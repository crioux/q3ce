...
void I2C(int Code, reg Dest, reg Op1, int Const)
{
	...
	{
		p = InstCreate32((NextCond << 28) | (Code & MODEMASK) | ((Dest & 15) << 16) | ((Op1 & 15) << 12) | (Const << 0),
				Dest, Op1, NONE, (NextCond != AL)?1:0, 0);
	}

	if (MODE(Code) == 9 && Const>=0 && Const<256)
	{
		p = InstCreate32((NextCond << 28) | (Code & MODEMASK) | ((Dest & 15) << 12) | ((Op1 & 15) << 16) | ((Const & 15) << 0) | ((Const & 0xF0) << 16),
				Dest, Op1, NONE, (NextCond != AL)?1:0, 0);
	}
	...
	if (MODE(Code) == 15)
	{
		...
		{
			p = InstCreate32((NextCond << 28) | (Code & MODEMASK) | ((Dest & 15) << 12) | ((Op1 & 15) << 16) | (Const << 0),
					NONE, Op1, NONE, (NextCond != AL)?1:0, 0);
	...
	if (Code >= 0 && Code < 16)
	{
		...
		if (Const >= 0 && Const <= 255)
			p = InstCreate32((NextCond << 28) | (1<<25) | (Code << 21) | ((NextSet?1:0)<<20) |
				...
				Dest, Op1, NONE, (NextCond != AL)?1:0, NextSet?1:0);
	}
		...
		Code == LDR_POSTSUB || Code == STR_POSTSUB)
	{
		...
		if (NextHalf || NextSign)
		{
			if (Const >= 0 && Const < 256 && (NextHalf || NextByte))
				p = InstCreate32((NextCond << 28) | 
			...
			if (Const >= 0 && Const < 4096)
				p = InstCreate32((NextCond << 28) | (1 << 26) |
					((Pre?1:0)<<24) | (Unsigned<<23) | (NextByte<<22) |
					((PreWrite?1:0)<<21) | (Load<<20) |
...
void I1P(int Code, reg Dest, dyninst* Block, int Ofs)
{
	...
	if (MODE(Code)==15)
	{
		p = InstCreate32((NextCond << 28) | (Code & MODEMASK) | (PC << 16) | ((Dest & 15) << 12),
			Dest, PC, NONE, (NextCond != AL)?1:0, 0);
...
void Break()
{
	dyninst* p = InstCreate32((NextCond << 28) | (15 << 24),NONE,NONE,NONE,0,0);
	if (p)
		p->Branch = 1;
	InstPost(p);
}
