 ...
 *
 * $Id: blit_arm_stretch.c 157 2005-01-02 00:39:09Z picard $
 *
...
static void Inc_RGB_UV_Pixel(blit_soft* p, int dY, int dUV)
{
...
static void Add_RGB_UV_Pixel(blit_soft* p, int Amount, int dY, int dUV, bool_t IncPtr)
{
...
static void Stretch_RGB_UV_Pixel(blit_soft* p, int Col, int dY, int dUV)
{
...
void Stretch_RGB_UV(blit_soft* p)
{
	...
	if (p->DirX<0 && !p->DstPalette) //adjust reversed destination for block size
		I2C(SUB,R9,R9,-p->DstStepX-(p->DstBPP >> 3));
	if (p->DstPalette && p->SwapXY)
	...
	Special = p->OnlyDiff && (p->FX.Flags & BLITFX_DITHER) && (p->SwapXY);
	if (Special)
	{
	...
	for (Col=0;Col<ColCount;)
	{
		...
		if (p->OnlyDiff)
		{
			...
			reg RA = (reg)((p->FX.Flags & BLITFX_DITHER) ? R7 : R1);
			reg RB = (reg)((p->FX.Flags & BLITFX_DITHER) ? R8 : R2);
		...
		if (p->DstPalette)
		{
			if (p->SwapXY)
			{
				...
				Byte(); I3(STR_POST,R0,R9,R5);
			}
			else
			{
				Byte(); I2C(STR_POST,R0,R9,p->DstStepX/2);
			}
		}
		else
		{
