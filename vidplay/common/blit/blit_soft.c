 ...
 *
 * $Id: blit_soft.c 206 2005-03-19 15:03:03Z picard $
 *
...
static const rgb Gray1[2] = { 
	{0,0,0},{255,255,255}
};
static const rgb Gray2[4] = { 
	{0,0,0},{85,85,85},{170,170,170},{255,255,255} 
};
static const rgb Gray4[16] = { 
			         ...
			         const video* Src, const blitfx* FX, int* OutCaps)
{
	...
	{
		p = (blitpack*) malloc(sizeof(blitpack));
		if (!p) return NULL;
...
#if defined(_M_IX86) || defined(NO_DYNCODE)
...
void Blit_PYUV_RGB32(blit_soft* This, uint8_t** DstPtr,uint8_t** SrcPtr,int DstPitch,int SrcPitch,
	                 int Width,int Height,int Src2SrcLast) 
{
	int i,x,y;
	uint8_t* Src[3];
...
void Blit_PYUV_RGB16(blit_soft* This, uint8_t** DstPtr,uint8_t** SrcPtr,int DstPitch,int SrcPitch,
	                 int Width,int Height,int Src2SrcLast) 
{
	int i,x,y;
	uint8_t* Src[3];
...
bool_t BlitCompile(blit_soft* p,
				   const video* NewDst,const video* NewSrc,
		  		   const blitfx* NewFX,bool_t NewOnlyDiff)
{
	...
	p->WMMX = (QueryPlatform(PLATFORM_CAPS) & CAPS_ARM_WMMX)!=0 && 
	          !QueryAdvanced(ADVANCED_NOWMMX) && (p->Dst.Flags & PF_16ALIGNED) && (p->Src.Flags & PF_16ALIGNED);
	p->QAdd = (QueryPlatform(PLATFORM_CAPS) & CAPS_ARM_5E)!=0 && 
...
#if defined(ARM)
	if ((p->Dst.Flags & PF_RGB) && p->Dst.BitCount==16 && (p->Src.Flags & PF_RGB))
		Any_RGB_RGB(p);
#endif

#if (defined(ARM) || defined(SH3) || defined(MIPS)) && !defined(NO_DYNCODE)
	if (SrcPlanarYUV)
	{
#if defined(ARM)
		if (DstPlanarYUV && 
