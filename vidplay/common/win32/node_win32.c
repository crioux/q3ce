...
{
	tcscpy(Base,T("SOFTWARE\\"));
	tcscat(Base,Context()->ProgramName);
	if (Class)
	{
		tcscat(Base,T("\\"));
		FourCCToString(Base+tcslen(Base),Class,(256-tcslen(Base))*sizeof(tchar_t));
	}
}
...
static void SaveValue(HKEY Key,int Id,const void* Data,int Size,int Type)
{
	...
	else
	{
		...
		case TYPE_STRING:
			RegType = REG_SZ;
			RegSize = (tcslen((const tchar_t*)Data)+1)*sizeof(tchar_t);
			break;
...
bool_t NodeRegLoadValue(int Class, int Id, void* Data, int Size, int Type)
{
	...
	NodeBase(Class,Base);
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, Base, 0, KEY_READ, &Key) == ERROR_SUCCESS)
	{
...
static void AddModule(const tchar_t* Path,HKEY* Key)
{
	...
	if (Date != -1)
	{
		...
		bool_t Changed = RegQueryValueEx(*Key, Path, 0, &RegType, (PBYTE)&Value, &RegSize) != ERROR_SUCCESS || Value != Date;

		NodeAddModule(Path,0,Date,Changed,0);

		if (Changed) // save file stamp to registry
...
static void FindPlugins(array* List)
{
	...
	NodeBase(0,Path);
	tcscat(Path,T("\\DLLStamp"));
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, Path, 0, NULL, 0, KEY_READ|KEY_WRITE, NULL, &KeyStamp, &Disp) == ERROR_SUCCESS)
	{
		...
		RegSize = sizeof(Path)/sizeof(tchar_t);
...
void Plugins_Init()
{
	...
	for (i=ARRAYBEGIN(List,int);i!=ARRAYEND(List,int);++i)
	{
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, Base, 0, KEY_READ, &Key) == ERROR_SUCCESS)
		{
			if (LoadValue(Key,NODE_MODULE_PATH,Path,sizeof(Path),TYPE_STRING) && NodeFindModule(Path,0)>=0)
			{
