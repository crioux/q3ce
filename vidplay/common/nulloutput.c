 ...
 *
 * $Id: nulloutput.c 157 2005-01-02 00:39:09Z picard $
 *
...
static int UpdateInput(nulloutput* p)
{
	...
	if ((p->Format.Type == PACKET_SUBTITLE) ||
		(p->Format.Type == PACKET_VIDEO && (p->Node.Class != NULLVIDEO_ID || Compressed(&p->Format.Video.Pixel))) ||
	    (p->Format.Type == PACKET_AUDIO && (p->Node.Class != NULLAUDIO_ID || p->Format.Audio.Format != AUDIOFMT_PCM)))
	{
