 ...
 *
 * $Id: idct.c 206 2005-03-19 15:03:03Z picard $
 *
...
int CodecIDCTSetFormat(codecidct* p, int Flags, int Width, int Height, int IDCTWidth, int IDCTHeight)
{
	int Result = ERR_NONE;
		...
		p->IDCT.Height != IDCTHeight)
	{
		...
		p->In.Format.Video.Width = Width;
		p->In.Format.Video.Height = Height;
