 ...
 *
 * $Id: player.h 178 2005-01-10 07:34:30Z picard $
 *
...
typedef bool_t (*playercomment)(void* This,int Stream,const tchar_t* Name,tchar_t* Value,int Size);
...
DLL int PlayerAdd(player* Player,int Index, const tchar_t* Path);
...
DLL tick_t PlayerGetChapter(player* Player,int No,tchar_t* OutName,int OutSize);
DLL bool_t PlayerGetStream(player* Player,int No,packetformat* OutFormat,tchar_t* OutName,int OutSize,int* OutPri);
