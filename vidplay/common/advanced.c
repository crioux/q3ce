 ...
 *
 * $Id: advanced.c 3 2004-07-13 11:26:13Z picard $
 *
...
static const datatable Params[] = 
{
...
#if defined(TARGET_WINCE)
	{ ADVANCED_ALLKEYS,	TYPE_BOOL, DF_SETUP|DF_CHECKLIST },
#endif
	...
	{ ADVANCED_DROPTOL,		TYPE_TICK, DF_SETUP|DF_MINMAX|DF_MSEC|DF_GAP, 0, TICKSPERSEC },
	{ ADVANCED_SKIPTOL,		TYPE_TICK, DF_SETUP|DF_MINMAX|DF_MSEC, 0, 5*TICKSPERSEC },
	{ ADVANCED_AVOFFSET,	TYPE_TICK, DF_SETUP|DF_MINMAX|DF_MSEC|DF_NEG|DF_RESYNC, -2*TICKSPERSEC, 2*TICKSPERSEC },
...
static int Enum(advanced* p, int* No, datadef* Param)
{
	...
	if (Result == ERR_NONE)
	{
		...
		if (Param->No == ADVANCED_NOWMMX && !(QueryPlatform(PLATFORM_CAPS) & CAPS_ARM_WMMX))
			Param->Flags |= DF_HIDDEN;
		if (Param->No == ADVANCED_ALLKEYS && QueryPlatform(PLATFORM_TYPENO) == TYPE_SMARTPHONE)
			Param->Flags |= DF_HIDDEN;
...
static int Get(advanced* p, int No, void* Data, int Size)
{
	...
	case ADVANCED_ALLKEYS: GETVALUE(p->AllKeys,bool_t); break;
...
static int Set(advanced* p, int No, const void* Data, int Size)
{
	...
	case ADVANCED_ALLKEYS: SETVALUE(p->AllKeys,bool_t,ERR_NONE); break;
...
static int Create(advanced* p)
{
...
#if defined(TARGET_WINCE)
	{
		...
		GetSystemPath(FileName,T("BtCeAvIf.dll"));
