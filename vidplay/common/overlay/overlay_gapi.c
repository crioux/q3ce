 ...
 *
 * $Id: overlay_gapi.c 200 2005-01-14 22:09:26Z picard $
 *
...
static bool_t LoadDriver(gapi* p)
{
	...
	if (p->Windows)
		GetSystemPath(Path,T("gx.dll"));
	else
		tcscpy(Path,T("gx.dll"));
...
static int Init(gapi* p)
{
	...
	p->Overlay.Output.Video.Direction = 0;
	p->Overlay.Output.Video.Aspect = ASPECT_ONE;

	if (Info.ffFormat & kfPalette)
	{
		QueryDesktop(&GDI);
		p->Overlay.Output.Video.Pixel.Flags = PF_PALETTE;
		p->Overlay.Output.Video.Pixel.BitCount = Info.cBPP;
		p->Overlay.Output.Video.Pixel.Palette = GDI.Pixel.Palette;
	}
	else
	if (Info.ffFormat & kfDirect444)
		DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.cBPP,4,4,4,0,0,0);
	else
	if (Info.ffFormat & kfDirect565)
	{
		if (Caps & CAPS_ONLY12BITRGB)
			DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.cBPP,4,4,4,1,2,1);
		else
			DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.cBPP,5,6,5,0,0,0);
	}
	else
	if (Info.ffFormat & kfDirect555)
		DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.cBPP,5,5,5,0,0,0);
	else
	if (Info.ffFormat & kfDirect888)
		DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.cBPP,8,8,8,0,0,0);

	if (Info.ffFormat & kfDirectInverted)
		p->Overlay.Output.Video.Pixel.Flags |= PF_INVERTED;
	...
	p->Overlay.Output.Video.Width = Info.cxWidth;
	p->Overlay.Output.Video.Height = Info.cyHeight;
	...
	{
		p->Overlay.Output.Video.Direction |= DIR_SWAPXY;
		p->Overlay.Output.Video.Pitch = abs(Info.cbxPitch) >> 3;
		SwapInt(&p->Overlay.Output.Video.Width,&p->Overlay.Output.Video.Height);

		if (Info.cbxPitch<0) 
			p->Overlay.Output.Video.Direction |= DIR_MIRRORUPDOWN;
		if (Info.cbyPitch<0) 
			p->Overlay.Output.Video.Direction |= DIR_MIRRORLEFTRIGHT;
	}
	else
	{
		p->Overlay.Output.Video.Pitch = abs(Info.cbyPitch) >> 3;
	...
	{
		if (p->Overlay.Output.Video.Width == 240 && p->Overlay.Output.Video.Height == 320 && p->Overlay.Output.Video.Pitch == 640)
			p->Overlay.Output.Video.Pitch = 480;
	}
...
static int Lock(gapi* p, planes Planes, bool_t OnlyAligned)
{
	...
	else
	{
		if (p->DRAM && p->GXSetViewport)
		{
			...
			if (p->Overlay.Output.Video.Pixel.Flags & PF_PIXELDOUBLE)
			{
				Top >>= 1;
				Height >>= 1;
			}
