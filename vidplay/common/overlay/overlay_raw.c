 ...
 *
 * $Id: overlay_raw.c 200 2005-01-14 22:09:26Z picard $
 *
...
bool_t GetRawFrameBuffer(RawFrameBufferInfo* Info)
{
	HDC DC = GetDC(NULL);
	...
	if (!Info->pFramePointer && 
		QueryPlatform(PLATFORM_VER) >= 421 && 
		QueryPlatform(PLATFORM_TYPENO) != TYPE_SMARTPHONE) // C500 should use GAPI
	{
		//try gxinfo
...
static int Init(raw* p)
{
	...
	memset(&p->Overlay.Output.Video,0,sizeof(video));
	p->Overlay.Output.Video.Width = Info.cxPixels;
	p->Overlay.Output.Video.Height = Info.cyPixels;
	...
	case FORMAT_555:
		DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.wBPP,5,5,5,0,0,0);
		break;
	case kfDirect565:
	case kfDirect|kfDirect565:
	case kfDirect|kfDirect565|kfLandscape:
	case FORMAT_565:
		DefaultRGB(&p->Overlay.Output.Video.Pixel,Info.wBPP,5,6,5,0,0,0);
		break;
	default:
		return ERR_DEVICE_ERROR;
	}
	...
	if (abs(Info.cyStride) < abs(Info.cxStride))
	{
		...
		p->Overlay.Output.Video.Pitch = abs(Info.cxStride);
	...
	{
		p->Overlay.Output.Video.Pitch = abs(Info.cyStride);
	...
	if (((int)Info.pFramePointer & 15)==0 && (p->Overlay.Output.Video.Pitch & 15)==0)
		p->Overlay.Output.Video.Pixel.Flags |= PF_16ALIGNED;

	p->Ptr = Info.pFramePointer;

	if (p->Overlay.Output.Video.Direction & DIR_SWAPXY)
		p->Overlay.SetFX |= BLITFX_VMEMROTATED;
	p->Overlay.SetFX = BLITFX_AVOIDTEARING;
	AdjustOrientation(&p->Overlay.Output.Video,1);
	return ERR_NONE;
}
...
{
	if (NodeEnumClass(NULL,GAPI_ID)) // skip when GAPI is already available (smartphone)
