 ...
 *
 * $Id: avi.c 206 2005-03-19 15:03:03Z picard $
 *
...
static NOINLINE int ReadLE32(const uint8_t* i)
{
	return i[0] | ((int)i[1]<<8) | ((int)i[2]<<16) | ((int)i[3]<<24);
}
			...
			if (Reader->Input->ReadBlock(Reader->Input,&Buffer,0,CHECKINDEX)==CHECKINDEX)
			{
				...
				while (--i>=Buffer.Ptr+16)
				{
					if (p->NameChar[*i])
					{
						...
						if (No<0)
						{
							if (Type == FOURCCLE('J','U','N','K') && ISDATA(ReadLE32(i-16)))
							{
								i -= 16;
								break;
							}
							if (ISDATA(Type) && (ReadLE32(i+4) & ~0x10)==0)
								break;
							Type <<= 8;
							No += 8;
						}
				...
				{
					if (ReadLE32(i-8) == FOURCCLE('i','d','x','1'))
						break;

					Type = ReadLE32(i);
					if (ISDATA(Type))
					{
						int Pos = ReadLE32(i+8);
						...
						if (ReadLE32(i+12)==0 && Pos==ReadLE32(i-16+8))
						{
							p->CheckForZero = 1;
							break;
						}
...
static bool_t SubtitleTiming(format_packet* Packet,format_reader* Reader,int Length)
{
	...
	if (Scanf(s,T("[%d:%d:%d.%d-%d:%d:%d.%d"),&Hour0,&Min0,&Sec0,&MSec0,&Hour1,&Min1,&Sec1,&MSec1)!=8)
	{
...
static void ReadMeta(avi* p, format_reader* Reader, int* DataLength)
{
	...
	if (Len>0)
	{
		*DataLength -= Len;
		UTF8[Len] = 0;
		tcscpy(Meta,T("LANGUAGE="));
		UTF8ToTcs(Meta+tcslen(Meta),UTF8,512-tcslen(Meta));
		if (Comment->Node)
...
static int ReadPacket(avi* p, format_reader* Reader, format_packet* Packet)
{
	...
	do
	{
		...
		if (ISDATA(Type))
		{
			...
			{
				//DEBUG_MSG3(DEBUG_FORMAT,"AVI Packet stream:%d pos:%08x length:%d",No,Reader->FilePos,DataLength);
		...
		case FOURCCLE('s','t','r','f'):
			if (p->HeadType == FOURCCLE('v','i','d','s'))
			{
				...
				if (Stream)
				{
					...
					if (Stream->Stream.Format.Video.Pixel.FourCC == FOURCC('D','X','S','B'))
					{
						PacketFormatClear(&Stream->Stream.Format);
						Stream->Stream.Format.Type = PACKET_SUBTITLE;
						Stream->Stream.Format.Subtitle.FourCC = FOURCC('D','X','S','B');
					}
					else
					{
						if (Stream->Stream.Format.Video.Pixel.FourCC == 4)
							Stream->Stream.Format.Video.Pixel.FourCC = UpperFourCC(p->VideoHandler);
			...
			if (p->HeadType == FOURCCLE('a','u','d','s'))
			{
				...
				if (Stream)
				{
					...
					if (!Stream->ByteRate)
					{
						...
						if (p->AudioRate.Num == Stream->Stream.Format.Audio.SampleRate)
					...
					else
					{
						...
						if (p->AudioSampleSize < Stream->Stream.Format.Audio.BlockAlign)
							p->AudioSampleSize = Stream->Stream.Format.Audio.BlockAlign;

						if (Stream->Stream.Format.Audio.Format == 0x55 /*MP3*/ && 
		...
		else
		{
			...
			if (Stream)
			{
				...
				else
				{
					...
					NeedKey = Stream->Stream.Format.Type == PACKET_VIDEO;
...
static void CheckIndex(avi* p, format_reader* Reader)
{
	...
	for (i=0;i<p->Format.StreamCount;++i)
	{
		...
		if (Stream->Stream.Format.Type == PACKET_VIDEO && Stream->SuperIndex)
		{
			OpenDML = 1;
	...
	for (i=1;i<MAXREADER;++i)
	{
		format_reader* Reader = &p->Format.Reader[i];
		if (Reader->Input && Reader->OffsetMax>0)
		{
			Reader->Seek(Reader,Reader->OffsetMin,SEEK_SET);
			FlushBuffers = 1;
		}
	}
