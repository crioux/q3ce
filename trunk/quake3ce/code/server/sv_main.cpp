		...
		if ( !Q_strncmp(cmd, client->reliableCommands[ index ], strlen("cs")) ) {
			...
			if ( csnum1 == csnum2 ) {
				...
				if ( client->netchan.remoteAddress.type != NA_BOT ) {
					Com_Printf( "WARNING: client %i removed lfixed pending config string %i: %s\n", client-svs.clients, csnum1, cmd );
				}
				*/
				return qtrue;
			}
...
void SVC_Status( netadr_t from ) {
	...
	if ( FIXED_TO_INT(Cvar_VariableValue( "g_gametype" )) == GT_SINGLE_PLAYER ) {
		return;
	}
...
void SV_Frame( int msec ) {
	...
	while ( sv.timeResidual >= frameMsec ) {
		...
		SysCallArgs args(1);
		args[0]=svs.time;
		VM_Call( gvm, GAME_RUN_FRAME, args);
	}
