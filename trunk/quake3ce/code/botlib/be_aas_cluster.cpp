...
int AAS_NonConvexFaces(aas_face_t *face1, aas_face_t *face2, int side1, int side2)
{
	...
	for (i = 0; i < face1->numedges; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
			if (FIXED_VEC3DOT(plane2->normal, aasworld.vertexes[edge->v[j]]) -
							plane2->dist < -BFIXED(0,01)) return qtrue;
		} //end for
	} //end for
	for (i = 0; i < face2->numedges; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
			if (FIXED_VEC3DOT(plane1->normal, aasworld.vertexes[edge->v[j]]) -
							plane1->dist < -BFIXED(0,01)) return qtrue;
		} //end for
	} //end for
...
{
	int i;
	bvec3_t edgevec1, edgevec2, normal1, normal2;
	bfixed dist1, dist2;
	aas_plane_t *plane;
	...
	dist1 = FIXED_VEC3DOT(normal1, aasworld.vertexes[edge1->v[0]]);
	CrossProduct(edgevec2, plane->normal, normal2);
	dist2 = FIXED_VEC3DOT(normal2, aasworld.vertexes[edge2->v[0]]);

	for (i = 0; i < 2; i++)
	{
		if (FIXED_VEC3DOT(aasworld.vertexes[edge1->v[i]], normal2) - dist2 < -BFIXED(0,01)) return qfalse;
	} //end for
	for (i = 0; i < 2; i++)
	{
		if (FIXED_VEC3DOT(aasworld.vertexes[edge2->v[i]], normal1) - dist1 < -BFIXED(0,01)) return qfalse;
	} //end for
	return qtrue;
...
void AAS_AddTeleporterPortals(void)
{
	...
	for (ent = entities; ent; ent = ent->next)
	{
		...
		if (classname && !strcmp(classname, "misc_teleporter"))
		{
			...
			//
			VectorSet(mins, -BFIXED(8,0), -BFIXED(8,0), BFIXED(8,0));
			VectorSet(maxs, BFIXED(8,0), BFIXED(8,0), BFIXED(24,0));
			//
...
void AAS_InitClustering(void)
{
	...
	if (aasworld.numclusters >= 1)
	{
		...
		if (FIXED_IS_ZERO(LibVarGetValue("forceclustering")) &&
			FIXED_IS_ZERO(LibVarGetValue("forcereachability"))) return;
#endif
	} //end if
