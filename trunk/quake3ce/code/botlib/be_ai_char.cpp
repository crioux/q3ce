...
void BotDumpCharacter(bot_character_t *ch)
{
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		switch(ch->c[i].type)
		{
			...
			case CT_FLOAT: Log_Write(" %4d %f\n", i, (double)ch->c[i].value._float); break;
...
{
	if (FIXED_IS_ZERO(LibVarGetValue("bot_reloadcharacters"))) return;
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "skill"))
		{
			...
			if (skill < 0 || token.intvalue == skill)
			{
				...
				ch->skill = MAKE_GFIXED(token.intvalue);
				while(PC_ExpectAnyToken(source, &token))
				{
					...
					if (token.type == TT_NUMBER)
					{
						...
						{
							ch->c[index].value._float = FIXED_TO_FLOAT(token.floatvalue);
					...
					{
						SourceError(source, "expected integer, bfixed or string, found %s\n", token.string);
						FreeSource(source);
						BotFreeCharacterStrings(ch);
						FreeMemory(ch);
						return NULL;
...
int BotFindCachedCharacter(const char *charfile, gfixed skill)
{
	...
	for (handle = 1; handle <= MAX_CLIENTS; handle++)
	{
		...
		if ( strcmp( botcharacters[handle]->filename, charfile ) == 0 &&
			(skill < GFIXED_0 || FIXED_ABS(botcharacters[handle]->skill - skill) < GFIXED(0,01)) )
		{
			return handle;
		} //end if
	} //end for
...
int BotLoadCachedCharacter(const char *charfile, gfixed skill, int reload)
{
	...
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			botimport.Print(PRT_MESSAGE, "loaded cached skill %f from %s\n", FIXED_TO_DOUBLE(skill), charfile);
			return cachedhandle;
		} //end if
	} //end else
	//
	intskill = FIXED_TO_INT(skill + GFIXED(0,5));
	...
	if (!reload)
	{
		...
		cachedhandle = BotFindCachedCharacter(charfile, -GFIXED_1);
		if (cachedhandle)
		{
			botimport.Print(PRT_MESSAGE, "loaded cached skill %f from %s\n", FIXED_TO_DOUBLE(botcharacters[cachedhandle]->skill), charfile);
			return cachedhandle;
		} //end if
	} //end if
	...
	{
		//try to load a cached character with any skill
		cachedhandle = BotFindCachedCharacter(DEFAULT_CHARACTER, -GFIXED_1);
		if (cachedhandle)
		{
...
int BotLoadCharacterSkill(const char *charfile, gfixed skill)
{
	...
	ch = BotLoadCachedCharacter(charfile, skill, FIXED_TO_INT(LibVarGetValue("bot_reloadcharacters")));
...
// Changes Globals:		-
//===========================================================================
int BotInterpolateCharacters(int handle1, int handle2, gfixed desiredskill)
{
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		...
		if (ch1->c[i].type == CT_FLOAT && ch2->c[i].type == CT_FLOAT)
		{
			...
			out->c[i].value._float = ch1->c[i].value._float +
								(ch2->c[i].value._float - ch1->c[i].value._float) * FIXED_TO_FLOAT(scale);
		} //end if
...
int BotLoadCharacter(const char *charfile, gfixed skill)
{
	...
	if (skill < GFIXED_1) skill = GFIXED_1;
	else if (skill > GFIXED(5,0)) skill = GFIXED(5,0);
	//skill 1, 4 and 5 should be available in the character files
	if (skill == GFIXED_1 || skill == GFIXED(4,0) || skill == GFIXED(5,0))
	{
	...
	{
		//load skill 1 and 4
		firstskill = BotLoadCharacterSkill(charfile, GFIXED_1);
		if (!firstskill) return 0;
		secondskill = BotLoadCharacterSkill(charfile, GFIXED(4,0));
		if (!secondskill) return firstskill;
	} //end if
	else
	{
		//load skill 4 and 5
		firstskill = BotLoadCharacterSkill(charfile, GFIXED(4,0));
		if (!firstskill) return 0;
		secondskill = BotLoadCharacterSkill(charfile, GFIXED(5,0));
		if (!secondskill) return firstskill;
	} //end else
...
gfixed Characteristic_Float(int character, int index)
{
	...
	ch = BotCharacterFromHandle(character);
	if (!ch) return GFIXED_0;
	//check if the index is in range
	if (!CheckCharacteristicIndex(character, index)) return GFIXED_0;
	//an integer will be converted to a bfixed
	if (ch->c[index].type == CT_INTEGER)
	{
		return MAKE_GFIXED(ch->c[index].value.integer);
	} //end if
	//floats are just returned
	else if (ch->c[index].type == CT_FLOAT)
	{
		return MAKE_GFIXED(ch->c[index].value._float);
	} //end else if
	//cannot convert a string pointer to a bfixed
	else
	{
		botimport.Print(PRT_ERROR, "characteristic %d is not a bfixed\n", index);
		return GFIXED_0;
	} //end else if
//	return 0;
