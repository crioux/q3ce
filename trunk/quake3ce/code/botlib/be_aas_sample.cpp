...
#define BBOX_NORMAL_EPSILON		BFIXED(0,001)

#define ON_EPSILON					BFIXED_0 //BFIXED(0,0005)

#define TRACEPLANE_EPSILON			BFIXED(0,125)

typedef struct aas_tracestack_s
{
	bvec3_t start;		//start point of the piece of line to trace
	bvec3_t end;			//end point of the piece of line to trace
...
void AAS_PresenceTypeBoundingBox(int presencetype, bvec3_t mins, bvec3_t maxs)
{
	...
	bvec3_t boxmins[3] = {{BFIXED_0, BFIXED_0, BFIXED_0}, {-BFIXED(15,0), -BFIXED(15,0), -BFIXED(24,0)}, {-BFIXED(15,0), -BFIXED(15,0), -BFIXED(24,0)}};
	bvec3_t boxmaxs[3] = {{BFIXED_0, BFIXED_0, BFIXED_0}, { BFIXED(15,0),  BFIXED(15,0),  BFIXED(32,0)}, { BFIXED(15,0),  BFIXED(15,0),   BFIXED(8,0)}};
...
void AAS_InitAASLinkHeap(void)
{
	...
	if (!aasworld.linkheap)
	{
		...
		max_aaslinks = 6144;
#else
		max_aaslinks = FIXED_TO_INT(LibVarValue("max_aaslinks", "6144"));
#endif
...
//		botimport.Print(PRT_MESSAGE, "[%d]", nodenum);
		...
		dist = FIXED_VEC3DOT(point, plane->normal) - plane->dist;
		if (dist > BFIXED_0) nodenum = node->children[0];
...
// Changes Globals:		-
//===========================================================================
int AAS_PointReachabilityAreaIndex( bvec3_t origin )
{
...
// Changes Globals:		-
//===========================================================================
bvec_t AAS_BoxOriginDistanceFromPlane(bvec3_t normal, bvec3_t mins, bvec3_t maxs, int side)
{
	bvec3_t v1, v2;
	int i;
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_AreaEntityCollision(int areanum, bvec3_t start, bvec3_t end,
										int presencetype, int passent, aas_trace_t *trace)
{
	int collision;
	bvec3_t boxmins, boxmaxs;
	aas_link_t *link;
...
aas_trace_t AAS_TraceClientBBox(bvec3_t start, bvec3_t end, int presencetype, int passent)
{
	...
	bfixed front, back, frac;
	bvec3_t cur_start, cur_end, cur_mid, v1, v2;
		...
		if (nodenum < 0)
		{
			...
			if (!(aasworld.areasettings[-nodenum].presencetype & presencetype))
			{
				...
				else
				{
					...
					trace.fraction = FIXED_RATIO_G(FIXED_VEC3LEN(v2),VectorNormalize(v1));
					FIXED_VEC3MA(tstack_p->start, -BFIXED(0,125), v1, tstack_p->start);
				} //end else
...
//				VectorSubtract(end, start, v1);
				...
				if (FIXED_VEC3DOT(v1, plane->normal) > BFIXED_0) trace.planenum ^= 1;
				return trace;
			} //end if
			else
			{
				if (passent >= 0)
				{
													...
													&trace))
					{
						if (!trace.startsolid)
						{
							...
							trace.fraction = FIXED_RATIO_G(FIXED_VEC3LEN(v2),FIXED_VEC3LEN(v1));
						} //end if
						return trace;
					} //end if
				} //end if
...
//			{
				front = FIXED_VEC3DOT(cur_start, plane->normal) - plane->dist;
				back = FIXED_VEC3DOT(cur_end, plane->normal) - plane->dist;
//				break;
//			} //end default
//		} //end switch
		...
		else
		{
			...
			if ( front == back ) front -= BFIXED(0,001); // bk0101022 - hack/FPE 
                	//calculate the hitpoint with the node (split point of the line)
			//put the crosspoint TRACEPLANE_EPSILON pixels on the near side
			if (front < BFIXED_0) frac = (front + TRACEPLANE_EPSILON)/(front-back);
			else frac = (front - TRACEPLANE_EPSILON)/(front-back); // bk010221
			//
			if (frac < BFIXED_0)
				frac = BFIXED(0,001); //0
			else if (frac > BFIXED_1)
				frac = BFIXED(0,999); //1
...
int AAS_TraceAreas(bvec3_t start, bvec3_t end, int *areas, bvec3_t *points, int maxareas)
{
	...
	int numareas;
	bfixed front, back, frac;
	bvec3_t cur_start, cur_end, cur_mid;
	aas_tracestack_t tracestack[127];
	aas_tracestack_t *tstack_p;
	aas_node_t *aasnode;
	aas_plane_t *plane;
...
//		} //end switch
		...
		//if the whole to be traced line is totally at the front of this node
		//only go down the tree with the front child
		if (front > BFIXED_0 && back > BFIXED_0)
		{
			//keep the current start and end point on the stack
			//and go down the tree with the front child
			tstack_p->nodenum = aasnode->children[0];
		...
		//if the whole to be traced line is totally at the back of this node
		//only go down the tree with the back child
		else if (front <= BFIXED_0 && back <= BFIXED_0)
		{
			//keep the current start and end point on the stack
			//and go down the tree with the back child
			tstack_p->nodenum = aasnode->children[1];
		...
		else
		{
			...
			//calculate the hitpoint with the node (split point of the line)
			//put the crosspoint TRACEPLANE_EPSILON pixels on the near side
			if (front < BFIXED_0) frac = (front)/(front-back);
			else frac = (front)/(front-back);
			if (frac < BFIXED_0) frac = BFIXED_0;
			else if (frac > BFIXED_1) frac = BFIXED_1;
			//frac = front / (front-back);
			//
			cur_mid[0] = cur_start[0] + (cur_end[0] - cur_start[0]) * frac;
			cur_mid[1] = cur_start[1] + (cur_end[1] - cur_start[1]) * frac;
...
// Changes Globals:		-
//===========================================================================
// void AAS_OrthogonalToVectors(bvec3_t v1, bvec3_t v2, bvec3_t res)
...
qboolean AAS_InsideFace(aas_face_t *face, avec3_t pnormal, bvec3_t point, bfixed epsilon)
{
	...
	bvec3_t v0;
	bvec3_t edgevec, pointvec, sepnormal;
	aas_edge_t *edge;
#ifdef AAS_SAMPLE_DEBUG
	int lastvertex = 0;
#endif //AAS_SAMPLE_DEBUG
	...
	for (i = 0; i < face->numedges; i++)
	{
		...
		if (FIXED_VEC3DOT(pointvec, sepnormal) < -epsilon) return qfalse;
	} //end for
	return qtrue;
...
aas_face_t *AAS_AreaGroundFace(int areanum, bvec3_t point)
{
	...
	for (i = 0; i < area->numfaces; i++)
	{
		...
		if (face->faceflags & FACE_GROUND)
		{
			...
			if (aasworld.planes[face->planenum].normal[2] < AFIXED_0) VectorNegate(up, normal);
			else VectorCopy(up, normal);
			//check if the point is in the face
			if (AAS_InsideFace(face, normal, point, BFIXED(0,01))) return face;
		} //end if
	} //end for
	return NULL;
...
/*			if (firstface)
			...
			if (AAS_InsideFace(face,
						aasworld.planes[face->planenum].normal, trace->endpos, BFIXED(0,01))) return face;
		} //end if
	} //end for
	return firstface;
...
// Changes Globals:		-
//===========================================================================
int AAS_BoxOnPlaneSide2(bvec3_t absmins, bvec3_t absmaxs, aas_plane_t *p)
{
	int i, sides;
	bfixed dist1, dist2;
	bvec3_t corners[2];
	...
	sides = 0;
	if (dist1 >= BFIXED_0) sides = 1;
	if (dist2 < BFIXED_0) sides |= 2;
...
// Changes Globals:		-
//===========================================================================
//int AAS_BoxOnPlaneSide(bvec3_t absmins, bvec3_t absmaxs, aas_plane_t *p)
