...
// Changes Globals:		-
//===========================================================================
int ReadValue(source_t *source, gfixed *value)
{
	token_t token;
	...
	*value = MAKE_GFIXED(token.floatvalue);
	return qtrue;
...
weightconfig_t *ReadWeightConfig(const char *filename)
{
	...
	if (FIXED_IS_ZERO(LibVarGetValue("bot_reloadcharacters")))
	{
		avail = -1;
...
// Changes Globals:		-
//===========================================================================
gfixed FuzzyWeight_r(int *inventory, fuzzyseperator_t *fs)
{
	gfixed scale, w1, w2;
	...
	else if (fs->next)
	{
		if (inventory[fs->index] < fs->next->value)
		{
			...
			//the scale factor
			scale = FIXED_INT32RATIO_G((inventory[fs->index] - fs->value),(fs->next->value - fs->value));
			//scale between the two weights
			return scale * w1 + (GFIXED_1 - scale) * w2;
		} //end if
...
// Changes Globals:		-
//===========================================================================
gfixed FuzzyWeightUndecided_r(int *inventory, fuzzyseperator_t *fs)
{
	gfixed scale, w1, w2;
...
// Changes Globals:		-
//===========================================================================
gfixed FuzzyWeight(int *inventory, weightconfig_t *wc, int weightnum)
{
...
// Changes Globals:		-
//===========================================================================
gfixed FuzzyWeightUndecided(int *inventory, weightconfig_t *wc, int weightnum)
{
#ifdef EVALUATERECURSIVELY
...
void EvolveFuzzySeperator_r(fuzzyseperator_t *fs)
{
	...
	else if (fs->type == WT_BALANCE)
	{
		...
		if (random() < GFIXED(0,01)) fs->weight += crandom() * (fs->maxweight - fs->minweight);
		else fs->weight += crandom() * (fs->maxweight - fs->minweight) * GFIXED(0,5);
...
// Changes Globals:		-
//===========================================================================
void ScaleFuzzySeperator_r(fuzzyseperator_t *fs, gfixed scale)
{
	if (fs->child)
	{
...
// Changes Globals:		-
//===========================================================================
void ScaleWeight(weightconfig_t *config, char *name, gfixed scale)
{
	int i;

	if (scale < GFIXED_0) scale = GFIXED_0;
	else if (scale > GFIXED_1) scale = GFIXED_1;
	for (i = 0; i < config->numweights; i++)
	{
...
// Changes Globals:		-
//===========================================================================
void ScaleFuzzySeperatorBalanceRange_r(fuzzyseperator_t *fs, gfixed scale)
{
	if (fs->child)
	{
	...
	{
		gfixed mid = (fs->minweight + fs->maxweight) * GFIXED(0,5);
		//get the weight between bounds
...
// Changes Globals:		-
//===========================================================================
void ScaleFuzzyBalanceRange(weightconfig_t *config, gfixed scale)
{
	int i;

	if (scale < GFIXED_0) scale = GFIXED_0;
	else if (scale > GFIXED(100,0)) scale = GFIXED(100,0);
	for (i = 0; i < config->numweights; i++)
	{
								...
								fuzzyseperator_t *fsout)
{
	...
	else if (fs1->type == WT_BALANCE)
	{
		...
		fsout->weight = FIXED_DIVPOW2((fs1->weight + fs2->weight),1);
