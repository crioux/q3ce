...
qboolean ReadNumber(source_t *source, fielddef_t *fd, void *p)
{
	...
	{
		if ((fd->type & FT_TYPE) != FT_FIXED)
		{
			SourceError(source, "unexpected fixed");
			return 0;
		} //end if
		...
		if (fd->type & FT_BOUNDED)
		{
			...
			{
				SourceError(source, "fixed out of range [%f, %f]", FIXED_TO_DOUBLE(fd->floatmin), FIXED_TO_DOUBLE(fd->floatmax));
				return 0;
			} //end if
		} //end if
		
		
		if((fd->type & FT_IS_AFIXED)==FT_IS_AFIXED)
		{
			*(afixed *) p = MAKE_AFIXED(floatval);
		}
		else if((fd->type & FT_IS_BFIXED)==FT_IS_BFIXED)
		{
			*(bfixed *) p = MAKE_BFIXED(floatval);
		}
		else if((fd->type & FT_IS_CFIXED)==FT_IS_CFIXED)
		{
			*(cfixed *) p = MAKE_CFIXED(floatval);
		}
		else if((fd->type & FT_IS_DFIXED)==FT_IS_DFIXED)
		{
			*(dfixed *) p = MAKE_DFIXED(floatval);
		}
		else
		{
			*(gfixed *) p = MAKE_GFIXED(floatval);
		}
	...
	if ((fd->type & FT_TYPE) == FT_CHAR || (fd->type & FT_TYPE) == FT_INT)
	{
		if (fd->type & FT_BOUNDED)
		{
			intmin = FIXED_TO_INT(FIXED_MAX(MAKE_LFIXED(intmin), fd->floatmin));
			intmax = FIXED_TO_INT(FIXED_MIN(MAKE_LFIXED(intmax), fd->floatmax));
		} //end if
	...
	else if ((fd->type & FT_TYPE) == FT_FIXED)
	{
		if (fd->type & FT_BOUNDED)
		{
			if (MAKE_LFIXED(intval) < fd->floatmin || MAKE_LFIXED(intval) > fd->floatmax)
			{
				SourceError(source, "value %d out of range [%f, %f]", intval, FIXED_TO_DOUBLE(fd->floatmin), FIXED_TO_DOUBLE(fd->floatmax));
				return 0;
			} //end if
		} //end if
	} //end else if
	...
	else if ((fd->type & FT_TYPE) == FT_FIXED)
	{
		if((fd->type & FT_IS_AFIXED)==FT_IS_AFIXED)
		{
			*(afixed *) p = MAKE_AFIXED(intval);
		}
		else if((fd->type & FT_IS_BFIXED)==FT_IS_BFIXED)
		{
			*(bfixed *) p = MAKE_BFIXED(intval);
		}
		else if((fd->type & FT_IS_CFIXED)==FT_IS_CFIXED)
		{
			*(cfixed *) p = MAKE_CFIXED(intval);
		}
		else if((fd->type & FT_IS_DFIXED)==FT_IS_DFIXED)
		{
			*(dfixed *) p = MAKE_DFIXED(intval);
		}
		else
		{
			*(gfixed *) p = MAKE_GFIXED(intval);
		}
	} //end else
	return 1;
		...
		while (num-- > 0)
		{
			...
			switch(fd->type & FT_TYPE)
			{
				...
				case FT_FIXED:
				{
					...
					if((fd->type & FT_IS_AFIXED)==FT_IS_AFIXED)	p = (char *) p + sizeof(afixed);
					else if((fd->type & FT_IS_BFIXED)==FT_IS_BFIXED) p = (char *) p + sizeof(bfixed);
					else if((fd->type & FT_IS_CFIXED)==FT_IS_CFIXED) p = (char *) p + sizeof(cfixed);
					else if((fd->type & FT_IS_DFIXED)==FT_IS_DFIXED) p = (char *) p + sizeof(dfixed);
					else p = (char *) p + sizeof(gfixed);
...
// Changes Globals:		-
//===========================================================================
int WriteFloat(FILE *fp, double value)
{
	char buf[128];
	int l;
		...
		while(num-- > 0)
		{
			switch(fd->type & FT_TYPE)
			{
				...
				case FT_FIXED:
				{
					if((fd->type & FT_IS_AFIXED)==FT_IS_AFIXED)
					{
						if (!WriteFloat(fp, FIXED_TO_DOUBLE(*(afixed *)p))) return qfalse;
						p = (char *) p + sizeof(afixed);
					}
					else if((fd->type & FT_IS_BFIXED)==FT_IS_BFIXED)
					{
						if (!WriteFloat(fp, FIXED_TO_DOUBLE(*(bfixed *)p))) return qfalse;
						p = (char *) p + sizeof(bfixed);
					}
					else if((fd->type & FT_IS_CFIXED)==FT_IS_CFIXED)
					{
						if (!WriteFloat(fp, FIXED_TO_DOUBLE(*(cfixed *)p))) return qfalse;
						p = (char *) p + sizeof(cfixed);
					}
					else if((fd->type & FT_IS_DFIXED)==FT_IS_DFIXED)
					{
						if (!WriteFloat(fp, FIXED_TO_DOUBLE(*(dfixed *)p))) return qfalse;
						p = (char *) p + sizeof(dfixed);
					}
					else
					{
						if (!WriteFloat(fp, FIXED_TO_DOUBLE(*(gfixed *)p))) return qfalse;
						p = (char *) p + sizeof(gfixed);
					}
					break;
				} //end case
				case FT_STRING:
