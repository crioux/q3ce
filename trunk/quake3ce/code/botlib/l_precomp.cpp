...
int PC_MergeTokens(token_t *t1, token_t *t2)
{
	...
	{
		//remove trailing lfixed quote
		t1->string[strlen(t1->string)-1] = '\0';
		//concat without leading lfixed quote
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	for (i = 0; builtin[i].string; i++)
	{
		...
		define->builtin = builtin[i]._builtin;
										...
										token_t **firsttoken, token_t **lasttoken)
{
	...
	switch(define->builtin)
	{
		case BUILTIN_LINE:
		{
			...
			token->floatvalue = MAKE_LFIXED(deftoken->line);
#endif //NUMBERVALUE
...
int PC_Directive_define(source_t *source)
{
	...
	if (last)
	{
		//check for merge _operators at the beginning or end
...
int PC__operatorPriority(int op)
{
	switch(op)
	{
	...
	return qfalse;
} //end of the function PC__operatorPriority

//#define AllocValue()			GetClearedMemory(sizeof(value_t));
//#define FreeValue(val)		FreeMemory(val)
//#define Alloc_operator(op)		op = (_operator_t *) GetClearedMemory(sizeof(_operator_t));
//#define Free_operator(op)		FreeMemory(op);

#define MAX_VALUES		64
#define MAX__operatorS	64
...
//
#define Alloc_operator(op)								\
	if (num_operators >= MAX__operatorS) {				\
		SourceError(source, "out of _operator space\n");	\
		error = 1;										\
		break;											\
	}													\
	else												\
		op = &_operator_heap[num_operators++];
#define Free_operator(op)

int PC_EvaluateTokens(source_t *source, token_t *tokens, signed long int *intvalue, lfixed *floatvalue, int integer)
{
	_operator_t *o, *first_operator, *last_operator;
	...
	lfixed questmarkfloatvalue = LFIXED_0;
	int gotquestmarkvalue = qfalse;
	int last_operatortype = 0;
	//
	_operator_t _operator_heap[MAX__operatorS];
	int num_operators = 0;
	value_t value_heap[MAX_VALUES];
	int numvalues = 0;

	first_operator = last_operator = NULL;
	firstvalue = lastvalue = NULL;
	if (intvalue) *intvalue = 0;
	if (floatvalue) *floatvalue = LFIXED_0;
	for (t = tokens; t; t = t->next)
	{
		switch(t->type)
		{
			case TT_NAME:
			{
				...
				{
					v->intvalue = 1;
					v->floatvalue = LFIXED_1;
				} //end if
				else
				{
					v->intvalue = 0;
					v->floatvalue = LFIXED_0;
				} //end else
			...
			case TT_PUNCTUATION:
			{
				...
				//check for invalid _operators on floating point values
				if (!integer)
				{
				...
				{
					//o = (_operator_t *) GetClearedMemory(sizeof(_operator_t));
					Alloc_operator(o);
					o->_operator = t->subtype;
					o->priority = PC__operatorPriority(t->subtype);
					o->parentheses = parentheses;
					o->next = NULL;
					o->prev = last_operator;
					if (last_operator) last_operator->next = o;
					else first_operator = o;
					last_operator = o;
					lastwasvalue = 0;
				} //end if
				break;
	...
	questmarkintvalue = 0;
	questmarkfloatvalue = LFIXED_0;
	//while there are _operators
	while(!error && first_operator)
	{
		v = firstvalue;
		for (o = first_operator; o->next; o = o->next)
		{
		...
		{
			Log_Write("_operator %s, value1 = %f", PunctuationFromNum(source->scriptstack, o->_operator), FIXED_TO_DOUBLE(v1->floatvalue));
			if (v2) Log_Write("value2 = %f", FIXED_TO_DOUBLE(v2->floatvalue));
		} //end else
#endif //DEBUG_EVAL
		switch(o->_operator)
		{
			case P_LOGIC_NOT:		v1->intvalue = !v1->intvalue;
									v1->floatvalue = MAKE_LFIXED((int)(!FIXED_TO_INT(v1->floatvalue))); break;
			case P_BIN_NOT:			v1->intvalue = ~v1->intvalue;
									break;
			case P_MUL:				v1->intvalue *= v2->intvalue;
									v1->floatvalue *= v2->floatvalue; break;
			case P_DIV:				if (!v2->intvalue || FIXED_IS_ZERO(v2->floatvalue))
									{
			...
			case P_LOGIC_AND:		v1->intvalue = v1->intvalue && v2->intvalue;
									v1->floatvalue = MAKE_LFIXED(FIXED_NOT_ZERO(v1->floatvalue) && FIXED_NOT_ZERO(v2->floatvalue)); break;
			case P_LOGIC_OR:		v1->intvalue = v1->intvalue || v2->intvalue;
									v1->floatvalue = MAKE_LFIXED(FIXED_NOT_ZERO(v1->floatvalue) || FIXED_NOT_ZERO(v2->floatvalue)); break;
			case P_LOGIC_GEQ:		v1->intvalue = v1->intvalue >= v2->intvalue;
									v1->floatvalue = MAKE_LFIXED(v1->floatvalue >= v2->floatvalue); break;
			...
			case P_COLON:
			{
				...
				{
					if (FIXED_IS_ZERO(questmarkfloatvalue)) v1->floatvalue = v2->floatvalue;
				} //end else
				gotquestmarkvalue = qfalse;
				break;
			} //end case
		...
		else Log_Write("result value = %f", FIXED_TO_DOUBLE(v1->floatvalue));
#endif //DEBUG_EVAL
		if (error) break;
		last_operatortype = o->_operator;
		...
		else first_operator = o->next;
		if (o->next) o->next->prev = o->prev;
		else last_operator = o->prev;
		//FreeMemory(o);
		Free_operator(o);
	} //end while
	if (firstvalue)
	...
	for (o = first_operator; o; o = last_operator)
	{
		last_operator = o->next;
		//FreeMemory(o);
		Free_operator(o);
	} //end for
...
// Changes Globals:		-
//============================================================================
int PC_Evaluate(source_t *source, signed long int *intvalue, lfixed *floatvalue, int integer)
{
	...
	else Log_Write("eval result: %f", FIXED_TO_DOUBLE(*floatvalue));
#endif //DEBUG_EVAL
	//
	return qtrue;
...
// Changes Globals:		-
//============================================================================
int PC_DollarEvaluate(source_t *source, signed long int *intvalue, lfixed *floatvalue, int integer)
{
...
int PC_Directive_evalfloat(source_t *source)
{
	...
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
	sprintf(token.string, "%1.2f", FIXED_ABS(value));
	token.type = TT_NUMBER;
	token.subtype = TT_FLOAT|TT_LONG|TT_DECIMAL;
	PC_UnreadSourceToken(source, &token);
	if (value < LFIXED_0) UnreadSignToken(source);
	return qtrue;
...
int PC_DollarDirective_evalint(source_t *source)
{
	...
	token.floatvalue = MAKE_LFIXED(value);
#endif //NUMBERVALUE
	PC_UnreadSourceToken(source, &token);
	if (value < 0) UnreadSignToken(source);
	return qtrue;
...
int PC_DollarDirective_evalfloat(source_t *source)
{
	...
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
	sprintf(token.string, "%1.2f", FIXED_TO_DOUBLE(FIXED_ABS(value)));
	token.type = TT_NUMBER;
	token.subtype = TT_FLOAT|TT_LONG|TT_DECIMAL;
#ifdef NUMBERVALUE
	token.intvalue = FIXED_TO_INT(value);
...
int PC_ExpectTokenType(source_t *source, int type, int subtype, token_t *token)
{
	...
	if (token->type == TT_NUMBER)
	{
		if ((token->subtype & subtype) != subtype)
		{
			...
			if (subtype & TT_FLOAT) strcat(str, " gfixed");
