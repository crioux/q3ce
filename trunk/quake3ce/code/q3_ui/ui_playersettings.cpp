...
static void PlayerSettings_DrawName( void *self ) {
	...
	if( focus ) {
		if ( _UI_trap_Key_GetOverstrikeMode() ) {
			c = 11;
		} else {
			c = 10;
		}
...
static void PlayerSettings_DrawEffects( void *self ) {
	...
	UI_DrawHandlePic( MAKE_GFIXED(item->generic.x + 64), MAKE_GFIXED(item->generic.y + PROP_HEIGHT + 8), GFIXED(128,0), GFIXED(8,0), s_playersettings.fxBasePic );
	UI_DrawHandlePic( MAKE_GFIXED(item->generic.x + 64 + item->curvalue * 16 + 8), MAKE_GFIXED(item->generic.y + PROP_HEIGHT + 6), GFIXED(16,0), GFIXED(12,0), s_playersettings.fxPic[item->curvalue] );
}
...
static void PlayerSettings_DrawPlayer( void *self ) {
	...
	if ( strcmp( buf, s_playersettings.playerModel ) != 0 ) {
		...
		viewangles[YAW]   = AFIXED(180 - 30,0);
		viewangles[PITCH] = AFIXED_0;
		viewangles[ROLL]  = AFIXED_0;
		UI_PlayerInfo_SetInfo( &s_playersettings.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, avec3_origin, WP_MACHINEGUN, qfalse );
	}

	b = (menubitmap_s*) self;
	UI_DrawPlayer( MAKE_GFIXED(b->generic.x), MAKE_GFIXED(b->generic.y), MAKE_GFIXED(b->width), MAKE_GFIXED(b->height), &s_playersettings.playerinfo, uis.realtime/2 );
}
...
static void PlayerSettings_SaveChanges( void ) {
	// name
	_UI_trap_Cvar_Set( "name", s_playersettings.name.field.buffer );

	// handicap
	_UI_trap_Cvar_SetValue( "handicap", MAKE_LFIXED(100 - s_playersettings.handicap.curvalue * 5) );

	// effects color
	_UI_trap_Cvar_SetValue( "color1", MAKE_LFIXED(uitogamecode[s_playersettings.effects.curvalue]) );
}
...
static void PlayerSettings_SetMenuItems( void ) {
	...
	// effects color
	c = FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "color1" )) - 1;
	if( c < 0 || c > 6 ) {
		c = 6;
	}
...
static void PlayerSettings_MenuEvent( void* ptr, int event ) {
	...
	case ID_HANDICAP:
		_UI_trap_Cvar_Set( "handicap", va( "%i", 100 - 25 * s_playersettings.handicap.curvalue ) );
		break;
...
static void PlayerSettings_MenuInit( void ) {
	...
	s_playersettings.banner.string        = strdup("PLAYER SETTINGS");
...
void PlayerSettings_Cache( void ) {
	...
	_UI_trap_R_RegisterShaderNoMip( ART_MODEL0 );
	_UI_trap_R_RegisterShaderNoMip( ART_MODEL1 );
	_UI_trap_R_RegisterShaderNoMip( ART_BACK0 );
	_UI_trap_R_RegisterShaderNoMip( ART_BACK1 );

	s_playersettings.fxBasePic = _UI_trap_R_RegisterShaderNoMip( ART_FX_BASE );
	s_playersettings.fxPic[0] = _UI_trap_R_RegisterShaderNoMip( ART_FX_RED );
	s_playersettings.fxPic[1] = _UI_trap_R_RegisterShaderNoMip( ART_FX_YELLOW );
	s_playersettings.fxPic[2] = _UI_trap_R_RegisterShaderNoMip( ART_FX_GREEN );
	s_playersettings.fxPic[3] = _UI_trap_R_RegisterShaderNoMip( ART_FX_TEAL );
	s_playersettings.fxPic[4] = _UI_trap_R_RegisterShaderNoMip( ART_FX_BLUE );
	s_playersettings.fxPic[5] = _UI_trap_R_RegisterShaderNoMip( ART_FX_CYAN );
	s_playersettings.fxPic[6] = _UI_trap_R_RegisterShaderNoMip( ART_FX_WHITE );
}
