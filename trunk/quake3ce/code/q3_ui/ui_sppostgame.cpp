...
static void UI_SPPostgameMenu_AgainEvent( void* ptr, int event )
{
	...
	UI_PopMenu();
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "map_restart 0\n" );
}
...
static void UI_SPPostgameMenu_MenuEvent( void* ptr, int event )
{
	...
	UI_PopMenu();
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "disconnect; levelselect\n" );
}
...
static sfxHandle_t UI_SPPostgameMenu_MenuKey( int key ) {
	...
	if( postgameMenuInfo.phase == 1 ) {
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "abort_podium\n" );
...
static void UI_SPPostgameMenu_DrawAwardsMedals( int max ) {
	...
	for( n = 0; n < max; n++ ) {
		...
		UI_DrawNamedPic( MAKE_GFIXED(x), MAKE_GFIXED(y), GFIXED(48,0), GFIXED(48,0), ui_medalPicNames[medal] );
...
static void UI_SPPostgameMenu_DrawAwardsPresentation( int timer ) {
	...
	color[3] = MAKE_GFIXED( AWARD_PRESENTATION_TIME - atimer ) / GFIXED(AWARD_PRESENTATION_TIME,0);
	...
	if( !postgameMenuInfo.playedSound[awardNum] ) {
		...
		_UI_trap_S_StartLocalSound( _UI_trap_S_RegisterSound( ui_medalSounds[postgameMenuInfo.awardsEarned[awardNum]], qfalse ), CHAN_ANNOUNCER );
	}
}
...
static void UI_SPPostgameMenu_MenuDrawScoreLine( int n, int y ) {
	...
	_UI_trap_GetConfigString( CS_PLAYERS + postgameMenuInfo.clientNums[n], info, MAX_INFO_STRING );
...
static void UI_SPPostgameMenu_MenuDraw( void ) {
	...
	_UI_trap_GetConfigString( CS_SYSTEMINFO, info, sizeof(info) );
	...
	if( postgameMenuInfo.phase == 1 ) {
		...
		if( timer >= 1000 && postgameMenuInfo.winnerSound ) {
			_UI_trap_S_StartLocalSound( postgameMenuInfo.winnerSound, CHAN_ANNOUNCER );
	...
	if( postgameMenuInfo.phase == 3 ) {
		if( uis.demoversion ) {
			if( postgameMenuInfo.won == 1 && UI_ShowTierVideo( 8 )) {
				_UI_trap_Cvar_Set( "nextmap", "" );
				_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "disconnect; cinematic demoEnd.RoQ\n" );
				return;
			}
		}
			...
			if( postgameMenuInfo.won == postgameMenuInfo.lastTier ) {
				_UI_trap_Cvar_Set( "nextmap", "" );
				_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "disconnect; cinematic end.RoQ\n" );
				return;
			}

			_UI_trap_Cvar_SetValue( "ui_spSelection", MAKE_LFIXED(postgameMenuInfo.won * ARENAS_PER_TIER) );
			_UI_trap_Cvar_Set( "nextmap", "levelselect" );
			_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "disconnect; cinematic tier%i.RoQ\n", postgameMenuInfo.won + 1 ) );
			return;
		}
	...
	// draw the scoreboard
	if( FIXED_IS_ZERO(_UI_trap_Cvar_VariableValue( "ui_spScoreboard" )) ) {
		return;
	}
...
void UI_SPPostgameMenu_Cache( void ) {
	...
	buildscript = FIXED_TO_INT(_UI_trap_Cvar_VariableValue("com_buildscript"));

	_UI_trap_R_RegisterShaderNoMip( ART_MENU0 );
	_UI_trap_R_RegisterShaderNoMip( ART_MENU1 );
	_UI_trap_R_RegisterShaderNoMip( ART_REPLAY0 );
	_UI_trap_R_RegisterShaderNoMip( ART_REPLAY1 );
	_UI_trap_R_RegisterShaderNoMip( ART_NEXT0 );
	_UI_trap_R_RegisterShaderNoMip( ART_NEXT1 );
	for( n = 0; n < 6; n++ ) {
		_UI_trap_R_RegisterShaderNoMip( ui_medalPicNames[n] );
		_UI_trap_S_RegisterSound( ui_medalSounds[n], qfalse );
	}

	if( buildscript ) {
		_UI_trap_S_RegisterSound( "music/loss.wav", qfalse );
		_UI_trap_S_RegisterSound( "music/win.wav", qfalse );
		_UI_trap_S_RegisterSound( "sound/player/announce/youwin.wav", qfalse );
	}
}
...
static void Prepname( int index ) {
	...
	_UI_trap_GetConfigString( CS_PLAYERS + postgameMenuInfo.clientNums[index], info, MAX_INFO_STRING );
	Q_strncpyz( name, Info_ValueForKey( info, "n" ), sizeof(name) );
	Q_CleanStr( name );
	len = strlen( name );
...
void UI_SPPostgameMenu_f( void ) {
	...
	if ( playerGameRank != 1 ) {
		postgameMenuInfo.winnerSound = _UI_trap_S_RegisterSound( va( "sound/player/announce/%s_wins.wav", postgameMenuInfo.placeNames[0] ), qfalse );
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "music music/loss\n" );
	}
	else {
		postgameMenuInfo.winnerSound = _UI_trap_S_RegisterSound( "sound/player/announce/youwin.wav", qfalse );
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "music music/win\n" );
	}
