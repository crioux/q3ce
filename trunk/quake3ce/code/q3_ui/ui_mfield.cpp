...
void MField_Draw( mfield_t *edit, int x, int y, int style, vec4_t color ) {
	...
	if ( drawLen >= MAX_STRING_CHARS ) {
		_UI_trap_Error( "drawLen >= MAX_STRING_CHARS" );
	}
...
void MField_Paste( mfield_t *edit ) {
	...
	_UI_trap_GetClipboardData( pasteBuffer, 64 );
...
void MField_KeyDownEvent( mfield_t *edit, int key ) {
	...
	if ( ( ( key == K_INS ) || ( key == K_KP_INS ) ) && _UI_trap_Key_IsDown( K_SHIFT ) ) {
		MField_Paste( edit );
		return;
	}
	...
	if ( key == K_INS || key == K_KP_INS ) {
		_UI_trap_Key_SetOverstrikeMode( !_UI_trap_Key_GetOverstrikeMode() );
		return;
	}
}
...
void MenuField_Draw( menufield_s *f )
{
	...
	{
		// draw cursor
		UI_FillRect( MAKE_GFIXED(f->generic.left), MAKE_GFIXED(f->generic.top), MAKE_GFIXED(f->generic.right-f->generic.left+1), MAKE_GFIXED(f->generic.bottom-f->generic.top+1), listbar_color ); 
