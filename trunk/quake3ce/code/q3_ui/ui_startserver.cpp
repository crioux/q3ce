...
static void StartServer_MenuEvent( void* ptr, int event ) {
	...
	case ID_STARTSERVERNEXT:
		_UI_trap_Cvar_SetValue( "g_gameType",MAKE_LFIXED(gametype_remap[s_startserver.gametype.curvalue]) );
...
static void StartServer_LevelshotDraw( void *self ) {
	...
	h =	b->height;
	if( b->shader ) {
		UI_DrawHandlePic( MAKE_GFIXED(x), MAKE_GFIXED(y), MAKE_GFIXED(w), MAKE_GFIXED(h), b->shader );
	}

	x = b->generic.x;
	y = b->generic.y + b->height;
	UI_FillRect( MAKE_GFIXED(x), MAKE_GFIXED(y), MAKE_GFIXED(b->width), GFIXED(28,0), colorBlack );
	...
	if( b->generic.flags & QMF_HIGHLIGHT ) {	
		UI_DrawHandlePic( MAKE_GFIXED(x), MAKE_GFIXED(y), MAKE_GFIXED(w), MAKE_GFIXED(h), b->focusshader );
	}
}
...
void StartServer_Cache( void )
{
	...
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_BACK0 );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_BACK1 );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_NEXT0 );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_NEXT1 );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_FRAMEL );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_FRAMER );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_SELECT );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_SELECTED );	
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_UNKNOWNMAP );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_ARROWS );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_ARROWSL );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_ARROWSR );

	precache = FIXED_TO_INT(_UI_trap_Cvar_VariableValue("com_buildscript"));
	...
	for( i = 0; i < s_startserver.nummaps; i++ ) {
		...
		if( precache ) {
			...
			_UI_trap_R_RegisterShaderNoMip(picname);
		}
	}
...
static void ServerOptions_Start( void ) {
	...
	case GT_FFA:
	default:
		_UI_trap_Cvar_SetValue( "ui_ffa_fraglimit", MAKE_LFIXED(fraglimit) );
		_UI_trap_Cvar_SetValue( "ui_ffa_timelimit", MAKE_LFIXED(timelimit) );
		break;

	case GT_TOURNAMENT:
		_UI_trap_Cvar_SetValue( "ui_tourney_fraglimit", MAKE_LFIXED(fraglimit) );
		_UI_trap_Cvar_SetValue( "ui_tourney_timelimit", MAKE_LFIXED(timelimit) );
		break;

	case GT_TEAM:
		_UI_trap_Cvar_SetValue( "ui_team_fraglimit", MAKE_LFIXED(fraglimit) );
		_UI_trap_Cvar_SetValue( "ui_team_timelimit", MAKE_LFIXED(timelimit) );
		_UI_trap_Cvar_SetValue( "ui_team_friendlt", MAKE_LFIXED(friendlyfire) );
		break;

	case GT_CTF:
		_UI_trap_Cvar_SetValue( "ui_ctf_fraglimit", MAKE_LFIXED(fraglimit) );
		_UI_trap_Cvar_SetValue( "ui_ctf_timelimit", MAKE_LFIXED(timelimit) );
		_UI_trap_Cvar_SetValue( "ui_ctf_friendlt", MAKE_LFIXED(friendlyfire) );
		break;
	}

	_UI_trap_Cvar_SetValue( "sv_maxclients", MAKE_LFIXED(Com_Clamp( GFIXED_0, GFIXED(12,0), MAKE_GFIXED(maxclients)) ) );
	_UI_trap_Cvar_SetValue( "dedicated", MAKE_LFIXED(Com_Clamp( GFIXED_0, GFIXED(2,0), MAKE_GFIXED(dedicated)) ) );
	_UI_trap_Cvar_SetValue ("timelimit", MAKE_LFIXED(Com_Clamp( GFIXED_0, MAKE_GFIXED(timelimit), MAKE_GFIXED(timelimit)) ) );
	_UI_trap_Cvar_SetValue ("fraglimit", MAKE_LFIXED(Com_Clamp( GFIXED_0, MAKE_GFIXED(fraglimit), MAKE_GFIXED(fraglimit)) ) );
	_UI_trap_Cvar_SetValue ("capturelimit", MAKE_LFIXED(Com_Clamp( GFIXED_0, MAKE_GFIXED(flaglimit), MAKE_GFIXED(flaglimit)) ) );
	_UI_trap_Cvar_SetValue( "g_friendlyfire", MAKE_LFIXED(friendlyfire) );
	_UI_trap_Cvar_SetValue( "sv_pure", MAKE_LFIXED(pure) );
	_UI_trap_Cvar_Set("sv_hostname", s_serveroptions.hostname.field.buffer );
	
	_UI_trap_Cvar_SetValue( "sv_punkbuster", MAKE_LFIXED(s_serveroptions.punkbuster.curvalue) );

	// the wait commands will allow the dedicated to take effect
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", s_startserver.maplist[s_startserver.currentmap] ) );

	// add bots
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "wait 3\n" );
	for( n = 1; n < PLAYER_SLOTS; n++ ) {
		if( s_serveroptions.playerType[n].curvalue != 1 ) {
			continue;
		}
		...
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, buf );
	}

	// set player's team
	if( dedicated == 0 && s_serveroptions.gametype >= GT_TEAM ) {
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait 5; team %s\n", playerTeam_list[s_serveroptions.playerTeam[0].curvalue] ) );
	}
}
...
static void ServerOptions_InitPlayerItems( void ) {
	...
	if( s_serveroptions.dedicated.curvalue == 0 ) {
		...
		_UI_trap_Cvar_VariableStringBuffer( "name", s_serveroptions.playerNameBuffers[0], sizeof(s_serveroptions.playerNameBuffers[0]) );
...
static void ServerOptions_LevelshotDraw( void *self ) {
	...
	x = b->generic.x;
	y = b->generic.y + b->height;
	UI_FillRect( MAKE_GFIXED(x), MAKE_GFIXED(y), MAKE_GFIXED(b->width), GFIXED(40,0), colorBlack );
...
static void ServerOptions_SetMenuItems( void ) {
	...
	case GT_FFA:
	default:
		Com_sprintf( s_serveroptions.fraglimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_ffa_fraglimit" )) )) );
		Com_sprintf( s_serveroptions.timelimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_ffa_timelimit" )) )) );
		break;

	case GT_TOURNAMENT:
		Com_sprintf( s_serveroptions.fraglimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_tourney_fraglimit" )) )) );
		Com_sprintf( s_serveroptions.timelimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_tourney_timelimit" )) )) );
		break;

	case GT_TEAM:
		Com_sprintf( s_serveroptions.fraglimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_team_fraglimit" )) )) );
		Com_sprintf( s_serveroptions.timelimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_team_timelimit" )) ) ));
		s_serveroptions.friendlyfire.curvalue = FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED_1, MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_team_friendly" )) ));
		break;

	case GT_CTF:
		Com_sprintf( s_serveroptions.flaglimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(100,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_ctf_capturelimit" ))) ) );
		Com_sprintf( s_serveroptions.timelimit.field.buffer, 4, "%i", FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(999,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_ctf_timelimit" )) )) );
		s_serveroptions.friendlyfire.curvalue = FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED_1, MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "ui_ctf_friendly" )) ));
		break;
	}

	Q_strncpyz( s_serveroptions.hostname.field.buffer, UI_Cvar_VariableString( "sv_hostname" ), sizeof( s_serveroptions.hostname.field.buffer ) );
	s_serveroptions.pure.curvalue = FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED_1, MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "sv_pure" )) ));
...
static void ServerOptions_MenuInit( qboolean multiplayer ) {
	...
	s_serveroptions.gametype = FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(5,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "g_gameType" )) ));
	s_serveroptions.punkbuster.curvalue = FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED_1, MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "sv_punkbuster" )) ));
...
void ServerOptions_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_BACK0 );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_BACK1 );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_FIGHT0 );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_FIGHT1 );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_SELECT );
	_UI_trap_R_RegisterShaderNoMip( GAMESERVER_UNKNOWNMAP );
}
...
void UI_BotSelectMenu_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_BACK0 );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_BACK1 );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_ACCEPT0 );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_ACCEPT1 );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_SELECT );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_SELECTED );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_ARROWS );
	_UI_trap_R_RegisterShaderNoMip( BOTSELECT_ARROWSL );
