...
void UI_SPArena_Start( const char *arenaInfo ) {
	...
	n = FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "sv_maxclients" ));
	if ( n < 8 ) {
		_UI_trap_Cvar_SetValue( "sv_maxclients", LFIXED(8 ,0));
	}
	...
	_UI_trap_Cvar_SetValue( "ui_spSelection", MAKE_LFIXED(level) );

	map = Info_ValueForKey( arenaInfo, "map" );
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "spmap %s\n", map ) );
}
