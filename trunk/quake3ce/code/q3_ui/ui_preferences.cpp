...
static void Preferences_SetMenuItems( void ) {
	s_preferences.crosshair.curvalue		= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_drawCrosshair" )) % NUM_CROSSHAIRS;
	s_preferences.simpleitems.curvalue		= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_simpleItems" )) != 0;
	s_preferences.brass.curvalue			= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_brassTime" ) )!= 0;
	s_preferences.wallmarks.curvalue		= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_marks" ) )!= 0;
	s_preferences.identifytarget.curvalue	= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_drawCrosshairNames" )) != 0;
	s_preferences.dynamiclights.curvalue	= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "r_dynamiclight" )) != 0;
	s_preferences.highqualitysky.curvalue	= FIXED_TO_INT(_UI_trap_Cvar_VariableValue ( "r_fastsky" )) == 0;
	s_preferences.synceveryframe.curvalue	= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "r_finish" )) != 0;
	s_preferences.forcemodel.curvalue		= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cg_forcemodel" )) != 0;
	s_preferences.drawteamoverlay.curvalue	= FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(3,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "cg_drawTeamOverlay" )) ));
	s_preferences.allowdownload.curvalue	= FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "cl_allowDownload" )) != 0;
}
...
static void Preferences_Event( void* ptr, int notification ) {
	...
	case ID_CROSSHAIR:
		...
		_UI_trap_Cvar_SetValue( "cg_drawCrosshair", MAKE_LFIXED(s_preferences.crosshair.curvalue) );
		break;

	case ID_SIMPLEITEMS:
		_UI_trap_Cvar_SetValue( "cg_simpleItems", MAKE_LFIXED(s_preferences.simpleitems.curvalue) );
		break;

	case ID_HIGHQUALITYSKY:
		_UI_trap_Cvar_SetValue( "r_fastsky", MAKE_LFIXED(!s_preferences.highqualitysky.curvalue) );
		break;

	case ID_EJECTINGBRASS:
		if ( s_preferences.brass.curvalue )
			_UI_trap_Cvar_Reset( "cg_brassTime" );
		else
			_UI_trap_Cvar_SetValue( "cg_brassTime", LFIXED_0 );
		break;

	case ID_WALLMARKS:
		_UI_trap_Cvar_SetValue( "cg_marks", MAKE_LFIXED(s_preferences.wallmarks.curvalue) );
		break;

	case ID_DYNAMICLIGHTS:
		_UI_trap_Cvar_SetValue( "r_dynamiclight", MAKE_LFIXED(s_preferences.dynamiclights.curvalue) );
		break;		

	case ID_IDENTIFYTARGET:
		_UI_trap_Cvar_SetValue( "cg_drawCrosshairNames", MAKE_LFIXED(s_preferences.identifytarget.curvalue) );
		break;

	case ID_SYNCEVERYFRAME:
		_UI_trap_Cvar_SetValue( "r_finish", MAKE_LFIXED(s_preferences.synceveryframe.curvalue) );
		break;

	case ID_FORCEMODEL:
		_UI_trap_Cvar_SetValue( "cg_forcemodel", MAKE_LFIXED(s_preferences.forcemodel.curvalue) );
		break;

	case ID_DRAWTEAMOVERLAY:
		_UI_trap_Cvar_SetValue( "cg_drawTeamOverlay", MAKE_LFIXED(s_preferences.drawteamoverlay.curvalue) );
		break;

	case ID_ALLOWDOWNLOAD:
		_UI_trap_Cvar_SetValue( "cl_allowDownload", MAKE_LFIXED(s_preferences.allowdownload.curvalue) );
		_UI_trap_Cvar_SetValue( "sv_allowDownload", MAKE_LFIXED(s_preferences.allowdownload.curvalue) );
		break;
...
static void Crosshair_Draw( void *self ) {
	...
	UI_DrawHandlePic( MAKE_GFIXED(x + SMALLCHAR_WIDTH), MAKE_GFIXED(y - 4), GFIXED(24,0), GFIXED(24,0), s_preferences.crosshairShader[s->curvalue] );
}
...
static void Preferences_MenuInit( void ) {
	...
	s_preferences.banner.string	   = strdup("GAME OPTIONS");
...
void Preferences_Cache( void ) {
	...
	for( n = 0; n < NUM_CROSSHAIRS; n++ ) {
		s_preferences.crosshairShader[n] = _UI_trap_R_RegisterShaderNoMip( va("gfx/2d/crosshair%c", 'a' + n ) );
	}
}
