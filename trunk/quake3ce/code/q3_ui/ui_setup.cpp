...
static void UI_SetupMenu_Init( void ) {
	...
	setupMenuInfo.banner.string						= strdup("SETUP");
	...
	setupMenuInfo.setupplayer.string				= strdup("PLAYER");
	...
	setupMenuInfo.setupcontrols.string				= strdup("CONTROLS");
	...
	setupMenuInfo.setupsystem.string				= strdup("SYSTEM");
	...
	setupMenuInfo.game.string						= strdup("GAME OPTIONS");
	...
	setupMenuInfo.cdkey.string						= strdup("CD Key");
	setupMenuInfo.cdkey.color						= color_red;
	setupMenuInfo.cdkey.style						= UI_CENTER;

	if( FIXED_IS_ZERO(_UI_trap_Cvar_VariableValue( "cl_paused" )) ) {
#if 0
		y += SETUP_MENU_VERTICAL_SPACING;
		...
		setupMenuInfo.defaults.string					= strdup("DEFAULTS");
