...
static vec4_t s_signup_color_prompt  = {GFIXED(1,00), GFIXED(0,43), GFIXED(0,00), GFIXED(1,00)};
...
static void Signup_MenuEvent( void* ptr, int event ) {
	...
	case ID_SIGNUP:
		...
		// set name
		//_UI_trap_Cvar_Set( "name", s_signup.name_box.field.buffer );
		/*
		_UI_trap_Cvar_Set( "rank_name", s_signup.name_box.field.buffer );
		_UI_trap_Cvar_Set( "rank_pwd", s_signup.password_box.field.buffer );
		*/
		...
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, cmd );
		*/
		_UI_trap_CL_UI_RankUserCreate(
			s_signup.name_box.field.buffer, 
			s_signup.password_box.field.buffer, 
			s_signup.email_box.field.buffer );
...
void Signup_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( SIGNUP_FRAME );
}
