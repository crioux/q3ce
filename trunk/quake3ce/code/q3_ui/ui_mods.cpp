...
static void UI_Mods_MenuEvent( void *ptr, int event ) {
	...
	case ID_GO:
		_UI_trap_Cvar_Set( "fs_game", s_mods.fs_gameList[s_mods.list.curvalue] );
...
static void UI_Mods_LoadModsFromFile( char *filename ) {
	...
	if ( len >= sizeof(buf) ) {
		_UI_trap_Print( va( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, sizeof(buf) ) );
...
static void UI_Mods_LoadMods( void ) {
	...
	_UI_trap_Print( va( "%i mods parsed\n", s_mods.list.numitems ) );
