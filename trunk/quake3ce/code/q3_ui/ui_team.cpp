...
static void TeamMain_MenuEvent( void* ptr, int event ) {
	...
	case ID_JOINRED:
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "cmd team red\n" );
		UI_ForceMenuOff();
		break;

	case ID_JOINBLUE:
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "cmd team blue\n" );
		UI_ForceMenuOff();
		break;

	case ID_JOINGAME:
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "cmd team free\n" );
		UI_ForceMenuOff();
		break;

	case ID_SPECTATE:
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "cmd team spectator\n" );
		UI_ForceMenuOff();
		break;
	}
}
...
void TeamMain_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( TEAMMAIN_FRAME );
}
