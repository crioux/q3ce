...
static void UI_RemoveBotsMenu_SetBotNames( void ) {
	...
	for ( n = 0; (n < 7) && (removeBotsMenuInfo.baseBotNum + n < removeBotsMenuInfo.numBots); n++ ) {
		_UI_trap_GetConfigString( CS_PLAYERS + removeBotsMenuInfo.botClientNums[removeBotsMenuInfo.baseBotNum + n], info, MAX_INFO_STRING );
...
void UI_RemoveBots_Cache( void ) {
	...
	_UI_trap_R_RegisterShaderNoMip( ART_DELETE0 );
	_UI_trap_R_RegisterShaderNoMip( ART_DELETE1 );
}
...
static void UI_RemoveBotsMenu_Init( void ) {
	...
	removeBotsMenuInfo.banner.string			= strdup("REMOVE BOTS");
