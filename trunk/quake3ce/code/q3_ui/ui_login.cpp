...
static vec4_t s_login_color_prompt  = {GFIXED(1,00), GFIXED(0,43), GFIXED(0,00), GFIXED(1,00)};
...
static void Login_MenuEvent( void* ptr, int event ) {
	...
	case ID_LOGIN:
		// set name								``
		//_UI_trap_Cvar_Set( "name", s_login.name_box.field.buffer );
		/*
		_UI_trap_Cvar_Set( "rank_name", s_login.name_box.field.buffer );
		_UI_trap_Cvar_Set( "rank_pwd", s_login.password_box.field.buffer );
		*/

		// login
		_UI_trap_CL_UI_RankUserLogin(
...
void Login_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( LOGIN_FRAME );
}
