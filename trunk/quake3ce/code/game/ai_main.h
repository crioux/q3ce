...
typedef struct bot_activategoal_s
{
	...
	gfixed time;								//time to activate something
	gfixed start_time;						//time starting to activate something
	gfixed justused_time;					//time the goal was used
	int shoot;								//true if bot has to shoot to activate
	int weapon;								//weapon to be used for activation
	bvec3_t target;							//target to shoot at to activate something
	bvec3_t origin;							//origin of the blocking entity to activate
...
typedef struct bot_state_s
{
	...
	gfixed walker;									//walker charactertic
	gfixed ltime;									//local bot time
	gfixed entergame_time;							//time the bot entered the game
	gfixed ltg_time;									//long term goal time
	gfixed nbg_time;									//nearby goal time
	gfixed respawn_time;								//time the bot takes to respawn
	gfixed respawnchat_time;							//time the bot started a chat during respawn
	gfixed chase_time;								//time the bot will chase the enemy
	gfixed enemyvisible_time;						//time the enemy was last visible
	gfixed check_time;								//time to check for nearby items
	gfixed stand_time;								//time the bot is standing still
	gfixed lastchat_time;							//time the bot last selected a chat
	gfixed kamikaze_time;							//time to check for kamikaze usage
	gfixed invulnerability_time;						//time to check for invulnerability usage
	gfixed standfindenemy_time;						//time to find enemy while standing
	gfixed attackstrafe_time;						//time the bot is strafing in one dir
	gfixed attackcrouch_time;						//time the bot will stop crouching
	gfixed attackchase_time;							//time the bot chases during actual attack
	gfixed attackjump_time;							//time the bot jumped during attack
	gfixed enemysight_time;							//time before reacting to enemy
	gfixed enemydeath_time;							//time the enemy died
	gfixed enemyposition_time;						//time the position and velocity of the enemy were stored
	gfixed defendaway_time;							//time away while defending
	gfixed defendaway_range;							//max travel time away from defend area
	gfixed rushbaseaway_time;						//time away from rushing to the base
	gfixed attackaway_time;							//time away from attacking the enemy base
	gfixed harvestaway_time;							//time away from harvesting
	gfixed ctfroam_time;								//time the bot is roaming in ctf
	gfixed killedenemy_time;							//time the bot killed the enemy
	gfixed arrive_time;								//time arrived (at companion)
	gfixed lastair_time;								//last time the bot had air
	gfixed teleport_time;							//last time the bot teleported
	gfixed camp_time;								//last time camped
	gfixed camp_range;								//camp range
	gfixed weaponchange_time;						//time the bot started changing weapons
	gfixed firethrottlewait_time;					//amount of time to wait
	gfixed firethrottleshoot_time;					//amount of time to shoot
	gfixed notblocked_time;							//last time the bot was not blocked
	gfixed blockedbyavoidspot_time;					//time blocked by an avoid spot
	gfixed predictobstacles_time;					//last time the bot predicted obstacles
	int predictobstacles_goalareanum;				//last goal areanum the bot predicted obstacles for
	bvec3_t aimtarget;
	bvec3_t enemyvelocity;							//enemy velocity GFIXED(0,5) secs ago during battle
	bvec3_t enemyorigin;								//enemy origin GFIXED(0,5) secs ago during battle
	//
	...
	bvec3_t lastenemyorigin;							//last origin of the enemy in the reachability area
	int weaponnum;									//current weapon number
	avec3_t viewangles;								//current view angles
	avec3_t ideal_viewangles;						//ideal view angles
	avec3_t viewanglespeed;
	//
	...
	gfixed order_time;								//time ordered to do something
	int owndecision_time;							//time the bot made it's own decision
	bot_goal_t teamgoal;							//the team goal
	bot_goal_t altroutegoal;						//alternative route goal
	gfixed reachedaltroutegoal_time;					//time the bot reached the alt route goal
	gfixed teammessage_time;							//time to message team mates what the bot is doing
	gfixed teamgoal_time;							//time to stop helping team mate
	gfixed teammatevisible_time;						//last time the team mate was NOT visible
	...
	gfixed lead_time;								//time leading someone
	gfixed leadvisible_time;							//last time the team mate was visible
	gfixed leadmessage_time;							//last time a messaged was sent to the team mate
	gfixed leadbackup_time;							//time backing up towards team mate
	//
	char teamleader[32];							//netname of the team leader
	gfixed askteamleader_time;						//time asked for team leader
	gfixed becometeamleader_time;					//time the bot will become the team leader
	gfixed teamgiveorders_time;						//time to give team orders
	gfixed lastflagcapture_time;						//last time a flag was captured
	...
	bfixed formation_dist;							//formation team mate intervening space
	char formation_teammate[16];					//netname of the team mate the bot uses for relative positioning
	afixed formation_angle;							//angle relative to the formation team mate
	bvec3_t formation_dir;							//the direction the formation is moving in
	bvec3_t formation_origin;						//origin the bot uses for relative positioning
