...
void P_DamageFeedback( gentity_t *player ) {
	...
	count = MAKE_GFIXED(client->damage_blood + client->damage_armor);
	if ( count == GFIXED_0 ) {
		return;		// didn't take any damage
	}

	if ( count > GFIXED(255 ,0)) {
		count = GFIXED(255,0);
	}
	...
	if ( client->damage_fromWorld ) {
		...
		client->ps.damagePitch = FIXED_TO_INT(angles[PITCH]/AFIXED(360,0) * AFIXED(256,0));
		client->ps.damageYaw = FIXED_TO_INT(angles[YAW]/AFIXED(360,0) * AFIXED(256,0));
	}
	...
	client->ps.damageCount = FIXED_TO_INT(count);
...
void	G_TouchTriggers( gentity_t *ent ) {
	...
	trace_t		trace;
	bvec3_t		mins, maxs;
	static bvec3_t	range = { BFIXED(40,0), BFIXED(40,0), BFIXED(52 ,0)};
	...
	num = _G_trap_EntitiesInBox( mins, maxs, touch, MAX_GENTITIES );
	...
	for ( i=0 ; i<num ; i++ ) {
		...
		if ( hit->s.eType == ET_ITEM ) {
			...
			if ( !_G_trap_EntityContact( mins, maxs, hit ) ) {
				continue;
			}
		}
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		pm.trace = _G_trap_Trace;
		pm.pointcontents = _G_trap_PointContents;
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( ! g_inactivity.integer ) {
		...
		if ( level.time > client->inactivityTime ) {
			_G_trap_DropClient( client - level.clients, "Dropped due to inactivity" );
			return qfalse;
		}
		if ( level.time > client->inactivityTime - 10000 && !client->inactivityWarning ) {
			client->inactivityWarning = qtrue;
			_G_trap_SendServerCommand( client - level.clients, "cp \"Ten seconds until inactivity drop!\n\"" );
		}
	}
	return qtrue;
}
	...
	while ( client->timeResidual >= 1000 ) {
		...
		if( maxHealth ) {
			if ( ent->health < maxHealth ) {
				ent->health += 15;
				if ( ent->health > maxHealth * GFIXED(1,1) ) {
					ent->health = maxHealth * GFIXED(1,1);
				}
		...
		if ( client->ps.powerups[PW_REGEN] ) {
			if ( ent->health < client->ps.stats[STAT_MAX_HEALTH]) {
				ent->health += 15;
				if ( ent->health > FIXED_TO_INT(MAKE_GFIXED(client->ps.stats[STAT_MAX_HEALTH]) * GFIXED(1,1)) ) {
					ent->health = FIXED_TO_INT(MAKE_GFIXED(client->ps.stats[STAT_MAX_HEALTH]) * GFIXED(1,1));
				}
				G_AddEvent( ent, EV_POWERUP_REGEN, 0 );
...
//	qboolean	fired;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_FALL_FAR:
			...
			VectorSet (dir, AFIXED_0, AFIXED_0, AFIXED_1);
		...
		case EV_USE_ITEM1:		// teleporter
			...
			if ( item ) {
				...
				drop->count = FIXED_TO_INT(MSECTIME_G( ent->client->ps.powerups[ j ] - level.time ));
				if ( drop->count < 1 ) {
					drop->count = 1;
				}
...
void BotTestSolid(bvec3_t origin);
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if ( pmove_msec.integer < 8 ) {
		_G_trap_Cvar_Set("pmove_msec", "8");
	}
	else if (pmove_msec.integer > 33) {
		_G_trap_Cvar_Set("pmove_msec", "33");
	}
	...
	client->ps.gravity = FIXED_TO_INT(g_gravity.value);

	// set speed
	client->ps.speed = FIXED_TO_INT(g_speed.value);

#ifdef MISSIONPACK
	if( bg_itemlist[client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_SCOUT ) {
		client->ps.speed *= GFIXED(1,5);
	}
	else
#endif
	if ( client->ps.powerups[PW_HASTE] ) {
		client->ps.speed = FIXED_TO_INT(MAKE_GFIXED(client->ps.speed) * GFIXED(1,3));
	}
	...
	if (client->ps.powerups[PW_INVULNERABILITY] ) {
		if ( !(client->ps.pm_flags & PMF_INVULEXPAND) ) {
			bvec3_t mins = { -42, -42, -42 };
			bvec3_t maxs = { 42, 42, 42 };
			bvec3_t oldmins, oldmaxs;
		...
		if (level.intermissionQueued != 0 && g_singlePlayer.integer) {
			if ( level.time - level.intermissionQueued >= 1000  ) {
				...
				if ( level.time - level.intermissionQueued >= 2000 && level.time - level.intermissionQueued <= 2500 ) {
					_G_trap_SendConsoleCommand( EXEC_APPEND, "centerview\n");
				}
...
void ClientThink( int clientNum ) {
	...
	_G_trap_GetUsercmd( clientNum, &ent->client->pers.cmd );
...
//	i = _G_trap_AAS_PointReachabilityAreaIndex( ent->client->ps.origin );
