...
bvec3_t lastteleport_origin;		//last teleport event origin
gfixed lastteleport_time;		//last teleport event time
...
void BotSetUserInfo(bot_state_t *bs, const char *key, const char *value) {
	...
	_G_trap_SetUserinfo(bs->client, userinfo);
...
int BotTeam(bot_state_t *bs) {
	...
	_G_trap_GetConfigstring(CS_PLAYERS+bs->client, info, sizeof(info));
	//
...
int BotSetLastOrderedTask(bot_state_t *bs) {
	...
	if ( bs->lastgoal_ltgtype ) {
		...
		bs->teamgoal_time = FloatTime() + GFIXED(300,0);
		BotSetTeamStatus(bs);
		//
		if ( gametype == GT_CTF ) {
			if ( bs->ltgtype == LTG_GETFLAG ) {
				...
				tb = BotTeamFlag(bs);
				eb = BotEnemyFlag(bs);
				tt = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, tb->areanum, TFL_DEFAULT);
				et = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, eb->areanum, TFL_DEFAULT);
...
void BotRefuseOrder(bot_state_t *bs) {
	...
	if (FIXED_NOT_ZERO(bs->order_time) && bs->order_time > FloatTime() - GFIXED(10 ,0)) {
		_G_trap_EA_Action(bs->client, ACTION_NEGATIVE);
		BotVoiceChat(bs, bs->decisionmaker, VOICECHAT_NO);
		bs->order_time = GFIXED_0;
	}
}
...
void BotCTFSeekGoals(bot_state_t *bs) {
	...
	if (BotCTFCarryingFlag(bs)) {
		...
		if (bs->ltgtype != LTG_RUSHBASE) {
			...
			switch(BotTeam(bs)) {
				...
				default: VectorSet(dir, BFIXED(999,0), BFIXED(999,0), BFIXED(999,0)); break;
			}
			// if the bot picked up the flag very close to the enemy base
			if ( FIXED_VEC3LEN(dir) < BFIXED(128 ,0)) {
	...
	if (flagstatus == 1) {
		//
		if (MAKE_GFIXED(bs->owndecision_time) < FloatTime()) {
			...
			if (!(bs->ltgtype == LTG_DEFENDKEYAREA &&
				...
				if (c >= 0 &&
					...
					//no message
					bs->teammessage_time = GFIXED_0;
					//no arrive message
					bs->arrive_time = GFIXED_1;
					//
					BotVoiceChat(bs, bs->teammate, VOICECHAT_ONFOLLOW);
					//get the team goal time
					bs->teamgoal_time = FloatTime() + GFIXED(TEAM_ACCOMPANY_TIME,0);
					bs->ltgtype = LTG_TEAMACCOMPANY;
					bs->formation_dist = BFIXED(3,5) * BFIXED(32,0);		//GFIXED(3,5) meter
					BotSetTeamStatus(bs);
					bs->owndecision_time = FIXED_TO_INT( FloatTime() + GFIXED(5,0) );
				}
			}
		}
		return;
	...
	if (BotAggression(bs) < GFIXED(50,0))
		return;
	...
	if (rnd < l1 && ctf_redflag.areanum && ctf_blueflag.areanum) {
		...
		bs->teamgoal_time = FloatTime() + GFIXED(TEAM_DEFENDKEYAREA_TIME,0);
		...
		bs->ctfroam_time = FloatTime() + GFIXED(CTF_ROAM_TIME,0);
		BotSetTeamStatus(bs);
	}
	bs->owndecision_time = FIXED_TO_INT(FloatTime() + GFIXED(5,0));
#ifdef DEBUG
		...
		if (bs->owndecision_time < FloatTime()) {
			...
			if (bs->ltgtype != LTG_TEAMACCOMPANY) {
				...
				if (c >= 0) {
					...
					bs->ltgtype = LTG_TEAMACCOMPANY;
					bs->formation_dist = GFIXED(3,5) * GFIXED(32,0);		//GFIXED(3,5) meter
					BotSetTeamStatus(bs);
...
int BotPointAreaNum(bvec3_t origin) {
	...
	areanum = _G_trap_AAS_PointAreaNum(origin);
	if (areanum) return areanum;
	VectorCopy(origin, end);
	end[2] += BFIXED(10,0);
	numareas = _G_trap_AAS_TraceAreas(origin, end, areas, NULL, 10);
...
const char *ClientName(int client, char *name, int size) {
	...
	_G_trap_GetConfigstring(CS_PLAYERS+client, buf, sizeof(buf));
...
void BotChooseWeapon(bot_state_t *bs) {
	...
	if (bs->cur_ps.weaponstate == WEAPON_RAISING ||
		...
		newweaponnum = _G_trap_BotChooseBestFightWeapon(bs->ws, bs->inventory);
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	initmove.viewoffset[2] += MAKE_BFIXED(bs->cur_ps.viewheight);
	...
	//
	if (bs->walker > GFIXED(0,5)) initmove.or_moveflags |= MFL_WALK;
	//
	VectorCopy(bs->viewangles, initmove.viewangles);
	//
	_G_trap_BotInitMoveState(bs->ms, &initmove);
}
		...
		if (offence) {
			if (!(bs->teamtaskpreference & TEAMTP_ATTACKER)) {
				...
				if (BotTeamLeader(bs)) {
					...
					//_G_trap_BotEnterChat(bs->cs, leader, CHAT_TELL);
				}
...
void BotUpdateBattleInventory(bot_state_t *bs, int enemy) {
	...
	bs->inventory[ENEMY_HEIGHT] = FIXED_TO_INT(dir[2]);
	dir[2] = BFIXED_0;
	bs->inventory[ENEMY_HORIZONTAL_DIST] = FIXED_TO_INT(FIXED_VEC3LEN(dir));
...
void BotUseKamikaze(bot_state_t *bs) {
	...
	if (bs->kamikaze_time > FloatTime())
		return;
	bs->kamikaze_time = FloatTime() + GFIXED(0,2);
	if (gametype == GT_CTF) {
		...
		if (c >= 0) {
			...
			VectorSubtract(entinfo.origin, bs->origin, dir);
			if (FIXED_VEC3LEN_SQ(dir) < Square(KAMIKAZE_DIST))
				return;
		}
		c = BotEnemyFlagCarrierVisible(bs);
		if (c >= 0) {
		...
		if (FIXED_VEC3LEN_SQ(dir) < Square(KAMIKAZE_DIST * GFIXED(0,9))) {
...
void BotUseInvulnerability(bot_state_t *bs) {
	...
	if (bs->invulnerability_time > FloatTime())
		return;
	bs->invulnerability_time = FloatTime() + GFIXED(0,2);
	if (gametype == GT_CTF) {
		//never use kamikaze if the team flag carrier is visible
		if (BotCTFCarryingFlag(bs))
			return;
		...
		target[2] += 1;
		VectorSubtract(bs->origin, target, dir);
		if (FIXED_VEC3LEN_SQ(dir) < Square(200)) {
			BotAI_Trace(&trace, bs->eye, NULL, NULL, target, bs->client, CONTENTS_SOLID);
			if (trace.fraction >= 1 || trace.ent == goal->entitynum) {
				_G_trap_EA_Use(bs->client);
				return;
		...
		target[2] += 1;
		VectorSubtract(bs->origin, target, dir);
		if (FIXED_VEC3LEN_SQ(dir) < Square(300)) {
			BotAI_Trace(&trace, bs->eye, NULL, NULL, target, bs->client, CONTENTS_SOLID);
			if (trace.fraction >= 1 || trace.ent == goal->entitynum) {
				_G_trap_EA_Use(bs->client);
				return;
...
qboolean BotIsObserver(bot_state_t *bs) {
	...
	_G_trap_GetConfigstring(CS_PLAYERS+bs->client, buf, sizeof(buf));
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
	...
	feet[2] -= BFIXED(23,0);
	return (_G_trap_AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME));
}
...
==================
*/
gfixed BotAggression(bot_state_t *bs) {
	//if the bot has quad
...
==================
*/
gfixed BotFeelingBad(bot_state_t *bs) {
	if (bs->weaponnum == WP_GAUNTLET) {
		return GFIXED(100,0);
	}
	if (bs->inventory[INVENTORY_HEALTH] < 40) {
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	rocketjumper = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_WEAPONJUMPING, GFIXED_0, GFIXED_1);
	if (rocketjumper < GFIXED(0,5)) return qfalse;
	return qtrue;
}
...
void BotGoCamp(bot_state_t *bs, bot_goal_t *goal) {
	...
	//get the team goal time
	camper = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CAMPER, GFIXED_0, GFIXED_1);
	if (camper > GFIXED(0,99)) bs->teamgoal_time = FloatTime() + GFIXED(10000,0);//GFIXED(99999,0);
	else bs->teamgoal_time = FloatTime() + GFIXED(120 ,0)+ GFIXED(180 ,0)* camper + random() * GFIXED(15,0);
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	camper = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CAMPER, GFIXED_0, GFIXED_1);
	if (camper < GFIXED(0,1)) return qfalse;
	...
	//if camped recently
	if (bs->camp_time > FloatTime() - GFIXED(60 ,0)+ GFIXED(300 ,0)* (GFIXED_1-camper)) return qfalse;
	//
	if (random() > camper) {
		bs->camp_time = FloatTime();
		return qfalse;
	...
	if (BotAggression(bs) < GFIXED(50,0)) return qfalse;
	...
	for (cs = _G_trap_BotGetNextCampSpotGoal(0, &goal); cs; cs = _G_trap_BotGetNextCampSpotGoal(cs, &goal)) {
		traveltime = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal.areanum, TFL_DEFAULT);
...
void BotDontAvoid(bot_state_t *bs, const char *itemname) {
	...
	num = _G_trap_BotGetLevelItemGoal(-1, itemname, &goal);
	while(num >= 0) {
		_G_trap_BotRemoveFromAvoidGoals(bs->gs, goal.number);
		num = _G_trap_BotGetLevelItemGoal(num, itemname, &goal);
	}
}
...
==================
*/
void BotRoamGoal(bot_state_t *bs, bvec3_t goal) {
	int pc, i;
	bfixed len;
	gfixed rnd;
	avec3_t dir;
	bvec3_t tmp, bestorg, belowbestorg;
	bsp_trace_t trace;
	...
	for (i = 0; i < 10; i++) {
		...
		if (rnd > GFIXED(0,25)) {
			...
			if (random() < GFIXED(0,5)) bestorg[0] -= BFIXED(800 ,0)* random_b() + BFIXED(100,0);
			else bestorg[0] += BFIXED(800 ,0)* random_b() + BFIXED(100,0);
		}
		if (rnd < GFIXED(0,75)) {
			//add a random value to the y-coordinate
			if (random() < GFIXED(0,5)) bestorg[1] -= BFIXED(800 ,0)* random_b() + BFIXED(100,0);
			else bestorg[1] += BFIXED(800 ,0)* random_b() + BFIXED(100,0);
		}
		//add a random value to the z-coordinate (NOTE: 48 = maxjump?)
		bestorg[2] += BFIXED(2 ,0)* BFIXED(48 ,0)* crandom_b();
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	gfixed attack_skill, jumper, croucher, strafechange_time;
	bfixed attack_dist, attack_range, dist;
	avec3_t forward, backward, sideward, hordir, up = {AFIXED_0, AFIXED_0, AFIXED_1};
	aas_entityinfo_t entinfo;
	...
	if (bs->attackchase_time > FloatTime()) {
		...
		VectorSet(goal.mins, -BFIXED(8,0), -BFIXED(8,0), -BFIXED(8,0));
		VectorSet(goal.maxs, BFIXED(8,0), BFIXED(8,0), BFIXED(8,0));
		//initialize the movement state
		BotSetupForMovement(bs);
		//move towards the goal
		_G_trap_BotMoveToGoal(&moveresult, bs->ms, &goal, tfl);
		return moveresult;
	}
	//
	memset(&moveresult, 0, sizeof(bot_moveresult_t));
	//
	attack_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_ATTACK_SKILL, GFIXED_0, GFIXED_1);
	jumper = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_JUMPER, GFIXED_0, GFIXED_1);
	croucher = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CROUCHER, GFIXED_0, GFIXED_1);
	//if the bot is really stupid
	if (attack_skill < GFIXED(0,2)) return moveresult;
	//initialize the movement state
	BotSetupForMovement(bs);
	...
	bvec3_t tmp;
	VectorSubtract(entinfo.origin, bs->origin, tmp);
	//the distance towards the enemy
	dist = VectorNormalizeB2A(tmp,forward);
	VectorNegate(forward, backward);
	//walk, crouch or jump
	movetype = MOVE_WALK;
	//
	if (bs->attackcrouch_time < FloatTime() - GFIXED_1) {
		if (random() < jumper) {
			movetype = MOVE_JUMP;
		}
		//wait at least one second before crouching again
		else if (bs->attackcrouch_time < FloatTime() - GFIXED_1 && random() < croucher) {
			bs->attackcrouch_time = FloatTime() + croucher * GFIXED(5,0);
		}
	}
	...
	if (movetype == MOVE_JUMP) {
		...
		if (bs->attackjump_time > FloatTime()) {
			...
			bs->attackjump_time = FloatTime() + GFIXED_1;
		}
	}
	if (bs->cur_ps.weapon == WP_GAUNTLET) {
		attack_dist = BFIXED_0;
		attack_range = BFIXED_0;
	}
	else {
		attack_dist = BFIXED(IDEAL_ATTACKDIST,0);
		attack_range = BFIXED(40,0);
	}
	//if the bot is stupid
	if (attack_skill <= GFIXED(0,4)) {
		//just walk to or away from the enemy
		if (dist > attack_dist + attack_range) {
			if (_G_trap_BotMoveInDirection(bs->ms, forward, BFIXED(400,0), movetype)) return moveresult;
		}
		if (dist < attack_dist - attack_range) {
			if (_G_trap_BotMoveInDirection(bs->ms, backward, BFIXED(400,0), movetype)) return moveresult;
		}
		return moveresult;
	}
	...
	strafechange_time = GFIXED(0,4) + (GFIXED_1 - attack_skill) * GFIXED(0,2);
	if (attack_skill > GFIXED(0,7)) strafechange_time += crandom() * GFIXED(0,2);
	//if the strafe direction should be changed
	if (bs->attackstrafe_time > strafechange_time) {
		//some magic number :)
		if (random() > GFIXED(0,935)) {
			//flip the strafe direction
			bs->flags ^= BFL_STRAFERIGHT;
			bs->attackstrafe_time = GFIXED_0;
		}
	}
	//
	for (i = 0; i < 2; i++) {
		...
		if (_G_trap_BotMoveInDirection(bs->ms, sideward, BFIXED(400,0), movetype))
			return moveresult;
...
//	bs->attackchase_time = AAS_Time() + GFIXED(6,0);
	return moveresult;
}
...
int BotSameTeam(bot_state_t *bs, int entnum) {
	...
	if ( gametype >= GT_TEAM ) {
		_G_trap_GetConfigstring(CS_PLAYERS+bs->client, info1, sizeof(info1));
		_G_trap_GetConfigstring(CS_PLAYERS+entnum, info2, sizeof(info2));
		//
...
==================
*/
qboolean InFieldOfVision(avec3_t viewangles, afixed fov, avec3_t angles)
{
	int i;
	afixed diff, angle;
	...
	for (i = 0; i < 2; i++) {
		...
		if (diff > AFIXED_0) {
			if (diff > fov * AFIXED(0,5)) return qfalse;
		}
		else {
			if (diff < -fov * AFIXED(0,5)) return qfalse;
		}
...
==================
*/
bfixed BotEntityVisible(int viewer, bvec3_t eye, avec3_t viewangles, afixed fov, int ent) {
	int i, contents_mask, passent, hitent, infog, inwater, otherinfog, pc;
	bfixed waterfactor, vis, bestvis;
	lfixed squaredfogdist;
	bsp_trace_t trace;
	aas_entityinfo_t entinfo;
	bvec3_t dir;
	bvec3_t start, end, middle;
	avec3_t entangles;
	lfixed lvec[3];
	...
	FIXED_VEC3SCALE(middle, BFIXED(0,5), middle);
	VectorAdd(entinfo.origin, middle, middle);
	//check if entity is within field of vision
	VectorSubtract(middle, eye, dir);
	vectoangles(dir, entangles);
	if (!InFieldOfVision(viewangles, fov, entangles)) return BFIXED_0;
	//
	pc = _G_trap_AAS_PointContents(eye);
	...
	for (i = 0; i < 3; i++) {
		...
		if (_G_trap_AAS_PointContents(middle) & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER)) {
		...
		if (trace.contents & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER)) {
			...
			if (1) {
				...
				waterfactor = BFIXED(0,5);
			}
		}
		//if a full trace or the hitent was hit
		if (trace.fraction >= GFIXED_1 || trace.ent == hitent) {
			//check for fog, assuming there's only one fog brush where
			//either the viewer or the entity is in or both are in
			otherinfog = (_G_trap_AAS_PointContents(middle) & CONTENTS_FOG);
			if (infog && otherinfog) {
				VectorSubtract(trace.endpos, eye, dir);
				lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
				squaredfogdist = FIXED_VEC3LEN_SQ(lvec);
			}
			else if (infog) {
				...
				squaredfogdist = LFIXED_0;
			}
			//decrease visibility with the view distance through fog
			vis = BFIXED_1 / (MAKE_BFIXED(squaredfogdist * LFIXED(0,001)) < BFIXED_1 ? BFIXED_1 : MAKE_BFIXED(squaredfogdist * LFIXED(0,001)));
			//if entering water visibility is reduced
			vis *= waterfactor;
			//
			if (vis > bestvis) bestvis = vis;
			//if pretty much no fog
			if (bestvis >= BFIXED(0,95)) return bestvis;
		}
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	gfixed alertness, easyfragger;
	bfixed vis;
	afixed f;
	lfixed squaredist, cursquaredist;
	aas_entityinfo_t entinfo, curenemyinfo;
	bvec3_t dir;
	lfixed lvec[3];
	avec3_t angles;

	alertness = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_ALERTNESS, GFIXED_0, GFIXED_1);
	easyfragger = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_EASY_FRAGGER, GFIXED_0, GFIXED_1);
	...
	if (curenemy >= 0) {
		...
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		cursquaredist = FIXED_VEC3LEN_SQ(lvec);
	}
	else {
		cursquaredist = LFIXED_0;
	}
#ifdef MISSIONPACK
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		if (easyfragger < GFIXED(0,5) && EntityIsChatting(&entinfo)) continue;
		//
		if (lastteleport_time > FloatTime() - GFIXED(3,0)) {
			VectorSubtract(entinfo.origin, lastteleport_origin, dir);
			lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
			squaredist = FIXED_VEC3LEN_SQ(lvec);
			if(squaredist < Square(LFIXED(70,0))) continue;
		}
		...
		//if the bot has no
		if (squaredist > Square(LFIXED(900,0) + MAKE_LFIXED(alertness) * LFIXED(4000,0))) continue;
		//if on the same team
		...
		if (curenemy < 0 && (healthdecrease || EntityIsShooting(&entinfo)))
			...
			f = AFIXED(90 ,0)+ AFIXED(90 ,0)- (AFIXED(90 ,0)-MAKE_AFIXED((squaredist > Square(LFIXED(810,0)) ? Square(LFIXED(810,0)) : squaredist) / (LFIXED(810 ,0)* LFIXED(9,0))));
		//check if the enemy is visible
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, f, i);
		if (vis <= BFIXED_0) continue;
		//if the enemy is quite far away, not shooting and the bot is not damaged
		if (curenemy < 0 && squaredist > Square(LFIXED(100,0)) && !healthdecrease && !EntityIsShooting(&entinfo))
		{
			//check if we can avoid this enemy
			VectorSubtract(bs->origin, entinfo.origin, dir);
			vectoangles(dir, angles);
			//if the bot isn't in the fov of the enemy
			if (!InFieldOfVision(entinfo.angles, AFIXED(90,0), angles)) {
		...
		if (curenemy >= 0) bs->enemysight_time = FloatTime() - GFIXED(2,0);
		else bs->enemysight_time = FloatTime();
		bs->enemysuicide = qfalse;
		bs->enemydeath_time = GFIXED_0;
		bs->enemyvisible_time = FloatTime();
		return qtrue;
	}
	return qfalse;
...
int BotTeamFlagCarrierVisible(bot_state_t *bs) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), i);
		if (vis <= BFIXED_0)
			continue;
		//
		return i;
...
==================
*/
void BotVisibleTeamMatesAndEnemies(bot_state_t *bs, int *teammates, int *enemies, gfixed range) {
	int i;
	bfixed vis;
	aas_entityinfo_t entinfo;
	bvec3_t dir;
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		VectorSubtract(entinfo.origin, bs->origin, dir);
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		if (FIXED_VEC3LEN_SQ(lvec) > Square(MAKE_LFIXED(range)))
			continue;
		//if the flag carrier is not visible
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), i);
		if (vis <= BFIXED_0)
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	bfixed dist;
	gfixed speed;
	gfixed aim_skill, aim_accuracy, reactiontime, f;
	bvec3_t dir, bestorigin, end, start, groundtarget, cmdmove, enemyvelocity;
	bvec3_t mins = {-BFIXED(4,0),-BFIXED(4,0),-BFIXED(4,0)}, maxs = {BFIXED(4,0), BFIXED(4,0), BFIXED(4,0)};
	weaponinfo_t wi;
	aas_entityinfo_t entinfo;
	bot_goal_t goal;
	bsp_trace_t trace;
	...
	//
	aim_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL, GFIXED_0, GFIXED_1);
	aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY,GFIXED_0, GFIXED_1);
	//
	if (aim_skill > GFIXED(0,95)) {
		//don't aim too early
		reactiontime = GFIXED(0,5) * _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, GFIXED_0, GFIXED_1);
		if (bs->enemysight_time > FloatTime() - reactiontime) return;
		if (bs->teleport_time > FloatTime() - reactiontime) return;
	}

	//get the weapon information
	_G_trap_BotGetWeaponInfo(bs->ws, bs->weaponnum, &wi);
	//get the weapon specific aim accuracy and or aim skill
	if (wi.number == WP_MACHINEGUN) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_SHOTGUN) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_GRENADE_LAUNCHER) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADELAUNCHER, GFIXED_0, GFIXED_1);
		aim_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_GRENADELAUNCHER, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_ROCKET_LAUNCHER) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_ROCKETLAUNCHER, GFIXED_0, GFIXED_1);
		aim_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_ROCKETLAUNCHER, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_LIGHTNING) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_RAILGUN) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_PLASMAGUN) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_PLASMAGUN, GFIXED_0, GFIXED_1);
		aim_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_PLASMAGUN, GFIXED_0, GFIXED_1);
	}
	else if (wi.number == WP_BFG) {
		aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BFG10K, GFIXED_0, GFIXED_1);
		aim_skill = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BFG10K, GFIXED_0, GFIXED_1);
	}
	//
	if (aim_accuracy <= GFIXED_0) aim_accuracy = GFIXED(0,0001);
	//get the enemy entity information
	BotEntityInfo(bs->enemy, &entinfo);
	//if the enemy is invisible then shoot crappy most of the time
	if (EntityIsInvisible(&entinfo)) {
		if (random() > GFIXED(0,1)) aim_accuracy *= GFIXED(0,4);
	}
	//
	VectorSubtract(entinfo.origin, entinfo.lastvisorigin, enemyvelocity);
	FIXED_VEC3SCALE(enemyvelocity, GFIXED_1 / entinfo.update_time, enemyvelocity);
	//enemy origin and velocity is remembered every GFIXED(0,5) seconds
	if (bs->enemyposition_time < FloatTime()) {
		//
		bs->enemyposition_time = FloatTime() + GFIXED(0,5);
		VectorCopy(enemyvelocity, bs->enemyvelocity);
		VectorCopy(entinfo.origin, bs->enemyorigin);
	}
	//if not extremely skilled
	if (aim_skill < GFIXED(0,9)) {
		VectorSubtract(entinfo.origin, bs->enemyorigin, dir);
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		//if the enemy moved a bit
		if (FIXED_VEC3LEN_SQ(lvec) > Square(LFIXED(48,0))) {
			//if the enemy changed direction
			if (DotProductLong(bs->enemyvelocity, enemyvelocity) < LFIXED_0) {
				//aim accuracy should be worse now
				aim_accuracy *= GFIXED(0,7);
			}
		}
	}
	//check visibility of enemy
	enemyvisible = FIXED_TO_INT(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->enemy));
	//if the enemy is visible
	if (enemyvisible) {
		//
		VectorCopy(entinfo.origin, bestorigin);
		bestorigin[2] += BFIXED(8,0);
		//get the start point shooting from
		//NOTE: the x and y projectile start offsets are ignored
		VectorCopy(bs->origin, start);
		start[2] += MAKE_BFIXED(bs->cur_ps.viewheight);
		start[2] += wi.offset[2];
		//
		BotAI_Trace(&trace, start, mins, maxs, bestorigin, bs->entitynum, MASK_SHOT);
		//if the enemy is NOT hit
		if (trace.fraction <= GFIXED_1 && trace.ent != entinfo.number) {
			bestorigin[2] += BFIXED(16,0);
		}
		//if it is not an instant hit weapon the bot might want to predict the enemy
		if (FIXED_NOT_ZERO(wi.speed)) {
			//
			...
			if (!(dist > BFIXED(100,0) && FIXED_VEC3LEN_SQ(lvec) < Square(LFIXED(32,0)))) {
				//if skilled anough do exact prediction
				if (aim_skill > GFIXED(0,8) &&
					...
					//
					FIXED_VEC3SCALE(dir,GFIXED_1 / entinfo.update_time, dir);
					//
					...
					_G_trap_AAS_PredictClientMovement(&move, bs->enemy, origin,
														PRESENCE_CROUCH, qfalse,
														dir, cmdmove, 0,
														FIXED_TO_INT(dist * (GFIXED(10,0) / wi.speed)), GFIXED(0,1), 0, 0, qfalse);
					VectorCopy(move.endpos, bestorigin);
					//BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f, frames = %f\n", FIXED_TO_DOUBLE(FloatTime()), FIXED_TO_DOUBLE(FIXED_VEC3LEN(dir)), FIXED_TO_DOUBLE(dist * 10 / wi.speed));
				}
				//if not that skilled do linear prediction
				else if (aim_skill > GFIXED(0,4)) {
					VectorSubtract(entinfo.origin, bs->origin, dir);
					//distance towards the enemy
					dist = FIXED_VEC3LEN(dir);
					//direction the enemy is moving in
					...
					dir[2] = BFIXED_0;
					//
					speed = FIXED_RATIO_G(VectorNormalize(dir),MAKE_BFIXED(entinfo.update_time));
					//botimport.Print(PRT_MESSAGE, "speed = %f, wi->speed = %f\n", FIXED_TO_DOUBLE(speed), FIXED_TO_DOUBLE(wi->speed));
					//best spot to aim at
					FIXED_VEC3MA(entinfo.origin, dist * (speed / wi.speed), dir, bestorigin);
				}
			}
		}
		//if the projectile does radial damage
		if (aim_skill > GFIXED(0,6) && wi.proj.damagetype & DAMAGETYPE_RADIAL) {
			//if the enemy isn't standing significantly higher than the bot
			if (entinfo.origin[2] < bs->origin[2] + BFIXED(16,0)) {
				...
				if (trace.startsolid) groundtarget[2] = entinfo.origin[2] - BFIXED(16,0);
				else groundtarget[2] = trace.endpos[2] - BFIXED(8,0);
				...
				if (FIXED_ABS(trace.endpos[2] - groundtarget[2]) < BFIXED(50,0)) {
					VectorSubtract(trace.endpos, groundtarget, dir);
					lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
					//if the hitpoint is near anough the ground target
					if (FIXED_VEC3LEN_SQ(lvec) < Square(LFIXED(60,0))) {
						VectorSubtract(trace.endpos, start, dir);
						lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
						//if the hitpoint is far anough from the bot
						if (FIXED_VEC3LEN_SQ(lvec) > Square(LFIXED(100,0))) {
							//check if the bot is visible from the ground target
							trace.endpos[2] += BFIXED_1;
		...
		bestorigin[0] += BFIXED(20 ,0)* crandom() * (GFIXED_1 - aim_accuracy);
		bestorigin[1] += BFIXED(20 ,0)* crandom() * (GFIXED_1 - aim_accuracy);
		bestorigin[2] += BFIXED(10 ,0)* crandom() * (GFIXED_1 - aim_accuracy);
	}
	else {
		//
		...
		if (aim_skill > GFIXED(0,5)) {
			...
			if (wi.number == WP_BFG ||
				...
				VectorSet(goal.maxs, BFIXED(8,0), BFIXED(8,0), BFIXED(8,0));
				//
				if (_G_trap_BotPredictVisiblePosition(bs->lastenemyorigin, bs->lastenemyareanum, &goal, TFL_DEFAULT, target)) {
					VectorSubtract(target, bs->eye, dir);
					lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
					if (FIXED_VEC3LEN_SQ(lvec) > Square(LFIXED(80,0))) {
						VectorCopy(target, bestorigin);
						bestorigin[2] -= BFIXED(20,0);
					}
				}
				aim_accuracy = GFIXED_1;
			}
		}
	}
	//
	...
	if (wi.number == WP_MACHINEGUN ||
		...
		//distance towards the enemy
		dist = FIXED_VEC3LEN(dir);
		if (dist > BFIXED(150,0)) dist = BFIXED(150,0);
		f = GFIXED(0,6) + FIXED_RATIO_G(dist, BFIXED(150 ,0)) * GFIXED(0,4);
		aim_accuracy *= f;
	}
	//add some random stuff to the aim direction depending on the aim accuracy
	if (aim_accuracy < GFIXED(0,8)) {
		VectorNormalize(dir);
		for (i = 0; i < 3; i++) dir[i] += BFIXED(0,3) * crandom() * (GFIXED_1 - aim_accuracy);
	}
	//set the ideal view angles
	vectoangles(dir, bs->ideal_viewangles);
	//take the weapon spread into account for lower skilled bots
	bs->ideal_viewangles[PITCH] += AFIXED(6 ,0) * MAKE_AFIXED(wi.vspread * crandom() * (GFIXED_1 - aim_accuracy));
	bs->ideal_viewangles[PITCH] = AngleMod(bs->ideal_viewangles[PITCH]);
	bs->ideal_viewangles[YAW] += AFIXED(6 ,0) * MAKE_AFIXED(wi.hspread * crandom() * (GFIXED_1 - aim_accuracy));
	bs->ideal_viewangles[YAW] = AngleMod(bs->ideal_viewangles[YAW]);
	//if the bots should be really challenging
	if (bot_challenge.integer) {
		//if the bot is really accurate and has the enemy in view for some time
		if (aim_accuracy > GFIXED(0,9) && bs->enemysight_time < FloatTime() - GFIXED_1) {
...
void BotCheckAttack(bot_state_t *bs) {
	gfixed points, reactiontime, firethrottle;
	afixed fov;
	int attackentity;
	bsp_trace_t bsptrace;
	//gfixed selfpreservation;
	avec3_t forward, right;
	bvec3_t start, end, dir;
	avec3_t angles;
	weaponinfo_t wi;
	bsp_trace_t trace;
	aas_entityinfo_t entinfo;
	...
	//
	reactiontime = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, GFIXED_0, GFIXED_1);
	if (bs->enemysight_time > FloatTime() - reactiontime) return;
	if (bs->teleport_time > FloatTime() - reactiontime) return;
	//if changing weapons
	if (bs->weaponchange_time > FloatTime() - GFIXED(0,1)) return;
	//check fire throttle characteristic
	if (bs->firethrottlewait_time > FloatTime()) return;
	firethrottle = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_FIRETHROTTLE, GFIXED_0, GFIXED_1);
	if (bs->firethrottleshoot_time < FloatTime()) {
		if (random() > firethrottle) {
			bs->firethrottlewait_time = FloatTime() + firethrottle;
			bs->firethrottleshoot_time = GFIXED_0;
		}
		else {
			bs->firethrottleshoot_time = FloatTime() + GFIXED_1 - firethrottle;
			bs->firethrottlewait_time = GFIXED_0;
		}
	}
	//
	//
	...
	lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
	lfixed dirlensq=FIXED_VEC3LEN_SQ(lvec)			;
	//
	if (bs->weaponnum == WP_GAUNTLET) {
		if (dirlensq > Square(LFIXED(60,0))) {
			return;
		}
	}
	if (dirlensq < Square(LFIXED(100,0)))
		fov = AFIXED(120,0);
	else
		fov = AFIXED(50,0);
	//
	...
	if (bsptrace.fraction < GFIXED_1 && bsptrace.ent != attackentity)
		return;
	...
	//end point aiming at
	FIXED_VEC3MA_R(start, BFIXED(1000,0), forward, end);
	//a little back to make sure not inside a very close enemy
	FIXED_VEC3MA_R(start, -BFIXED(12,0), forward, start);
	...
	if (trace.ent != attackentity || attackentity >= MAX_CLIENTS) {
		...
		if (wi.proj.damagetype & DAMAGETYPE_RADIAL) {
			if (trace.fraction * BFIXED(1000 ,0)< wi.proj.radius) {
				points = (MAKE_GFIXED(wi.proj.damage) - GFIXED(0,5) * trace.fraction * GFIXED(1000,0)) * GFIXED(0,5);
				if (points > GFIXED_0) {
					return;
				}
			}
...
void BotMapScripts(bot_state_t *bs) {
	...
	_G_trap_GetServerinfo(info, sizeof(info));

	strncpy(mapname, Info_ValueForKey( info, "mapname" ), sizeof(mapname)-1);
	mapname[sizeof(mapname)-1] = '\0';

	if (!Q_stricmp(mapname, "q3tourney6")) {
		bvec3_t mins = {BFIXED(700,0), BFIXED(204,0), BFIXED(672,0)}, maxs = {BFIXED(964,0), BFIXED(468,0), BFIXED(680,0)};
		bvec3_t buttonorg = {BFIXED(304,0), BFIXED(352,0), BFIXED(920,0)};
		...
		if (shootbutton) {
			...
			vectoangles(dir, bs->ideal_viewangles);
			aim_accuracy = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY, GFIXED_0, GFIXED_1);
			bs->ideal_viewangles[PITCH] += AFIXED(8 ,0) * MAKE_AFIXED(crandom() * (GFIXED_1 - aim_accuracy));
			bs->ideal_viewangles[PITCH] = AngleMod(bs->ideal_viewangles[PITCH]);
			bs->ideal_viewangles[YAW] += AFIXED(8 ,0) * MAKE_AFIXED(crandom() * (GFIXED_1 - aim_accuracy));
			bs->ideal_viewangles[YAW] = AngleMod(bs->ideal_viewangles[YAW]);
			//
			if (InFieldOfVision(bs->viewangles, AFIXED(20,0), bs->ideal_viewangles)) {
				_G_trap_EA_Attack(bs->client);
			}
		}
	}
...
void BotSetMovedir(avec3_t angles, avec3_t movedir) {
...
==================
*/
int BotModelMinsMaxs(int modelindex, int eType, int contents, bvec3_t mins, bvec3_t maxs) {
	gentity_t *ent;
	int i;
...
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	...
	afixed angle;
	gfixed lip, health;
	bvec3_t size, start, end, mins, maxs, points[10];
	avec3_t angles,movedir;
	bvec3_t origin, goalorigin, bboxmins, bboxmaxs;
	bvec3_t extramins = {BFIXED_1, BFIXED_1, BFIXED_1}, extramaxs = {-BFIXED_1, -BFIXED_1, -BFIXED_1};
	bsp_trace_t bsptrace;

	activategoal->shoot = qfalse;
	VectorClear(activategoal->target);
	//create a bot goal towards the button
	_G_trap_AAS_ValueForBSPEpairKey(bspent, "model", model, sizeof(model));
	if (!*model)
		return qfalse;
	...
	_G_trap_AAS_FloatForBSPEpairKey(bspent, "lip", &lip);
	if (FIXED_IS_ZERO(lip)) lip = GFIXED(4,0);
	//get the move direction from the angle
	gfixed gf;
	_G_trap_AAS_FloatForBSPEpairKey(bspent, "angle", &gf);
	angle=MAKE_AFIXED(gf);
	VectorSet(angles, AFIXED_0, angle, AFIXED_0);
	BotSetMovedir(angles, movedir);
	//button size
	VectorSubtract(maxs, mins, size);
	//button origin
	VectorAdd(mins, maxs, origin);
	FIXED_VEC3SCALE(origin, BFIXED(0,5), origin);
	//touch distance of the button
	dist = FIXED_ABS(movedir[0]) * size[0] + FIXED_ABS(movedir[1]) * size[1] + FIXED_ABS(movedir[2]) * size[2];
	dist *= BFIXED(0,5);
	//
	_G_trap_AAS_FloatForBSPEpairKey(bspent, "health", &health);
	//if the button is shootable
	if (FIXED_NOT_ZERO(health)) {
		//calculate the shoot target
		FIXED_VEC3MA_R(origin, -dist, movedir, goalorigin);
		//
		...
		if (bsptrace.fraction >= GFIXED_1 || bsptrace.ent == entitynum) {
			//
			...
			VectorSet(activategoal->goal.mins, -BFIXED(8,0), -BFIXED(8,0), -BFIXED(8,0));
			VectorSet(activategoal->goal.maxs, BFIXED(8,0), BFIXED(8,0), BFIXED(8,0));
			//
			return qtrue;
		}
		else {
			...
			_G_trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, bboxmins, bboxmaxs);
			for (i = 0; i < 3; i++) {
				if (movedir[i] < AFIXED_0) dist += FIXED_ABS(movedir[i]) * FIXED_ABS(bboxmaxs[i]);
				else dist += FIXED_ABS(movedir[i]) * FIXED_ABS(bboxmins[i]);
			}
			...
			numareas = _G_trap_AAS_TraceAreas(start, end, areas, points, 10);
			//
			for (i = numareas-1; i >= 0; i--) {
				if (_G_trap_AAS_AreaReachability(areas[i])) {
					break;
				}
			}
			if (i < 0) {
			...
			if (i >= 0) {
				...
				for (i = 0; i < 3; i++)
				{
					if (movedir[i] < AFIXED_0) activategoal->goal.maxs[i] += FIXED_ABS(movedir[i]) * FIXED_ABS(extramaxs[i]);
					else activategoal->goal.mins[i] += FIXED_ABS(movedir[i]) * FIXED_ABS(extramins[i]);
				} //end for
				//
		...
		numareas = _G_trap_AAS_TraceAreas(start, end, areas, NULL, 10);
		//
...
int BotTriggerMultipleActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	int i, areas[10], numareas, modelindex, entitynum;
	char model[128];
	bvec3_t start, end, mins, maxs;
	avec3_t angles;
	bvec3_t origin, goalorigin;
...
int BotPushOntoActivateGoalStack(bot_state_t *bs, bot_activategoal_t *activategoal) {
	...
	best = -1;
	besttime = FloatTime() + GFIXED(9999,0);
	//
...
void BotEnableActivateGoalAreas(bot_activategoal_t *activategoal, int enable) {
	...
	for (i = 0; i < activategoal->numareas; i++)
		_G_trap_AAS_EnableRoutingArea( activategoal->areas[i], enable );
...
int BotIsGoingToActivateEntity(bot_state_t *bs, int entitynum) {
	...
	for (i = 0; i < MAX_ACTIVATESTACK; i++) {
		...
		if (bs->activategoalheap[i].goal.entitynum == entitynum) {
			...
			if (bs->activategoalheap[i].justused_time > FloatTime() - GFIXED(2,0))
				return qtrue;
		}
	}
	return qfalse;
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	...
	bvec3_t origin, angles, absmins, absmaxs;
	...
	for (ent = _G_trap_AAS_NextBSPEntity(0); ent; ent = _G_trap_AAS_NextBSPEntity(ent)) {
		if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "model", tmpmodel, sizeof(tmpmodel))) continue;
	...
	_G_trap_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname));
	if (!classname) {
	...
	if (!strcmp(classname, "func_door")) {
		if (_G_trap_AAS_FloatForBSPEpairKey(ent, "health", &health)) {
		...
		//
		_G_trap_AAS_IntForBSPEpairKey(ent, "spawnflags", &spawnflags);
		// if the door starts open then just wait for the door to return
		if ( spawnflags & 1 )
			return 0;
		//get the door origin
		if (!_G_trap_AAS_VectorForBSPEpairKey(ent, "origin", origin)) {
			VectorClear(origin);
		}
		...
		_G_trap_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model));
		if (*model) {
			modelindex = atoi(model+1);
			if (modelindex) {
				VectorClear(angles);
				...
				//
				numareas = _G_trap_AAS_BBoxAreas(absmins, absmaxs, areas, MAX_ACTIVATEAREAS*2);
				...
				for (i = 0; i < numareas; i++) {
					...
					_G_trap_AAS_AreaInfo(areas[i], &areainfo);
	...
	if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[0], sizeof(targetname[0]))) {
	...
	cur_entities[0] = _G_trap_AAS_NextBSPEntity(0);
	for (i = 0; i >= 0 && i < 10;) {
		for (ent = cur_entities[i]; ent; ent = _G_trap_AAS_NextBSPEntity(ent)) {
			if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "target", target, sizeof(target))) continue;
			if (!strcmp(targetname[i], target)) {
				cur_entities[i] = _G_trap_AAS_NextBSPEntity(ent);
				break;
			}
		}
		if (!ent) {
		...
		if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname))) {
			if (bot_developer.integer) {
		...
		if (!strcmp(classname, "func_button")) {
			...
			if ( bs->activatestack && bs->activatestack->inuse &&
				 ...
				 bs->activatestack->start_time < FloatTime() - GFIXED(2,0))
				continue;
			// if the bot is in a reachability area
			if ( _G_trap_AAS_AreaReachability(bs->areanum) ) {
				// disable all areas the blocking entity is in
				BotEnableActivateGoalAreas( activategoal, qfalse );
				//
				t = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, activategoal->goal.areanum, bs->tfl);
				// if the button is not reachable
				if (!t) {
					continue;
				}
				activategoal->time = FloatTime() + MAKE_GFIXED(t) * GFIXED(0,01) + GFIXED(5,0);
			}
			return ent;
		}
		...
		// the actual button or trigger might be linked through a target_relay or target_delay
			...
			if (_G_trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[i+1], sizeof(targetname[0]))) {
				i++;
				cur_entities[i] = _G_trap_AAS_NextBSPEntity(0);
			}
		}
	}
#ifdef OBSTACLEDEBUG
...
int BotGoForActivateGoal(bot_state_t *bs, bot_activategoal_t *activategoal) {
	...
	if (FIXED_IS_ZERO(activategoal->time))
		activategoal->time = FloatTime() + GFIXED(10,0);
...
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent) {
	...
	_G_trap_AAS_ValueForBSPEpairKey(bspent, "classname", classname, sizeof(classname));
...
void BotRandomMove(bot_state_t *bs, bot_moveresult_t *moveresult) {
	...
	angles[0] = AFIXED_0;
	angles[1] = random_a() * AFIXED(360,0);
	angles[2] = AFIXED_0;
	AngleVectors(angles, dir, NULL, NULL);

	_G_trap_BotMoveInDirection(bs->ms, dir, BFIXED(400,0), MOVE_WALK);
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
	int movetype, bspent;
	bvec3_t start, end, mins, maxs;
	avec3_t hordir, sideward, up = {AFIXED_0, AFIXED_0, AFIXED_1};
	avec3_t angles;
	aas_entityinfo_t entinfo;
	...
	if (activate && entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex) {
		...
		if (bspent) {
			...
			if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) &&
				_G_trap_AAS_AreaReachability(bs->areanum))
				return;
		}
		else {
			// enable any routing areas that were disabled
	...
	if (VectorNormalize(hordir) < AFIXED(0,1)) {
		VectorSet(angles, AFIXED_0, AFIXED(360,0) * random_a(), AFIXED_0);
	...
	VectorCopy(bs->origin, start);
	start[2] += BFIXED(18,0);
	FIXED_VEC3MA_R(start, BFIXED(5,0), hordir, end);
	VectorSet(mins, -BFIXED(16,0), -BFIXED(16,0), -BFIXED(24,0));
	VectorSet(maxs, BFIXED(16,0), BFIXED(16,0), BFIXED(4,0));
	//
	...
	if (movetype != MOVE_CROUCH || !_G_trap_BotMoveInDirection(bs->ms, hordir, BFIXED(400,0), movetype)) {
		// perform the movement
		if (!_G_trap_BotMoveInDirection(bs->ms, sideward, BFIXED(400,0), movetype)) {
			...
			FIXED_VEC3MA(sideward, -BFIXED_1, hordir, sideward);
			// move in the other direction
			_G_trap_BotMoveInDirection(bs->ms, sideward, BFIXED(400,0), movetype);
		}
	}
	//
	if (bs->notblocked_time < FloatTime() - GFIXED(0,4)) {
		// just reset goals and hope the bot will go into another direction?
		// is this still needed??
		if (bs->ainode == AINode_Seek_NBG) bs->nbg_time = GFIXED_0;
		else if (bs->ainode == AINode_Seek_LTG) bs->ltg_time = GFIXED_0;
	}
}
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (bs->predictobstacles_goalareanum == goal->areanum &&
		bs->predictobstacles_time > FloatTime() - GFIXED(6,0)) {
		return qfalse;
	}
	...
	_G_trap_AAS_PredictRoute(&route, bs->areanum, bs->origin,
...
void BotCheckConsoleMessages(bot_state_t *bs) {
	...
	//
	while((handle = _G_trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		//if the chat state is flooded with messages the bot will read them quickly
		if (_G_trap_BotNumConsoleMessages(bs->cs) < 10) {
			//if it is a chat message the bot needs some time to read it
			if (m.type == CMS_CHAT && m.time > FloatTime() - (GFIXED_1 + random())) break;
		}
		//
		ptr = m.message;
		...
		if (!BotMatchMessage(bs, m.message)) {
			...
			if (m.type == CMS_CHAT && !bot_nochat.integer) {
				//
				if (!_G_trap_BotFindMatch(m.message, &match, MTCONTEXT_REPLYCHAT)) {
					_G_trap_BotRemoveConsoleMessage(bs->cs, handle);
					continue;
				}
				...
				_G_trap_BotMatchVariable(&match, MESSAGE, message, sizeof(message));
				...
				//unify the message
				_G_trap_UnifyWhiteSpaces(message);
				//
				_G_trap_Cvar_Update(&bot_testrchat);
				if (bot_testrchat.integer) {
					//
					_G_trap_BotLibVarSet("bot_testrchat", "1");
					//if bot replies with a chat message
					if (_G_trap_BotReplyChat(bs->cs, message, context, CONTEXT_REPLY,
															NULL, NULL,
															NULL, NULL,
															NULL, NULL,
															botname, netname)) {
				...
				//if at a valid chat position and not chatting already and not in teamplay
					...
					chat_reply = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_REPLY, GFIXED_0, GFIXED_1);
					if (random() < GFIXED(1,5) / MAKE_GFIXED(NumBots()+1) && random() < chat_reply) {
						//if bot replies with a chat message
						if (_G_trap_BotReplyChat(bs->cs, message, context, CONTEXT_REPLY,
																NULL, NULL,
																NULL, NULL,
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	_G_trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, BFIXED(160,0), AVOID_ALWAYS);
}
...
void BotCheckForProxMines(bot_state_t *bs, entityState_t *state) {
	...
	_G_trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 160, AVOID_ALWAYS);
	//
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
	...
	switch(event) {
		...
		case EV_GLOBAL_SOUND:
		{
			...
			_G_trap_GetConfigstring(CS_SOUNDS + state->eventParm, buf, sizeof(buf));
			/*
...
void BotCheckSnapshot(bot_state_t *bs) {
	...
	_G_trap_BotAddAvoidSpot(bs->ms, bvec3_origin, BFIXED_0, AVOID_CLEAR);
	//reset kamikaze body
	bs->kamikazebody = 0;
...
void BotCheckAir(bot_state_t *bs) {
	if (bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0) {
		if (_G_trap_AAS_PointContents(bs->eye) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)) {
			return;
		}
	}
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (bs->altroutegoal.areanum) {
		//
		if (FIXED_NOT_ZERO(bs->reachedaltroutegoal_time))
			return goal;
		// travel time towards alternative route goal
		t = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->altroutegoal.areanum, bs->tfl);
		if (t && t < 20) {
...
int BotGetAlternateRouteGoal(bot_state_t *bs, int base) {
	...
	if (!numaltroutegoals)
		return qfalse;
	rnd = FIXED_TO_INT(random() * MAKE_GFIXED(numaltroutegoals));
	...
	goal->flags = 0;
	//
	bs->reachedaltroutegoal_time = GFIXED_0;
	return qtrue;
}
...
#ifdef MISSIONPACK
	if (gametype == GT_CTF) {
		if (_G_trap_BotGetLevelItemGoal(-1, "Neutral Flag", &ctf_neutralflag) < 0)
			BotAI_Print(PRT_WARNING, "no alt routes without Neutral Flag\n");
		if (ctf_neutralflag.areanum) {
			//
			red_numaltroutegoals = _G_trap_AAS_AlternativeRouteGoals(
			...
			blue_numaltroutegoals = _G_trap_AAS_AlternativeRouteGoals(
										ctf_neutralflag.origin, ctf_neutralflag.areanum,
		...
		if (_G_trap_BotGetLevelItemGoal(-1, "Neutral Obelisk", &neutralobelisk) < 0)
...
==================
*/
void BotDeathmatchAI(bot_state_t *bs, gfixed thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		_G_trap_Characteristic_String(bs->character, CHARACTERISTIC_GENDER, gender, sizeof(gender));
		//set the bot gender
		_G_trap_GetUserinfo(bs->client, userinfo, sizeof(userinfo));
		...
		if ( !bs->map_restart && g_gametype.integer != GT_TOURNAMENT ) {
			...
			_G_trap_EA_Command(bs->client, buf);
		}
		//set the chat gender
		if (gender[0] == 'm') _G_trap_BotSetChatGender(bs->cs, CHAT_GENDERMALE);
		else if (gender[0] == 'f')  _G_trap_BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
		else  _G_trap_BotSetChatGender(bs->cs, CHAT_GENDERLESS);
		//set the chat name
		ClientName(bs->client, name, sizeof(name));
		_G_trap_BotSetChatName(bs->cs, name, bs->client);
		//
	...
	if (!bs->entergamechat && bs->entergame_time > FloatTime() - GFIXED(8,0)) {
	...
	if (i >= MAX_NODESWITCHES) {
		_G_trap_BotDumpGoalStack(bs->gs);
		_G_trap_BotDumpAvoidGoals(bs->gs);
		BotDumpNodeSwitches(bs);
		ClientName(bs->client, name, sizeof(name));
		BotAI_Print(PRT_ERROR, "%s at %1.1f switched more than %d AI nodes\n", name, FIXED_TO_DOUBLE(FloatTime()), MAX_NODESWITCHES);
	}
	//
	bs->lastframe_health = bs->inventory[INVENTORY_HEALTH];
	bs->lasthitcount = bs->cur_ps.persistant[PERS_HITS];
...
void BotSetEntityNumForGoalWithModel(bot_goal_t *goal, int eType, const char *modelname) {
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		...
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		if (FIXED_VEC3LEN(lvec) < Square(LFIXED(10,0))) {
			goal->entitynum = i;
			return;
		}
	}
...
int BotGoalForBSPEntity( const char *classname, bot_goal_t *goal ) {
	...
	for (ent = _G_trap_AAS_NextBSPEntity(0); ent; ent = _G_trap_AAS_NextBSPEntity(ent)) {
		if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "classname", value, sizeof(value)))
			continue;
...
void BotSetupDeathmatchAI(void) {
	...
	gametype = _G_trap_Cvar_VariableIntegerValue("g_gametype");
	maxclients = _G_trap_Cvar_VariableIntegerValue("sv_maxclients");

	_G_trap_Cvar_Register(&bot_rocketjump, "bot_rocketjump", "1", 0);
	_G_trap_Cvar_Register(&bot_grapple, "bot_grapple", "0", 0);
	_G_trap_Cvar_Register(&bot_fastchat, "bot_fastchat", "0", 0);
	_G_trap_Cvar_Register(&bot_nochat, "bot_nochat", "0", 0);
	_G_trap_Cvar_Register(&bot_testrchat, "bot_testrchat", "0", 0);
	_G_trap_Cvar_Register(&bot_challenge, "bot_challenge", "0", 0);
	_G_trap_Cvar_Register(&bot_predictobstacles, "bot_predictobstacles", "1", 0);
	_G_trap_Cvar_Register(&g_spSkill, "g_spSkill", "2", 0);
	//
	if (gametype == GT_CTF) {
		if (_G_trap_BotGetLevelItemGoal(-1, "Red Flag", &ctf_redflag) < 0)
			BotAI_Print(PRT_WARNING, "CTF without Red Flag\n");
		if (_G_trap_BotGetLevelItemGoal(-1, "Blue Flag", &ctf_blueflag) < 0)
		...
		if (_G_trap_BotGetLevelItemGoal(-1, "Red Obelisk", &redobelisk) < 0)
			BotAI_Print(PRT_WARNING, "Obelisk without red obelisk\n");
		BotSetEntityNumForGoal(&redobelisk, "team_redobelisk");
		if (_G_trap_BotGetLevelItemGoal(-1, "Blue Obelisk", &blueobelisk) < 0)
	...
	max_bspmodelindex = 0;
	for (ent = _G_trap_AAS_NextBSPEntity(0); ent; ent = _G_trap_AAS_NextBSPEntity(ent)) {
		if (!_G_trap_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model))) continue;
		if (model[0] == '*') {
