...
char *BotRandomOpponentName(bot_state_t *bs) {
	...
	count = FIXED_TO_INT(random() * MAKE_GFIXED(numopponents));
...
#ifdef MISSIONPACK
	rnd = FIXED_TO_INT(random() * GFIXED(11,9));
#else
	rnd = FIXED_TO_INT(random() * GFIXED(8,9));
#endif
	switch(rnd) {
...
int BotValidChatPosition(bot_state_t *bs) {
	bvec3_t point, start, end, mins, maxs;
	bsp_trace_t trace;
	...
	point[2] -= BFIXED(24,0);
	if (_G_trap_PointContents(point,bs->entitynum) & (CONTENTS_LAVA|CONTENTS_SLIME)) return qfalse;
	//do not chat if under water
	VectorCopy(bs->origin, point);
	point[2] += BFIXED(32,0);
	if (_G_trap_PointContents(point,bs->entitynum) & MASK_WATER) return qfalse;
	//must be standing on the world entity
	VectorCopy(bs->origin, start);
	VectorCopy(bs->origin, end);
	start[2] += BFIXED_1;
	end[2] -= BFIXED(10,0);
	_G_trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, mins, maxs);
...
int BotChat_EnterGame(bot_state_t *bs) {
	...
	if (bs->lastchat_time > FloatTime() - GFIXED(TIME_BETWEENCHATTING,0)) return qfalse;
	//don't chat in teamplay
	if (TeamPlayIsOn()) return qfalse;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_ENTEREXITGAME, GFIXED_0, GFIXED_1);
...
int BotChat_StartLevel(bot_state_t *bs) {
	...
	//don't chat in teamplay
	if (TeamPlayIsOn()) {
	    _G_trap_EA_Command(bs->client, "vtaunt");
	    return qfalse;
	}
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_STARTENDLEVEL, GFIXED_0, GFIXED_1);
	if (!bot_fastchat.integer) {
		if (random() > rnd) return qfalse;
	}
	if (BotNumActivePlayers() <= 1) return qfalse;
...
int BotChat_Death(bot_state_t *bs) {
	...
	if (bot_nochat.integer) return qfalse;
	if (bs->lastchat_time > FloatTime() - GFIXED(TIME_BETWEENCHATTING,0)) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_DEATH, GFIXED_0, GFIXED_1);
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
	...
	else
	{
		...
		if (bs->botdeathtype == MOD_WATER)
...
#ifdef MISSIONPACK
		else if (bs->botdeathtype == MOD_KAMIKAZE && _G_trap_BotNumInitialChats(bs->cs, "death_kamikaze"))
			...
			if ((bs->botdeathtype == MOD_GAUNTLET ||
				...
				bs->botdeathtype == MOD_BFG_SPLASH) && random() < GFIXED(0,5)) {
			...
			else if (random() < _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_INSULT, GFIXED_0, GFIXED_1)) {
...
int BotChat_Kill(bot_state_t *bs) {
	...
	if (bot_nochat.integer) return qfalse;
	if (bs->lastchat_time > FloatTime() - GFIXED(TIME_BETWEENCHATTING,0)) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_KILL, GFIXED_0, GFIXED_1);
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
	//if fast chat is off
	if (!bot_fastchat.integer) {
...
#ifdef MISSIONPACK
		else if (bs->botdeathtype == MOD_KAMIKAZE && _G_trap_BotNumInitialChats(bs->cs, "kill_kamikaze"))
...
int BotChat_HitTalking(bot_state_t *bs) {
	...
	//
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITTALKING, GFIXED_0, GFIXED_1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return qfalse;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
	//if fast chat is off
	if (!bot_fastchat.integer) {
		if (random() > rnd * GFIXED(0,5)) return qfalse;
	}
	if (!BotValidChatPosition(bs)) return qfalse;
	//
...
int BotChat_HitNoDeath(bot_state_t *bs) {
	...
	if (bs->lastchat_time > FloatTime() - GFIXED(TIME_BETWEENCHATTING,0)) return qfalse;
	if (BotNumActivePlayers() <= 1) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNODEATH, GFIXED_0, GFIXED_1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return qfalse;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
...
int BotChat_HitNoKill(bot_state_t *bs) {
	...
	if (bs->lastchat_time > FloatTime() - GFIXED(TIME_BETWEENCHATTING,0)) return qfalse;
	if (BotNumActivePlayers() <= 1) return qfalse;
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNOKILL, GFIXED_0, GFIXED_1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return qfalse;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
...
int BotChat_Random(bot_state_t *bs) {
	...
	//
	rnd = _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_RANDOM, GFIXED_0, GFIXED_1);
	if (random() > bs->thinktime * GFIXED(0,1)) return qfalse;
	if (!bot_fastchat.integer) {
		if (random() > rnd) return qfalse;
		if (random() > GFIXED(0,25)) return qfalse;
	}
	if (BotNumActivePlayers() <= 1) return qfalse;
	//
	if (!BotValidChatPosition(bs)) return qfalse;
	...
	//
	if (random() < _G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_MISC, GFIXED_0, GFIXED_1)) {
...
==================
*/
gfixed BotChatTime(bot_state_t *bs) {
	int cpm;

	cpm = _G_trap_Characteristic_BInteger(bs->character, CHARACTERISTIC_CHAT_CPM, 1, 4000);

	return GFIXED(2,0);	//MAKE_GFIXED _G_trap_BotChatLength(bs->cs) * 30 / cpm;
}
...
void BotChatTest(bot_state_t *bs) {
	...
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "game_enter",
					...
					BotMapTitle(),							// 4
					NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "game_exit");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "game_exit",
					EasyClientName(bs->client, name, 32),	// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "level_start");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_start",
					EasyClientName(bs->client, name, 32),	// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "level_end_victory");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end_victory",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "level_end_lose");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end_lose",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "level_end");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_drown");
	for (i = 0; i < num; i++)
	{
		//
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_slime");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_slime", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "death_lava");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_lava", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "death_cratered");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_cratered", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "death_suicide");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_suicide", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "death_telefrag");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_telefrag", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "death_gauntlet");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_gauntlet",
				name,												// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_rail");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_rail",
				name,												// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_bfg");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_bfg",
				name,												// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_insult",
					name,												// 0
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "death_praise");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_praise",
					name,												// 0
	...
	EasyClientName(bs->lastkilledplayer, name, 32);
	//
	num = _G_trap_BotNumInitialChats(bs->cs, "kill_gauntlet");
	for (i = 0; i < num; i++)
	{
		//
		BotAI_BotInitialChat(bs, "kill_gauntlet", name, NULL);
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "kill_rail");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_rail", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "kill_telefrag");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_telefrag", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "kill_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_insult", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "kill_praise");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_praise", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "enemy_suicide");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "enemy_suicide", name, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	...
	ClientName(g_entities[bs->client].client->lasthurt_client, name, sizeof(name));
	weap = BotWeaponNameForMeansOfDeath(g_entities[bs->client].client->lasthurt_client);
	num = _G_trap_BotNumInitialChats(bs->cs, "hit_talking");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_talking", name, weap, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "hit_nodeath");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_nodeath", name, weap, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = _G_trap_BotNumInitialChats(bs->cs, "hit_nokill");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_nokill", name, weap, NULL);
		_G_trap_BotEnterChat(bs->cs, 0, CHAT_ALL);
	...
	//
	num = _G_trap_BotNumInitialChats(bs->cs, "random_misc");
	for (i = 0; i < num; i++)
	{
		//
		BotAI_BotInitialChat(bs, "random_misc",
	...
	num = _G_trap_BotNumInitialChats(bs->cs, "random_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "random_insult",
					BotRandomOpponentName(bs),	// 0
