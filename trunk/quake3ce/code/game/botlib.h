#ifndef __INC_BOTLIB_H
#define __INC_BOTLIB_H
...
{
	gfixed thinktime;		//time since last output (in seconds)
	avec3_t dir;				//movement direction
	bfixed speed;			//speed in the range [0, 400]
	avec3_t viewangles;		//the view angles
...
typedef struct bsp_trace_s
{
	...
	bfixed			exp_dist;	// expanded plane distance
...
typedef struct bot_entitystate_s
{
	...
	bvec3_t	origin;			// origin of the entity
	avec3_t	angles;			// angles of the model
	bvec3_t	old_origin;		// for lerping
	bvec3_t	mins;			// bounding box minimums
	bvec3_t	maxs;			// bounding box maximums
...
typedef struct botlib_import_s
{
	...
	void		(*EntityTrace)(bsp_trace_t *trace, bvec3_t start, bvec3_t mins, bvec3_t maxs, bvec3_t end, int entnum, int contentmask);
	//retrieve the contents at the given point
	int			(*PointContents)(bvec3_t point);
	//check if the point is in potential visible sight
	int			(*inPVS)(bvec3_t p1, bvec3_t p2);
	...
	void		(*DebugLineShow)(int line, bvec3_t start, bvec3_t end, int color);
	//
	int			(*DebugPolygonCreate)(int color, int numPoints, bvec3_t *points);
...
typedef struct aas_export_s
{
	...
	// be_aas_altroute.c
	//--------------------------------------------
	int			(*AAS_AlternativeRouteGoals)(bvec3_t start, int startareanum, bvec3_t goal, int goalareanum, int travelflags,
...
typedef struct ea_export_s
{
	...
	void	(*EA_Move)(int client, avec3_t dir, bfixed speed);
	void	(*EA_View)(int client, avec3_t viewangles);
	//send regular input to the server
	void	(*EA_EndRegular)(int client, gfixed thinktime);
	void	(*EA_GetInput)(int client, gfixed thinktime, bot_input_t *input);
...
typedef struct ai_export_s
{
	...
	int		(*BotMovementViewTarget)(int movestate, struct bot_goal_s *goal, int travelflags, bfixed lookahead, bvec3_t target);
	int		(*BotPredictVisiblePosition)(bvec3_t origin, int areanum, struct bot_goal_s *goal, int travelflags, bvec3_t target);
...
typedef struct botlib_export_s
{
	...
	int (*BotLibStartFrame)(gfixed time);
	//load a new map in the bot library
	int (*BotLibLoadMap)(const char *mapname);
	//entity updates
	int (*BotLibUpdateEntity)(int ent, bot_entitystate_t *state);
	//just for testing
	int (*Test)(int parm0, char *parm1, bvec3_t parm2, avec3_t parm3);
} botlib_export_t;
