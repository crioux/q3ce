...
/*
	...
	if ( interpret == VMI_NATIVE ) {
		if ( FIXED_NOT_ZERO(Cvar_VariableValue( "fs_restrict" ))) {
...
void *VM_ArgPtr( void *value ) {
	...
	if ( currentVM->entryPoint ) {
		return (void *)(currentVM->dataBase + ((int)value) );
	}
	else {
		return (void *)(currentVM->dataBase + (((int)value)  & currentVM->dataMask));
	}
}
...
void *VM_ExplicitArgPtr( vm_t *vm, void *value ) {
	...
	//
	if ( vm->entryPoint ) {
		return (void *)(vm->dataBase + (int)value);
	}
	else {
...
//	wchar_t foo[256];	
//	_snwprintf(foo,256,L"vm_call %p,%d\n",vm,callnum);
