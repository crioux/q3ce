...
void CMod_LoadSubmodels( lump_t *l ) {
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		{	// spread the mins / maxs by a pixel
			out->mins[j] = LittleFixed_IS_FLOAT_B(in->mins[j]) - BFIXED_1;
			out->maxs[j] = LittleFixed_IS_FLOAT_B(in->maxs[j]) + BFIXED_1;
//			out->mins[j] = in->mins[j] - GFIXED_1;
//			out->maxs[j] = in->maxs[j] + GFIXED_1;
		}
...
#ifdef _DEBUG
		ASSERT(out->normal[0]!=AFIXED_0 || out->normal[1]!=AFIXED_0 || out->normal[2]!=AFIXED_0);
		ASSERT(FIXED_VEC3LEN_SQ(out->normal) > AFIXED(0,9999));
#endif
...
void CMod_LoadPatches( lump_t *surfs, lump_t *verts ) {
	...
	int			c;
	cPatch_t	*patch;
	bvec3_t		points[MAX_PATCH_VERTS];
	int			width, height;
	int			shaderNum;
	...
	for ( i = 0 ; i < count ; i++, in++ ) {
		...
		for ( j = 0 ; j < c ; j++, dv_p++ ) {
			points[j][0] = LittleFixed_IS_FLOAT_B( dv_p->xyz[0] );
			points[j][1] = LittleFixed_IS_FLOAT_B( dv_p->xyz[1] );
			points[j][2] = LittleFixed_IS_FLOAT_B( dv_p->xyz[2] );
		}
...
===================
*/
clipHandle_t CM_TempBoxModel( const bvec3_t mins, const bvec3_t maxs, int capsule ) {
...
===================
*/
void CM_ModelBounds( clipHandle_t model, bvec3_t mins, bvec3_t maxs ) {
	cmodel_t	*cmod;
