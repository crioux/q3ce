	...
	if (cgs.orderPending) {
		...
		if (cg_currentSelectedPlayer.integer == numSortedTeamPlayers) {
			...
			if (sortedTeamPlayers[cg_currentSelectedPlayer.integer] == cg.snap->ps.clientNum && p1) {
				_CG_trap_SendConsoleCommand(va("teamtask %i\n", cgs.currentOrder));
				//_CG_trap_SendConsoleCommand(va("cmd say_team %s\n", p2));
				_CG_trap_SendConsoleCommand(va("cmd vsay_team %s\n", p1));
			} else if (p2) {
				//_CG_trap_SendConsoleCommand(va("cmd say_team %s, %s\n", ci->name,p));
				_CG_trap_SendConsoleCommand(va("cmd vtell %d %s\n", sortedTeamPlayers[cg_currentSelectedPlayer.integer], p2));
			}
		}
		if (b) {
			_CG_trap_SendConsoleCommand(b);
		}
...
static void CG_SetSelectedPlayerName() {
  if (cg_currentSelectedPlayer.integer >= 0 && cg_currentSelectedPlayer.integer < numSortedTeamPlayers) {
		clientInfo_t *ci = cgs.clientinfo + sortedTeamPlayers[cg_currentSelectedPlayer.integer];
	  if (ci) {
			_CG_trap_Cvar_Set("cg_selectedPlayerName", ci->name);
			_CG_trap_Cvar_Set("cg_selectedPlayer", va("%d", sortedTeamPlayers[cg_currentSelectedPlayer.integer]));
			cgs.currentOrder = ci->teamTask;
	  }
	} else {
		_CG_trap_Cvar_Set("cg_selectedPlayerName", "Everyone");
	}
}
...
static void CG_DrawPlayerArmorIcon( rectDef_t *rect, qboolean draw2D ) {
  ...
  if ( cg_drawStatus.integer == 0 ) {
    ...
    origin[0] = 90;
  	origin[1] = 0;
  	origin[2] = -10;
  	angles[YAW] = ( cg.time & 2047 ) * 360 / GFIXED(2048,0);
...
static void CG_DrawPlayerArmorValue(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle) {
	char	num[16];
  int value;
	centity_t	*cent;
	playerState_t	*ps;
    ...
    icon = cg_weapons[ cg.predictedPlayerState.weapon ].ammoIcon;
  	...
  	if ( cent->currentState.weapon && cg_weapons[ cent->currentState.weapon ].ammoModel ) {
	    VectorClear( angles );
  		...
  		origin[1] = 0;
  		origin[2] = 0;
  		angles[YAW] = 90 + 20 * sin( cg.time / GFIXED(1000,0) );
  		CG_Draw3DModel( rect->x, rect->y, rect->w, rect->h, cg_weapons[ cent->currentState.weapon ].ammoModel, 0, origin, angles );
  	}
  }
}

static void CG_DrawPlayerAmmoValue(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle) {
	char	num[16];
	int value;
	centity_t	*cent;
	playerState_t	*ps;
...
static void CG_DrawPlayerHead(rectDef_t *rect, qboolean draw2D) {
	...
	if ( cg.damageTime && cg.time - cg.damageTime < DAMAGE_TIME ) {
		frac = MAKE_GFIXED(cg.time - cg.damageTime ) / DAMAGE_TIME;
		size = rect->w * GFIXED(1,25) * ( GFIXED(1,5) - frac * GFIXED(0,5) );

		stretch = size - rect->w * GFIXED(1,25);
		// kick in the direction of damage
		x -= stretch * GFIXED(0,5) + cg.damageX * stretch * GFIXED(0,5);

		cg.headStartYaw = 180 + cg.damageX * 45;

		cg.headEndYaw = 180 + 20 * cos( crandom()*GFIXED_PI );
		cg.headEndPitch = 5 * cos( crandom()*GFIXED_PI );
...
static void CG_DrawSelectedPlayerHealth( rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	clientInfo_t *ci;
	int value;
	char num[16];
...
static void CG_DrawSelectedPlayerArmor( rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	clientInfo_t *ci;
	int value;
	char num[16];
  ci = cgs.clientinfo + sortedTeamPlayers[CG_GetSelectedPlayer()];
...
static void CG_DrawSelectedPlayerName( rectDef_t *rect, gfixed scale, vec4_t color, qboolean voice, int textStyle) {
	clientInfo_t *ci;
...
static void CG_DrawSelectedPlayerLocation( rectDef_t *rect, gfixed scale, vec4_t color, int textStyle ) {
	clientInfo_t *ci;
  ci = cgs.clientinfo + sortedTeamPlayers[CG_GetSelectedPlayer()];
  if (ci) {
...
static void CG_DrawPlayerLocation( rectDef_t *rect, gfixed scale, vec4_t color, int textStyle  ) {
	clientInfo_t *ci = &cgs.clientinfo[cg.snap->ps.clientNum];
  if (ci) {
		const char *p = CG_ConfigString(CS_LOCATIONS + ci->location);
		if (!p || !*p) {
...
static void CG_DrawPlayerScore( rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
  char num[16];
...
static void CG_DrawPlayerItem( rectDef_t *rect, gfixed scale, qboolean draw2D) {
	int		value;
  bvec3_t origin, angles;
    ...
    y = rect->y;
		...
		for (j = 0; j < PW_NUM_POWERUPS; j++) {
			if (ci->powerups & (1 << j)) {
				...
				if (item) {
				  CG_DrawPic( x, y, rect->w, rect->h, _CG_trap_R_RegisterShader( item->icon ) );
					x += 3;
					y += 3;
          return;
				}
...
static void CG_DrawSelectedPlayerHead( rectDef_t *rect, qboolean draw2D, qboolean voice ) {
  ...
  if (ci) {
  	if ( cg_draw3dIcons.integer ) {
	  	...
	  	origin[2] = -GFIXED(0,5) * ( mins[2] + maxs[2] );
  		origin[1] = GFIXED(0,5) * ( mins[1] + maxs[1] );

	  	// calculate distance so the head nearly fills the box
  		// assume heads are taller than wide
  		len = GFIXED(0,7) * ( maxs[2] - mins[2] );		
...
static void CG_DrawPlayerHealth(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	playerState_t	*ps;
  int value;
	char	num[16];
...
static void CG_DrawRedScore(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	int value;
	char num[16];
...
static void CG_DrawBlueScore(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	int value;
	char num[16];
...
static void CG_DrawRedName(rectDef_t *rect, gfixed scale, vec4_t color, int textStyle ) {
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cg_redTeamName.string , 0, 0, textStyle);
}

static void CG_DrawBlueName(rectDef_t *rect, gfixed scale, vec4_t color, int textStyle ) {
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cg_blueTeamName.string, 0, 0, textStyle);
}

static void CG_DrawBlueFlagName(rectDef_t *rect, gfixed scale, vec4_t color, int textStyle ) {
  int i;
...
static void CG_DrawBlueFlagHead(rectDef_t *rect) {
  ...
  for ( i = 0 ; i < cgs.maxclients ; i++ ) {
      ...
      bvec3_t angles;
      VectorClear( angles );
 		  angles[YAW] = 180 + 20 * sin( cg.time / GFIXED(650,0) );;
...
static void CG_DrawRedFlagName(rectDef_t *rect, gfixed scale, vec4_t color, int textStyle ) {
  int i;
  for ( i = 0 ; i < cgs.maxclients ; i++ ) {
...
static void CG_HarvesterSkulls(rectDef_t *rect, gfixed scale, vec4_t color, qboolean force2D, int textStyle ) {
	char num[16];
	bvec3_t origin, angles;
	qhandle_t handle;
...
static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, gfixed special, gfixed scale, vec4_t color) {
	char num[16];
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		if (item) {
			...
			if ( t - cg.time >= POWERUP_BLINKS * POWERUP_BLINK_TIME ) {
				...
				f = MAKE_GFIXED( t - cg.time ) / POWERUP_BLINK_TIME;
				f -= (int)f;
				modulate[0] = modulate[1] = modulate[2] = modulate[3] = f;
				_CG_trap_R_SetColor( modulate );
			}

			CG_DrawPic( r2.x, r2.y, r2.w * .75, r2.h, _CG_trap_R_RegisterShader( item->icon ) );
...
static void CG_DrawAreaSystemChat(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader) {
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, systemChat, 0, 0, 0);
}

static void CG_DrawAreaTeamChat(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader) {
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,teamChat1, 0, 0, 0);
}

static void CG_DrawAreaChat(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader) {
...
static void CG_DrawKiller(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	// fragged by ... line
	if ( cg.killerName[0] ) {
...
static void CG_DrawCapFragLimit(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle) {
...
static void CG_Draw1stPlace(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle) {
	if (cgs.scores1 != SCORE_NOT_PRESENT) {
		CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i", cgs.scores1),0, 0, textStyle); 
	}
}

static void CG_Draw2ndPlace(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle) {
	if (cgs.scores2 != SCORE_NOT_PRESENT) {
...
static void CG_DrawGameStatus(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
...
static void CG_DrawGameType(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader, int textStyle ) {
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, CG_GameTypeString(), 0, 0, textStyle);
}

static void CG_Text_Paint_Limit(gfixed *maxX, gfixed x, gfixed y, gfixed scale, vec4_t color, const char* text, gfixed adjust, int limit) {
  int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
  if (text) {
...
void CG_DrawNewTeamInfo(rectDef_t *rect, gfixed text_x, gfixed text_y, gfixed scale, vec4_t color, qhandle_t shader) {
	int xx;
	gfixed y;
	int i, j, len, count;
	const char *p;
	vec4_t		hcolor;
	gfixed pwidth, lwidth, maxx, leftOver;
	clientInfo_t *ci;
	gitem_t	*item;
	qhandle_t h;
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			for (j = 0; j <= PW_NUM_POWERUPS; j++) {
				if (ci->powerups & (1 << j)) {
					...
					if (item) {
						CG_DrawPic( xx, y, PIC_WIDTH, PIC_WIDTH, _CG_trap_R_RegisterShader( item->icon ) );
						xx += PIC_WIDTH;
					}
				}
			}
...
void CG_DrawTeamSpectators(rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader) {
	if (cg.spectatorLen) {
		gfixed maxX;
...
void CG_DrawMedal(int ownerDraw, rectDef_t *rect, gfixed scale, vec4_t color, qhandle_t shader) {
...
void CG_KeyEvent(int key, qboolean down) {
  ...
  //if (key == _CG_trap_Key_GetKey("teamMenu") || !Display_CaptureItem(cgs.cursorX, cgs.cursorY)) {
...
void CG_ShowResponseHead() {
  Menus_OpenByName("voiceMenu");
	_CG_trap_Cvar_Set("cl_conXOffset", "72");
...
void CG_GetTeamColor(vec4_t *color) {
  if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED) {
    (*color)[0] = GFIXED_1;
    (*color)[3] = GFIXED(0,25);
    (*color)[1] = (*color)[2] = GFIXED_0;
  } else if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE) {
    (*color)[0] = (*color)[1] = GFIXED_0;
    (*color)[2] = GFIXED_1;
    (*color)[3] = GFIXED(0,25);
  } else {
