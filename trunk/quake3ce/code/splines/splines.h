...
//extern void glBox(idVec3_t &color, idVec3_t &point, gfixed size);
//extern void glLabeledPoint(idVec3_t &color, idVec3_t &point, gfixed size, const char *label);

const mvec4_t blue(GFIXED_0, GFIXED_0, GFIXED_1, GFIXED_1);
const mvec4_t red(GFIXED_1, GFIXED_0, GFIXED_0, GFIXED_1);
...
public:
	...
	virtual void addPoint(const gfixed x, const gfixed y, const gfixed z) {}
	virtual void addPoint(const idVec3_t &v) {}
	virtual void removePoint(int index) {}
	virtual idVec3_t *getPoint(int index) { return NULL; }
	
	int	selectPointByRay(gfixed ox, gfixed oy, gfixed oz, gfixed dx, gfixed dy, gfixed dz, bool single) {
	...
	int	selectPointByRay(const idVec3_t origin, const idVec3_t direction, bool single) {
		...
		for (i=0; i < count; i++) {
			...
			temp2 = temp;
			temp -= origin;
			d = FIXED_VEC3DOT((gfixed *)&temp, (gfixed *)&direction);
	...
	virtual void updateSelection(gfixed x, gfixed y, gfixed z) {
		idVec3_t move(x, y, z);
...
public:
	...
	void clear() {
		...
		dirty = true;
		activeSegment = 0;
		granularity = GFIXED(0,025);
		pathColor.set(GFIXED_1, GFIXED(0,5), GFIXED_0);
		controlColor.set(GFIXED(0,7), GFIXED_0, GFIXED_1);
		segmentColor.set(GFIXED_0, GFIXED_0, GFIXED_1);
		activeColor.set(GFIXED_1, GFIXED_0, GFIXED_0);
	}
...
//	void draw(bool editMode);
	...
	void addPoint(gfixed x, gfixed y, gfixed z) {
	...
	void setGranularity(gfixed f) {
		granularity = f;
	}

	gfixed getGranularity() {
		return granularity;
	}
	...
	void setSegmentTime(int index, int time) {
		assert(index >= 0 && index < splinePoints.Num());
		splineTime[index] = MAKE_LFIXED(time);
	}

	lfixed getSegmentTime(int index) {
		assert(index >= 0 && index < splinePoints.Num());
...
protected:
	idStr name;
	gfixed calcSpline(int step, gfixed tension);
	idList<idVec3_t*> controlPoints;
	idList<idVec3_t*> splinePoints;
	idList<lfixed> splineTime;
	idVec3_t *selected;
...
// velocity where GFIXED_1 equal rough walking speed
struct idVelocity {
	idVelocity(long start, long duration, gfixed s) {
		startTime = start;
		time = duration;
		speed = s;
	}
...
public:
	...
	void addVelocity(long start, long duration, gfixed speed) {
...
//	virtual void draw() {};
	...
	void calcVelocity(gfixed distance) {
		gfixed secs = MSECTIME_G(time);
...
public:
	...
	idCameraFOV(int s, int e, long t) {
		startFOV = MAKE_GFIXED(s);
		endFOV = MAKE_GFIXED(e);
		time = t;
	}
	...
	void setFOV(gfixed f) {
		fov = f;
	}

	gfixed getFOV(long t) {
		if (time) {
			assert(startTime);
			gfixed percent = MAKE_GFIXED(t) / MAKE_GFIXED(startTime);
			gfixed temp = startFOV - endFOV;
			temp *= percent;
			fov = startFOV + temp;
		}
		return fov;
...
public:
	...
	void clear() {
		...
		cameraRunning = false;
		lastDirection.Zero();
		baseTime = GFIXED(30,0);
		activeTarget = 0;
		name = "camera01";
		fov.setFOV(GFIXED(90,0));
		int i;
	...
	static idCameraPosition *newFromType(idCameraPosition::positionType t) {
		switch (t) {
			case idCameraPosition::GFIXED : return new idFixedPosition();
	...
	idCameraPosition *getActiveTarget() {
		if (targetPositions.Num() == 0) {
			addTarget(NULL, idCameraPosition::GFIXED);
		}
	...
	void getActiveSegmentInfo(int segment, idVec3_t &origin, idVec3_t &direction, gfixed *fv);

	bool getCameraInfo(long time, idVec3_t &origin, idVec3_t &direction, gfixed *fv);
	bool getCameraInfo(long time, gfixed *origin, gfixed *direction, gfixed *fv) {
		idVec3_t org, dir;
		org[0] = origin[0];
		org[1] = origin[1];
		org[2] = origin[2];
