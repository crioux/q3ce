...
*/
	...
	if ( size == SMALLCHAR_WIDTH ) {
		...
		SCR_DrawBigString( x, y, str, GFIXED_1 );
	}
	...
	if ( size == SMALLCHAR_WIDTH ) {
		...
		str[0] = cursorChar;
		str[1] = 0;
		SCR_DrawBigString( x + ( edit->cursor - prestep - i ) * size, y, str, GFIXED_1 );
  ...
  if (key == K_ENTER)
  {
	...
	if ( key == K_ESCAPE && down ) {
		...
		if (cls.keyCatchers & KEYCATCH_CGAME) {
			...
			SysCallArgs args(1);
			args[0]=CGAME_EVENT_NONE;
			VM_Call (cgvm, CG_EVENT_HANDLING, args);
			return;
		}
		...
		VM_Call( uivm, UI_KEY_EVENT, args );
		return;
	}
	...
	if (!down) {
		...
		if ( cls.keyCatchers & KEYCATCH_UI && uivm ) {
			...
			args[0]=key;
			args[1]=down;
			VM_Call( cgvm, CG_KEY_EVENT, args );
		} 
