...
void S_TransferPaintBuffer(int endtime)
{
	...
	if ( s_testsound->integer ) {
		...
		// write a gfixed sine wave
		count = (endtime - s_paintedtime);
		for (i=0 ; i<count ; i++)
			paintbuffer[i].left = paintbuffer[i].right = FIXED_TO_INT(FIXED_SIN(MAKE_GFIXED(s_paintedtime+i)*GFIXED(0,1))*GFIXED(20000,0)*GFIXED(256,0));
	}
...
static void S_PaintChannelFrom16( channel_t *ch, const sfx_t *sc, int count, int sampleOffset, int bufferOffset ) {
	...
	sndBuffer				*chunk;
	short					*samples;
	gfixed					ooff, fleftvol, frightvol;
	int						fdata, fdiv;
	...
	if (ch->doppler) {
		sampleOffset = FIXED_TO_INT(MAKE_GFIXED(sampleOffset)*ch->oldDopplerScale);
	}
	...
	if (!ch->doppler || ch->dopplerScale==GFIXED_1) {
#if idppc_altivec
		...
		fleftvol = MAKE_GFIXED(ch->leftvol*snd_vol);
		frightvol = MAKE_GFIXED(ch->rightvol*snd_vol);

		ooff = MAKE_GFIXED(sampleOffset);
		samples = chunk->sndChunk;
		
		for ( i=0 ; i<count ; i++ ) {

			aoff = FIXED_TO_INT(ooff);
			ooff = ooff + ch->dopplerScale;
			boff = FIXED_TO_INT(ooff);
			fdata = 0;
			for (j=aoff; j<boff; j++) {
				if (j == SND_CHUNK_SIZE) {
					chunk = chunk->next;
					...
					samples = chunk->sndChunk;
					ooff -= MAKE_GFIXED(SND_CHUNK_SIZE);
				}
				fdata  += samples[j&(SND_CHUNK_SIZE-1)];
			}
			fdiv = 256 * (boff-aoff);
			samp[i].left += FIXED_INT32SCALE(fdata, fleftvol) / fdiv;
			samp[i].right += FIXED_INT32SCALE(fdata, frightvol) / fdiv;
		}
	}
}
...
void S_PaintChannelFromMuLaw( channel_t *ch, sfx_t *sc, int count, int sampleOffset, int bufferOffset ) {
	...
	if (!ch->doppler) {
		...
		samples = (byte *)chunk->sndChunk;
		for ( i=0 ; i<count ; i++ ) {
			data  = mulawToShort[samples[FIXED_TO_INT(ooff)]];
			ooff = ooff + ch->dopplerScale;
			samp[i].left += (data * leftvol)>>8;
			samp[i].right += (data * rightvol)>>8;
			if (FIXED_TO_INT(ooff) >= SND_CHUNK_SIZE*2) {
				chunk = chunk->next;
				if (!chunk) {
					chunk = sc->soundData;
				}
...
void S_PaintChannels( int endtime ) {
	...
	snd_vol = FIXED_TO_INT(s_volume->value*LFIXED(255,0));
...
//			Com_Printf ("full stream\n");
		...
		for ( i = 0; i < MAX_CHANNELS ; i++, ch++ ) {		
			if ( !ch->thesfx || (ch->leftvol==0 && ch->rightvol==0 )) {
				continue;
			}
