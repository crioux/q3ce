...
SysCallArg CL_UISystemCalls( int callnum, SysCallArgs &args ) {
	...
	case UI_CVAR_VARIABLEVALUE:
		return SysCallArg::Fixed( Cvar_VariableValue( args[0] ) );

	case UI_CVAR_VARIABLESTRINGBUFFER:
		Cvar_VariableStringBuffer( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_CVAR_SETVALUE_L:
		Cvar_SetValue( args[0], (lfixed)args[1] );
		return SysCallArg();

	case UI_CVAR_SETVALUE_G:
		Cvar_SetValue( args[0], MAKE_LFIXED((gfixed)args[1]) );
		return SysCallArg();
	...
	case UI_CVAR_CREATE:
		Cvar_Get( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_CVAR_INFOSTRINGBUFFER:
		Cvar_InfoStringBuffer( args[0], args[1], args[2] );
		return SysCallArg();
  ...
  case UI_R_MODELBOUNDS:
	...
	case UI_KEY_KEYNUMTOSTRINGBUF:
		Key_KeynumToStringBuf( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_KEY_GETBINDINGBUF:
		Key_GetBindingBuf( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_KEY_SETBINDING:
		Key_SetBinding( args[0], args[1] );
		return SysCallArg();
	...
	case UI_KEY_GETOVERSTRIKEMODE:
		return SysCallArg::Int(Key_GetOverstrikeMode());

	case UI_KEY_SETOVERSTRIKEMODE:
		Key_SetOverstrikeMode( args[0] );
		return SysCallArg();
	...
	case UI_GETCLIPBOARDDATA:
		CL_GetClipboardData( args[0], args[1] );
		return SysCallArg();

	case UI_GETCLIENTSTATE:
		GetClientState( args[0] );
		return SysCallArg();		

	case UI_GETGLCONFIG:
		CLUI_GetGlconfig( args[0] );
		return SysCallArg();

	case UI_GETCONFIGSTRING:
		return SysCallArg::Int(GetConfigString( args[0], args[1], args[2] ));
	...
	case UI_LAN_ADDSERVER:
		return SysCallArg::Int(LAN_AddServer(args[0], args[1], args[2]));

	case UI_LAN_REMOVESERVER:
		LAN_RemoveServer(args[0], args[1]);
		return SysCallArg();

	case UI_LAN_GETPINGQUEUECOUNT:
		return SysCallArg::Int(LAN_GetPingQueueCount());

	case UI_LAN_CLEARPING:
		LAN_ClearPing( args[0] );
		return SysCallArg();

	case UI_LAN_GETPING:
		LAN_GetPing( args[0], args[1], args[2], args[3] );
		return SysCallArg();

	case UI_LAN_GETPINGINFO:
		LAN_GetPingInfo( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_LAN_GETSERVERCOUNT:
		return SysCallArg::Int(LAN_GetServerCount(args[0]));

	case UI_LAN_GETSERVERADDRESSSTRING:
		LAN_GetServerAddressString( args[0], args[1], args[2], args[3] );
		return SysCallArg();

	case UI_LAN_GETSERVERINFO:
		LAN_GetServerInfo( args[0], args[1], args[2], args[3] );
		return SysCallArg();

	case UI_LAN_GETSERVERPING:
		return SysCallArg::Int(LAN_GetServerPing( args[0], args[1] ));

	case UI_LAN_MARKSERVERVISIBLE:
		LAN_MarkServerVisible( args[0], args[1], args[2] );
		return SysCallArg();

	case UI_LAN_SERVERISVISIBLE:
		return SysCallArg::Int(LAN_ServerIsVisible( args[0], args[1] ));

	case UI_LAN_UPDATEVISIBLEPINGS:
		return SysCallArg::Int(LAN_UpdateVisiblePings( args[0] ));

	case UI_LAN_RESETPINGS:
		LAN_ResetPings( args[0] );
		return SysCallArg();

	case UI_LAN_SERVERSTATUS:
		return SysCallArg::Int(LAN_GetServerStatus( args[0], args[1], args[2] ));

	case UI_LAN_COMPARESERVERS:
		return SysCallArg::Int(LAN_CompareServers( args[0], args[1], args[2], args[3], args[4] ));

	case UI_MEMORY_REMAINING:
		return SysCallArg::Int(Hunk_MemoryRemaining());

	case UI_GET_CDKEY:
		CLUI_GetCDKey( args[0], args[1] );
		return SysCallArg();

	case UI_SET_CDKEY:
		CLUI_SetCDKey( args[0] );
		return SysCallArg();
	...
	case UI_VERIFY_CDKEY:
		return SysCallArg::Int(CL_CDKeyValidate(args[0], args[1]));


		
	default:
		Com_Error( ERR_DROP, "Bad UI system trap: %i", callnum );
...
void CL_ShutdownUI( void ) {
	...
	VM_Call( uivm, UI_SHUTDOWN, SysCallArgs(0));
	VM_Free( uivm );
	uivm = NULL;
}
...
void CL_InitUI( void ) {
	...
	// sanity check
	v = VM_Call( uivm, UI_GETAPIVERSION, SysCallArgs(0) );
	if (v == UI_OLD_API_VERSION) {
//		Com_Printf(S_COLOR_YELLOW "WARNING: loading old Quake III Arena User Interface version %d\n", v );
		// init for this gamestate
		SysCallArgs args(1);
		args[0]=(cls.state >= CA_AUTHORIZING && cls.state < CA_ACTIVE);
		VM_Call( uivm, UI_INIT, args);
	}
...
qboolean UI_usesUniqueCDKey() {
	if (uivm) {
		return ((int)VM_Call( uivm, UI_HASUNIQUECDKEY, SysCallArgs(0)) == qtrue);
	} else {
		return qfalse;
	}
}
...
qboolean UI_GameCommand( void ) {
	...
	SysCallArgs args(1);
	args[0]=cls.realtime;
	return VM_Call( uivm, UI_CONSOLE_COMMAND, args );
}
