...
#define		SOUND_ATTENUATE		AFIXED(0,0008)
...
static bvec3_t		listener_origin;
static avec3_t		listener_axis[3];
...
=================
*/
void S_SpatializeOrigin (bvec3_t origin, int master_vol, int *left_vol, int *right_vol)
{
    avec_t		dot;
    bfixed		dist;
	afixed		adist;
    avec_t		lscale, rscale, scale;
    avec3_t		source_vec;
    avec3_t		vec;

	const afixed dist_mult = SOUND_ATTENUATE;
	
	// calculate stereo seperation and distance attenuation
	bvec3_t tmp;
	VectorSubtract(origin, listener_origin, tmp);

	dist = VectorNormalizeB2A(tmp, source_vec);
	dist -= BFIXED(SOUND_FULLVOLUME,0);
	if (dist < BFIXED_0)
		dist = BFIXED_0;			// close enough to be at full volume
	adist = MAKE_AFIXED(dist * dist_mult);		// different attenuation levels
	...
	{
		rscale = FIXED_DIVPOW2((AFIXED_1 + dot),1);
		lscale = FIXED_DIVPOW2((AFIXED_1 - dot),1);
		//rscale = s_separation->value + ( GFIXED_1 - s_separation->value ) * dot;
		//lscale = s_separation->value - ( GFIXED_1 - s_separation->value ) * dot;
		if ( rscale < AFIXED_0 ) {
			rscale =AFIXED_0;
		}
		if ( lscale < AFIXED_0 ) {
	...
	scale = (AFIXED_1 - adist) * rscale;
	*right_vol = FIXED_INT32SCALE(master_vol,scale);
	if (*right_vol < 0)
		*right_vol = 0;

	scale = (AFIXED_1 - adist) * lscale;
	*left_vol = FIXED_INT32SCALE(master_vol,scale);
	if (*left_vol < 0)
		*left_vol = 0;
}
...
====================
*/
void S_StartSound(bvec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle ) {
	channel_t	*ch;
	sfx_t		*sfx;
...
==================
*/
void S_AddLoopingSound( int entityNum, const bvec3_t origin, const bvec3_t velocity, sfxHandle_t sfxHandle ) {
	sfx_t *sfx;
	...
	loopSounds[entityNum].oldDopplerScale = GFIXED_1;
	loopSounds[entityNum].dopplerScale = GFIXED_1;
	loopSounds[entityNum].sfx = sfx;

	lfixed lvec[3];
	lvec[0]=MAKE_LFIXED(velocity[0]); lvec[1]=MAKE_LFIXED(velocity[1]); lvec[2]=MAKE_LFIXED(velocity[2]);

	if (s_doppler->integer && FIXED_VEC3LEN_SQ(lvec)>LFIXED_0) {
		bvec3_t	out;
		lfixed	lena, lenb;

		loopSounds[entityNum].doppler = qtrue;
		lena = DistanceSquaredLong(loopSounds[listener_number].origin, loopSounds[entityNum].origin);
		VectorAdd(loopSounds[entityNum].origin, loopSounds[entityNum].velocity, out);
		lenb = DistanceSquaredLong(loopSounds[listener_number].origin, out);
		if ((loopSounds[entityNum].framenum+1) != cls.framecount) {
			loopSounds[entityNum].oldDopplerScale = GFIXED_1;
		} else {
			loopSounds[entityNum].oldDopplerScale = loopSounds[entityNum].dopplerScale;
		}
		loopSounds[entityNum].dopplerScale = MAKE_GFIXED(lenb/(lena*LFIXED(100,0)));
		if (loopSounds[entityNum].dopplerScale<=GFIXED_1) {
...
==================
*/
void S_AddRealLoopingSound( int entityNum, const bvec3_t origin, const bvec3_t velocity, sfxHandle_t sfxHandle ) {
	sfx_t *sfx;
...
============
*/
void S_RawSamples( int samples, int rate, int width, int s_channels, const byte *data, gfixed volume ) {
	int		i;
	int		src, dst;
	gfixed	scale;
	int		intVolume;
	...
	intVolume = FIXED_TO_INT(GFIXED(256,0) * volume);
	...
	scale = FIXED_INT32RATIO_G(rate,dma.speed);
...
======================
*/
void S_UpdateEntityPosition( int entityNum, const bvec3_t origin ) {
...
============
*/
void S_Respatialize( int entityNum, const bvec3_t head, avec3_t axis[3], int inwater ) {
	int			i;
	channel_t	*ch;
	bvec3_t		origin;
...
// check to make sure that we haven't overshot
	...
	if ( dma.submission_chunk < 256 ) {
		s_paintedtime = s_soundtime + FIXED_TO_INT(s_mixPreStep->value * MAKE_LFIXED(dma.speed));
	} else {
...
void S_Update_(void) {
	...
	ma = s_mixahead->value * MAKE_LFIXED(dma.speed);
	op = s_mixPreStep->value + MAKE_LFIXED(sane*dma.speed)*LFIXED(0,01);
	...
	endtime = s_soundtime + FIXED_TO_INT(ma);
...
void S_UpdateBackgroundTrack( void ) {
	...
	int		fileBytes;
	//int		r;
	static	gfixed	musicVolume = GFIXED(0,5);
	...
	musicVolume = (musicVolume + (MAKE_GFIXED(s_musicVolume->value) * GFIXED(2,0)))/GFIXED(4,0);

	// don't bother playing anything if musicvolume is 0
	if ( musicVolume <= GFIXED_0 ) {
		return;
	}
