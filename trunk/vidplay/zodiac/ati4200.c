 ...
 *
 * $Id: ati4200.c 157 2005-01-02 00:39:09Z picard $
 *
...
static void GetMode(ati4200* p)
{
	...
	p->p.Output.Video.Direction = GetOrientation() ^ (DIR_MIRRORUPDOWN | DIR_MIRRORLEFTRIGHT);

	if (p->p.Output.Video.Direction & DIR_SWAPXY)
	{
		...
		SwapInt(&p->p.Output.Video.Width,&p->p.Output.Video.Height);

		if (Ofs>=0 && Ofs<h*2)
			h -= Ofs>>1;

		p->Offset.y = h - p->p.Output.Video.Height;
	}
	else
		p->Offset.y = 0;
}
...
static int Update(ati4200* p)
{
	...
	p->Overlay.Width = ALIGN16(p->IDCT?p->IDCTWidth:p->p.Input.Video.Width);
	p->Overlay.Height = ALIGN16(p->IDCT?p->IDCTHeight:p->p.Input.Video.Height);
	p->Overlay.Direction = (p->p.FX.Direction ^ p->p.Input.Video.Direction) & DIR_SWAPXY;
...
static void StretchPlane(ati4200* p, int SrcOffset, int SrcPitch, rect* SrcRect, int DstOffset, int DstPitch, rect* DstRect)
{
	...
	DWidth = (SrcRect->Width << 8) / RScaleX;
	if ((SrcRect->Width << 8) % DWidth) 
		++DWidth;

	DHeight = (SrcRect->Height << 8) / RScaleY;
	if ((SrcRect->Height << 8) % DHeight) 
		++DHeight;
