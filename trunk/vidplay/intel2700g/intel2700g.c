 ...
 *
 * $Id: intel2700g.c 11 2004-07-15 13:46:20Z picard $
 *
...
static bool_t UpdateDirection(pvr* p)
{
	...
	if (p->IDCT && (!p->SetupIDCTRotate || p->YUV422))
	{
		...
		p->OvlFX.Direction = CombineDir(p->p.InputDirection,p->p.OrigFX.Direction,p->p.Output.Video.Direction);
	}
	else
	{
		p->SoftFX.Direction = CombineDir(p->p.InputDirection,p->p.OrigFX.Direction,p->NativeDirection);
		p->OvlFX.Direction = CombineDir(p->NativeDirection,0,p->p.Output.Video.Direction);
	}
	...
	p->DisableVSync = !p->SetupVSync || ((p->OvlFX.Direction ^ p->NativeDirection ^ p->p.Output.Video.Direction) & DIR_SWAPXY) != 0;
#endif

	New.Width = p->Width16+16;
	New.Height = p->Height16+16;
	New.Direction = CombineDir(p->p.Input.Video.Direction,p->SoftFX.Direction,0);
...
static int Init(pvr* p)
{
	...
	if (p->IDCTInit)
	{
		if (!PlanarYUV420(&p->p.Input.Video.Pixel))
		{
			if (PlanarYUV422(&p->p.Input.Video.Pixel))
				p->YUV422 = 1;
			else
	...
	if (!PlanarYUV(&p->p.Input.Video.Pixel,NULL,NULL,NULL))
		return ERR_NOT_SUPPORTED;
	...
	if (InitM24VA(p))
	{
		if (p->VDISP_Initialise(NULL) == VDISPError_OK)
		{
			...
			p->IDCTRounding = 0;
			p->Width16 = ALIGN16(p->IDCT ? p->IDCTWidth:p->p.Input.Video.Width)-16;
			if (p->Width16<0) p->Width16=0;
			p->Height16 = ALIGN16(p->IDCT ? p->IDCTHeight:p->p.Input.Video.Height)-16;
			if (p->Height16<0) p->Height16=0;

			if (p->IDCT)
			{
				if (p->p.Input.Video.Direction & DIR_SWAPXY)
					SwapInt(&p->p.Input.Video.Width,&p->p.Input.Video.Height);
				p->p.Input.Video.Direction = 0;
			}
			else
				if (p->p.Input.Video.Direction & DIR_SWAPXY)
...
{	
	...
	p->p.PrefDirection = CombineDir(NativeFX,0,p->p.Input.Video.Direction);
	...
	if (p->AllCount>=2)
	{
		...
		Attribs.bCKeyOn = 1;//p->p.ColorKey != -1;
		Attribs.ui32CKeyValue = RGBToFormat(p->p.ColorKey,&p->p.Output.Video.Pixel); 
...
static int IDCTGet(void* p, int No, void* Data, int Size )
{
	...
	case IDCT_FORMAT: GETVALUECOND(PVR(p)->p.Input.Video,video,PVR(p)->IDCT); break;
