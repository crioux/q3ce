 ...
 *
 * $Id: overlay.c 153 2004-12-20 16:46:49Z picard $
 *
...
static int DefaultBlit(overlay* p, const constplanes Data, const constplanes DataLast)
{
	planes Planes;
	int Result = p->Lock(p,Planes,1);
	if (Result==ERR_NONE)
	{
		BlitImage(p->Soft,Planes,Data,DataLast,p->Output.Video.Pitch,-1);
		p->Unlock(p);
	}
	return Result;
}
...
void OverlayClearBorder(overlay* p)
{
	...
	{
		uint32_t c = RGBToFormat(0,&p->Output.Video.Pixel);
		rect Viewport;
		VirtToPhy(&p->Viewport,&Viewport,&p->Output.Video);

		FillColor(Planes[0],p->Output.Video.Pitch,Viewport.x,Viewport.y,
			Viewport.Width,p->DstAlignedRect.y-Viewport.y,
			p->Output.Video.Pixel.BitCount,c);

		FillColor(Planes[0],p->Output.Video.Pitch,Viewport.x,p->DstAlignedRect.y,
			p->DstAlignedRect.x-Viewport.x,p->DstAlignedRect.Height,
			p->Output.Video.Pixel.BitCount,c);

		FillColor(Planes[0],p->Output.Video.Pitch,p->DstAlignedRect.x+p->DstAlignedRect.Width,p->DstAlignedRect.y,
			(Viewport.x+Viewport.Width)-(p->DstAlignedRect.x+p->DstAlignedRect.Width),p->DstAlignedRect.Height,
			p->Output.Video.Pixel.BitCount,c);

		FillColor(Planes[0],p->Output.Video.Pitch,Viewport.x,p->DstAlignedRect.y+p->DstAlignedRect.Height,
...
int OverlayUpdateAlign(overlay* p)
{
	...
	VirtToPhy(&p->Viewport,&p->DstAlignedRect,&p->Output.Video);
	VirtToPhy(NULL,&p->SrcAlignedRect,&p->Input.Video);

	BlitRelease(p->Soft);
	p->Soft = BlitCreate(&p->Output.Video,&p->Input.Video,&p->FX,&p->Caps);

	BlitAlign(p->Soft,&p->DstAlignedRect, &p->SrcAlignedRect);
	PhyToVirt(&p->DstAlignedRect,&p->GUIAlignedRect,&p->Output.Video);
...
{
	if (p->Inited && !p->UpdateInputFormat && p->PrefDirection != p->Input.Video.Direction && p->Pin.Node)
	{	
		packetformat Format = p->Input;
		if ((p->PrefDirection ^ Format.Video.Direction) & DIR_SWAPXY)
			SwapInt(&Format.Video.Width,&Format.Video.Height);
		Format.Video.Direction = p->PrefDirection;
...
int OverlayUpdateFX(overlay* p, bool_t ForceUpdate)
{
	...
	if (Aspect.Num<0)
	{
		...
		{
			Aspect.Num *= -p->Input.Video.Width;
			Aspect.Den *= p->Input.Video.Height;
		}
		else
		{
			Aspect.Num *= -p->Input.Video.Height;
			Aspect.Den *= p->Input.Video.Width;
		}
	}
	...
	p->FX.Direction = CombineDir(p->InputDirection,p->OrigFX.Direction,p->Output.Video.Direction);
	p->PrefDirection = CombineDir(p->FX.Direction,0,p->Input.Video.Direction);
	...
	if (p->OrigFX.ScaleX<=0)
	{
		...
		if (p->InputDirection & DIR_SWAPXY)
		{
			ScaleX = Scale(Width,SCALE_ONE,p->Input.Video.Width);
			ScaleY = Scale(Height,SCALE_ONE,Scale(p->Input.Video.Height,Aspect.Num,Aspect.Den));
		}
		else
		{
			ScaleX = Scale(Width,SCALE_ONE,Scale(p->Input.Video.Width,Aspect.Num,Aspect.Den));
			ScaleY = Scale(Height,SCALE_ONE,p->Input.Video.Height);
		}
	...
	if (p->Output.Video.Pixel.Flags & PF_PIXELDOUBLE)
	{
		p->FX.ScaleX >>= 1;
		p->FX.ScaleY >>= 1;
	}
...
static int UpdateInputDirection(overlay* p,bool_t Update)
{
	...
	{
		p->InputDirection = p->Input.Video.Direction;
		p->PreRotate = 0;

		if (p->AutoPrerotate)
		{
			rect r;
			PhyToVirt(NULL,&r,&p->Input.Video);
...
static int Updating(overlay* p)
{
	if (!p->Updating)
	{
		...
		OverlayUpdateFX(p,p->ViewportChanged);
...
static int UpdateInput(overlay* p)
{
	...
	{
		if (Compressed(&p->Input.Video.Pixel))
			return ERR_INVALID_DATA;

		if (p->Input.Video.Width<=0) 
		{
			p->Disabled = 1;
			p->Input.Video.Width = 2;
		}

		if (p->Input.Video.Height<=0) 
		{
			p->Disabled = 1;
			p->Input.Video.Height = 2;
		}
...
int OverlaySet(overlay* p,int No,const void* Data,int Size)
{
	...
	case OUT_INPUT|PIN_FORMAT:
		if (p->Inited && PacketFormatRotatedVideo(&p->Input,(const packetformat*)Data,DIR_SWAPXY|DIR_MIRRORLEFTRIGHT|DIR_MIRRORUPDOWN))
		{
