...
static tchar_t DocumentPath[MAX_PATH];
static tchar_t SystemPath[MAX_PATH];
...
void PlatformDetect(platform* p)
{
	...
	stprintf(p->Version,T("%d.%02d"),p->Ver/100,p->Ver%100);
...
#else
	tcscpy(p->PlatformType,T("Windows"));
	p->Type = TYPE_WIN32;
#endif
}
...
void _Assert(const char* Exp,const char* File,int Line)
{
	TCHAR TExp[MAXPATH];
	TCHAR TFile[MAXPATH];
	AsciiToTcs(TExp,File,MAXPATH);
	stprintf(TFile,T("%s:%d"),TExp,Line);
	AsciiToTcs(TExp,Exp,MAXPATH);
#ifndef NDEBUG
	DebugBreak();
#endif
...
//#ifdef NDEBUG
	{
		...
		sec = t / 1000;
		t -= sec*1000;
		stprintf(s+tcslen(s), T("%02d:%02d.%03d "),min,sec,t);
	}
#endif
...
void GetDebugPath(tchar_t* Path, const tchar_t* FileName)
{
	stprintf(Path,T("%s\\%s"),DocumentPath,FileName);
}

void GetSystemPath(tchar_t* Path, const tchar_t* FileName)
{
	stprintf(Path,T("%s\\%s"),SystemPath,FileName);
}
...
bool_t SaveDocument(const tchar_t* Name, const char* Text,tchar_t* URL)
{
	stream* f;
	stprintf(URL,T("%s\\%s.txt"),DocumentPath,Name);

	f = StreamOpen(URL,1);
	if (!f)
		return 0;

	StreamWrite(f,Text,strlen(Text));
	StreamClose(f);
	return 1;
}
...
int SafeException(void* p)
{
	...
	if (File)
	{
		...
		if (!NodeLocatePtr(Record->ExceptionAddress,DllName,&DllBase))
		{
			DllName[0] = 0;
		...
		if (Record->ExceptionCode == STATUS_ACCESS_VIOLATION)
		{
			...
			if (NodeLocatePtr((void*)Record->ExceptionInformation[1],DllName,&DllBase))
		...
		for (r=Reg;r->Name;++r)
		{
			...
			if (NodeLocatePtr(Ptr,DllName,&DllBase))
				StreamPrintf(File,T(" (%s:%08x)"),DllName,DllBase);
		}
...
bool_t CheckModule(const tchar_t* Name)
{
	...
	RegSize = sizeof(Path);
	stprintf(Path,T("SOFTWARE\\%s\\DLLPath"),Context()->ProgramName);
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, Path, 0, NULL, 0, KEY_READ|KEY_WRITE, NULL, &Key, &Disp) == ERROR_SUCCESS)
	{
