 ...
 *
 * $Id: node.c 202 2005-01-25 01:27:33Z picard $
 *
...
static NOINLINE int FindModule(context* p,const tchar_t* Path,int Id)
{
	...
	for (No=0;No<Count;++No)
	{
		...
		bool_t SameName = TcsICmp(Path,StringDef(MODULE_PATH,No))==0;
...
const tchar_t* NodeGetModule(int No,int* Id,int64_t* Date)
{
	...
	*Date = ARRAYBEGIN(Context()->NodeModule,nodemodule)[No].Date;
	return StringDef(MODULE_PATH,No);
}
...
static NOINLINE int ReleaseModules(context* p,bool_t Force)
{
	...
	for (j=ARRAYBEGIN(p->NodeModule,nodemodule);j!=ARRAYEND(p->NodeModule,nodemodule);++j)
		...
		{
			DEBUG_MSG1(-1,T("FreeModule %s"),StringDef(MODULE_PATH,j-ARRAYBEGIN(p->NodeModule,nodemodule)));
			FreeModule(j);
			++n;
		}
	LockLeave(p->NodeLock);
...
static NOINLINE nodemodule* LoadModule(context* p,int No)
{
	...
	{	
		DEBUG_MSG1(-1,T("LoadModule %s"),StringDef(MODULE_PATH,No));
		p->LoadModuleNo = No;
		Module->Module = NodeLoadModule(StringDef(MODULE_PATH,No),&Module->Id,&Module->Func,&Module->Db);
		p->LoadModuleNo = 0;
		if (!Module->Module)
			return NULL;
	}
...
static void Register(context* p,const nodedef* Def,bool_t Registered,int ModuleNo)
{
	...
	if (Registered)
	{
		...
		if (ModuleNo>0 && (Class->ModuleNo != ModuleNo || Class->Def.ParentClass != Def->ParentClass || Class->Def.Flags != Def->Flags))
		{
			...
			NodeRegSaveValue(Def->Class,NODE_MODULE_PATH,StringDef(MODULE_PATH,ModuleNo),MAXPATH,TYPE_STRING);
			NodeRegSaveValue(Def->Class,NODE_PARENT,&Def->ParentClass,sizeof(int),TYPE_INT);
			NodeRegSaveValue(Def->Class,NODE_FLAGS,Def->Flags?&Def->Flags:NULL,sizeof(int),TYPE_INT);
		}
#endif
	}
	...
	if (p && !p->InHibernate)
	{
		...
		for (Mode=0;!Changed && Mode<HIBERNATEMODES;++Mode)
			for (No=0;GetNode(p,No,&Node);++No)
				if (Node && Node->Set(Node,NODE_HIBERNATE,&Mode,sizeof(int))==ERR_NONE)
					Changed = 1;
...
static void DumpPtr(stream* File, tchar_t* s, node* Ptr, int No)
{
	if (Ptr)
	{
		...
		if (No)
			...
			v[0] = 0;
		StreamPrintf(File,T("%s%s%s (0x%08x)\n"),s,StringDef(Ptr->Class,0),v,(int)Ptr);
	}
	else
...
static void DumpFormat(stream* File, tchar_t* s,const packetformat* Format)
{
	...
	case PACKET_VIDEO:
		...
		if (Format->Video.Pixel.Flags & PF_FOURCC) 
		{
			FourCCToString(v,Format->Video.Pixel.FourCC,sizeof(v));
			StreamPrintf(File,T(" fourcc=%s"),v);
		}
		else
			StreamPrintf(File,T(" bitcount=%d rmask=%04x gmask=%04x bmask=%04x"),Format->Video.Pixel.BitCount,
				Format->Video.Pixel.BitMask[0],Format->Video.Pixel.BitMask[1],Format->Video.Pixel.BitMask[2]);
		if (Format->Video.Width) StreamPrintf(File,T(" width=%d"),Format->Video.Width);
		if (Format->Video.Height) StreamPrintf(File,T(" height=%d"),Format->Video.Height);
		if (Format->Video.Pitch) StreamPrintf(File,T(" pitch=%d"),Format->Video.Pitch);
		if (Format->Video.Direction) StreamPrintf(File,T(" dir=%d"),Format->Video.Direction);
		if (Format->ByteRate) StreamPrintf(File,T(" bitrate=%d"),Format->ByteRate*8);
		StreamPrintf(File,T("\n"));
		break;
	case PACKET_AUDIO:
		StreamPrintf(File,T("%saudio"),s);
		if (Format->Audio.Format) StreamPrintf(File,T(" fmt=%04x"),Format->Audio.Format);
		if (Format->Audio.SampleRate) StreamPrintf(File,T(" rate=%d"),Format->Audio.SampleRate);
		if (Format->Audio.Channels) StreamPrintf(File,T(" ch=%d"),Format->Audio.Channels);
		if (Format->Audio.Bits) StreamPrintf(File,T(" bits=%d"),Format->Audio.Bits);
		if (Format->ByteRate) StreamPrintf(File,T(" bitrate=%d"),Format->ByteRate*8);
		StreamPrintf(File,T("\n"));
		break;
	case PACKET_SUBTITLE:
		FourCCToString(v,Format->Subtitle.FourCC,sizeof(v));
...
void NodeDump(struct stream* File)
{
	...
	for (No=0,j=ARRAYBEGIN(p->NodeModule,nodemodule);j!=ARRAYEND(p->NodeModule,nodemodule);++j,++No)
		TRY_BEGIN 
		{
			...
			StreamPrintf(File,T("%s %08x-%08x %d\n"),StringDef(MODULE_PATH,No),j->Min,j->Max,j->ObjectCount);
		}
		TRY_END
	...
	for (i=ARRAYBEGIN(p->Node,node*);i!=ARRAYEND(p->Node,node*);++i)
		if (*i)
			TRY_BEGIN 
			{
				...
				StreamPrintf(File,T("%s (0x%08x)\n"),StringDef(p->Class,0),(int)p);

				for (No=0;NodeEnum(p,No,&DataDef)==ERR_NONE;++No)
				{
					TRY_BEGIN 
					{
						Name = StringDef(DataDef.Class,DataDef.No);
						if (!Name[0]) Name = DataDef.Name;
						stprintf(s,T("  %s(%d)="),Name,DataDef.No);
						...
						case TYPE_BOOL:
							...
							{
								BoolToString(Buffer,Bool,sizeof(Buffer));
						...
						case TYPE_TICK:
							...
							{
								TickToString(Buffer,Tick,sizeof(Buffer),Tick<TICKSPERSEC,1,0);
								StreamPrintf(File,T("%s%s\n"),s,Buffer);
							}
							break;
						case TYPE_INT:
							...
							{
								IntToString(Buffer,Int,sizeof(Buffer),(DataDef.Flags & DF_HEX)!=0);
								StreamPrintf(File,T("%s%s\n"),s,Buffer);
							}
							break;
						case TYPE_HOTKEY:
							if (p->Get(p,DataDef.No,&Int,sizeof(int))==ERR_NONE)
							{
								HotKeyToString(Buffer,Int,sizeof(Buffer));
								StreamPrintf(File,T("%s%s\n"),s,Buffer);
							}
							break;
						case TYPE_FRACTION:
							...
							{
								FractionToString(Buffer,&Fraction,sizeof(Buffer),1,2);
								StreamPrintf(File,T("%s%s\n"),s,Buffer);
							}
							break;
						case TYPE_STRING:
...
int NodeEnumTable(int* No, datadef* Out, const datatable* p)
{
	...
	if (*No < Count)
	{
		...
		case TYPE_RGB: Out->Size = sizeof(rgb); break;
		case TYPE_BINARY: Out->Size = Out->Format1; break;
		case TYPE_EQUALIZER: Out->Size = sizeof(equalizer); break;
		...
		Out->Name = String(Out->Class,Out->No);
		return ERR_NONE;
	}
...
int NodeEnumClassEx(array* List, int Class, const tchar_t* ContentType, const tchar_t* URL, const void* Data, int Length)
{
	...
	for (i=ARRAYBEGIN(p->NodeClassPri,nodeclass*);i!=ARRAYEND(p->NodeClassPri,nodeclass*);++i)
		if (((*i)->Def.Flags & CF_SIZE) && !TmpModule(p,(*i)->ModuleNo)) // skip abstract
			for (v=*i;v;v=v->Parent)
				if (v->Def.Class == Class)
				{
					...
					if ((ContentType || URL || Data) &&
						(!ContentType || !CheckContentType(ContentType,StringDef(Id,NODE_CONTENTTYPE))) &&
						(!URL || !CheckExts(URL,StringDef(Id,NODE_EXTS))) &&
						(!Data || !DataProbe(Data,Length,StringDef(Id,NODE_PROBE))))
						continue;
...
bool_t NodeLocatePtr(void* Ptr, tchar_t* OutName, int* OutBase)
{
	if (Ptr)
	{
		int No;
		...
		for (No=0,i=ARRAYBEGIN(p->NodeModule,nodemodule);i!=ARRAYEND(p->NodeModule,nodemodule);++i,++No)
		{
			if (i->Func)
			{
				...
				if (i->Min <= v && i->Max > v)
				{
					tcscpy(OutName,StringDef(MODULE_PATH,No));
					*OutBase = v - i->Min;
					return 1;
				}
			}
...
{
	const tchar_t* s = StringDef(Class,Id);
	int Len = tcslen(s);
	if (Len)
		++Len;
	else 
