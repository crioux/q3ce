 ...
 *
 * $Id: buffer.c 206 2005-03-19 15:03:03Z picard $
 *
...
bool_t BufferAlloc(buffer* p, int Size, int Align)
{
	uint8_t* Data;
	Align--;
...
bool_t BufferWrite(buffer* p, const void* Ptr, int Length, int Align)
{
	//append new data to buffer
	int WritePos = p->WritePos + Length;
...
bool_t BufferRead(buffer* p, const uint8_t** Ptr, int Length)
{
	if (p->WritePos - p->ReadPos < Length)
		return 0;
...
void BufferPack(buffer* p, int Length)
{
	int Skip = p->ReadPos + Length;
	if (p->WritePos > Skip)
	{
		p->WritePos -= Skip;
		if (Skip)
...
bool_t ArrayAlloc(array* p,int Total,int Align)
{
	uint8_t* Data;
	--Align;
...
bool_t ArrayAppend(array* p, const void* Ptr, int Length, int Align)
{
	int Total = p->_End - p->_Begin + Length;
...
void ArraySort(array* p, int Count, int Width, arraycmp Cmp)
{
	qsort(p->_Begin,Count,Width,Cmp);
}

int ArrayFind(const array* p, int Count, int Width, const void* Data, arraycmp Cmp, bool_t* Found)
{
	if (Cmp)
	{
		int i;
...
bool_t ArrayAdd(array* p,int Count, int Width, const void* Data, arraycmp Cmp,int Align)
{
	int Pos;
	bool_t Found;
...
bool_t ArrayRemove(array* p, int Count, int Width, const void* Data, arraycmp Cmp)
{
	bool_t Found;
...
bool_t ArrayUnlock(array* p,int Align)
{
#ifdef TARGET_PALMOS
	if (p->_Block.Ptr)
	{
