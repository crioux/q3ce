 ...
 *
 * $Id: format_base.c 206 2005-03-19 15:03:03Z picard $
 *
...
bool_t DropBuffer(format_base* p)
{
	bool_t Dropped = 0;
	int No=0;
...
void CalcDuration(format_base* p)
{
	...
	if (p->StreamCount && p->AutoReadSize)
	{
		int ByteRate = SumByteRate(p);
...
void Format_BufferRelease(format_base* p, format_buffer* Buffer)
{
	...
	assert(Buffer->RefCount>0 && Buffer->RefCount<500);
...
static void ReleaseStream(format_base* p,format_stream* Stream)
{
	...
	if (p->ReleaseStream && Stream != p->SubTitle)
...
int FormatBaseGet(format_base* p,int No,void* Data,int Size)
{
	...
	if (No >= FORMAT_COMMENT && No < FORMAT_COMMENT+p->StreamCount)  
		GETVALUE(p->Streams[No-FORMAT_COMMENT]->Comment,pin)
	else
	if (No >= FORMAT_STREAM && No < FORMAT_STREAM+p->StreamCount)
		GETVALUE(p->Streams[No-FORMAT_STREAM]->Pin,pin)
	else
	if (No >= (FORMAT_STREAM|PIN_PROCESS) && No < (FORMAT_STREAM|PIN_PROCESS)+p->StreamCount)
		GETVALUE(p->Streams[No-(FORMAT_STREAM|PIN_PROCESS)]->Process,packetprocess)
	else
	if (No >= (FORMAT_STREAM|PIN_FORMAT) && No < (FORMAT_STREAM|PIN_FORMAT)+p->StreamCount)
	{
	...
	case FORMAT_FILEPOS: GETVALUECOND(p->Reader[0].FilePos,int,p->Reader[0].FilePos>=0); break;
	case FORMAT_FILEALIGN: GETVALUE(p->FileAlign,int); break;
	case FORMAT_STREAM_COUNT: GETVALUE(p->StreamCount,int); break;
	...
	case FORMAT_SUBTITLE: GETVALUECOND(p->SubTitle->Pin,pin,p->SubTitle!=NULL); break;
	case FORMAT_SUBTITLE|PIN_PROCESS: GETVALUECOND(p->SubTitle->Process,packetprocess,p->SubTitle!=NULL); break;
	case FORMAT_SUBTITLE|PIN_FORMAT: GETVALUECOND(p->SubTitle->Format,packetformat,p->SubTitle!=NULL); break;
	}
	return Result;
}
...
int FormatBaseSet(format_base* p,int No,const void* Data,int Size)
{
	...
	case FORMAT_SUBTITLE|PIN_PROCESS:
		SETVALUECOND(p->SubTitle->Process,packetprocess,ERR_NONE,p->SubTitle!=NULL);
		break;

	case FORMAT_SUBTITLE:
		SETVALUECOND(p->SubTitle->Pin,pin,UpdateStreamPin(p,p->SubTitle),p->SubTitle!=NULL);
		break;
	}
	return Result;
}
...
int Format_Sync(format_base* p,tick_t Time,int FilePos,bool_t PrevKey)
{
...
static int Synced(format_base* p,format_stream* Stream)
{
	p->SyncMode = 0;
...
int Format_Send(format_base* p,format_stream* Stream)
{
	...
	{
		if (CurrTime >= 0 && Stream->Packet.RefTime < CurrTime - p->DropTolerance)
		{
			if (Stream->Packet.RefTime < CurrTime - p->SkipTolerance - p->DropTolerance)
			{
				Stream->State.DropLevel = 2;
				Stream->DropCount = DROP_SHOW;
			}
...
int Format_FillQueue(format_base* p,format_reader* Reader)
{
	...
	if (!Reader->BufferAvailable && !Reader->ReadBuffer && 
	...
	if (Result == ERR_NONE && Packet->Stream && Packet->Stream->Pin.Node)
	{
		...
		if (Packet->RefTime>=0)
		{
			...
			if (s->LastTime > Packet->RefTime && Packet->RefTime-p->GlobalOffset < TICKSPERSEC)
...
bool_t Format_AllocBufferBlock(format_base* p,format_stream* Stream,int Size)
{
...
int Format_ProcessStream(format_base* p,format_stream* Stream)
{
	format_packet* Packet;
	format_ref* Ref;
	int Result = ERR_NONE;
	...
	for (No=0;No<Burst;++No)
	{
		...
		if (p->SyncMode && Stream != p->SyncStream && p->SyncStream)
		{
			...
			while (Packet && Packet->RefTime < p->SyncStream->LastTime)
			{
				Stream->PacketFirst = Packet->Next;
				if (Stream->PacketLast == Packet)
					Stream->PacketLast = NULL;
...
void Format_Sended(format_base* p, format_stream* Stream)
{
	format_packet* Packet = Stream->PacketFirst;
	if (Packet)
	{
...
format_stream* Format_DefSyncStream(format_base* p, format_reader* Reader)
{
...
int Format_Process(format_base* p,processstate* State)
{
	int No;
	int Result;
	...
	if (!p->HeaderLoaded && p->Reader[0].FilePos <= HEADERLOAD)
	{
...
bool_t Format_ReadBuffer(format_reader* Reader, bool_t ToRead)
{
	...
	if (!Reader->BufferFirst && !Reader->NoMoreInput)
	{
		...
		if (Buffer)
		{
			int Length = Reader->Input->ReadBlock(Reader->Input,&Buffer->Block,Buffer->Length,BLOCKSIZE - Buffer->Length);
			if (Length < 0)
			{
				// failed
				if (!Reader->InputBuffer)
...
{
	int64_t v = Reader_ReadLE32(p);
	v |= (int64_t)Reader_ReadLE32(p) << 32;
	return v;
}

int GetRate(format_reader* p,format_reader* Last)
{
	if (p->NoMoreInput)
		return MAX_INT;
	...
	return p->Ratio * (p->BufferAvailable - (p==Last?READER_BONUS:0));
}

int Format_ReadInput(format_base* p,int BufferMax,int* BufferUsed)
{
	int No;
	int Left,Length;
	int Result = ERR_NONE;
	...
	*BufferUsed = p->BufferUsed;
	...
	else
	{
		...
		{
			if (!Length || Buffer->Length == BLOCKSIZE)
			{
				LockEnter(p->BufferLock);
				Format_BufferInsert(Reader,Buffer);
				LockLeave(p->BufferLock);
...
void Format_FindSubtitles(format_base* p)
{
/*
//!SUBTITLE
	...
	tchar_t URL[MAXPATH];
	LockEnter(p->InputLock);
	if (p->Input->Get(p->Input,STREAM_URL,URL,sizeof(URL)) != ERR_NONE)
		URL[0] = 0;
	LockLeave(p->InputLock);
...
void Format_PrepairStream(format_base* p,format_stream* Stream)
{
	...
	if (Stream->Format.Type == PACKET_AUDIO && p->StreamCount==1)
		p->ProcessMinBuffer = PROCESSMINAUDIO;
	else
		p->ProcessMinBuffer = PROCESSMINVIDEO;

	if (Stream->Format.Type == PACKET_AUDIO && Stream->Format.Audio.Format == AUDIOFMT_PCM)
	{
...
int Format_BitmapInfoMem(format_stream* s,void* Data,int Length)
{
	if (Length>=4)
	{
		...
		if (Length >= Size)
		{
			...
			s->Format.Video.Width = INT32LE(*(int32_t*)(p+4));
			s->Format.Video.Height = INT32LE(*(int32_t*)(p+8));
			s->Format.Video.Pixel.BitCount = INT16LE(*(int16_t*)(p+14));
			s->Format.Video.Pixel.FourCC = INT32LE(*(int32_t*)(p+16));
			s->Format.Video.Aspect = ASPECT_ONE;
...
void Format_BitmapInfo(format_reader* p,format_stream* s,int Length)
{
	if (Length>=4)
	{
		...
		if (Length >= Size)
		{
			PacketFormatClear(&s->Format);
			s->Format.Type = PACKET_VIDEO;
			s->Format.Video.Width = p->ReadLE32(p); 
			s->Format.Video.Height = p->ReadLE32(p); 
			p->ReadLE16(p); //planes
			s->Format.Video.Pixel.BitCount = p->ReadLE16(p);
			s->Format.Video.Pixel.FourCC = p->ReadLE32(p);
			s->Format.Video.Aspect = ASPECT_ONE;
...
int Format_WaveFormatMem(format_stream* s,void* Data,int Length)
{
	if (Length >= 16)
	{
		...
		s->Format.Audio.Format = INT16LE(*(int16_t*)(p+0));
		s->Format.Audio.Channels =INT16LE(*(int16_t*)(p+2));
		s->Format.Audio.SampleRate = INT32LE(*(int32_t*)(p+4));
		s->Format.ByteRate = INT32LE(*(int32_t*)(p+8));
		s->Format.Audio.BlockAlign = INT16LE(*(int16_t*)(p+12));
		s->Format.Audio.Bits = INT16LE(*(int16_t*)(p+14));
		Length -= 16;
...
void Format_WaveFormat(format_reader* p,format_stream* s,int Length)
{
	if (Length >= 16)
	{
		PacketFormatClear(&s->Format);
		s->Format.Type = PACKET_AUDIO;
		s->Format.Audio.Format = p->ReadLE16(p);
		s->Format.Audio.Channels = p->ReadLE16(p);
		s->Format.Audio.SampleRate = p->ReadLE32(p);
		s->Format.ByteRate = p->ReadLE32(p);
		s->Format.Audio.BlockAlign = p->ReadLE16(p);
		s->Format.Audio.Bits = p->ReadLE16(p);
		Length -= 16;
...
#ifdef TARGET_PALMOS
	p->UseBufferBlock = QueryPlatform(PLATFORM_LOWMEMORY);
#endif
