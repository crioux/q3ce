...
static bool_t BuildI420(dmo* p)
{
	...
	memcpy(&p->Codec.Out.Format.Video,&p->Codec.In.Format.Video,sizeof(video));
	p->Codec.Out.Format.Video.Pixel.Flags = PF_FOURCC;
	p->Codec.Out.Format.Video.Pixel.FourCC = FOURCC_I420;
	p->Codec.Out.Format.Video.Pixel.BitCount = 16;
	p->Codec.Out.Format.Video.Pitch = p->Codec.Out.Format.Video.Width;
	...
	VideoInfo->bmiHeader.biWidth = p->Codec.Out.Format.Video.Width;
	VideoInfo->bmiHeader.biHeight = p->Codec.Out.Format.Video.Height;
	VideoInfo->bmiHeader.biCompression = p->Codec.Out.Format.Video.Pixel.FourCC;
	VideoInfo->bmiHeader.biBitCount = (WORD)p->Codec.Out.Format.Video.Pixel.BitCount;
	VideoInfo->bmiHeader.biPlanes = 1;
	VideoInfo->bmiHeader.biSizeImage = (p->Codec.Out.Format.Video.Width * 
		p->Codec.Out.Format.Video.Height * p->Codec.Out.Format.Video.Pixel.BitCount)/8;

	VideoInfo->rcSource.left   = 0;
	VideoInfo->rcSource.top    = 0;
	VideoInfo->rcSource.right  = p->Codec.Out.Format.Video.Width;
	VideoInfo->rcSource.bottom = p->Codec.Out.Format.Video.Height;
...
/*	PacketFormatClear(&p->Codec.Out.Format);
	...
	memcpy(&p->Codec.Out.Format.Video,&p->Codec.In.Format.Video,sizeof(video));
	p->Codec.Out.Format.Video.Pixel.Flags = PF_FOURCC;
	p->Codec.Out.Format.Video.Pixel.FourCC = FOURCC_YUY2;
	p->Codec.Out.Format.Video.Pixel.BitCount = 16;
	p->Codec.Out.Format.Video.Pitch = p->Codec.Out.Format.Video.Width*2;
	...
	VideoInfo->bmiHeader.biSize = 44;//sizeof(VideoInfo->bmiHeader);
	VideoInfo->bmiHeader.biWidth = p->Codec.Out.Format.Video.Width;
	VideoInfo->bmiHeader.biHeight = p->Codec.Out.Format.Video.Height;
	VideoInfo->bmiHeader.biCompression = p->Codec.Out.Format.Video.Pixel.FourCC;
	VideoInfo->bmiHeader.biBitCount = (WORD)p->Codec.Out.Format.Video.Pixel.BitCount;
...
static bool_t MatchOutVideo(dmo* p) 
{
	int No;
    for (No=0;;++No)
	{
			...
			p->OutType.pbFormat)
		{
			...
			PacketFormatClear(&p->Codec.Out.Format);
			p->Codec.Out.Format.Type = PACKET_VIDEO;
			p->Codec.Out.Format.Video.Pixel.FourCC = Info->bmiHeader.biCompression;
			p->Codec.Out.Format.Video.Pixel.BitCount = Info->bmiHeader.biBitCount;
			p->Codec.Out.Format.Video.Width = Info->bmiHeader.biWidth;
			p->Codec.Out.Format.Video.Height = Info->bmiHeader.biHeight;
			p->Codec.Out.Format.Video.Aspect = ASPECT_ONE;
			if (Info->bmiHeader.biCompression == 3)
			{
				p->Codec.Out.Format.Video.Pixel.BitMask[0] = 0xF800;
				p->Codec.Out.Format.Video.Pixel.BitMask[1] = 0x07E0;
				p->Codec.Out.Format.Video.Pixel.BitMask[2] = 0x001F;
			}
			PacketFormatDefault(&p->Codec.Out.Format);

			if (p->Codec.Out.Format.Video.Pixel.BitCount==16 && 
				p->Codec.Out.Format.Video.Pixel.FourCC==3 && p->ForceRGB)
				return 1;

			if (PlanarYUV420(&p->Codec.Out.Format.Video.Pixel) && !p->ForceRGB)
				return 1;
		}
...
static bool_t BuildPCM(dmo* p)
{
	...
	WaveFormat->wFormatTag = (WORD)p->Codec.Out.Format.Audio.Format;
	WaveFormat->nChannels = (WORD)p->Codec.Out.Format.Audio.Channels;
	WaveFormat->nSamplesPerSec = p->Codec.Out.Format.Audio.SampleRate;
	WaveFormat->nAvgBytesPerSec = p->Codec.Out.Format.ByteRate;
	WaveFormat->nBlockAlign = (WORD)p->Codec.Out.Format.Audio.BlockAlign;
	WaveFormat->wBitsPerSample = (WORD)p->Codec.Out.Format.Audio.Bits;
	WaveFormat->cbSize = 0;
...
static int UpdateInput(dmo* p)
{
	...
	if (p->Codec.In.Format.Type != PACKET_NONE)
	{
		...
		case PACKET_VIDEO:
			...
			if ((p->Codec.In.Format.Video.Pixel.FourCC == FOURCC('W','M','V','3') || 
				 p->Codec.In.Format.Video.Pixel.FourCC == FOURCC('W','M','V','2')) && Extra>4)
			...
			VideoInfo->bmiHeader.biWidth = p->Codec.In.Format.Video.Width;
			VideoInfo->bmiHeader.biHeight = p->Codec.In.Format.Video.Height;
			VideoInfo->bmiHeader.biCompression = p->Codec.In.Format.Video.Pixel.FourCC;
			VideoInfo->bmiHeader.biBitCount = (WORD)p->Codec.In.Format.Video.Pixel.BitCount;
			VideoInfo->bmiHeader.biPlanes = 1;
			VideoInfo->bmiHeader.biSizeImage = (p->Codec.In.Format.Video.Width * 
				p->Codec.In.Format.Video.Height * p->Codec.In.Format.Video.Pixel.BitCount)/8;
			if (p->WMPVersion > 9)
			...
			VideoInfo->rcSource.left   = 0;
			VideoInfo->rcSource.top    = 0;
			VideoInfo->rcSource.right  = p->Codec.In.Format.Video.Width;
			VideoInfo->rcSource.bottom = p->Codec.In.Format.Video.Height;
			VideoInfo->rcTarget        = VideoInfo->rcSource;
			...
			if (p->WMPVersion==9 && PlanarYUV420(&p->Codec.Out.Format.Video.Pixel))
			{
				// override default pitch, it has to be dword aligned
				p->Codec.Out.Format.Video.Pitch = (p->Codec.Out.Format.Video.Pitch+3) & ~3;
				MinBufferSize = 1;
			}

			AutoBufferSize = GetImageSize(&p->Codec.Out.Format.Video);
		...
		case PACKET_AUDIO:
			...
			WaveFormat->wFormatTag = (WORD)p->Codec.In.Format.Audio.Format;
			WaveFormat->nChannels = (WORD)p->Codec.In.Format.Audio.Channels;
			WaveFormat->nSamplesPerSec = p->Codec.In.Format.Audio.SampleRate;
			WaveFormat->nAvgBytesPerSec = p->Codec.In.Format.ByteRate;
			WaveFormat->nBlockAlign = (WORD)p->Codec.In.Format.Audio.BlockAlign;
			WaveFormat->wBitsPerSample = (WORD)p->Codec.In.Format.Audio.Bits;
			...
			p->InType.subtype = WMCMEDIASUBTYPE;
			p->InType.subtype.v1 = p->Codec.In.Format.Audio.Format;
...
{
	p->Module = LoadLibrary(String(p->Codec.Node.Class,DMO_MODULEPPC));
	GetProc(&p->Module,&p->CreateCodecDMO,T("CreateCodecDMO"),0);
	GetProc(&p->Module,&p->QueryUnloadCodecDMO,T("QueryUnloadCodecDMO"),1);

	if (!p->Module && StringToGUID(String(p->Codec.Node.Class,DMO_CLASSID),&p->ClassId))
	{
		p->Module = LoadLibrary(String(p->Codec.Node.Class,DMO_MODULE));
...
{
	if (CheckModule(String(Class,DMO_MODULEPPC)))
		return 2;
	if (CheckModule(String(Class,DMO_MODULE)))
		return 1;
	return 0;
}
