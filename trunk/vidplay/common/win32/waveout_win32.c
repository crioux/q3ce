...
static int SetVolumeSoft(waveout* p,int v,bool_t m)
{
	...
	if (p->SoftwareVolume)
	{
		if (p->Handle && (p->Mute || p->VolumeSoft==0)!=OldSkip)
		{
			if (!OldSkip)
				Pause(p);
			else 
	...
	if (p->Handle)
	{
		...
		if (Adjust != 256)
		{
			...
			for (List=p->Buffers;List;List=List->GlobalNext)
				VolumeMul(Adjust,(void*)List->Block.Ptr,p->BufferLength,&p->Output.Audio);
			LeaveCriticalSection(&p->Section);
		}
	}
...
static void UpdateBenchAvg(waveout* p)
{
	...
	n = p->Input.Audio.Bits >> 3;
	if (!(p->Input.Audio.Flags & PCM_PLANES))
		n *= p->Input.Audio.Channels;
	n = p->Output.Audio.SampleRate;
	if (n>0)
...
static void ReleaseBuffer(waveout* p,wavebuffer* Buffer,bool_t Update)
{
	EnterCriticalSection(&p->Section);
...
static int Send(waveout* p, const constplanes Planes, int Length, tick_t RefTime, tick_t CurrTime, int Speed)
{
	...
	while (Length > 0)
	{
		...
		if (p->VolumeRamp < RAMPLIMIT)
			p->VolumeRamp = VolumeRamp(p->VolumeRamp,DstPlanes[0],DstLength,&p->Output.Audio);
	...
	if (Length && p->Input.Audio.BlockAlign>0)
		p->Skip = p->Input.Audio.BlockAlign - Length % p->Input.Audio.BlockAlign;
...
static void UpdatePCM(waveout* p,const audio* InputFormat)
{
	...
	p->AdjustedRate = Scale(p->Output.Audio.SampleRate,p->Speed.Den,p->Speed.Num);

	PCMRelease(p->PCM);
	p->PCM = PCMCreate(&p->Output.Audio,InputFormat,p->Dither,p->SoftwareVolume);
}
...
static int UpdateInput(waveout* p)
{
	...
	if (p->Input.Type == PACKET_AUDIO)
	{
		...
		do
		{
			...
			p->Format.nChannels = (WORD)p->Output.Audio.Channels;
			p->Format.nSamplesPerSec = p->Output.Audio.SampleRate;
			p->Format.wBitsPerSample = (WORD)p->Output.Audio.Bits;
			...
			if (MMResult==WAVERR_BADFORMAT)
			{
				...
				if (Try==1)
				{
					if (p->Output.Audio.SampleRate > 35000)
						p->Output.Audio.SampleRate = 44100;
					else
						++Try;
				}
...
static int Set(waveout* p, int No, const void* Data, int Size)
{
	...
	case NODE_SETTINGSCHANGED:
		p->SoftwareVolume = QueryAdvanced(ADVANCED_SYSTEMVOLUME)==0;
