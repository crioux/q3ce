 ...
 *
 * $Id: blit_soft.c 113 2004-11-25 06:44:11Z picard $
 *
...
{
	p->Overlay.Output.Video.Width = p->Driver->Width;
	p->Overlay.Output.Video.Height = p->Driver->Height;
	p->Overlay.Output.Video.Direction = 0;
	p->Overlay.Output.Video.Aspect = ASPECT_ONE;
	p->Overlay.Output.Video.Pitch = p->Overlay.Output.Video.Width*2;
	DefaultRGB(&p->Overlay.Output.Video.Pixel,16,5,6,5,0,0,0);
	...
	if (p->Overlay.Output.Video.Direction & DIR_SWAPXY)
	{
		...
		if (Model == MODEL_IPAQ_H3800 || Model == MODEL_IPAQ_H3900)
			p->Overlay.Output.Video.Direction ^= DIR_MIRRORLEFTRIGHT|DIR_MIRRORUPDOWN;
	}
...
static int UpdateAlign(xscale* p)
{
	...
	PhyToVirt(NULL,&FullScreen,&p->Overlay.Output.Video);
	...
	{
		int n;
		for (n=0;n<p->Overlay.Output.Video.Height;++n)
			p->Driver->Map[n]=n;

		if (p->Driver->SetupStretch && 
			((p->Overlay.Node.Class == XSCALE_LOW_ID && *ScaleY > SCALE_ONE/2) || *ScaleY > SCALE_ONE) && 
			!(p->Overlay.Output.Video.Width & 1) && p->Overlay.Output.Video.Height>0)
		{
			int Num,Den;
			int Adj;
			...
			*ScaleY = p->Overlay.Node.Class == XSCALE_LOW_ID ? SCALE_ONE/2 : SCALE_ONE;
			...
			VirtToPhy(&p->Overlay.Viewport,&DstAlignedRect,&p->Overlay.Output.Video);
			VirtToPhy(NULL,&SrcAlignedRect,&p->Overlay.Input.Video);

			BlitRelease(p->Overlay.Soft);
			p->Overlay.Soft = BlitCreate(&p->Overlay.Output.Video,&p->Overlay.Input.Video,&FX,&p->Overlay.Caps);
...
#ifdef X50V
	p->X50v = QueryPlatform(PLATFORM_MODEL)==MODEL_AXIM_X50 && FileExists(T("\\windows\\pvrvadd.dll"));
	if (p->X50v)
	{
		p->Width = 480;
		p->Height = 640;
		...
		{
			video Output;
			QueryRawFrameBuffer(&Output,&p->OrigBuffer);
			if (Output.Width != 480 || Output.Height != 640 || Output.Pixel.BitCount != 16)
				p->OrigBuffer = NULL;
		}
	}
#endif
