 ...
 *
 * $Id: overlay_hires.c 157 2005-01-02 00:39:09Z picard $
 *
...
static int Init(hires* p)
{
	...
	p->Bits = QueryScreen(&p->Overlay.Output.Video,&p->Offset,&Mode);
	if (!p->Bits)
	...
	p->OrigPitch = p->Overlay.Output.Video.Pitch;
	p->Next = 0;
	p->LowLevelFailed = 0;
	return ERR_NONE;
}
...
static void Hide(hires* p)
{
	if (p->Overlay.Overlay)
	{
		p->LowLevelSetOrig(p);
		p->Overlay.Overlay = 0;
		p->Overlay.Output.Video.Pitch = p->OrigPitch;
	}
}
...
{
	int Pitch = p->Overlay.Output.Video.Pitch;
	Init(p);
	if (p->Overlay.Overlay)
		p->Overlay.Output.Video.Pitch = Pitch;
	return ERR_NONE;
}
...
static int XScaleGetOrig(hires* p)
{
	...
	if (p->Overlay.Output.Video.Width > p->PhyWidth ||
		p->Overlay.Output.Video.Height > p->PhyHeight)
	...
	p->Overlay.Output.Video.Pitch = p->PhyWidth*2;
	if (p->UseBorder)
		p->Overlay.Output.Video.Pitch += (p->Border[0]+p->Border[1])*2;
...
static int XScaleAlloc(hires* p)
{
	...
	if (!AllocBlock(16+sizeof(xscaledesc)+256+p->SurfaceSize,&Buffer->Buffer,0,HEAP_ANYWR))
		return ERR_OUT_OF_MEMORY;
	Buffer->Desc = (xscaledesc*)ALIGN16(Buffer->Buffer.Ptr);
...
static int OMAPAlloc(hires* p)
{
	...
	if (!AllocBlock(p->SurfaceSize+16+OMAP_PAL_SIZE,&Buffer->Buffer,0,HEAP_ANYWR))
		return ERR_OUT_OF_MEMORY;
	Buffer->Surface = (uint8_t*)ALIGN16(Buffer->Buffer.Ptr)+OMAP_PAL_SIZE;
...
static int Create(hires* p)
{
	...
	p->UseBorder = p->Model==MODEL_LIFEDRIVE;
	return ERR_NONE;
}
