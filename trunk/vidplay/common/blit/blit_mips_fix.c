 ...
 *
 * $Id: blit_arm_fix.c 3 2004-07-13 11:26:13Z picard $
 *
...
static void Fix_RGB_UV_LoadUV(blit_soft* p,int OfsUV,int OfsUV2)
{
	if (p->DstPalette)
	{
		if (!p->Scale)
...
static void Fix_RGB_UV_Pixel(blit_soft* p, int Row, int Col)
{
	int Shift;
	reg v;
	reg Y1,Y2;
...
// R3,R2 for temp
static void Write(blit_soft* p,reg Dst,reg A,reg B,bool_t Prepare)
{
	switch (p->DstBPP)
	{
	case 8:
...
void Fix_RGB_UV(blit_soft* p)
{
	...
	p->Scale =  !(p->RScaleX == 8 || p->RScaleX == 16) || 
				!(p->RScaleY == 8 || p->RScaleY == 16) ||
				p->SrcUVX2!=1 || p->SrcUVY2!=1;

	if (p->Scale)
		p->ColorLookup = p->DstDoubleX = p->DstDoubleY = 0;
