 ...
 *
 * $Id: ati3200.c 145 2004-12-12 18:18:28Z picard $
 *
...
static bool_t CheckVersion(ahi* p)
{
	int a,b;

	StrToTcs(p->Version,p->DevInfo.Version,sizeof(p->Version)/sizeof(tchar_t));
...
static void GetMode(ahi* p)
{
	...
	memset(&p->p.Output.Video,0,sizeof(video));
	p->p.Output.Video.Width = p->ModeInfo.Width;
	p->p.Output.Video.Height = p->ModeInfo.Height;
	p->p.Output.Video.Direction = 0;
	p->p.Output.Video.Aspect = ASPECT_ONE;

	if (p->ModeInfo.Rotation!=0 && p->ModeInfo.Rotation!=2)
		p->p.Output.Video.Direction ^= DIR_SWAPXY;

	switch (p->ModeInfo.Rotation)
	{
	case 1:
		p->p.Output.Video.Direction ^= DIR_MIRRORLEFTRIGHT;
		break;
	case 2:
		p->p.Output.Video.Direction ^= DIR_MIRRORLEFTRIGHT|DIR_MIRRORUPDOWN;
		break;
	case 3:
		p->p.Output.Video.Direction ^= DIR_MIRRORUPDOWN;
		break;
	}
	...
	Ahi2PixelFormat(p->ModeInfo.PixelFormat,&p->p.Output.Video.Pixel);
	FillInfo(&p->p.Output.Video.Pixel);

	AdjustOrientation(&p->p.Output.Video,0);
}
...
static int GetFreeMemory(ahi* p,int Mem)
{
	...
	if (p->AhiSurfGetLargestFreeBlockSize)
	{
		...
		if (p->AhiInit(0)==SUCCESS)
		{
			int No;
			for (No=0;;No++)
			{
				int Result = p->AhiDevEnum(&p->Device,&p->DevInfo,No);
				if (Result == SUCCESS && CheckVersion(p))
				{
					if (p->AhiDevOpen(&p->Context,p->Device,"TCPMP",2)==SUCCESS)
					{
...
static int Get(ahi* p,int No,void* Data,int Size)
{
	...
	{
	case AHI_NAME: 
		StrToTcs((tchar_t*)Data,p->DevInfo.Name,Size/sizeof(tchar_t));
		Result = ERR_NONE;
		break;

	case AHI_VERSION: 
		StrToTcs((tchar_t*)Data,p->DevInfo.Version,Size/sizeof(tchar_t));
		Result = ERR_NONE;
		break;
...
static void UpdateColorKey(ahi* p)
{
	...
	if (p->p.FullScreenViewport)
		p->p.ColorKey = 0; // black colorkey because of lowbitdepth
...
static void UpdateAlign(ahi* p,int ScaleMode)
{
	...
	PhyToVirt(NULL,&FullScreen,&p->p.Output.Video);

	VirtToPhy(&p->p.Viewport,&p->p.DstAlignedRect,&p->p.Output.Video);
	VirtToPhy(NULL,&p->OverlayRect,&p->p.Input.Video);
...
static void UpdateAHI(ahi* p)
{
	...
	if (p->p.ColorKey != -1) {
		p->Param.ColorMask = 
			p->p.Output.Video.Pixel.BitMask[0] | 
			p->p.Output.Video.Pixel.BitMask[1] |
			p->p.Output.Video.Pixel.BitMask[2];
		p->Param.ColorKey = RGBToFormat(p->p.ColorKey,&p->p.Output.Video.Pixel);
	}
	else
	{
		p->Param.ColorMask = 0;
...
ReTry:
	...
	p->Overlay.Direction = CombineDir(p->p.InputDirection,p->p.OrigFX.Direction,p->p.Output.Video.Direction);
	Overlay = p->Overlay;
	Overlay.Width = p->p.Input.Video.Width;
	Overlay.Height = p->p.Input.Video.Height;
	if (p->BlitDownScale)
	{
		Overlay.Width >>= 1;
		Overlay.Height >>= 1;
	...
	BlitRelease(p->p.Soft);
	p->p.Soft = BlitCreate(&Overlay,&p->p.Input.Video,&p->SoftFX,&p->p.Caps);
	BlitAlign(p->p.Soft,&p->OverlayRect,&p->p.SrcAlignedRect);

	if (!p->Buf[0] || p->Overlay.Width != p->OverlayRect.Width || p->Overlay.Height != p->OverlayRect.Height)
	{
		FreeBuffer(p,0,NULL);
		p->Overlay.Width = p->OverlayRect.Width;
		p->Overlay.Height = p->OverlayRect.Height;
		if (!AllocBuffer(p,0,p->OverlayRect.Width,p->OverlayRect.Height,MEM_ANY_INT))
		{
			if (!p->BlitDownScale)
			{
				p->BlitDownScale = 1;
...
static int Orientation(void* This)
{
	...
	return p->p.Output.Video.Direction;
}
...
static int Init(ahi* p)
{
	...
	if (p->AhiInit(0)==SUCCESS)
	{
		...
		p->Overlay.Width = ALIGN16(p->IDCTInit ? p->IDCTWidth:p->p.Input.Video.Width);
		p->Overlay.Height = ALIGN16(p->IDCTInit ? p->IDCTHeight:p->p.Input.Video.Height);
		p->Overlay.Direction = p->IDCTInit ? 0:p->p.Input.Video.Direction;
		p->Overlay.Aspect = ASPECT_ONE;
		p->BlitDownScale = 0;

		if (p->IDCTInit && !PlanarYUV420(&p->p.Input.Video.Pixel))
			...
			if (AtiResult == SUCCESS)
			{
				...
				if (p->AhiDevOpen(&(p->Context),p->Device,"TCPMP",2)==SUCCESS)
				{
					...
					if (p->DevInfo.ExternalMemSize>0 &&
						PlanarYUV(&p->p.Input.Video.Pixel,NULL,NULL,NULL))
					{
					...
					if (p->IDCTInit)
					{
						...
						if (!p->IDCT)
						{
							p->AhiDevClose(p->Context);
							p->Context = NULL;
							ShowError(AHI_ID,AHI_ID,AHI_OPEN_IDCT);
...
static int Update(ahi* p)
{	
	UpdateDirection(p,(p->p.FX.Direction ^ p->p.Input.Video.Direction) & DIR_SWAPXY);
