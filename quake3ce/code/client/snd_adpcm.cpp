...
void S_AdpcmEncode( short indata[], char outdata[], int len, struct adpcm_state *state ) {
    ...
    for ( ; len > 0 ; len-- ) {
		...
		**    delta = diff*4/step;
		**    vpdiff = (delta+GFIXED(0,5))*step/4;
...
/* static */ void S_AdpcmDecode( const char indata[], short *outdata, int len, struct adpcm_state *state ) {
    ...
    for ( ; len > 0 ; len-- ) {
		...
		/*
		** Computes 'vpdiff = (delta+GFIXED(0,5))*step/4', but see comment
		** in adpcm_coder.
		*/
		vpdiff = step >> 3;
...
int S_AdpcmMemoryNeeded( const wavinfo_t *info ) {
	...
	scale = FIXED_INT32RATIO_G(info->rate,dma.speed);

	// calc number of samples at playback sampling rate
	scaledSampleCount = FIXED_INT32SCALE(info->samples,scale);
