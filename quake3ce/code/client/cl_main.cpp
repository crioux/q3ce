...
void CL_Record_f( void ) {
  ...
  // sync 0 doesn't prevent recording, so not forcing it off .. everyone does g_sync 1 ; record ; g_sync 0 ..
	if ( !Cvar_VariableIntegerValue( "g_synchronousClients" ) ) {
...
void CL_CheckTimeout( void ) {
	...
	if ( ( !cl_paused->integer || !sv_paused->integer ) 
	    ...
	    && cls.realtime - clc.lastPacketTime > FIXED_TO_INT(cl_timeout->value*LFIXED(1000,0))) {
...
void CL_Frame ( int msec ) {
	...
	if ( cl_avidemo->integer && msec) {
		...
		// gfixed time for next frame'
		msec = FIXED_TO_INT((MAKE_LFIXED(1000 / cl_avidemo->integer) * com_timescale->value));
		if (msec == 0) {
			msec = 1;
		}
	}
	...
	if ( cl_timegraph->integer ) {
		SCR_DebugGraph ( MAKE_GFIXED(cls.realFrametime) * GFIXED(0,25), 0 );
	}
...
int CL_ScaledMilliseconds(void) {
	return FIXED_TO_INT(MAKE_LFIXED(Sys_Milliseconds())*com_timescale->value);
}
...
void CL_Init( void ) {
	...
	Com_Printf( "- Client Init Complete --\n" );
}
