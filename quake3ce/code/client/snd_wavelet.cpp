...
long myftol( gfixed f );

#define C0 GFIXED(0,4829629131445341)
#define C1 GFIXED(0,8365163037378079)
#define C2 GFIXED(0,2241438680420134)
#define C3 -GFIXED(0,1294095225512604)

void daub4(gfixed b[], unsigned long n, int isign)
{
	gfixed wksp[4097];
	gfixed	*a=b-1;						// numerical recipies so a[1] = b[0]
...
void wt1(gfixed a[], unsigned long n, int isign)
{
	unsigned long nn;
...
void encodeWavelet( sfx_t *sfx, short *packets) {
	...
	if (!madeTable) {
		for (i=0;i<256;i++) {
			mulawToShort[i] = MuLawDecode((byte)i);
		}
		madeTable = qtrue;
	}
	chunk = NULL;
	...
	while(samples>0) {
		...
		for(i=0;i<size;i++) {
			temp = wksp[i];
			if (temp > GFIXED(32767,0)) temp = GFIXED(32767,0); else if (temp<-GFIXED(32768,0)) temp = -GFIXED(32768,0);
			out[i] = MuLawEncode((short)FIXED_TO_INT(temp));
		}
...
void decodeWavelet(sndBuffer *chunk, short *to) {
	...
	out = (byte *)chunk->sndChunk;
	for(i=0;i<size;i++) {
		wksp[i] = MAKE_GFIXED(mulawToShort[out[i]]);
	}
