...
extern CXMalloc g_mem_zone;
extern CXMalloc g_mem_hunk;
...
cvar_t	*com_dropsim;		// GFIXED_0 to GFIXED_1, simulated packet drops
cvar_t	*com_journal;
cvar_t	*com_maxfps;
cvar_t	*com_timedemo;
...
int Com_RealTime(qtime_t *qtime) {
	...
	qtime->tm_sec = st.wSecond;
	qtime->tm_min = st.wMinute;
	qtime->tm_hour = st.wHour;
	qtime->tm_mday = st.wDayOfWeek;
	qtime->tm_mon = st.wMonth;
	qtime->tm_year = st.wYear;
	qtime->tm_wday = st.wDay;
	qtime->tm_yday = st.wDay;
	qtime->tm_isdst = 0;
...
void Com_InitZoneMemory( void ) {
	...
	s_zoneTotal=g_mem_zone.GetTotalSize();

	void *mem=g_mem_zone.xmalloc(s_zoneTotal);
	if ( !mem ) {
		Com_Error( ERR_FATAL, "Zone data failed to allocate %i megs", s_zoneTotal / (1024*1024) );
	}
	Com_Memset(mem,0,s_zoneTotal);
	
	zone.Create(mem,s_zoneTotal);
}
...
void Com_InitHunkMemory( void ) {
	...
	s_hunkTotal = g_mem_hunk.GetTotalSize()-256;
	
	// bk001205 - was malloc
	//s_hunkData = calloc( s_hunkTotal + 31, 1 );
	//s_hunkhandle=CreateFileMapping(INVALID_HANDLE_VALUE,NULL,PAGE_READWRITE,0,s_hunkTotal + 31,NULL);
	//s_hunkData=(byte *)MapViewOfFile(s_hunkhandle,FILE_MAP_ALL_ACCESS,0,0,0);
	s_hunkData=(byte *)g_mem_hunk.xmalloc(s_hunkTotal+31);
	if ( !s_hunkData ) {
		Com_Error( ERR_FATAL, "Hunk data failed to allocate %i megs", s_hunkTotal / (1024*1024) );
	}
	Com_Memset(s_hunkData,0,s_hunkTotal+31);
...
==================
*/
EXTERN_C void Hunk_FreeTempMemory( void *buf ) {
	hunkHeader_t	*hdr;
...
EXTERN_C void Hunk_ClearTempMemory( void ) {
	if ( s_hunkData != NULL ) {
...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
		...
		case SE_PACKET:
			...
			if ( com_dropsim->value > LFIXED_0 ) {
				static int seed;

				if ( MAKE_LFIXED(Q_random( &seed )) < com_dropsim->value ) {
...
static void Com_Freeze_f (void) {
	...
	s = MAKE_GFIXED(atof( Cmd_Argv(1) ));

	start = Com_Milliseconds();

	while ( 1 ) {
		now = Com_Milliseconds();
		if ( MSECTIME_G( now - start ) > s ) {
			break;
		}
	}
}
...
void Com_Init( char *commandLine ) {
  ...
  // bk001129 - do this before anything else decides to push events
	Com_Printf( "Preparing zone memory...\n" );
	...
	Com_Printf( "Parsing command line...\n" );
...
//	Swap_Init ();
	...
	Com_Printf( "Preparing hunk memory...\n" );
...
int Com_ModifyMsec( int msec ) {
	...
	} else if ( FIXED_NOT_ZERO(com_timescale->value) ) {
		msec = FIXED_TO_INT(MAKE_LFIXED(msec) * com_timescale->value);
	} else if (com_cameraMode->integer) {
		msec = FIXED_TO_INT(com_timescale->value);
	}
	
	// don't let it scale below 1 msec
	if ( msec < 1 && FIXED_NOT_ZERO(com_timescale->value)) {
		msec = 1;
	}
...
void Com_Frame( void ) {
	...
	if ( com_viewlog->modified ) {
		if ( FIXED_IS_ZERO(com_dedicated->value) ) {
			Sys_ShowConsole( com_viewlog->integer, qfalse );
		}
