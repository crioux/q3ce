...
==================
*/
int CM_PointLeafnum_r( const bvec3_t p, int num ) {
	bfixed		d;
	cNode_t		*node;
	cplane_t	*plane;
	...
	while (num >= 0)
	{
		...
		if (plane->type < 3)
			...
			d = FIXED_VEC3DOT_R (plane->normal, p) - plane->dist;
		if (d < BFIXED_0)
			num = node->children[1];
		else
			num = node->children[0];
...
int CM_PointLeafnum( const bvec3_t p ) {
...
==================
*/
int	CM_BoxLeafnums( const bvec3_t mins, const bvec3_t maxs, int *list, int listsize, int *lastLeaf) {
	leafList_t	ll;
...
==================
*/
int CM_BoxBrushes( const bvec3_t mins, const bvec3_t maxs, cbrush_t **list, int listsize ) {
	leafList_t	ll;
...
==================
*/
int CM_PointContents( const bvec3_t p, clipHandle_t model ) {
	int			leafnum;
	int			i, k;
	int			brushnum;
	cLeaf_t		*leaf;
	...
	for (k=0 ; k<leaf->numLeafBrushes ; k++) {
		...
		for ( i = 0 ; i < b->numsides ; i++ ) {
			d = FIXED_VEC3DOT( p, b->sides[i].plane->normal );
// FIXME test for Cash
...
==================
*/
int	CM_TransformedPointContents( const bvec3_t p, clipHandle_t model, const bvec3_t origin, const avec3_t angles) {
	bvec3_t		p_l;
	bvec3_t		temp;
	...
	(FIXED_NOT_ZERO(angles[0]) || FIXED_NOT_ZERO(angles[1]) || FIXED_NOT_ZERO(angles[2])) )
	{
		...
		VectorCopy (p_l, temp);
		p_l[0] = FIXED_VEC3DOT (temp, forward);
		p_l[1] = -FIXED_VEC3DOT (temp, right);
		p_l[2] = FIXED_VEC3DOT (temp, up);
	}
