...
// use a CVAR_SET for rom sets, get won't override
	...
	var->value = MAKE_LFIXED(atof(var->string));
...
void Cvar_SetValue( const char *var_name, lfixed value) {
	...
	if ( value == FIXED_SNAP(value) ) {
		Com_sprintf (val, sizeof(val), "%i",FIXED_TO_INT(value));
	} else {
		Com_sprintf (val, sizeof(val), "%f",FIXED_TO_DOUBLE(value));
	}
