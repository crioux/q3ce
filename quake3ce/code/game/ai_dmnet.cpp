...
int BotGetAirGoal(bot_state_t *bs, bot_goal_t *goal) {
	...
	bvec3_t end, mins = {-BFIXED(15,0), -BFIXED(15,0), -BFIXED(2,0)}, maxs = {BFIXED(15,0), BFIXED(15,0), BFIXED(2,0)};
	int areanum;
	...
	if (bsptrace.fraction > GFIXED_0) {
		areanum = BotPointAreaNum(bsptrace.endpos);
		if (areanum) {
			VectorCopy(bsptrace.endpos, goal->origin);
			goal->origin[2] -= BFIXED(2,0);
			goal->areanum = areanum;
			goal->mins[0] = -BFIXED(15,0);
			goal->mins[1] = -BFIXED(15,0);
			goal->mins[2] = -BFIXED_1;
			goal->maxs[0] = BFIXED(15,0);
			goal->maxs[1] = BFIXED(15,0);
			goal->maxs[2] = BFIXED_1;
			goal->flags = GFL_AIR;
			goal->number = 0;
			goal->iteminfo = 0;
			goal->entitynum = 0;
...
==================
*/
int BotGoForAir(bot_state_t *bs, int tfl, bot_goal_t *ltg, gfixed range) {
	bot_goal_t goal;

	//if the bot needs air
	if (bs->lastair_time < FloatTime() - GFIXED(6,0)) {
		//
#ifdef DEBUG
		...
		if (BotGetAirGoal(bs, &goal)) {
			_G_trap_BotPushGoal(bs->gs, &goal);
			return qtrue;
		}
		else {
			//get a nearby goal outside the water
			while(_G_trap_BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range)) {
				_G_trap_BotGetTopGoal(bs->gs, &goal);
				//if the goal is not in water
				if (!(_G_trap_AAS_PointContents(goal.origin) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA))) {
					return qtrue;
				}
				_G_trap_BotPopGoal(bs->gs);
			}
			_G_trap_BotResetAvoidGoals(bs->gs);
		}
	}
	return qfalse;
}
...
==================
*/
int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, gfixed range) {
	int ret;
	...
	if (BotCTFCarryingFlag(bs)) {
		...
		if (_G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
	...
	//
	ret = _G_trap_BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range);
	/*
	if (ret)
	{
		char buf[128];
		...
		_G_trap_BotGetTopGoal(bs->gs, &goal);
		_G_trap_BotGoalName(goal.number, buf, sizeof(buf));
		BotAI_Print(PRT_MESSAGE, "%1.1f: new nearby goal %s\n", FIXED_TO_DOUBLE(FloatTime()), buf);
	}
    */
	return ret;
}
...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
	if (goal->flags & GFL_ITEM) {
		...
		if (_G_trap_BotTouchingGoal(bs->origin, goal)) {
			if (!(goal->flags & GFL_DROPPED)) {
				_G_trap_BotSetAvoidGoalTime(bs->gs, goal->number, -GFIXED_1);
			}
			return qtrue;
		}
		//if the goal isn't there
		if (_G_trap_BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye, bs->viewangles, goal)) {
			/*
			gfixed avoidtime;
			int t;

			avoidtime = _G_trap_BotAvoidGoalTime(bs->gs, goal->number);
			if (avoidtime > 0) {
				t = _G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal->areanum, bs->tfl);
				if (MAKE_GFIXED t * GFIXED(0,009) < avoidtime)
					return qtrue;
			}
			*/
			return qtrue;
		...
		if (bs->areanum == goal->areanum) {
			if (bs->origin[0] > goal->origin[0] + goal->mins[0] && bs->origin[0] < goal->origin[0] + goal->maxs[0]) {
				if (bs->origin[1] > goal->origin[1] + goal->mins[1] && bs->origin[1] < goal->origin[1] + goal->maxs[1]) {
					if (!_G_trap_AAS_Swimming(bs->origin)) {
						return qtrue;
					}
				}
			}
		...
		//if touching the goal
		if (_G_trap_BotTouchingGoal(bs->origin, goal)) return qtrue;
		//if the bot got air
		if (bs->lastair_time > FloatTime() - GFIXED_1) return qtrue;
	}
	else {
		//if touching the goal
		if (_G_trap_BotTouchingGoal(bs->origin, goal)) return qtrue;
...
int BotGetItemLongTermGoal(bot_state_t *bs, int tfl, bot_goal_t *goal) {
	...
	if (!_G_trap_BotGetTopGoal(bs->gs, goal)) {
	...
	if (bs->ltg_time < FloatTime()) {
		...
		//choose a new goal
		//BotAI_Print(PRT_MESSAGE, "%6.1f client %d: BotChooseLTGItem\n", FIXED_TO_DOUBLE(FloatTime()), bs->client);
		if (_G_trap_BotChooseLTGItem(bs->gs, bs->origin, bs->inventory, tfl)) {
			/*
			char buf[128];
			//get the goal at the top of the stack
			_G_trap_BotGetTopGoal(bs->gs, goal);
            ...
            */
			bs->ltg_time = FloatTime() + GFIXED(20,0);
		}
			...
			_G_trap_BotResetAvoidGoals(bs->gs);
			_G_trap_BotResetAvoidReach(bs->ms);
		}
		//get the goal at the top of the stack
		return _G_trap_BotGetTopGoal(bs->gs, goal);
	}
	return qtrue;
}
...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal) {
	...
	if (bs->ltgtype == LTG_TEAMHELP && !retreat) {
		...
		if (FIXED_NOT_ZERO(bs->teammessage_time) && bs->teammessage_time < FloatTime()) {
			BotAI_BotInitialChat(bs, "help_start", EasyClientName(bs->teammate, netname, sizeof(netname)), NULL);
			_G_trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
		...
		if (bs->teammatevisible_time < FloatTime() - GFIXED(10,0)) bs->ltgtype = 0;
		//get entity information of the companion
		BotEntityInfo(bs->teammate, &entinfo);
		//if the team mate is visible
		if (FIXED_NOT_ZERO(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->teammate))) {
			//if close just stand still there
			VectorSubtract(entinfo.origin, bs->origin, dir);
			lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
			if (FIXED_VEC3LEN(lvec) < Square(LFIXED(100,0))) {
				_G_trap_BotResetAvoidReach(bs->ms);
				return qfalse;
			}
		}
	...
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !retreat) {
		...
		if (bs->teamgoal_time < FloatTime()) {
			...
			_G_trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
			bs->ltgtype = 0;
		}
		//get entity information of the companion
		BotEntityInfo(bs->teammate, &entinfo);
		...
		if (FIXED_NOT_ZERO(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->teammate))) {
			...
			bs->teammatevisible_time = FloatTime();
			VectorSubtract(entinfo.origin, bs->origin, dir);
			if (FIXED_VEC3LEN_SQ(dir) < Square(bs->formation_dist)) {
				//
				...
				if (botinfo.origin[2] + botinfo.maxs[2] > entinfo.origin[2] + entinfo.mins[2]) {
					...
					if (botinfo.origin[0] + botinfo.maxs[0] > entinfo.origin[0] + entinfo.mins[0] - BFIXED(4,0) &&
						botinfo.origin[0] + botinfo.mins[0] < entinfo.origin[0] + entinfo.maxs[0] + BFIXED(4,0)) {
						if (botinfo.origin[1] + botinfo.maxs[1] > entinfo.origin[1] + entinfo.mins[1] - BFIXED(4 ,0)&&
							botinfo.origin[1] + botinfo.mins[1] < entinfo.origin[1] + entinfo.maxs[1] + BFIXED(4,0)) {
							if (botinfo.origin[2] + botinfo.maxs[2] > entinfo.origin[2] + entinfo.mins[2] - BFIXED(4 ,0)&&
								botinfo.origin[2] + botinfo.mins[2] < entinfo.origin[2] + entinfo.maxs[2] + BFIXED(4,0)) {
								// if the followed client looks in the direction of this bot
								AngleVectors(entinfo.angles, adir, NULL, NULL);
								adir[2] = AFIXED_0;
								VectorNormalize(adir);
								//VectorSubtract(entinfo.origin, entinfo.lastvisorigin, dir);
								VectorSubtract(bs->origin, entinfo.origin, dir2);
								VectorNormalizeB2A(dir2,adir2);
								if (FIXED_VEC3DOT(adir, adir2) > AFIXED(0,7)) {
									// back up
									BotSetupForMovement(bs);
									_G_trap_BotMoveInDirection(bs->ms, adir2, BFIXED(400,0), MOVE_WALK);
								}
							}
						}
					}
				...
				if (bs->attackcrouch_time < FloatTime() - GFIXED(5,0)) {
					...
					if (random() < bs->thinktime * croucher) {
						bs->attackcrouch_time = FloatTime() + GFIXED(5 ,0)+ croucher * GFIXED(15,0);
					}
				}
				//don't crouch when swimming
				if (_G_trap_AAS_Swimming(bs->origin)) bs->attackcrouch_time = FloatTime() - GFIXED_1;
				//if not arrived yet or arived some time ago
				if (bs->arrive_time < FloatTime() - GFIXED(2,0)) {
					//if not arrived yet
					if (FIXED_IS_ZERO(bs->arrive_time)) {
						_G_trap_EA_Gesture(bs->client);
					...
					//if the bot wants to crouch
						...
						_G_trap_EA_Crouch(bs->client);
					}
					//else do some model taunts
					else if (random() < bs->thinktime * GFIXED(0,05)) {
						//do a gesture :)
						_G_trap_EA_Gesture(bs->client);
					}
				}
				...
				if (bs->arrive_time > FloatTime() - GFIXED(2,0)) {
					...
					bs->ideal_viewangles[2] *= AFIXED(0,5);
				}
				//else look strategically around for enemies
				else if (random() < bs->thinktime * GFIXED(0,8)) {
				...
				if (BotGoForAir(bs, bs->tfl, &bs->teamgoal, GFIXED(400,0))) {
					_G_trap_BotResetLastAvoidReach(bs->ms);
					//get the goal at the top of the stack
					//_G_trap_BotGetTopGoal(bs->gs, &tmpgoal);
					//_G_trap_BotGoalName(tmpgoal.number, buf, 144);
					//BotAI_Print(PRT_MESSAGE, "new nearby goal %s\n", buf);
					//time the bot gets to pick up the nearby goal item
					bs->nbg_time = FloatTime() + GFIXED(8,0);
		...
		if (bs->teammatevisible_time < FloatTime() - GFIXED(60,0)) {
	...
	if (bs->ltgtype == LTG_DEFENDKEYAREA) {
		if (MAKE_GFIXED(_G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
	...
	if (bs->ltgtype == LTG_DEFENDKEYAREA && !retreat &&
		...
		//check for bot typing status message
		if (FIXED_NOT_ZERO(bs->teammessage_time) && bs->teammessage_time < FloatTime()) {
			_G_trap_BotGoalName(bs->teamgoal.number, buf, sizeof(buf));
		...
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		if (FIXED_VEC3LEN(lvec) < Square(LFIXED(70,0))) {
			_G_trap_BotResetAvoidReach(bs->ms);
			bs->defendaway_time = FloatTime() + GFIXED(3 ,0)+ GFIXED(3 ,0)* random();
			if (BotHasPersistantPowerupAndWeapon(bs)) {
				bs->defendaway_range = GFIXED(100,0);
			}
			else {
				bs->defendaway_range = GFIXED(350,0);
			}
		}
		return qtrue;
	}
	...
	if ((bs->ltgtype == LTG_CAMP || bs->ltgtype == LTG_CAMPORDER) && !retreat) {
		...
		VectorSubtract(goal->origin, bs->origin, dir);
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		if (FIXED_VEC3LEN(lvec) < Square(LFIXED(60,0)))
		{
			//if not arrived yet
			if (FIXED_IS_ZERO(bs->arrive_time)) {
				if (bs->ltgtype == LTG_CAMPORDER) {
			...
			if (random() < bs->thinktime * GFIXED(0,8)) {
				BotRoamGoal(bs, target);
				VectorSubtract(target, bs->origin, dir);
				vectoangles(dir, bs->ideal_viewangles);
				bs->ideal_viewangles[2] *= AFIXED(0,5);
			...
			if (_G_trap_PointContents(bs->eye,bs->entitynum) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)) {
				if (bs->ltgtype == LTG_CAMPORDER) {
					BotAI_BotInitialChat(bs, "camp_stop", NULL);
					_G_trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
					//
	...
	if (bs->ltgtype == LTG_PATROL && !retreat) {
		...
		//if the bot touches the current goal
		if (_G_trap_BotTouchingGoal(bs->origin, &bs->curpatrolpoint->goal)) {
	...
	if (gametype == GT_CTF) {
		...
		if (bs->ltgtype == LTG_RUSHBASE && bs->rushbaseaway_time < FloatTime()) {
			...
			if (_G_trap_BotTouchingGoal(bs->origin, goal)) {
				...
				if (BotCTFCarryingFlag(bs)) {
					_G_trap_BotResetAvoidReach(bs->ms);
					bs->rushbaseaway_time = FloatTime() + GFIXED(5 ,0)+ GFIXED(10 ,0)* random();
		...
		if (bs->ltgtype == LTG_RETURNFLAG) {
			...
			//if touching the flag
			if (_G_trap_BotTouchingGoal(bs->origin, goal)) bs->ltgtype = 0;
			//stop after 3 minutes
			if (bs->teamgoal_time < FloatTime()) {
				bs->ltgtype = 0;
			}
		...
		if (bs->ltgtype == LTG_ATTACKENEMYBASE &&
			...
			if (FIXED_VEC3LEN_SQ(dir) < Square(60)) {
				bs->attackaway_time = FloatTime() + 3 + 5 * random();
			}
			//quit rushing after 2 minutes
			if (bs->teamgoal_time < FloatTime()) {
...
int BotLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal) {
	...
	if (bs->lead_time > GFIXED_0 && !retreat) {
		...
		//
		if (bs->leadmessage_time < GFIXED_0 && -bs->leadmessage_time < FloatTime()) {
		...
		//if the team mate is visible
		if (FIXED_NOT_ZERO(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->lead_teammate))) {
			bs->leadvisible_time = FloatTime();
		}
		//if the team mate is not visible for 1 seconds
		if (bs->leadvisible_time < FloatTime() - GFIXED_1) {
			bs->leadbackup_time = FloatTime() + GFIXED(2,0);
		}
		//distance towards the team mate
		VectorSubtract(bs->origin, bs->lead_teamgoal.origin, dir);
		lvec[0]=MAKE_LFIXED(dir[0]);lvec[1]=MAKE_LFIXED(dir[1]);lvec[2]=MAKE_LFIXED(dir[2]);
		squaredist = FIXED_VEC3LEN(lvec);
		//if backing up towards the team mate
		if (bs->leadbackup_time > FloatTime()) {
			if (bs->leadmessage_time < FloatTime() - GFIXED(20,0)) {
				BotAI_BotInitialChat(bs, "followme", EasyClientName(bs->lead_teammate, teammate, sizeof(teammate)), NULL);
				_G_trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
				bs->leadmessage_time = FloatTime();
			}
			//if very close to the team mate
			if (squaredist < Square(LFIXED(100,0))) {
				bs->leadbackup_time = GFIXED_0;
			}
			...
			if (squaredist > Square(LFIXED(500,0))) {
				if (bs->leadmessage_time < FloatTime() - GFIXED(20,0)) {
					BotAI_BotInitialChat(bs, "followme", EasyClientName(bs->lead_teammate, teammate, sizeof(teammate)), NULL);
					_G_trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
					bs->leadmessage_time = FloatTime();
...
void AIEnter_Intermission(bot_state_t *bs, char *s) {
	...
	if (BotChat_EndLevel(bs)) {
		_G_trap_BotEnterChat(bs->cs, 0, bs->chatto);
	}
...
int AINode_Intermission(bot_state_t *bs) {
	...
	if (!BotIntermission(bs)) {
		if (BotChat_StartLevel(bs)) {
			...
			bs->stand_time = FloatTime() + GFIXED(2,0);
		}
...
void AIEnter_Stand(bot_state_t *bs, char *s) {
	...
	bs->standfindenemy_time = FloatTime() + GFIXED_1;
...
int AINode_Stand(bot_state_t *bs) {
	...
	if (bs->lastframe_health > bs->inventory[INVENTORY_HEALTH]) {
		if (BotChat_HitTalking(bs)) {
			bs->standfindenemy_time = FloatTime() + BotChatTime(bs) + GFIXED(0,1);
			bs->stand_time = FloatTime() + BotChatTime(bs) + GFIXED(0,1);
		}
	}
...
void AIEnter_Respawn(bot_state_t *bs, char *s) {
	...
	//reset some states
	_G_trap_BotResetMoveState(bs->ms);
	_G_trap_BotResetGoalState(bs->gs);
	_G_trap_BotResetAvoidGoals(bs->gs);
	_G_trap_BotResetAvoidReach(bs->ms);
	...
	if (BotChat_Death(bs)) {
		...
		bs->respawn_time = FloatTime() + GFIXED_1 + random();
		bs->respawnchat_time = GFIXED_0;
	}
	//set respawn state
...
int AINode_Respawn(bot_state_t *bs) {
	...
	if (bs->respawn_wait) {
		...
		_G_trap_EA_Respawn(bs->client);
		//
		if (FIXED_NOT_ZERO(bs->respawnchat_time)) {
			_G_trap_BotEnterChat(bs->cs, 0, bs->chatto);
			bs->enemy = -1;
		}
	}
	if (FIXED_NOT_ZERO(bs->respawnchat_time) && bs->respawnchat_time < FloatTime() - GFIXED(0,5)) {
		_G_trap_EA_Talk(bs->client);
	}
	//
	return qtrue;
...
void BotClearPath(bot_state_t *bs, bot_moveresult_t *moveresult) {
		...
		if ( !(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)) ) {
			...
			if (moveresult->weapon) {
				...
				if (bs->cur_ps.weapon == moveresult->weapon) {
					...
					if (InFieldOfVision(bs->viewangles, AFIXED(20,0), moveresult->ideal_viewangles)) {
						//
						BotAI_Trace(&bsptrace, bs->eye, NULL, NULL, target, bs->entitynum, MASK_SHOT);
						// if the mine is visible from the current position
						if (bsptrace.fraction >= GFIXED_1 || bsptrace.ent == state.number) {
							// shoot at the mine
	...
	if (moveresult->flags & MOVERESULT_BLOCKEDBYAVOIDSPOT) {
		bs->blockedbyavoidspot_time = FloatTime() + GFIXED(5,0);
	}
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	if (bs->activatestack->shoot) {
		...
		if (bsptrace.fraction >= GFIXED_1 || bsptrace.ent == goal->entitynum) {
			targetvisible = qtrue;
			// if holding the right weapon
			if (bs->cur_ps.weapon == bs->activatestack->weapon) {
				...
				// if the bot is pretty close with it's aim
				if (InFieldOfVision(bs->viewangles, AFIXED(20,0), ideal_viewangles)) {
					_G_trap_EA_Attack(bs->client);
				}
			}
		}
	...
	if (targetvisible) {
...
#endif //DEBUG
			bs->activatestack->time = GFIXED(0,0);
		}
		...
		if (bs->activatestack->time < FloatTime()) {
			...
			if (bs->activatestack) {
				bs->activatestack->time = FloatTime() + GFIXED(10,0);
				return qfalse;
			}
...
#endif //DEBUG
				bs->activatestack->time = GFIXED_0;
			}
		}
		// if the activate goal has been activated or the bot takes too long
		if (bs->activatestack->time < FloatTime()) {
		...
		_G_trap_BotMoveToGoal(&moveresult, bs->ms, goal, bs->tfl);
	...
	// if waiting for something
		...
		if (_G_trap_BotMovementViewTarget(bs->ms, goal, bs->tfl, BFIXED(300,0), target)) {
			VectorSubtract(target, bs->origin, dir);
			vectoangles(dir, bs->ideal_viewangles);
		}
		else {
	...
	if (BotFindEnemy(bs, -1)) {
		if (BotWantsToRetreat(bs)) {
			...
			_G_trap_BotEmptyGoalStack(bs->gs);
			//go fight
...
void AIEnter_Seek_NBG(bot_state_t *bs, char *s) {
	...
	if (_G_trap_BotGetTopGoal(bs->gs, &goal)) {
		_G_trap_BotGoalName(goal.number, buf, 144);
...
int AINode_Seek_NBG(bot_state_t *bs) {
	...
	bs->enemy = -1;
	//if the bot has no goal
	if (!_G_trap_BotGetTopGoal(bs->gs, &goal)) bs->nbg_time = GFIXED_0;
	//if the bot touches the current goal
	else if (BotReachedGoal(bs, &goal)) {
		BotChooseWeapon(bs);
		bs->nbg_time = GFIXED_0;
	}
	//
	if (bs->nbg_time < FloatTime()) {
		...
		bs->check_time = FloatTime() + GFIXED(0,05);
		//go back to seek ltg
	...
	//if waiting for something
		...
		if (!_G_trap_BotGetSecondGoal(bs->gs, &goal)) _G_trap_BotGetTopGoal(bs->gs, &goal);
		if (_G_trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, BFIXED(300,0), target)) {
			VectorSubtract(target, bs->origin, dir);
			vectoangles(dir, bs->ideal_viewangles);
		}
...
int AINode_Seek_LTG(bot_state_t *bs)
{
	...
	bs->enemy = -1;
	//
	if (bs->killedenemy_time > FloatTime() - GFIXED(2,0)) {
		if (random() < bs->thinktime * GFIXED_1) {
			_G_trap_EA_Gesture(bs->client);
		}
	}
	//if there is an enemy
	...
	if (bs->check_time < FloatTime()) {
		bs->check_time = FloatTime() + GFIXED(0,5);
...
#endif
		//
		if (BotNearbyGoal(bs, bs->tfl, &goal, MAKE_GFIXED(range))) {
			_G_trap_BotResetLastAvoidReach(bs->ms);
			//get the goal at the top of the stack
			//_G_trap_BotGetTopGoal(bs->gs, &tmpgoal);
			//_G_trap_BotGoalName(tmpgoal.number, buf, 144);
			//BotAI_Print(PRT_MESSAGE, "new nearby goal %s\n", buf);
			//time the bot gets to pick up the nearby goal item
			bs->nbg_time = FloatTime() + GFIXED(4 ,0)+ MAKE_GFIXED(range) * GFIXED(0,01);
	...
	//if waiting for something
		...
		//FIXME: look at cluster portals?
		else if (FIXED_NOT_ZERO(FIXED_VEC3LEN_SQ(moveresult.movedir))) {
			vectoangles(moveresult.movedir, bs->ideal_viewangles);
		}
		else if (random() < bs->thinktime * GFIXED(0,8)) {
			BotRoamGoal(bs, target);
...
int AINode_Battle_Fight(bot_state_t *bs) {
	...
	if (FIXED_NOT_ZERO(bs->enemydeath_time)) {
		if (bs->enemydeath_time < FloatTime() - GFIXED_1) {
	...
	if (FIXED_IS_ZERO(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->enemy))) {
...
int AINode_Battle_Chase(bot_state_t *bs)
{
	...
	if (FIXED_NOT_ZERO(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, AFIXED(360,0), bs->enemy))) {
	...
	if (_G_trap_BotTouchingGoal(bs->origin, &goal)) bs->chase_time = GFIXED_0;
	//if there's no chase time left
	if (FIXED_IS_ZERO(bs->chase_time) || bs->chase_time < FloatTime() - GFIXED(10,0)) {
		AIEnter_Seek_LTG(bs, "battle chase: time out");
		return qfalse;
	}
	//check for nearby goals periodicly
	if (bs->check_time < FloatTime()) {
		bs->check_time = FloatTime() + GFIXED_1;
		range = GFIXED(150,0);
		//
		if (BotNearbyGoal(bs, bs->tfl, &goal, range)) {
			//the bot gets 5 seconds to pick up the nearby goal item
			bs->nbg_time = FloatTime() + GFIXED(0,1) * range + GFIXED_1;
			_G_trap_BotResetLastAvoidReach(bs->ms);
	...
	if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET|MOVERESULT_MOVEMENTVIEW|MOVERESULT_SWIMVIEW)) {
		...
		if (bs->chase_time > FloatTime() - GFIXED(2,0)) {
			BotAimAtEnemy(bs);
		}
		else {
			if (_G_trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, BFIXED(300,0), target)) {
	...
	if (bs->areanum == bs->lastenemyareanum) bs->chase_time = GFIXED_0;
...
int AINode_Battle_Retreat(bot_state_t *bs) {
	...
	if (bs->enemyvisible_time < FloatTime() - GFIXED(4,0)) {
	...
	if (bs->check_time < FloatTime()) {
		...
		if (BotNearbyGoal(bs, bs->tfl, &goal, range)) {
			_G_trap_BotResetLastAvoidReach(bs->ms);
			//time the bot gets to pick up the nearby goal item
			bs->nbg_time = FloatTime() + range / GFIXED(100 ,0) + GFIXED_1;
	...
	//choose the best weapon to fight with
	BotChooseWeapon(bs);
	//if the view is gfixed for the movement
