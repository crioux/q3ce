...
/*QUAKED info_camp (0 GFIXED(0,5) 0) (-4 -4 -4) (4 4 4)
...
/*QUAKED info_null (0 GFIXED(0,5) 0) (-4 -4 -4) (4 4 4)
Used as a positional target for calculations in the utilities (spotlights, etc), but removed during gameplay.
*/
...
/*QUAKED info_notnull (0 GFIXED(0,5) 0) (-4 -4 -4) (4 4 4)
...
void TeleportPlayer( gentity_t *player, bvec3_t origin, avec3_t angles ) {
	...
	_G_trap_UnlinkEntity (player);

	VectorCopy ( origin, player->client->ps.origin );
	player->client->ps.origin[2] += BFIXED_1;

	// spit the player out
	avec3_t tmp;
	AngleVectors( angles, tmp, NULL, NULL );
	FIXED_VEC3SCALE_R( tmp, BFIXED(400,0), player->client->ps.velocity );
...
void SP_misc_model( gentity_t *ent ) {
	...
	VectorSet (ent->mins, -GFIXED(16,0), -GFIXED(16,0), -GFIXED(16,0));
	VectorSet (ent->maxs, GFIXED(16,0), GFIXED(16,0), GFIXED(16,0));
...
void locateCamera( gentity_t *ent ) {
	...
	if ( target ) {
		bvec3_t tmp;
		VectorSubtract( target->s.origin, owner->s.origin, tmp);
...
void SP_misc_portal_camera(gentity_t *ent) {
	...
	ent->s.clientNum = FIXED_TO_INT(roll/GFIXED(360,0) * GFIXED(256,0));
}
...
void Use_Shooter( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->enemy ) {
		bvec3_t tmp; 
		VectorSubtract( ent->enemy->r.currentOrigin, ent->s.origin, tmp );
		VectorNormalizeB2A( tmp,dir );
	} else {
	...
	deg = crandom_a() * MAKE_AFIXED(ent->random);
	FIXED_VEC3MA( dir, deg, up, dir );

	deg = crandom_a() * MAKE_AFIXED(ent->random);
	FIXED_VEC3MA( dir, deg, right, dir );
...
void InitShooter( gentity_t *ent, int weapon ) {
	...
	if ( FIXED_IS_ZERO(ent->random) ) {
		ent->random = GFIXED_1;
	}
	ent->random = FIXED_SIN( GFIXED_PI * ent->random / GFIXED(180 ,0));
...
"random" the number of degrees of deviance from the taget. (GFIXED_1 default)
*/
...
Fires at either the target or the current direction.
"random" is the number of degrees of deviance from the taget. (GFIXED_1 default)
*/
