...
void QDECL PrintMsg( gentity_t *ent, const char *fmt, ... ) {
	...
	// lfixed quotes are bad
	while ((p = strchr(msg, '"')) != NULL)
		*p = '\'';

	_G_trap_SendServerCommand ( ( (ent == NULL) ? -1 : ent-g_entities ), va("print \"%s\"", msg ));
}
...
==============
*/
void AddTeamScore(bvec3_t origin, int team, int score) {
	gentity_t	*te;
...
void Team_SetFlagStatus( int team, flagStatus_t status ) {
	...
	if( modified ) {
		...
		_G_trap_SetConfigstring( CS_FLAGSTATUS, st );
	}
}
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if (targ->client->ps.powerups[enemy_flag_pw]) {
		attacker->client->pers.teamState.lastfraggedcarrier = MAKE_GFIXED(level.time);
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			...
			if (ent->inuse && ent->client->sess.sessionTeam == otherteam)
				ent->client->pers.teamState.lasthurtcarrier = GFIXED_0;
		}
		return;
	}
	...
	if (FIXED_NOT_ZERO(targ->client->pers.teamState.lasthurtcarrier) &&
		MAKE_GFIXED(level.time) - targ->client->pers.teamState.lasthurtcarrier < GFIXED(CTF_CARRIER_DANGER_PROTECT_TIMEOUT,0) &&
		...
		targ->client->pers.teamState.lasthurtcarrier = GFIXED_0;
	...
	if ( ( ( FIXED_VEC3LEN(v1) < BFIXED(CTF_TARGET_PROTECT_RADIUS,0) &&
		_G_trap_InPVS(flag->r.currentOrigin, targ->r.currentOrigin ) ) ||
		( FIXED_VEC3LEN(v2) < BFIXED(CTF_TARGET_PROTECT_RADIUS,0) &&
		_G_trap_InPVS(flag->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
	...
	if (carrier && carrier != attacker) {
		...
		if ( ( ( FIXED_VEC3LEN(v1) < BFIXED(CTF_ATTACKER_PROTECT_RADIUS,0) &&
			_G_trap_InPVS(carrier->r.currentOrigin, targ->r.currentOrigin ) ) ||
			( FIXED_VEC3LEN(v2) < BFIXED(CTF_ATTACKER_PROTECT_RADIUS,0) &&
				_G_trap_InPVS(carrier->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
			attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
void Team_CheckHurtCarrier(gentity_t *targ, gentity_t *attacker)
{
	...
	if (targ->client->ps.powerups[flag_pw] &&
		...
		attacker->client->pers.teamState.lasthurtcarrier = MAKE_GFIXED(level.time);
...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if ( ent->flags & FL_DROPPED_ITEM ) {
		...
		other->client->pers.teamState.lastreturnedflag = MAKE_GFIXED(level.time);
	...
	teamgame.last_flag_capture = MAKE_GFIXED(level.time);
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			cl->sess.sessionTeam) {
			player->client->pers.teamState.lasthurtcarrier = -GFIXED(5,0);
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				GFIXED(CTF_RETURN_FLAG_ASSIST_TIMEOUT,0) > MAKE_GFIXED(level.time)) {
				...
				GFIXED(CTF_FRAG_CARRIER_ASSIST_TIMEOUT,0) > MAKE_GFIXED(level.time)) {
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	cl->pers.teamState.flagsince = MAKE_GFIXED(level.time);
...
//				level.sortedClients[i], player->client->pers.teamState.location, h, a, 
	...
	_G_trap_SendServerCommand( ent-g_entities, va("tinfo %i %s", cnt, string) );
}
...
gentity_t *SpawnObelisk( bvec3_t origin, int team, int spawnflags) {
	trace_t		tr;
	bvec3_t		dest;
	gentity_t	*ent;
	...
	VectorSet( ent->r.mins, -GFIXED(15,0), -GFIXED(15,0), GFIXED(0 ,0));
	VectorSet( ent->r.maxs, GFIXED(15,0), GFIXED(15,0), GFIXED(87 ,0));
	...
	if ( spawnflags & 1 ) {
		...
		// drop to floor
		VectorSet( dest, ent->s.origin[0], ent->s.origin[1], ent->s.origin[2] - GFIXED(4096 ,0));
