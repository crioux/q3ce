...
int BotClientTravelTimeToGoal(int client, bot_goal_t *goal) {
	...
	if (!areanum) return 1;
	return _G_trap_AAS_AreaTravelTimeToGoalArea(areanum, ps.origin, goal->areanum, TFL_DEFAULT);
}
...
void BotSayTeamOrderAlways(bot_state_t *bs, int toclient) {
	...
	if (bs->client == toclient) {
		...
		_G_trap_BotGetChatMessage(bs->cs, buf, sizeof(buf));
		ClientName(bs->client, name, sizeof(name));
		Com_sprintf(teamchat, sizeof(teamchat), EC"(%s"EC")"EC": %s", name, buf);
		_G_trap_BotQueueConsoleMessage(bs->cs, CMS_CHAT, teamchat);
	}
	else {
		_G_trap_BotEnterChat(bs->cs, toclient, CHAT_TELL);
	}
}
...
void BotVoiceChat(bot_state_t *bs, int toclient, char *voicechat) {
	...
	if (toclient == -1)
		// voice only say team
		_G_trap_EA_Command(bs->client, va("vsay_team %s", voicechat));
	else
		// voice only tell single player
		_G_trap_EA_Command(bs->client, va("vtell %d %s", toclient, voicechat));
#endif
}
...
void BotVoiceChatOnly(bot_state_t *bs, int toclient, char *voicechat) {
	...
	if (toclient == -1)
		// voice only say team
		_G_trap_EA_Command(bs->client, va("vosay_team %s", voicechat));
	else
		// voice only tell single player
		_G_trap_EA_Command(bs->client, va("votell %d %s", toclient, voicechat));
#endif
}
...
void BotCTFOrders_BothFlagsNotAtBase(bot_state_t *bs) {
	...
	switch(bs->numteammates) {
		...
		default:
		{
			defenders = FIXED_TO_INT(MAKE_GFIXED(numteammates) * GFIXED(0,4) + GFIXED(0,5));
			if (defenders > 4) defenders = 4;
			attackers = FIXED_TO_INT(MAKE_GFIXED(numteammates) * GFIXED(0,5) + GFIXED(0,5));
	...
	if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
		...
		switch(bs->numteammates) {
			...
			default:
			{
				...
				defenders = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,3) + GFIXED(0,5) );
				if (defenders > 3) defenders = 3;
				attackers = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,7) + GFIXED(0,5) );
		...
		switch(bs->numteammates) {
			...
			{
				//keep some people near the base for when the flag is returned
				defenders = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,2) + GFIXED(0,5) );
...
void BotCTFOrders_EnemyFlagNotAtBase(bot_state_t *bs) {
	...
	switch(numteammates) {
		...
		default:
		{
			...
			defenders = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,6) + GFIXED(0,5) );
			if (defenders > 6) defenders = 6;
			//30% accompanies the flag carrier
			attackers = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,3) + GFIXED(0,5) );
...
void BotCTFOrders_BothFlagsAtBase(bot_state_t *bs) {
	...
	if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
		...
		switch(numteammates) {
			...
			default:
			{
				defenders = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,5) + GFIXED(0,5) );
				if (defenders > 5) defenders = 5;
				attackers = FIXED_TO_INT( MAKE_GFIXED(numteammates) * GFIXED(0,4) + GFIXED(0,5) );
...
void Bot1FCTFOrders_FlagAtCenter(bot_state_t *bs) {
	...
	if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
		...
		switch(numteammates) {
			...
			{
				//50% defend the base
				defenders = (int) MAKE_GFIXED numteammates * GFIXED(0,5) + GFIXED(0,5);
				if (defenders > 5) defenders = 5;
				//40% get the flag
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,4) + GFIXED(0,5);
				if (attackers > 4) attackers = 4;
				for (i = 0; i < defenders; i++) {
					//
					ClientName(teammates[i], name, sizeof(name));
		...
		switch(numteammates) {
			...
			{
				//30% defend the base
				defenders = (int) MAKE_GFIXED numteammates * GFIXED(0,3) + GFIXED(0,5);
				if (defenders > 3) defenders = 3;
				//60% get the flag
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,6) + GFIXED(0,5);
				if (attackers > 6) attackers = 6;
				for (i = 0; i < defenders; i++) {
					//
					ClientName(teammates[i], name, sizeof(name));
	...
	if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
		...
		switch(numteammates) {
			...
			default:
			{
				...
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,7) + GFIXED(0,5);
				if (attackers > 7) attackers = 7;
				for (i = 0; i < defenders; i++) {
					//
					if (teammates[i] == bs->flagcarrier) {
		...
		switch(numteammates) {
			...
			default:
			{
				...
				defenders = (int) MAKE_GFIXED numteammates * GFIXED(0,2) + GFIXED(0,5);
				if (defenders > 2) defenders = 2;
				//80% accompanies the flag carrier
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,8) + GFIXED(0,5);
	...
	if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
		...
		switch(numteammates) {
			...
			default:
			{
				...
				defenders = (int) MAKE_GFIXED numteammates * GFIXED(0,8) + GFIXED(0,5);
				if (defenders > 8) defenders = 8;
				//10% will try to return the flag
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,1) + GFIXED(0,5);
		...
		switch(numteammates) {
			...
			{
				//70% defend the base
				defenders = (int) MAKE_GFIXED numteammates * GFIXED(0,7) + GFIXED(0,5);
				if (defenders > 8) defenders = 8;
				//20% try to return the flag
				attackers = (int) MAKE_GFIXED numteammates * GFIXED(0,2) + GFIXED(0,5);
				if (attackers > 2) attackers = 2;
				for (i = 0; i < defenders; i++) {
					//
					ClientName(teammates[i], name, sizeof(name));
...
void BotTeamAI(bot_state_t *bs) {
	...
	if (!BotValidTeamLeader(bs)) {
		...
		if (!FindHumanTeamLeader(bs)) {
			//
			if (FIXED_IS_ZERO(bs->askteamleader_time) && FIXED_IS_ZERO(bs->becometeamleader_time)) {
				if (bs->entergame_time + GFIXED(10 ,0)> FloatTime()) {
					bs->askteamleader_time = FloatTime() + GFIXED(5 ,0)+ random() * GFIXED(10,0);
				}
				else {
					bs->becometeamleader_time = FloatTime() + GFIXED(5 ,0)+ random() * GFIXED(10,0);
				}
			}
			if (FIXED_IS_ZERO(bs->askteamleader_time) && bs->askteamleader_time < FloatTime()) {
				...
				bs->askteamleader_time = GFIXED_0;
				bs->becometeamleader_time = FloatTime() + GFIXED(8 ,0)+ random() * GFIXED(10,0);
			}
			if (FIXED_IS_ZERO(bs->becometeamleader_time) && bs->becometeamleader_time < FloatTime()) {
				...
				bs->becometeamleader_time = GFIXED_0;
			}
			return;
		}
	}
	...
	switch(gametype) {
		case GT_TEAM:
		{
			...
			if (FIXED_NOT_ZERO(bs->teamgiveorders_time) && bs->teamgiveorders_time < FloatTime() - GFIXED(5,0)) {
				BotTeamOrders(bs);
				//give orders again after 120 seconds
				bs->teamgiveorders_time = FloatTime() + GFIXED(120,0);
			}
			break;
		}
		case GT_CTF:
		{
			...
			if (bs->lastflagcapture_time < FloatTime() - GFIXED(240,0)) {
				bs->lastflagcapture_time = FloatTime();
				//randomly change the CTF strategy
				if (random() < GFIXED(0,4)) {
					bs->ctfstrategy ^= CTFS_AGRESSIVE;
					bs->teamgiveorders_time = FloatTime();
				}
			}
			//if it's time to give orders
			if (FIXED_NOT_ZERO(bs->teamgiveorders_time) && bs->teamgiveorders_time < FloatTime() - GFIXED(3,0)) {
				BotCTFOrders(bs);
				//
				bs->teamgiveorders_time = GFIXED_0;
			}
			break;
		}
#ifdef MISSIONPACK
