...
qboolean	BG_PlayerTouchesItem( playerState_t *ps, entityState_t *item, int atTime ) {
	...
	if ( ps->origin[0] - origin[0] > BFIXED(44,0)
		|| ps->origin[0] - origin[0] < -BFIXED(50,0)
		|| ps->origin[1] - origin[1] > BFIXED(36,0)
		|| ps->origin[1] - origin[1] < -BFIXED(36,0)
		|| ps->origin[2] - origin[2] > BFIXED(36,0)
		|| ps->origin[2] - origin[2] < -BFIXED(36 ,0)) {
		return qfalse;
	}
...
================
*/
void BG_EvaluateTrajectory( const atrajectory_t *tr, int atTime, avec3_t result ) {
	afixed		deltaTime;
	afixed		phase;
	...
	case TR_INTERPOLATE:
		...
		break;
	case TR_LINEAR:
		deltaTime = MSECTIME_A( atTime - tr->trTime );	// milliseconds to seconds
		FIXED_VEC3MA( tr->trBase, MAKE_AFIXED(deltaTime), tr->trDelta, result );
		break;
	case TR_SINE:
		deltaTime = FIXED_INT32RATIO_A(( atTime - tr->trTime ),tr->trDuration);
		phase = FIXED_SIN( deltaTime * (AFIXED_PI * AFIXED(2,0)));
		FIXED_VEC3MA( tr->trBase, phase, tr->trDelta, result );
		break;
	case TR_LINEAR_STOP:
		...
		deltaTime = MSECTIME_A( atTime - tr->trTime );	// milliseconds to seconds
		if ( deltaTime < AFIXED_0 ) {
			deltaTime = AFIXED_0;
		}
		FIXED_VEC3MA( tr->trBase, deltaTime, tr->trDelta, result );
		break;
	case TR_GRAVITY:
		deltaTime = MSECTIME_A( atTime - tr->trTime );	// milliseconds to seconds
		FIXED_VEC3MA( tr->trBase, deltaTime, tr->trDelta, result );
		result[2] -= AFIXED(DEFAULT_GRAVITY/2,0) * deltaTime * deltaTime;		// FIXME: local gravity...
		break;
	default:
...
void BG_EvaluateTrajectory( const btrajectory_t *tr, int atTime, bvec3_t result ) {
	bfixed		deltaTime;
	bfixed		phase;
	...
	case TR_INTERPOLATE:
		...
		break;
	case TR_LINEAR:
		deltaTime = MSECTIME_B( atTime - tr->trTime );	// milliseconds to seconds
		FIXED_VEC3MA( tr->trBase, MAKE_BFIXED(deltaTime), tr->trDelta, result );
		break;
	case TR_SINE:
		deltaTime = FIXED_INT32RATIO_B( atTime - tr->trTime,tr->trDuration);
		phase = FIXED_SIN( deltaTime * (BFIXED_PI * BFIXED(2,0)));
		FIXED_VEC3MA( tr->trBase, phase, tr->trDelta, result );
		break;
	case TR_LINEAR_STOP:
		if ( atTime > tr->trTime + tr->trDuration ) {
	...
	case TR_GRAVITY:
		deltaTime = MSECTIME_B( atTime - tr->trTime );	// milliseconds to seconds
		FIXED_VEC3MA( tr->trBase, deltaTime, tr->trDelta, result );
		result[2] -= BFIXED(DEFAULT_GRAVITY/2,0) * deltaTime * deltaTime;		// FIXME: local gravity...
		break;
	default:
		Com_Error( ERR_DROP, "BG_EvaluateTrajectory: unknown trType: %i", tr->trTime );
		break;
...
================
*/
void BG_EvaluateTrajectoryDelta( const atrajectory_t *tr, int atTime, avec3_t result ) {
	afixed	deltaTime;
	afixed	phase;
	...
	case TR_SINE:
		deltaTime = FIXED_INT32RATIO_A( atTime - tr->trTime,tr->trDuration);
		phase = FIXED_COS( deltaTime * AFIXED_PI * AFIXED(2 ,0));	// derivative of sin = cos
		phase = FIXED_DIVPOW2(phase,1);
		FIXED_VEC3SCALE( tr->trDelta, phase, result );
		break;
	case TR_LINEAR_STOP:
		if ( atTime > tr->trTime + tr->trDuration ) {
			VectorClear( result );
	...
	case TR_GRAVITY:
		deltaTime = MSECTIME_A(atTime - tr->trTime);	// milliseconds to seconds
		VectorCopy( tr->trDelta, result );
		result[2] -= AFIXED(DEFAULT_GRAVITY ,0)* deltaTime;		// FIXME: local gravity...
		break;
	default:
...
================
*/
void BG_EvaluateTrajectoryDelta( const btrajectory_t *tr, int atTime, bvec3_t result ) {
	bfixed	deltaTime;
	bfixed	phase;
	...
	case TR_SINE:
		deltaTime = FIXED_INT32RATIO_B( atTime - tr->trTime,tr->trDuration);
		phase = FIXED_COS( deltaTime * BFIXED_PI * BFIXED(2,0));	// derivative of sin = cos
		phase = FIXED_DIVPOW2(phase,1);
		FIXED_VEC3SCALE( tr->trDelta, phase, result );
		break;
	case TR_LINEAR_STOP:
	...
	case TR_GRAVITY:
		deltaTime = MSECTIME_B(atTime - tr->trTime);	// milliseconds to seconds
		VectorCopy( tr->trDelta, result );
		result[2] -= BFIXED(DEFAULT_GRAVITY,0)* deltaTime;		// FIXME: local gravity...
		break;
	default:
		Com_Error( ERR_DROP, "BG_EvaluateTrajectoryDelta: unknown trType: %i", tr->trTime );
		break;
...
void BG_TouchJumpPad( playerState_t *ps, entityState_t *jumppad ) {
	...
	if ( ps->jumppad_ent != jumppad->number ) {
		...
		p = FIXED_ABS( AngleNormalize180( angles[PITCH] ) );
		if( p < AFIXED(45 ,0)) {
			effectNum = 0;
		} else {
			effectNum = 1;
...
void BG_PlayerStateToEntityState( playerState_t *ps, entityState_t *s, qboolean snap ) {
	...
	s->angles2[YAW] = MAKE_AFIXED(ps->movementDir);
