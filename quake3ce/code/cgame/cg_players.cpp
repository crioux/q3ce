...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName ) {
	...
	if ( soundName[0] != '*' ) {
		return _CG_trap_S_RegisterSound( soundName, qfalse );
	}
	...
	while ( 1 ) {
		...
		if ( !Q_stricmp( token, "footsteps" ) ) {
			...
			for ( i = 0 ; i < 3 ; i++ ) {
				...
				ci->headOffset[i] = MAKE_BFIXED(atof( token ));
			}
			continue;
...
static qboolean	CG_FileExists(const char *filename) {
	...
	len = _CG_trap_FS_FOpenFile( filename, 0, FS_READ );
	if (len>0) {
		return qtrue;
	}
	return qfalse;
...
static qboolean	CG_RegisterClientSkin( clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName ) {
	...
	ci->legsSkin = _CG_trap_R_RegisterSkin( filename );
	if (!ci->legsSkin) {
	...
	ci->torsoSkin = _CG_trap_R_RegisterSkin( filename );
	if (!ci->torsoSkin) {
	...
	*/
	...
	if ( CG_FindClientHeadFile( filename, sizeof(filename), ci, teamName, headModelName, headSkinName, "head", "skin" ) ) {
		ci->headSkin = _CG_trap_R_RegisterSkin( filename );
	}
	if (!ci->headSkin) {
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	ci->legsModel = _CG_trap_R_RegisterModel( filename );
	if ( !ci->legsModel ) {
	...
	ci->torsoModel = _CG_trap_R_RegisterModel( filename );
	if ( !ci->torsoModel ) {
	...
	ci->headModel = _CG_trap_R_RegisterModel( filename );
	...
	if ( CG_FindClientHeadFile( filename, sizeof(filename), ci, teamName, headName, headSkinName, "icon", "skin" ) ) {
		ci->modelIcon = _CG_trap_R_RegisterShaderNoMip( filename );
	}
...
static void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	if ( ci->torsoModel ) {
		...
		if ( _CG_trap_R_LerpTag( &tag, ci->torsoModel, 0, 0, GFIXED_1, "tag_flag" ) ) {
			ci->newAnims = qtrue;
		}
	}
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ ) {
		...
		if (modelloaded) {
			ci->sounds[i] = _CG_trap_S_RegisterSound( va("sound/player/%s/%s", dir, s + 1), qfalse );
		}
		if ( !ci->sounds[i] ) {
			ci->sounds[i] = _CG_trap_S_RegisterSound( va("sound/player/%s/%s", fallback, s + 1), qfalse );
		}
	}
...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		forceDefer = _CG_trap_MemoryRemaining() < 4000000;
...
void CG_LoadDeferredPlayers( void ) {
	...
	for ( i = 0, ci = cgs.clientinfo ; i < cgs.maxclients ; i++, ci++ ) {
		if ( ci->infoValid && ci->deferred ) {
			...
			if ( _CG_trap_MemoryRemaining() < 4000000 ) {
				CG_Printf( "Memory is low.  Using deferred model.\n" );
				ci->deferred = qfalse;
				continue;
			}
...
===============
*/
static void CG_RunLerpFrame( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, gfixed speedScale ) {
	int			f, numFrames;
	animation_t	*anim;
	...
	if ( cg.time >= lf->frameTime ) {
		...
		f = FIXED_INT32SCALE(f,speedScale);		// adjust for haste, etc
	...
	if ( lf->frameTime == lf->oldFrameTime ) {
		...
		lf->backlerp = GFIXED_1 - FIXED_INT32RATIO_G( cg.time - lf->oldFrameTime , lf->frameTime - lf->oldFrameTime );
	}
}
...
===============
*/
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, gfixed *legsBackLerp,
	...
	if ( cent->currentState.powerups & ( 1 << PW_HASTE ) ) {
		speedScale = GFIXED(1,5);
	} else {
		speedScale = GFIXED(1,0);
	}
...
==================
*/
static void CG_SwingAngles( afixed destination, afixed swingTolerance, afixed clampTolerance,
	...
	if ( !*swinging ) {
		...
		afixed aswing = AngleSubtract( *angle, destination );
		if ( aswing > swingTolerance || aswing < -swingTolerance ) {
			*swinging = qtrue;
		}
	}
	...
	if ( scale < FIXED_DIVPOW2(swingTolerance,1)) {
		scale = AFIXED(0,5);
	...
	if ( swing >= AFIXED_0 ) {
		lmove = MAKE_LFIXED(cg.frametime) * MAKE_LFIXED(scale) * MAKE_LFIXED(speed);
		if ( lmove >= MAKE_LFIXED(swing) ) {
			move = MAKE_BFIXED(swing);
			*swinging = qfalse;
		}
		else
		{
			move=MAKE_BFIXED(FIXED_MOD(lmove,LFIXED(360,0)));
		}
		*angle = AngleModB2A( MAKE_BFIXED(*angle) + move );
...
=================
*/
static void CG_AddPainTwitch( centity_t *cent, avec3_t torsoAngles ) {
	int		t;
	afixed	f;
	...
	f = AFIXED_1 - FIXED_INT32RATIO_A(t,PAIN_TWITCH_TIME);

	if ( cent->pe.painDirection ) {
		torsoAngles[ROLL] += AFIXED(20,0) * f;
	} else {
		torsoAngles[ROLL] -= AFIXED(20,0) * f;
	}
}
...
===============
*/
static void CG_PlayerAngles( centity_t *cent, avec3_t legs[3], avec3_t torso[3], avec3_t head[3] ) {
	...
	if ( cent->currentState.eFlags & EF_DEAD ) {
		...
		dir = FIXED_TO_INT(cent->currentState.angles2[YAW]);
		if ( dir < 0 || dir > 7 ) {
			CG_Error( "Bad player movement angle" );
		}
	}
	legsAngles[YAW] = headAngles[YAW] + MAKE_AFIXED(movementOffsets[ dir ]);
	torsoAngles[YAW] = headAngles[YAW] + AFIXED(0,25) * MAKE_AFIXED(movementOffsets[ dir ]);

	// torso
	CG_SwingAngles( torsoAngles[YAW], AFIXED(25,0), AFIXED(90,0), MAKE_AFIXED(cg_swingSpeed.value), &cent->pe.torso.yawAngle, &cent->pe.torso.yawing );
	CG_SwingAngles( legsAngles[YAW], AFIXED(40,0), AFIXED(90,0), MAKE_AFIXED(cg_swingSpeed.value), &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
	...
	CG_SwingAngles( dest, AFIXED(15,0), AFIXED(30,0), AFIXED(0,1), &cent->pe.torso.pitchAngle, &cent->pe.torso.pitching );
	...
	if ( FIXED_NOT_ZERO(speed) ) {
		...
		side = speed * FIXED_VEC3DOT( velocity, axis[1] );
		legsAngles[ROLL] -= MAKE_AFIXED(side);

		side = speed * FIXED_VEC3DOT( velocity, axis[0] );
		legsAngles[PITCH] += MAKE_AFIXED(side);
	}
	...
	if ( clientNum >= 0 && clientNum < MAX_CLIENTS ) {
		...
		if ( ci->fixedlegs ) {
			...
			legsAngles[PITCH] = AFIXED_0;
			legsAngles[ROLL] = AFIXED_0;
		}
	}
...
static void CG_HasteTrail( centity_t *cent ) {
	...
	origin[2] -= BFIXED(16,0);

	smoke = CG_SmokePuff( origin, bvec3_origin, 
				  BFIXED(8,0), 
...
static void CG_BreathPuffs( centity_t *cent, refEntity_t *head) {
	...
	contents = _CG_trap_CM_PointContents( head->origin, 0 );
	...
	VectorSet( up, GFIXED_0, GFIXED_0, GFIXED(8,0) );
	FIXED_VEC3MA(head->origin, GFIXED(9,0), head->axis[0], origin);
	FIXED_VEC3MA(origin, -GFIXED(4,0), head->axis[2], origin);
	CG_SmokePuff( origin, up, 16, 1, 1, 1, GFIXED(0,66), 1500, cg.time, cg.time + 400, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
...
static void CG_DustTrail( centity_t *cent ) {
	...
	VectorSet(vel, GFIXED_0, GFIXED_0, -GFIXED(30,0));
	dust = CG_SmokePuff( end, vel,
				  24,
				  .8f, .8f, GFIXED(0,7), GFIXED(0,33),
				  500,
				  cg.time,
				  0,
				  0,
...
static void CG_TrailItem( centity_t *cent, qhandle_t hModel ) {
	...
	FIXED_VEC3MA_R( cent->lerpOrigin, -BFIXED(16,0), axis[0], ent.origin );
	ent.origin[2] += BFIXED(16,0);
	angles[YAW] += AFIXED(90,0);
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hSkin, refEntity_t *torso ) {
	...
	_CG_trap_R_AddRefEntityToScene( &pole );
	...
	if ( updateangles ) {
		...
		dir[2] += BFIXED(100,0);
		VectorNormalize( dir );
		d = FIXED_VEC3DOT_R(pole.axis[2], dir);
		// if there is anough movement orthogonal to the flag pole
		if (FIXED_ABS(d) < BFIXED(0,9)) {
			//
			d = FIXED_VEC3DOT_R(pole.axis[0], dir);
			if (d > BFIXED_1) {
				d = BFIXED_1;
			}
			else if (d < -BFIXED_1) {
			...
			angle = FIXED_ACOS(MAKE_AFIXED(d));

			d = FIXED_VEC3DOT_R(pole.axis[1], dir);
			if (d < BFIXED_0) {
				angles[YAW] = AFIXED(360,0) - angle * AFIXED(180,0) / AFIXED_PI;
			}
			else {
				angles[YAW] = angle * AFIXED(180,0) / AFIXED_PI;
			}
			if (angles[YAW] < AFIXED_0)
				angles[YAW] += AFIXED(360,0);
			if (angles[YAW] > AFIXED(360,0))
				angles[YAW] -= AFIXED(360,0);

			//vectoangles( cent->currentState.pos.trDelta, tmpangles );
			//angles[YAW] = tmpangles[YAW] + 45 - cent->pe.torso.yawAngle;
			// change the yaw angle
			CG_SwingAngles( angles[YAW], AFIXED(25,0), AFIXED(90,0), AFIXED(0,15), &cent->pe.flag.yawAngle, &cent->pe.flag.yawing );
		}

		/*
		d = FIXED_VEC3DOT(pole.axis[2], dir);
		angle = Q_acos(d);

		d = FIXED_VEC3DOT(pole.axis[1], dir);
		if (d < 0) {
			angle = 360 - angle * 180 / GFIXED_PI;
		}
		else {
			angle = angle * 180 / GFIXED_PI;
		}
	...
	CG_RunLerpFrame( ci, &cent->pe.flag, flagAnim, GFIXED_1 );
	...
	_CG_trap_R_AddRefEntityToScene( &flag );
}
...
static void CG_PlayerTokens( centity_t *cent, int renderfx ) {
	...
	for (i = 0; i < trail->numpositions; i++) {
		...
		if (VectorNormalize(dir) > 30) {
			FIXED_VEC3MA(origin, GFIXED(30,0), dir, trail->positions[i]);
		}
	...
	for (i = 0; i < trail->numpositions; i++) {
		...
		VectorSet(ent.axis[2], GFIXED_0, GFIXED_0, GFIXED_1);
		CrossProduct(ent.axis[0], ent.axis[2], ent.axis[1]);

		VectorCopy(trail->positions[i], ent.origin);
		angle = (((cg.time + 500 * MAX_SKULLTRAIL - 500 * i) / 16) & 255) * (GFIXED_PI * 2) / 255;
...
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso ) {
	...
	if ( powerups & ( 1 << PW_QUAD ) ) {
		_CG_trap_R_AddLightToScene( cent->lerpOrigin, MAKE_GFIXED(200 + (rand()&31)), GFIXED(0,2), GFIXED(0,2), GFIXED_1 );
	}

	// flight plays a looped sound
	if ( powerups & ( 1 << PW_FLIGHT ) ) {
		_CG_trap_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, bvec3_origin, cgs.media.flightSound );
	}
	...
	if ( powerups & ( 1 << PW_REDFLAG ) ) {
		...
		_CG_trap_R_AddLightToScene( cent->lerpOrigin, MAKE_GFIXED(200 + (rand()&31)), GFIXED_1, GFIXED(0,2), GFIXED(0,2) );
	}
	...
	if ( powerups & ( 1 << PW_NEUTRALFLAG ) ) {
		...
		_CG_trap_R_AddLightToScene( cent->lerpOrigin, MAKE_GFIXED(200 + (rand()&31)), GFIXED_1, GFIXED_1, GFIXED_1 );
	}
...
static qboolean CG_PlayerShadow( centity_t *cent, bfixed *shadowPlane ) {
	bvec3_t		end, mins = {-BFIXED(15,0), -BFIXED(15,0), BFIXED_0}, maxs = {BFIXED(15,0), BFIXED(15,0), BFIXED(2,0)};
	trace_t		trace;
	gfixed		alpha;

	*shadowPlane = BFIXED_0;
	...
	end[2] -= BFIXED(SHADOW_DISTANCE,0);

	_CG_trap_CM_BoxTrace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID );

	// no shadow if too high
	if ( trace.fraction == GFIXED_1 || trace.startsolid || trace.allsolid ) {
		return qfalse;
	}

	*shadowPlane = trace.endpos[2] + BFIXED_1;
	...
	alpha = GFIXED_1 - trace.fraction;

	// bk0101022 - hack / FPE - bogus planes?
	//assert( FIXED_VEC3DOT( trace.plane.normal, trace.plane.normal ) != GFIXED_0 ) 

	// add the mark as a temporary, so it goes directly to the renderer
	// without taking a spot in the cg_marks array
	CG_ImpactMark( cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal, 
		cent->pe.legs.yawAngle, alpha,alpha,alpha,GFIXED_1, qfalse, BFIXED(24,0), qtrue );
...
static void CG_PlayerSplash( centity_t *cent ) {
	...
	contents = _CG_trap_CM_PointContents( end, 0 );
	if ( !( contents & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) ) {
		return;
	}
	...
	contents = _CG_trap_CM_PointContents( start, 0 );
	if ( contents & ( CONTENTS_SOLID | CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) {
		return;
	}

	// trace down to find the surface
	_CG_trap_CM_BoxTrace( &trace, start, end, NULL, NULL, 0, ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) );
	...
	verts[0].xyz[0] -= BFIXED(32,0);
	verts[0].xyz[1] -= BFIXED(32,0);
	...
	verts[1].xyz[0] -= BFIXED(32,0);
	verts[1].xyz[1] += BFIXED(32,0);
	...
	verts[2].xyz[0] += BFIXED(32,0);
	verts[2].xyz[1] += BFIXED(32,0);
	...
	verts[3].xyz[0] += BFIXED(32,0);
	verts[3].xyz[1] -= BFIXED(32,0);
	...
	_CG_trap_R_AddPolyToScene( cgs.media.wakeMarkShader, 4, verts );
}
...
=================
*/
int CG_LightVerts( bvec3_t normal, int numVerts, polyVert_t *verts )
{
	int				i, j;
	bfixed			incoming;
	vec3_t			ambientLight;
	...
	_CG_trap_R_LightForPoint( verts[0].xyz, ambientLight, directedLight, lightDir );

	for (i = 0; i < numVerts; i++) {
		incoming = FIXED_VEC3DOT (normal, lightDir);
		if ( incoming <= BFIXED_0 ) {
			verts[i].modulate[0] = FIXED_TO_INT(ambientLight[0]);
			verts[i].modulate[1] = FIXED_TO_INT(ambientLight[1]);
			verts[i].modulate[2] = FIXED_TO_INT(ambientLight[2]);
...
void CG_Player( centity_t *cent ) {
	...
	if ( cent->currentState.eFlags & EF_KAMIKAZE ) {
		...
		if ( cent->currentState.eFlags & EF_DEAD ) {
			...
			angle = ((cg.time / 7) & 255) * (GFIXED_PI * 2) / 255;
			if (angle > GFIXED_PI * 2)
				angle -= MAKE_GFIXEDGFIXED_PI * 2;
			dir[0] = sin(angle) * 20;
			dir[1] = cos(angle) * 20;
			angle = ((cg.time / 4) & 255) * (GFIXED_PI * 2) / 255;
			...
			angles[1] = (angle * 180 / GFIXED_PI) + 90;
			if (angles[1] > 360)
				angles[1] -= 360;
			angles[2] = 0;
			...
			*/
			...
			angle = ((cg.time / 4) & 255) * (GFIXED_PI * 2) / 255 + GFIXED_PI;
			if (angle > GFIXED_PI * 2)
				angle -= MAKE_GFIXEDGFIXED_PI * 2;
			dir[0] = sin(angle) * 20;
			dir[1] = cos(angle) * 20;
			...
			angles[0] = cos(angle - GFIXED(0,5) * GFIXED_PI) * 30;
			angles[1] = 360 - (angle * 180 / GFIXED_PI);
			if (angles[1] > 360)
				angles[1] -= 360;
			angles[2] = 0;
			AnglesToAxis( angles, skull.axis );
			...
			*/
			...
			angle = ((cg.time / 3) & 255) * (GFIXED_PI * 2) / 255 + GFIXED(0,5) * GFIXED_PI;
			if (angle > GFIXED_PI * 2)
				angle -= MAKE_GFIXEDGFIXED_PI * 2;
			dir[0] = sin(angle) * 20;
			dir[1] = cos(angle) * 20;
	...
	if ( cent->currentState.powerups & ( 1 << PW_GUARD ) ) {
		...
		_CG_trap_R_AddRefEntityToScene( &powerup );
	}
	...
	if ( (cent->currentState.powerups & ( 1 << PW_INVULNERABILITY ) ) ||
		...
		if ( cg.time - ci->invulnerabilityStartTime < 250 ) {
			c = MAKE_GFIXED (cg.time - ci->invulnerabilityStartTime) / 250;
		}
		else if (cg.time - ci->invulnerabilityStopTime < 250 ) {
			c = MAKE_GFIXED (250 - (cg.time - ci->invulnerabilityStopTime)) / 250;
		}
		else {
			c = 1;
		}
		VectorSet( powerup.axis[0], c, GFIXED_0, GFIXED_0 );
		VectorSet( powerup.axis[1], GFIXED_0, c, GFIXED_0 );
		VectorSet( powerup.axis[2], GFIXED_0, GFIXED_0, c );
		_CG_trap_R_AddRefEntityToScene( &powerup );
	}
	...
	if ( ci->medkitUsageTime && t < 500 ) {
		...
		VectorCopy(cent->lerpOrigin, powerup.origin);
		powerup.origin[2] += -24 + MAKE_GFIXED t * 80 / 500;
		if ( t > 400 ) {
			c = MAKE_GFIXED (t - 1000) * 0xff / 100;
...
void CG_ResetPlayerEntity( centity_t *cent ) {
	...
	cent->pe.legs.pitchAngle = AFIXED_0;
