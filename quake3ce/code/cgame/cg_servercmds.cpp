...
void CG_ParseServerinfo( void ) {
	...
	info = (const char *)CG_ConfigString( CS_SERVERINFO );
	cgs.gametype = (gametype_t)atoi( Info_ValueForKey( info, "g_gametype" ) );
	_CG_trap_Cvar_Set("g_gametype", va("%i", cgs.gametype));
	...
	_CG_trap_Cvar_Set("g_redTeam", cgs.redTeam);
	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
	_CG_trap_Cvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ParseWarmup( void ) {
	...
	if ( warmup == 0 && cg.warmup ) {
		...
		if (cgs.gametype >= GT_CTF && cgs.gametype <= GT_HARVESTER) {
			_CG_trap_S_StartLocalSound( cgs.media.countPrepareTeamSound, CHAN_ANNOUNCER );
		} else
#endif
		{
			_CG_trap_S_StartLocalSound( cgs.media.countPrepareSound, CHAN_ANNOUNCER );
		}
	}
	...
	while (o && *o) {
		...
		if (n && *n) {
			...
			if (o) {
				...
				timeOffset[o-t] = 0;
				o++;
				_CG_trap_R_RemapShader( originalShader, newShader, timeOffset );
			}
		} else {
			break;
		}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
...
#ifdef MISSIONPACK
		_CG_trap_S_StartLocalSound( cgs.media.voteNow, CHAN_ANNOUNCER );
#endif //MISSIONPACK
		...
		cgs.gameModels[ num-CS_MODELS ] = _CG_trap_R_RegisterModel( str );
	} else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_MODELS ) {
		if ( str[0] != '*' ) {	// player specific sounds don't register here
			cgs.gameSounds[ num-CS_SOUNDS] = _CG_trap_S_RegisterSound( str, qfalse );
		}
...
static void CG_MapRestart( void ) {
	...
	if ( cg.warmup == 0 /* && cgs.gametype == GT_TOURNAMENT */) {
		_CG_trap_S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
		CG_CenterPrint( "FIGHT!", 120, GIANTCHAR_WIDTH*2 );
	}
#ifdef MISSIONPACK
	if (cg_singlePlayerActive.integer) {
		_CG_trap_Cvar_Set("ui_matchStartTime", va("%i", cg.time));
		if (cg_recordSPDemo.integer && cg_recordSPDemoName.string && *cg_recordSPDemoName.string) {
			_CG_trap_SendConsoleCommand(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
		}
	}
#endif
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "cp" ) ) {
		CG_CenterPrint( CG_Argv(1), FIXED_TO_INT(GFIXED(SCREEN_HEIGHT,0) * GFIXED(0,30)), BIGCHAR_WIDTH );
		return;
	}
	...
	if ( !strcmp( cmd, "print" ) ) {
		...
		if ( !Q_stricmpn( cmd, "vote failed", 11 ) || !Q_stricmpn( cmd, "team vote failed", 16 )) {
			_CG_trap_S_StartLocalSound( cgs.media.voteFailed, CHAN_ANNOUNCER );
		} else if ( !Q_stricmpn( cmd, "vote passed", 11 ) || !Q_stricmpn( cmd, "team vote passed", 16 ) ) {
			_CG_trap_S_StartLocalSound( cgs.media.votePassed, CHAN_ANNOUNCER );
		}
#endif
		return;
	}
...
*/
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
		if (_CG_trap_Argc() == 4) {
			_CG_trap_R_RemapShader(CG_Argv(1), CG_Argv(2), CG_Argv(3));
		}
	}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( _CG_trap_GetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}
