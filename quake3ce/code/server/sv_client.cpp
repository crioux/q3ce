...
void SV_DirectConnect( netadr_t from ) {
	...
	if ( !NET_IsLocalAddress (from) ) {
		...
		if ( !Sys_IsLANAddress( from ) ) {
			if ( FIXED_NOT_ZERO(sv_minPing->value) && MAKE_LFIXED(ping) < sv_minPing->value ) {
			...
			if ( FIXED_NOT_ZERO(sv_maxPing->value) && MAKE_LFIXED(ping) > sv_maxPing->value ) {
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	SysCallArgs args(1);
	args[0]=drop - svs.clients;
	VM_Call( gvm, GAME_CLIENT_DISCONNECT, args);
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	SysCallArgs args(1);
	args[0]=client - svs.clients;
	VM_Call( gvm, GAME_CLIENT_BEGIN, args);
}
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	SysCallArgs args(1);
	args[0]=cl - svs.clients;
	VM_Call( gvm, GAME_CLIENT_USERINFO_CHANGED, args);
}
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
	...
	if (clientOK) {
		...
		if (!u->name && sv.state == SS_GAME) {
			SysCallArgs args(1);
			args[0]=cl-svs.clients;
			VM_Call( gvm, GAME_CLIENT_COMMAND, args);
		}
	}
	else if (!bProcessed)
...
void SV_ClientThink (client_t *cl, usercmd_t *cmd) {
	...
	SysCallArgs args(1);
	args[0]=cl - svs.clients;
	VM_Call( gvm, GAME_CLIENT_THINK, args);
}
