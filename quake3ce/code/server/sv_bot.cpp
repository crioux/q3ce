...
void BotDrawDebugPolygons(void (*drawPoly)(int color, int numPoints, bfixed *points), int value) {
	...
	for (i = 0; i < bot_maxdebugpolys; i++) {
		...
		drawPoly(poly->color, poly->numPoints, (bfixed *)poly->points);
...
==================
*/
void BotImport_Trace(bsp_trace_t *bsptrace, bvec3_t start, bvec3_t mins, bvec3_t maxs, bvec3_t end, int passent, int contentmask) {
	trace_t trace;
...
==================
*/
void BotImport_EntityTrace(bsp_trace_t *bsptrace, bvec3_t start, bvec3_t mins, bvec3_t maxs, bvec3_t end, int entnum, int contentmask) {
	trace_t trace;
...
==================
*/
int BotImport_inPVS(bvec3_t p1, bvec3_t p2) {
...
==================
*/
void BotImport_BSPModelMinsMaxsOrigin(int modelnum, avec3_t angles, bvec3_t outmins, bvec3_t outmaxs, bvec3_t origin) {
	clipHandle_t h;
	bvec3_t mins, maxs;
	bfixed max;
	int	i;
	...
	if ((FIXED_NOT_ZERO(angles[0]) || FIXED_NOT_ZERO(angles[1]) || FIXED_NOT_ZERO(angles[2]))) {
		// expand for rotation
...
==================
*/
int BotImport_DebugPolygonCreate(int color, int numPoints, bvec3_t *points) {
	bot_debugpoly_t *poly;
	int i;
	...
	Com_Memcpy(poly->points, points, numPoints * sizeof(bvec3_t));
	//
	return i;
}
...
==================
*/
void BotImport_DebugPolygonShow(int id, int color, int numPoints, bvec3_t *points) {
	bot_debugpoly_t *poly;
...
==================
*/
void BotImport_DebugLineShow(int line, bvec3_t start, bvec3_t end, int color) {
	bvec3_t points[4], tmp;
	...
	FIXED_VEC3MA_R(points[0], BFIXED(2,0), cross, points[0]);
	FIXED_VEC3MA_R(points[1], -BFIXED(2,0), cross, points[1]);
	FIXED_VEC3MA_R(points[2], -BFIXED(2,0), cross, points[2]);
	FIXED_VEC3MA_R(points[3], BFIXED(2,0), cross, points[3]);
...
void SV_BotFrame( int time ) 
{
	...
	SysCallArgs args(1);
	args[0]=time;
	VM_Call( gvm, BOTAI_START_FRAME, args);
}
