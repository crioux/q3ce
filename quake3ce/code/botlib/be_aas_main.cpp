...
// Changes Globals:		-
//===========================================================================
void AAS_ContinueInit(gfixed time)
{
	...
	if (aasworld.savefile || FIXED_NOT_ZERO(LibVarGetValue("forcewrite")))
	{
		//optimize the AAS data
		if (FIXED_NOT_ZERO(LibVarValue("aasoptimize", "0"))) AAS_Optimize();
		//save the AAS file
...
// Changes Globals:		-
//===========================================================================
int AAS_StartFrame(gfixed time)
{
	aasworld.time = time;
	...
	if (bl_bot_developer)
	{
		if (FIXED_NOT_ZERO(LibVarGetValue("showcacheupdates")))
		{
			AAS_RoutingInfo();
			LibVarSet("showcacheupdates", "0");
		} //end if
		if (FIXED_NOT_ZERO(LibVarGetValue("showmemoryusage")))
		{
			PrintUsedMemorySize();
			LibVarSet("showmemoryusage", "0");
		} //end if
		if (FIXED_NOT_ZERO(LibVarGetValue("memorydump")))
		{
			PrintMemoryLabels();
	...
	//
	if (FIXED_NOT_ZERO(saveroutingcache->value))
	{
		AAS_WriteRouteCache();
...
// Changes Globals:		-
//===========================================================================
void AAS_ProjectPointOntoVector( bvec3_t point, bvec3_t vStart, bvec3_t vEnd, bvec3_t vProj )
{
	bvec3_t pVec, vec;
	...
	FIXED_VEC3MA( vStart, FIXED_VEC3DOT( pVec, vec ), vec, vProj );
