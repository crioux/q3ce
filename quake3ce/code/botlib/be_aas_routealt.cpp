										 ...
										 int type)
{
	...
	for (i = 1; i < aasworld.numareas; i++)
	{
		...
		if (MAKE_GFIXED(starttime) > GFIXED(1,1) * MAKE_GFIXED(goaltraveltime)) continue;
		//travel time from the area to the goal area
		goaltime = AAS_AreaTravelTimeToGoalArea(i, NULL, goalareanum, travelflags);
		if (!goaltime) continue;
		//if the travel time from the area to the goal is greater than the shortest goal travel time
		if (MAKE_GFIXED(goaltime) > GFIXED(0,8) * MAKE_GFIXED(goaltraveltime)) continue;
	...
	for (i = 1; i < aasworld.numareas; i++)
	{
		...
		FIXED_VEC3SCALE(mid, FIXED_INT32RATIO_B(1, numclusterareas), mid);
