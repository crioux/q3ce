...
#define INSIDEUNITS							BFIXED(2,0)
#define INSIDEUNITS_WALKEND					BFIXED(5,0)
#define INSIDEUNITS_WALKSTART				BFIXED(0,1)
#define INSIDEUNITS_WATERJUMP				BFIXED(15,0)
...
typedef struct aas_lreachability_s
{
	...
	bvec3_t start;					//start point of inter area movement
	bvec3_t end;						//end point of inter area movement
...
// Changes Globals:		-
//===========================================================================
bfixed AAS_FaceArea(aas_face_t *face)
{
	int i, edgenum, side;
	bfixed total;
	bvec_t *v;
	...
	for (i = 1; i < face->numedges - 1; i++)
	{
		...
		total += BFIXED(0,5) * FIXED_VEC3LEN(cross);
	} //end for
	return total;
...
// Changes Globals:		-
//===========================================================================
bfixed AAS_AreaVolume(int areanum)
{
	...
	for (i = 0; i < area->numfaces; i++)
	{
		...
		d = -(FIXED_VEC3DOT (corner, plane->normal) - plane->dist);
		a = AAS_FaceArea(face);
		volume += d * a;
	} //end for
...
// Changes Globals:		-
//===========================================================================
int AAS_GetJumpPadInfo(int ent, bvec3_t areastart, bvec3_t absmins, bvec3_t absmaxs, bvec3_t velocity)
{
	int modelnum, ent2;
	bfixed speed, height, gravity, time, dist, forward;
	bvec3_t origin, teststart, ent2origin;
	avec3_t angles;
	aas_trace_t trace;
	...
	if (FIXED_IS_ZERO(speed)) speed = BFIXED(1000,0);
	VectorClear(angles);
	...
	time = FIXED_SQRT( height / ( BFIXED(0,5) * gravity ) );
	if (FIXED_IS_ZERO(time))
	{
	...
	forward = dist / time;
	//FIXME: why multiply by BFIXED(1,1)
	forward *= BFIXED(1,1);
	FIXED_VEC3SCALE(velocity, forward, velocity);
...
// Changes Globals:		-
//===========================================================================
int AAS_BestReachableFromJumpPadArea(bvec3_t origin, bvec3_t mins, bvec3_t maxs)
{
	int area2num, ent, bot_visualizejumppads, bestareanum;
	bfixed volume, bestareavolume;
	bvec3_t areastart, cmdmove, bboxmins, bboxmaxs;
	bvec3_t absmins, absmaxs, velocity;
...
#else
	bot_visualizejumppads = FIXED_TO_INT(LibVarValue("bot_visualizejumppads", "0"));
#endif
...
// Changes Globals:		-
//===========================================================================
int AAS_BestReachableArea(bvec3_t origin, bvec3_t mins, bvec3_t maxs, bvec3_t goalorigin)
{
	int areanum, i, j, k, l;
	aas_link_t *areas;
	bvec3_t absmins, absmaxs;
	//bvec3_t bbmins, bbmaxs;
	bvec3_t start, end;
	aas_trace_t trace;
	...
	for (i = 0; i < 5 && !areanum; i++)
	{
		for (j = 0; j < 5 && !areanum; j++)
		{
			for (k = -1; k <= 1 && !areanum; k++)
			{
				...
				{
					VectorCopy(origin, start);
					start[0] += MAKE_BFIXED(j * 4 * k);
					start[1] += MAKE_BFIXED(j * 4 * l);
					start[2] += MAKE_BFIXED(i * 4);
					areanum = AAS_PointAreaNum(start);
				} //end for
			} //end for
		} //end for
	...
	if (areanum)
	{
		...
		else
		{
			...
			{
				Log_Write("point %f %f %f in area %d but trace startsolid", FIXED_TO_DOUBLE(start[0]), FIXED_TO_DOUBLE(start[1]), FIXED_TO_DOUBLE(start[2]), areanum);
...
// Changes Globals:		-
//===========================================================================
bfixed AAS_AreaGroundFaceArea(int areanum)
{
	int i;
	bfixed total;
	aas_area_t *area;
...
// Changes Globals:		-
//===========================================================================
void AAS_FaceCenter(int facenum, bvec3_t center)
{
	int i;
	bfixed scale;
	aas_face_t *face;
	...
	scale = BFIXED(0,5) / MAKE_BFIXED(face->numedges);
	FIXED_VEC3SCALE(center, scale, center);
...
{
	bfixed maxzvelocity, gravity, t;

	maxzvelocity = FIXED_SQRT(BFIXED(30,0) * BFIXED(10000,0));
	gravity = aassettings.phys_gravity;
	t = maxzvelocity / gravity;
	return FIXED_TO_INT(BFIXED(0,5) * gravity * t * t);
} //end of the function AAS_FallDamageDistance
//===========================================================================
// distance = BFIXED(0,5) * gravity * t * t
// vel = t * gravity
// damage = vel * vel * BFIXED(0,0001)
//
// Parameter:			-
// Returns:				-
// Changes Globals:		-
//===========================================================================
bfixed AAS_FallDelta(bfixed distance)
{
	bfixed t, delta, gravity;

	gravity = aassettings.phys_gravity;
	t = FIXED_SQRT(FIXED_ABS(distance) * BFIXED(2,0) / gravity);
	delta = t * gravity;
	return delta * delta * BFIXED(0,0001);
...
// Changes Globals:		-
//===========================================================================
bfixed AAS_MaxJumpHeight(bfixed phys_jumpvel)
{
	bfixed phys_gravity;

	phys_gravity = aassettings.phys_gravity;
	//maximum height a player can jump with the given initial z velocity
	return BFIXED(0,5) * phys_gravity * (phys_jumpvel / phys_gravity) * (phys_jumpvel / phys_gravity);
...
// Changes Globals:		-
//===========================================================================
bfixed AAS_MaxJumpDistance(bfixed phys_jumpvel)
{
	bfixed phys_gravity, phys_maxvelocity, t;

	phys_gravity = aassettings.phys_gravity;
	phys_maxvelocity = aassettings.phys_maxvelocity;
	//time a player takes to fall the height
	t = FIXED_SQRT(aassettings.rs_maxjumpfallheight / (BFIXED(0,5) * phys_gravity));
   //maximum distance
...
{
	return FIXED_TO_INT(aassettings.phys_jumpvel / (aassettings.phys_gravity * BFIXED(0,1)));
...
// Changes Globals:		-
//===========================================================================
int AAS_NearbySolidOrGap(bvec3_t start, bvec3_t end)
{
	bvec3_t tmp;
	avec3_t dir;
	bvec3_t testpoint;
	...
	FIXED_VEC3MA_R(end, BFIXED(48,0), dir, testpoint);

	areanum = AAS_PointAreaNum(testpoint);
	if (!areanum)
	{
		testpoint[2] += BFIXED(16,0);
		areanum = AAS_PointAreaNum(testpoint);
		if (!areanum) return qtrue;
	} //end if
	FIXED_VEC3MA_R(end, BFIXED(64,0), dir, testpoint);
	areanum = AAS_PointAreaNum(testpoint);
	if (areanum)
	{
...
int AAS_Reachability_Swim(int area1num, int area2num)
{
	...
	for (i = 0; i < 3; i++)
	{
		if (area1->mins[i] > area2->maxs[i] + BFIXED(10,0)) return qfalse;
		if (area1->maxs[i] < area2->mins[i] - BFIXED(10,0)) return qfalse;
	} //end for
		...
		for (j = 0; j < area2->numfaces; j++)
		{
			...
			if (face1num == face2num)
			{
				...
				if (AAS_PointContents(start) & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER))
				{
					...
					FIXED_VEC3MA_R(lreach->start, -INSIDEUNITS, plane->normal, lreach->end);
					lreach->traveltype = TRAVEL_SWIM;
					lreach->traveltime = 1;
					//if the volume of the area is rather small
					if (AAS_AreaVolume(area2num) < BFIXED(800,0))
						lreach->traveltime += 200;
					//if (!(AAS_PointContents(start) & MASK_WATER)) lreach->traveltime += BFIXED(500,0);
...
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num)
{
	...
	bfixed height, bestheight, length, bestlength;
	bvec3_t start, end, tmp;
	avec3_t normal, invgravity, gravitydirection = {AFIXED_0, AFIXED_0, -AFIXED_1};
	bvec3_t edgevec;
	aas_area_t *area1, *area2;
			...
			for (edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++)
			{
				for (edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++)
				{
					...
					VectorSubtract(aasworld.vertexes[edge->v[1]],
								aasworld.vertexes[edge->v[0]], bdir);
					length = FIXED_VEC3LEN(bdir);
					//get the start point
					VectorAdd(aasworld.vertexes[edge->v[0]],
								aasworld.vertexes[edge->v[1]], start);
					FIXED_VEC3SCALE(start, BFIXED(0,5), start);
					VectorCopy(start, end);
					...
					CrossProduct(edgevec, plane2->normal, tmp);
					VectorNormalizeB2A(tmp,normal);
					//
					//FIXED_VEC3MA(start, -BFIXED_1, normal, start);
					FIXED_VEC3MA_R(end, INSIDEUNITS_WALKEND, normal, end);
					FIXED_VEC3MA_R(start, INSIDEUNITS_WALKSTART, normal, start);
					end[2] += BFIXED(0,125);
					//
					height = FIXED_VEC3DOT_R(invgravity, start);
					...
					if (height < bestheight ||
							(height < bestheight + BFIXED_1 && length > bestlength))
					{
						bestheight = height;
						bestlength = length;
						//create a new reachability link
	...
	if (foundreach)
	{
		...
		{
			lreach->traveltime += FIXED_TO_INT(aassettings.rs_startcrouch);
		} //end if
		/*
		//NOTE: if there's nearby solid or a gap area after this area
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num)
{
	...
	bfixed dist, dist1, dist2, diff, ortdot;
	afixed invgravitydot;
	bfixed x1, x2, x3, x4, y1, y2, y3, y4, tmp, y;
	bfixed length, ground_bestlength, water_bestlength, ground_bestdist, water_bestdist;
	bvec3_t v1, v2, v3, v4, tmpv, p1area1, p1area2, p2area1, p2area2;
	bvec3_t normal, ort, edgevec, start, end, dir;
	bvec3_t ground_beststart, ground_bestend, ground_bestnormal;
	bvec3_t water_beststart, water_bestend, water_bestnormal;
	avec3_t invgravity = {AFIXED_0, AFIXED_0, AFIXED_1};
	bvec3_t testpoint;
	aas_plane_t *plane;
	aas_area_t *area1, *area2;
	...
	ground_bestdist = BFIXED(99999,0);
	ground_bestlength = BFIXED_0;
	ground_bestarea2groundedgenum = 0;
	//
	water_foundreach = qfalse;
	water_bestdist = BFIXED(99999,0);
	water_bestlength = BFIXED_0;
	...
	for (i = 0; i < area1->numfaces; i++)
	{
		...
		if (!(groundface1->faceflags & FACE_GROUND))
		{
			...
			if (area1swim)
			{
				...
				if (FIXED_VEC3DOT_R(plane->normal, invgravity) < AFIXED(0,7)) continue;
			} //end if
			else
			{
		...
		for (k = 0; k < groundface1->numedges; k++)
		{
			...
			for (j = 0; j < area2->numfaces; j++)
			{
				...
				for (l = 0; l < groundface2->numedges; l++)
				{
					...
					diff = FIXED_VEC3DOT(normal, v3) - dist;
					if (diff < -BFIXED(0,1) || diff > BFIXED(0,1)) continue;
					diff = FIXED_VEC3DOT(normal, v4) - dist;
					if (diff < -BFIXED(0,1) || diff > BFIXED(0,1)) continue;
					//
					...
					invgravitydot = FIXED_VEC3DOT(invgravity, invgravity);
					ortdot = FIXED_VEC3DOT(ort, ort);
					//projection into the step plane
					//NOTE: since gravity is vertical this is just the z coordinate
					y1 = v1[2];//FIXED_VEC3DOT(v1, invgravity) / invgravitydot;
					y2 = v2[2];//FIXED_VEC3DOT(v2, invgravity) / invgravitydot;
					y3 = v3[2];//FIXED_VEC3DOT(v3, invgravity) / invgravitydot;
					y4 = v4[2];//FIXED_VEC3DOT(v4, invgravity) / invgravitydot;
					//
					x1 = FIXED_VEC3DOT(v1, ort) / ortdot;
					x2 = FIXED_VEC3DOT(v2, ort) / ortdot;
					x3 = FIXED_VEC3DOT(v3, ort) / ortdot;
					x4 = FIXED_VEC3DOT(v4, ort) / ortdot;
					//
					if (x1 > x2)
					{
						tmp = x1; x1 = x2; x2 = tmp;
...
//						Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num);
					...
					if ((x1 - BFIXED(0,5) < x3 && x4 < x2 + BFIXED(0,5)) &&
							(x3 - BFIXED(0,5) < x1 && x2 < x4 + BFIXED(0,5)))
					{
						dist1 = y3 - y1;
						dist2 = y4 - y2;
					...
					else
					{
						...
						if (x1 > x3 - BFIXED(0,1) && x1 < x3 + BFIXED(0,1))
						{
							dist1 = y3 - y1;
							VectorCopy(v1, p1area1);
							VectorCopy(v3, p1area2);
						...
						//if the points are equal
						if (x2 > x4 - BFIXED(0,1) && x2 < x4 + BFIXED(0,1))
						{
							dist2 = y4 - y2;
							VectorCopy(v2, p2area1);
							VectorCopy(v4, p2area2);
					...
					if (dist1 > dist2 - BFIXED_1 && dist1 < dist2 + BFIXED_1)
					{
						dist = dist1;
						VectorAdd(p1area1, p2area1, start);
						FIXED_VEC3SCALE(start, BFIXED(0,5), start);
						VectorAdd(p1area2, p2area2, end);
						FIXED_VEC3SCALE(end, BFIXED(0,5), end);
					} //end if
					else if (dist1 < dist2)
					{
						dist = dist1;
					...
					if (groundface1->faceflags & FACE_GROUND)
					{
						...
						if (dist < ground_bestdist ||
								...
								(dist < ground_bestdist + BFIXED_1 && length > ground_bestlength))
						{
					...
					else
					{
						...
						if (dist < water_bestdist ||
								//or the vertical distance is pretty much the same
								//but the overlapping part of the edges is longer
								(dist < water_bestdist + BFIXED_1 && length > water_bestlength))
						{
							water_bestdist = dist;
	...
	if (ground_foundreach)
	{
		...
		if (ground_bestdist >= BFIXED_0 && ground_bestdist < aassettings.phys_maxstep)
		{
			...
			FIXED_VEC3MA(ground_beststart, INSIDEUNITS_WALKSTART, ground_bestnormal, lreach->start);
			FIXED_VEC3MA(ground_bestend, INSIDEUNITS_WALKEND, ground_bestnormal, lreach->end);
	...
	if (water_foundreach)
	{
		...
		FIXED_VEC3MA(water_bestend, -INSIDEUNITS, water_bestnormal, testpoint);
		...
		if (aasworld.areasettings[AAS_PointAreaNum(testpoint)].areaflags & AREA_LIQUID)
		{
			...
			//the water surface
			if (water_bestdist < aassettings.phys_maxwaterjump + BFIXED(24,0))
			{
						...
						(aasworld.areasettings[area2num].presencetype & PRESENCE_NORMAL))
				{
					...
					FIXED_VEC3MA(water_bestend, INSIDEUNITS_WATERJUMP, water_bestnormal, lreach->end);
					lreach->traveltype = TRAVEL_WATERJUMP;
					lreach->traveltime = FIXED_TO_INT(aassettings.rs_waterjump);
					lreach->next = areareachability[area1num];
					areareachability[area1num] = lreach;
	...
	if (ground_foundreach)
	{
		...
		if (ground_bestdist > BFIXED_0 && ground_bestdist < aassettings.phys_maxbarrier)
		{
			//if no water in area1 or a very thin layer of water on the ground
			if (!water_foundreach || (ground_bestdist - water_bestdist < BFIXED(16,0)))
			{
				...
				if (!AAS_AreaCrouch(area1num) && !AAS_AreaCrouch(area2num))
				{
					...
					lreach->traveltime = FIXED_TO_INT(aassettings.rs_barrierjump);//AAS_BarrierJumpTravelTime();
					lreach->next = areareachability[area1num];
					areareachability[area1num] = lreach;
	...
	if (ground_foundreach)
	{
		if (ground_bestdist < BFIXED_0)
		{
			...
			if (FIXED_IS_ZERO(aassettings.rs_maxfallheight) || FIXED_ABS(ground_bestdist) < aassettings.rs_maxfallheight) {
				//trace a bounding box vertically to check for solids
				FIXED_VEC3MA(ground_bestend, INSIDEUNITS, ground_bestnormal, ground_bestend);
				...
				if (!trace.startsolid && trace.fraction >= GFIXED_1)
				{
					...
					if (AAS_PointAreaNum(trace.endpos) == area2num)
					{
						...
						if (i >= numareas)
						{
							...
							lreach->traveltime = FIXED_TO_INT( aassettings.rs_startwalkoffledge + FIXED_ABS(ground_bestdist) * BFIXED(50,0) / aassettings.phys_gravity );
							...
							if (!AAS_AreaSwim(area2num) && !AAS_AreaJumpPad(area2num))
							{
								...
								{
									lreach->traveltime += FIXED_TO_INT( aassettings.rs_falldamage5 );
								} //end if
								if (AAS_FallDelta(ground_bestdist) > aassettings.phys_falldelta10)
								{
									lreach->traveltime += FIXED_TO_INT( aassettings.rs_falldamage10 );
								} //end if
							} //end if
							lreach->next = areareachability[area1num];
							areareachability[area1num] = lreach;
...
// Changes Globals:		-
//===========================================================================
bfixed VectorDistance(bvec3_t v1, bvec3_t v2)
{
	bvec3_t dir;

	VectorSubtract(v2, v1, dir);
	return FIXED_VEC3LEN(dir);
...
// Changes Globals:		-
//===========================================================================
int VectorBetweenVectors(bvec3_t v, bvec3_t v1, bvec3_t v2)
{
	bvec3_t dir1, dir2;

	VectorSubtract(v, v1, dir1);
	VectorSubtract(v, v2, dir2);
	return (FIXED_VEC3DOT(dir1, dir2) <= BFIXED_0);
...
// Changes Globals:		-
//===========================================================================
void VectorMiddle(bvec3_t v1, bvec3_t v2, bvec3_t middle)
{
...
//===========================================================================
/*
bfixed AAS_ClosestEdgePoints(bvec3_t v1, bvec3_t v2, bvec3_t v3, bvec3_t v4,
							aas_plane_t *plane1, aas_plane_t *plane2,
							bvec3_t beststart, bvec3_t bestend, bfixed bestdist)
{
	bvec3_t dir1, dir2, p1, p2, p3, p4;
	bfixed a1, a2, b1, b2, dist;
	int founddist;
	...
	if (dir2[0])
	{
		...
		p1[0] = (FIXED_VEC3DOT(v1, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
		p1[1] = a2 * p1[0] + b2;
		//point on the edge vector of area2 closest to v2
		p2[0] = (FIXED_VEC3DOT(v2, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
		p2[1] = a2 * p2[0] + b2;
	} //end if
	else
	{
	...
	if (dir1[0])
	{
		...
		p3[0] = (FIXED_VEC3DOT(v3, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
		p3[1] = a1 * p3[0] + b1;
		//point on the edge vector of area1 closest to v4
		p4[0] = (FIXED_VEC3DOT(v4, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
		p4[1] = a1 * p4[0] + b1;
	} //end if
	else
	{
	...
	p1[2] = (plane2->dist - FIXED_VEC3DOT(plane2->normal, p1)) / plane2->normal[2];
	p2[2] = (plane2->dist - FIXED_VEC3DOT(plane2->normal, p2)) / plane2->normal[2];
	p3[2] = (plane1->dist - FIXED_VEC3DOT(plane1->normal, p3)) / plane1->normal[2];
	p4[2] = (plane1->dist - FIXED_VEC3DOT(plane1->normal, p4)) / plane1->normal[2];
	//
	founddist = qfalse;
	//
	if (VectorBetweenVectors(p1, v3, v4))
	{
		...
		if (dist > bestdist - BFIXED(0,5) && dist < bestdist + BFIXED(0,5))
		{
...
bfixed AAS_ClosestEdgePoints(bvec3_t v1, bvec3_t v2, bvec3_t v3, bvec3_t v4,
							aas_plane_t *plane1, aas_plane_t *plane2,
							bvec3_t beststart1, bvec3_t bestend1,
							bvec3_t beststart2, bvec3_t bestend2, bfixed bestdist)
{
	bvec3_t dir1, dir2, p1, p2, p3, p4;
	bfixed a1, a2, b1, b2, dist, dist1, dist2;
	int founddist;
	...
	p4[2] = BFIXED_0;
	//calculate the z-coordinates from the ground planes
	p1[2] = (plane2->dist - FIXED_VEC3DOT_R(plane2->normal, p1)) / MAKE_BFIXED(plane2->normal[2]);
	p2[2] = (plane2->dist - FIXED_VEC3DOT_R(plane2->normal, p2)) / MAKE_BFIXED(plane2->normal[2]);
	p3[2] = (plane1->dist - FIXED_VEC3DOT_R(plane1->normal, p3)) / MAKE_BFIXED(plane1->normal[2]);
	p4[2] = (plane1->dist - FIXED_VEC3DOT_R(plane1->normal, p4)) / MAKE_BFIXED(plane1->normal[2]);
	//
	founddist = qfalse;
	//
	if (VectorBetweenVectors(p1, v3, v4))
...
int AAS_Reachability_Jump(int area1num, int area2num)
{
	...
	bfixed phys_jumpvel, maxjumpdistance, maxjumpheight, height, bestdist, speed;
	bvec_t *v1, *v2, *v3, *v4;
	bvec3_t beststart, beststart2, bestend, bestend2;
	bvec3_t teststart, testend, tmp, velocity, cmdmove;
	avec3_t dir;
	bvec3_t bdir;
	avec3_t up = {AFIXED_0, AFIXED_0, AFIXED_1}, sidewards;
	aas_area_t *area1, *area2;
	aas_face_t *face1, *face2;
	aas_edge_t *edge1, *edge2;
	...
	maxjumpdistance = BFIXED(2,0) * AAS_MaxJumpDistance(phys_jumpvel);
	//maximum height a player can jump with the given initial z velocity
	...
	if (bestdist > BFIXED(4,0) && bestdist < maxjumpdistance)
	{
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, FIXED_TO_DOUBLE(bestdist));
		// if very close and almost no height difference then the bot can walk
		if (bestdist <= BFIXED(48,0) && FIXED_ABS(beststart[2] - bestend[2]) < BFIXED(8,0))
		{
			speed = BFIXED(400,0);
			traveltype = TRAVEL_WALKOFFLEDGE;
		} //end if
		else if (AAS_HorizontalVelocityForJump(BFIXED_0, beststart, bestend, &speed))
		{
			//FIXME: why multiply with BFIXED(1,2)???
			speed *= BFIXED(1,2);
			traveltype = TRAVEL_WALKOFFLEDGE;
		} //end else if
		else
		{
			...
			VectorSubtract(bestend, beststart, tmp);
			tmp[2] = BFIXED_0;
			if (FIXED_VEC3LEN(tmp) < BFIXED(10,0))
				return qfalse;
		} //end if
		//
		VectorSubtract(bestend, beststart, tmp);
		VectorNormalizeB2A(tmp,dir);
		FIXED_VEC3MA_R(beststart, BFIXED_1, dir, teststart);
		//
		VectorCopy(teststart, testend);
		testend[2] -= BFIXED(100,0);
		...
		if (trace.fraction < GFIXED_1)
		{
			...
			if (FIXED_VEC3DOT_R(plane->normal, up) >= AFIXED(0,7))
			{
		...
		//
		FIXED_VEC3MA_R(bestend, -BFIXED_1, dir, teststart);
		//
		VectorCopy(teststart, testend);
		testend[2] -= BFIXED(100,0);
		trace = AAS_TraceClientBBox(teststart, testend, PRESENCE_NORMAL, -1);
		...
		//
		stopevent = PSE_HITGROUND|PSE_ENTERWATER|PSE_ENTERSLIME|PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE;
		if (!AAS_AreaClusterPortal(area1num) && !AAS_AreaClusterPortal(area2num))
			stopevent |= PSE_TOUCHCLUSTERPORTAL;
		//
		for (i = 0; i < 3; i++)
		{
			//
			if (i == 1)
			{
				testend[0]+=MAKE_BFIXED(sidewards[0]);
				testend[1]+=MAKE_BFIXED(sidewards[1]);
				testend[2]+=MAKE_BFIXED(sidewards[2]);
				//VectorAdd(testend, sidewards, testend);
			}
			else if (i == 2)
			{
				testend[0]=bestend[0]-MAKE_BFIXED(sidewards[0]);
				testend[1]=bestend[1]-MAKE_BFIXED(sidewards[1]);
				testend[2]=bestend[2]-MAKE_BFIXED(sidewards[2]);
			...
			VectorSubtract(testend, beststart, tmp);
			tmp[2] = BFIXED_0;
			VectorNormalizeB2A(tmp,dir);
			...
			if (move.stopevent & (PSE_ENTERSLIME|PSE_ENTERLAVA))
				return qfalse;
			// never jump or fall through a cluster portal
			if (move.stopevent & PSE_TOUCHCLUSTERPORTAL)
				return qfalse;
			...
			FIXED_VEC3MA_R(move.endpos, -BFIXED(64,0), dir, teststart);
			teststart[2] += BFIXED_1;
		...
		VectorSubtract(bestend, beststart, bdir);
		height = bdir[2];
		bdir[2] = BFIXED_0;
		if ((traveltype & TRAVELTYPE_MASK) == TRAVEL_WALKOFFLEDGE && height > FIXED_VEC3LEN(bdir))
		{
			lreach->traveltime = FIXED_TO_INT(aassettings.rs_startwalkoffledge + height * BFIXED(50,0) / aassettings.phys_gravity);
		}
		else
		{
			lreach->traveltime = FIXED_TO_INT(aassettings.rs_startjump + VectorDistance(bestend, beststart) * BFIXED(240,0) / aassettings.phys_maxwalkvelocity);
		} //end if
		//
...
int AAS_Reachability_Ladder(int area1num, int area2num)
{
	...
	bfixed face1area, face2area, bestface1area, bestface2area;
	bfixed phys_jumpvel, maxjumpheight;
	bvec3_t area1point, area2point, v1, v2,tmp;
	avec3_t up = {AFIXED_0, AFIXED_0, AFIXED_1};
	bvec3_t mid, lowestpoint, start, end, sharededgevec;
	avec3_t dir;
	aas_area_t *area1, *area2;
	...
	bestface1area = -BFIXED(9999,0);
	bestface2area = -BFIXED(9999,0);
	...
	if (ladderface1 && ladderface2)
	{
		...
		FIXED_VEC3SCALE(area1point, BFIXED(0,5), area1point);
		...
		CrossProduct(plane1->normal, sharededgevec, tmp);
		VectorNormalizeB2A(tmp,dir);
		//NOTE: 32 because that's larger than 16 (bot bbox x,y)
		FIXED_VEC3MA_R(area1point, -BFIXED(32,0), dir, area1point);
		FIXED_VEC3MA_R(area2point, BFIXED(32,0), dir, area2point);
		//
		ladderface1vertical = MAKE_BFIXED(abs(FIXED_TO_INT(FIXED_VEC3DOT(plane1->normal, up)))) < BFIXED(0,1);
		ladderface2vertical = MAKE_BFIXED(abs(FIXED_TO_INT(FIXED_VEC3DOT(plane2->normal, up)))) < BFIXED(0,1);
		...
		if (ladderface1vertical && ladderface2vertical
					...
					&& FIXED_VEC3DOT(plane1->normal, plane2->normal) > AFIXED(0,7)
					//and the shared edge is not too vertical
					&& MAKE_BFIXED(abs(FIXED_TO_INT(FIXED_VEC3DOT(sharededgevec, up)))) < BFIXED(0,7))
		{
			//create a new reachability link
			lreach = AAS_AllocReachability();
			if (!lreach) return qfalse;
			...
			FIXED_VEC3MA_R(area2point, -BFIXED(3,0), plane1->normal, lreach->end);
			...
			FIXED_VEC3MA_R(area1point, -BFIXED(3,0), plane1->normal, lreach->end);
			lreach->traveltype = TRAVEL_LADDER;
			lreach->traveltime = 10;
		...
		if (ladderface1vertical && (ladderface2->faceflags & FACE_GROUND))
		{
			...
			VectorCopy(area1point, lreach->start);
			VectorCopy(area2point, lreach->end);
			lreach->end[2] += BFIXED(16,0);
			FIXED_VEC3MA_R(lreach->end, -BFIXED(15,0), plane1->normal, lreach->end);
			lreach->traveltype = TRAVEL_LADDER;
			lreach->traveltime = 10;
			lreach->next = areareachability[area1num];
			areareachability[area1num] = lreach;
		...
		if (ladderface1vertical)
		{
			...
			FIXED_VEC3MA_R(lowestpoint, BFIXED(5,0), plane1->normal, start);
			VectorCopy(start, end);
			start[2] += BFIXED(5,0);
			end[2] -= BFIXED(100,0);
			...
			for (i = 0; i < area2->numfaces; i++)
			{
				...
				{
					plane2 = &aasworld.planes[face2->planenum];
					if (MAKE_BFIXED(abs(FIXED_TO_INT(FIXED_VEC3DOT(plane2->normal, up)))) < BFIXED(0,1)) break;
				} //end if
			} //end for
						...
						!AAS_ReachabilityExists(area2num, area1num))
			{
				...
				if (start[2] - trace.endpos[2] < maxjumpheight)
				{
					...
					FIXED_VEC3MA_R(lowestpoint, -BFIXED(5,0), plane1->normal, lreach->end);
					//get the end point a little higher
					lreach->end[2] += BFIXED(10,0);
													...
													| AREACONTENTS_LAVA))
			{
				...
				{
					//trace down in the middle of this edge
					FIXED_VEC3MA(lowestpoint, MAKE_BFIXED(i), plane1->normal, start);
					VectorCopy(start, end);
					start[2] += 5;
					end[2] -= 100;
					//trace without entity collision
...
void AAS_Reachability_Teleport(void)
{
	...
	int ent, dest;
	afixed angle;
	bvec3_t origin, destorigin, mins, maxs, end;
	avec3_t angles;
	bvec3_t mid, velocity, cmdmove;
	aas_lreachability_t *lreach;
	aas_clientmove_t move;
	aas_trace_t trace;
	aas_link_t *areas, *link;
...
//#endif REACH_DEBUG
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num))
		{
			...
			else*/
			{
				...
				if (FIXED_NOT_ZERO(angle))
				{
					VectorSet(angles, AFIXED_0, angle, AFIXED_0);
					avec3_t tmp;
					AngleVectors(angles, tmp, NULL, NULL);
					velocity[0]=BFIXED(400,0)*tmp[0];
					velocity[1]=BFIXED(400,0)*tmp[1];
					velocity[2]=BFIXED(400,0)*tmp[2];
				...
				AAS_PredictClientMovement(&move, -1, destorigin, PRESENCE_NORMAL, qfalse,
										velocity, cmdmove, 0, 30, GFIXED(0,1),
										PSE_HITGROUND|PSE_ENTERWATER|PSE_ENTERSLIME|
										PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE|PSE_TOUCHJUMPPAD|PSE_TOUCHTELEPORTER, 0, qfalse); //qtrue);
		...
		//botimport.Print(PRT_MESSAGE, "teleporter brush mins = %f %f %f\n", FIXED_TO_DOUBLE(mins[0]), FIXED_TO_DOUBLE(mins[1]), FIXED_TO_DOUBLE(mins[2]));
		//botimport.Print(PRT_MESSAGE, "teleporter brush maxs = %f %f %f\n", FIXED_TO_DOUBLE(maxs[0]), FIXED_TO_DOUBLE(maxs[1]), FIXED_TO_DOUBLE(maxs[2]));
		...
		for (link = areas; link; link = link->next_area)
		{
			...
			lreach->traveltime = FIXED_TO_INT(aassettings.rs_teleport);
			lreach->next = areareachability[area1num];
			areareachability[area1num] = lreach;
			//
			reach_teleport++;
...
void AAS_Reachability_Elevator(void)
{
	...
	bfixed lip, height, speed;
	char model[MAX_EPAIRKEY], classname[MAX_EPAIRKEY];
	int ent;
	bvec3_t mins, maxs, origin;
	avec3_t angles = {AFIXED_0, AFIXED_0, AFIXED_0};
	bvec3_t pos1, pos2, mids, platbottom, plattop;
	bvec3_t bottomorg, toporg, start, end, dir;
	bvec_t xvals[8], yvals[8], xvals_top[8], yvals_top[8];
	aas_lreachability_t *lreach;
	aas_trace_t trace;
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!strcmp(classname, "func_plat"))
		{
			...
			if (FIXED_IS_ZERO(lip)) lip = BFIXED(8,0);
			//get the movement height of the plat
			AAS_FloatForBSPEpairKey(ent, "height", &height);
			if (FIXED_IS_ZERO(height)) height = (maxs[2] - mins[2]) - lip;
			//get the speed of the plat
			AAS_FloatForBSPEpairKey(ent, "speed", &speed);
			if (FIXED_IS_ZERO(speed)) speed = BFIXED(200,0);
			//get bottom position below pos1
			pos2[2] -= height;
			//
			//get a point just above the plat in the bottom position
			VectorAdd(mins, maxs, mids);
			FIXED_VEC3MA(pos2, BFIXED(0,5), mids, platbottom);
			platbottom[2] = maxs[2] - (pos1[2] - pos2[2]) + BFIXED(2,0);
			//get a point just above the plat in the top position
			VectorAdd(mins, maxs, mids);
			FIXED_VEC3MA(pos2, BFIXED(0,5), mids, plattop);
			plattop[2] = maxs[2] + BFIXED(2,0);
			//
			/*if (!area1num)
			{
			...
			//
			VectorAdd(mins, maxs, mids);
			FIXED_VEC3SCALE(mids, BFIXED(0,5), mids);
			//
			...
			for (i = 0; i < 9; i++)
			{
				if (i < 8) //check at the sides of the plat
				{
					...
					bottomorg[2] = platbottom[2] + BFIXED(16,0);
				...
				for (n = 0; n < 3; n++)
				{
					...
					for (j = 0; j < 8; j++)
					{
						...
						toporg[2] = plattop[2] + BFIXED(16,0);
						...
						for (l = 0; l < 16; l++)
						{
							if (area2num)
							{
								if (AAS_AreaGrounded(area2num) || AAS_AreaSwim(area2num))
								{
									...
									trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1);
									if (trace.fraction >= GFIXED_1) break;
								} //end if
							} //end if
							toporg[2] += BFIXED(4,0);
							area2num = AAS_PointAreaNum(toporg);
						...
						VectorNormalize(dir);
						dir[0] = bottomorg[0] + BFIXED(24,0) * dir[0];
						dir[1] = bottomorg[1] + BFIXED(24,0) * dir[1];
						dir[2] = bottomorg[2];
						//
						for (p = 0; p < 3; p++)
						...
						lreach->edgenum = FIXED_TO_INT(height);
						//
						VectorCopy(dir, lreach->start);
						VectorCopy(toporg, lreach->end);
						lreach->traveltype = TRAVEL_ELEVATOR;
						lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
						lreach->traveltime = FIXED_TO_INT(aassettings.rs_startelevator + height * BFIXED(100,0) / speed);
						lreach->next = areareachability[area1num];
						areareachability[area1num] = lreach;
...
// Changes Globals:		-
//===========================================================================
aas_lreachability_t *AAS_FindFaceReachabilities(bvec3_t *facepoints, int numpoints, aas_plane_t *plane, int towardsface)
{
	int i, j, k, l;
	int facenum, edgenum, bestfacenum;
	bfixed *v1, *v2, *v3, *v4;
	bfixed bestdist, speed, hordist, dist;
	bvec3_t beststart, beststart2, bestend, bestend2, tmp, hordir, testpoint;
	...
	for (i = 1; i < aasworld.numareas; i++)
	{
		...
		//
		if (bestdist > BFIXED(192,0)) continue;
		//
		VectorMiddle(beststart, beststart2, beststart);
		VectorMiddle(bestend, bestend2, bestend);
		//
		...
		hordir[2] = BFIXED_0;
		hordist = FIXED_VEC3LEN(hordir);
		//
		if (hordist > BFIXED(2,0) * AAS_MaxJumpDistance(aassettings.phys_jumpvel)) continue;
		//the end point should not be significantly higher than the start point
		if (bestend[2] - BFIXED(32,0) > beststart[2]) continue;
		//don't fall down too far
		if (bestend[2] < beststart[2] - BFIXED(128,0)) continue;
		//the distance should not be too far
		if (hordist > BFIXED(32,0))
		{
			//check for walk off ledge
			if (!AAS_HorizontalVelocityForJump(BFIXED_0, beststart, bestend, &speed)) continue;
		} //end if
		//
		beststart[2] += BFIXED_1;
		bestend[2] += BFIXED_1;
		//
		if (towardsface) VectorCopy(bestend, testpoint);
		else VectorCopy(beststart, testpoint);
		testpoint[2] = BFIXED_0;
		testpoint[2] = (bestfaceplane->dist - FIXED_VEC3DOT_R(bestfaceplane->normal, testpoint)) / MAKE_BFIXED(bestfaceplane->normal[2]);
		//
		if (!AAS_PointInsideFace(bestfacenum, testpoint, BFIXED(0,1)))
		{
			//if the faces are not overlapping then only go down
			if (bestend[2] - BFIXED(16,0) > beststart[2]) continue;
		} //end if
		lreach = AAS_AllocReachability();
...
void AAS_Reachability_FuncBobbing(void)
{
	...
	char classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
	bvec3_t origin, move_end, move_start, move_start_top, move_end_top;
	bvec3_t mins, maxs;
	avec3_t angles = {AFIXED_0, AFIXED_0, AFIXED_0};
	bvec3_t start_edgeverts[4], end_edgeverts[4], mid;
	bvec3_t org, start, end, dir, points[10];
	bfixed height;
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		AAS_FloatForBSPEpairKey(ent, "height", &height);
		if (FIXED_IS_ZERO(height)) height = BFIXED(32,0);
		//
		if (!AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY))
		{
		...
		if (!AAS_VectorForBSPEpairKey(ent, "origin", origin))
			VectorSet(origin, BFIXED_0, BFIXED_0, BFIXED_0);
		//
		...
		*/
		...
		for (i = 0; i < 4; i++)
		{
			...
			start_edgeverts[i][2] += BFIXED(24,0);	//+ player origin to ground dist
		} //end for
		...
		VectorSet(start_plane.normal, AFIXED_0, AFIXED_0, AFIXED_1);
		//
		for (i = 0; i < 4; i++)
		{
			VectorCopy(move_end, end_edgeverts[i]);
			end_edgeverts[i][2] += maxs[2] - mid[2]; //+ bbox maxs z
			end_edgeverts[i][2] += BFIXED(24,0);	//+ player origin to ground dist
		} //end for
		...
		VectorSet(end_plane.normal, AFIXED_0, AFIXED_0, AFIXED_1);
		//
#ifndef BSPC
#if 0
		for (i = 0; i < 4; i++)
		...
		move_start_top[2] += maxs[2] - mid[2] + BFIXED(24,0); //+ bbox maxs z
		VectorCopy(move_end, move_end_top);
		move_end_top[2] += maxs[2] - mid[2] + BFIXED(24,0); //+ bbox maxs z
		//
		...
		for (i = 0; i < 2; i++)
		{
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach)
			{
				...
				//if (trace.fraction < BFIXED_1) continue;
				//
				for (endreach = firstendreach; endreach; endreach = nextendreach)
				{
					...
					FIXED_VEC3MA(startreach->start, BFIXED_1, dir, start);
					start[2] += BFIXED_1;
					FIXED_VEC3MA(startreach->start, BFIXED(16,0), dir, end);
					end[2] += BFIXED_1;
					//
					...
					if (i == 0) lreach->edgenum = (FIXED_TO_INT(move_start[axis]) << 16) | (FIXED_TO_INT(move_end[axis]) & 0x0000ffff);
					else lreach->edgenum = (FIXED_TO_INT(move_end[axis]) << 16) | (FIXED_TO_INT(move_start[axis]) & 0x0000ffff);
...
//					AAS_PermanentLine(lreach->start, lreach->end, 1);
					...
					lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
					lreach->traveltime = FIXED_TO_INT(aassettings.rs_funcbob);
					reach_funcbob++;
...
void AAS_Reachability_JumpPad(void)
{
	...
	//int modelnum, ent2;
	//bfixed dist, time, height, gravity, forward;
	bfixed speed, zvel, hordist;
	aas_face_t *face2;
	aas_area_t *area2;
	aas_lreachability_t *lreach;
	bvec3_t areastart, facecenter, dir, cmdmove;
	bvec3_t velocity, absmins, absmaxs;
	//bvec3_t origin, ent2origin, angles, teststart;
	aas_clientmove_t move;
	//aas_trace_t trace;
	aas_link_t *areas, *link;
...
//		FIXED_VEC3SCALE(velocity, speed, velocity);
		VectorClear(angles);
		//get the mins, maxs and origin of the model
		AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY);
		if (model[0]) modelnum = atoi(model+1);
		...
		//
#ifdef REACH_DEBUG
		botimport.Print(PRT_MESSAGE, "absmins = %f %f %f\n", FIXED_TO_DOUBLE(absmins[0]), FIXED_TO_DOUBLE(absmins[1]), FIXED_TO_DOUBLE(absmins[2]));
		botimport.Print(PRT_MESSAGE, "absmaxs = %f %f %f\n", FIXED_TO_DOUBLE(absmaxs[0]), FIXED_TO_DOUBLE(absmaxs[1]), FIXED_TO_DOUBLE(absmaxs[2]));
#endif REACH_DEBUG
		VectorAdd(absmins, absmaxs, origin);
		FIXED_VEC3SCALE (origin, BFIXED(0,5), origin);
...
//		botimport.Print(PRT_MESSAGE, "found a trigger_push with velocity %f %f %f\n", FIXED_TO_DOUBLE(velocity[0]), FIXED_TO_DOUBLE(velocity[1]), FIXED_TO_DOUBLE(velocity[2]));
		...
		if (FIXED_NOT_ZERO(velocity[0]) || FIXED_NOT_ZERO(velocity[1]))
		{
			VectorSet(cmdmove, BFIXED_0, BFIXED_0, BFIXED_0);
			...
			for (i = 0; i < 20; i++)
			{
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
										velocity, cmdmove, 0, 30, GFIXED(0,1),
										PSE_HITGROUND|PSE_ENTERWATER|PSE_ENTERSLIME|
										PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE|PSE_TOUCHJUMPPAD|PSE_TOUCHTELEPORTER, 0, bot_visualizejumppads);
			...
			if (area2num && i < 20)
			{
				for (link = areas; link; link = link->next_area)
				{
					...
					lreach->facenum = FIXED_TO_INT(velocity[2]);
					//NOTE: the edgenum is the horizontal velocity
					lreach->edgenum = FIXED_TO_INT(FIXED_VEC2LEN(velocity));
					VectorCopy(areastart, lreach->start);
					VectorCopy(move.endpos, lreach->end);
					lreach->traveltype = TRAVEL_JUMPPAD;
					lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
					lreach->traveltime = FIXED_TO_INT(aassettings.rs_jumppad);
		...
		//
		if (FIXED_ABS(velocity[0]) > BFIXED(100,0) || FIXED_ABS(velocity[1]) > BFIXED(100,0)) continue;
		...
		for (area2num = 1; area2num < aasworld.numareas; area2num++)
		{
			...
			for (i = 0; i < area2->numfaces; i++)
			{
				...
				if (ret && speed < BFIXED(150,0))
				{
					//direction towards the face center
					VectorSubtract(facecenter, areastart, dir);
					dir[2] = BFIXED_0;
					hordist = VectorNormalize(dir);
					//if (hordist < BFIXED(1,6) * facecenter[2] - areastart[2])
					{
						//get command movement
						FIXED_VEC3SCALE(dir, speed, cmdmove);
						//
						AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
													velocity, cmdmove, 30, 30,GFIXED(0,1),
													PSE_ENTERWATER|PSE_ENTERSLIME|
													PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE|
													PSE_TOUCHJUMPPAD|PSE_TOUCHTELEPORTER|PSE_HITGROUNDAREA, area2num, visualize);
						//if prediction time wasn't enough to fully predict the movement
						//don't enter slime or lava and don't fall from too high
						if (move.frames < 30 && 
								!(move.stopevent & (PSE_ENTERSLIME|PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE))
								&& (move.stopevent & (PSE_HITGROUNDAREA|PSE_TOUCHJUMPPAD|PSE_TOUCHTELEPORTER)))
						{
							...
							if (!link)
							{
								for (link = areas; link; link = link->next_area)
								{
									...
									lreach->facenum = FIXED_TO_INT(velocity[2]);
									//NOTE: the edgenum is the horizontal velocity
									lreach->edgenum = FIXED_TO_INT(FIXED_VEC2LEN(cmdmove));
									VectorCopy(areastart, lreach->start);
									VectorCopy(facecenter, lreach->end);
									lreach->traveltype = TRAVEL_JUMPPAD;
									lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
									lreach->traveltime = FIXED_TO_INT(aassettings.rs_aircontrolledjumppad);
									lreach->next = areareachability[link->areanum];
									areareachability[link->areanum] = lreach;
									//
									reach_jumppad++;
...
int AAS_Reachability_Grapple(int area1num, int area2num)
{
	...
	bfixed mingrappleangle, z, hordist;
	bsp_trace_t bsptrace;
	aas_trace_t trace;
	aas_face_t *face2;
	aas_area_t *area1, *area2;
	aas_lreachability_t *lreach;
	bvec3_t areastart, facecenter, start, end, dir, down = {BFIXED_0, BFIXED_0, -BFIXED_1};
	bvec_t *v;
	...
	if (!AAS_AreaSwim(area1num))
	{
		if (!AAS_PointAreaNum(start)) Log_Write("area %d center %f %f %f in solid?\r\n", area1num,
								FIXED_TO_DOUBLE(start[0]), FIXED_TO_DOUBLE(start[1]), FIXED_TO_DOUBLE(start[2]));
		VectorCopy(start, end);
		end[2] -= BFIXED(1000,0);
		trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1);
	...
	for (i = 0; i < area2->numfaces; i++)
	{
		...
		if (FIXED_VEC3DOT_R(aasworld.planes[face2->planenum].normal, dir) > BFIXED_0) continue;
		//get the center of the face
		AAS_FaceCenter(face2num, facecenter);
		//only go higher up with the grapple
		if (facecenter[2] < areastart[2] + BFIXED(64,0)) continue;
		//only use vertical faces or downward facing faces
		if (FIXED_VEC3DOT_R(aasworld.planes[face2->planenum].normal, down) < BFIXED_0) continue;
		//direction towards the face center
		VectorSubtract(facecenter, areastart, dir);
		//
		z = dir[2];
		dir[2] = BFIXED_0;
		hordist = FIXED_VEC3LEN(dir);
		if (FIXED_IS_ZERO(hordist)) continue;
		//if too far
		if (hordist > BFIXED(2000,0)) continue;
		//check the minimal angle of the movement
		mingrappleangle = BFIXED(15,0); //15 degrees
		if (z / hordist < FIXED_TAN(BFIXED(2,0) * BFIXED_PI * mingrappleangle / BFIXED(360,0))) continue;
		//
		VectorCopy(facecenter, start);
		FIXED_VEC3MA_R(facecenter, -BFIXED(500,0), aasworld.planes[face2->planenum].normal, end);
		//
		...
		if ((bsptrace.surface.flags & SURF_SKY) || (bsptrace.fraction * GFIXED(500,0) > GFIXED(32,0))) continue;
		//trace a full bounding box from the area center on the ground to
		//the center of the face
		VectorSubtract(facecenter, areastart, dir);
		VectorNormalize(dir);
		FIXED_VEC3MA(areastart, BFIXED(4,0), dir, start);
		VectorCopy(bsptrace.endpos, end);
		trace = AAS_TraceClientBBox(start, end, PRESENCE_NORMAL, -1);
		VectorSubtract(trace.endpos, facecenter, dir);
		if (FIXED_VEC3LEN(dir) > BFIXED(24,0)) continue;
		//
		VectorCopy(trace.endpos, start);
		VectorCopy(trace.endpos, end);
		end[2] -= MAKE_BFIXED(AAS_FallDamageDistance());
		trace = AAS_TraceClientBBox(start, end, PRESENCE_NORMAL, -1);
		if (trace.fraction >= GFIXED_1) continue;
		//area to end in
		areanum = AAS_PointAreaNum(trace.endpos);
		...
		lreach->traveltime = FIXED_TO_INT(aassettings.rs_startgrapple + FIXED_VEC3LEN(dir) * BFIXED(0,25));
		lreach->next = areareachability[area1num];
		areareachability[area1num] = lreach;
		//
		reach_grapple++;
...
{
	int ent, i;
	bvec3_t mins = {-BFIXED(15,0), -BFIXED(15,0), -BFIXED(15,0)}, maxs = {BFIXED(15,0), BFIXED(15,0), BFIXED(15,0)};
	bvec3_t origin;
...
int AAS_Reachability_WeaponJump(int area1num, int area2num)
{
	...
	aas_area_t *area1, *area2;
	aas_lreachability_t *lreach;
	bvec3_t areastart, facecenter, start, end, dir, cmdmove;// teststart;
	bvec3_t velocity;
	aas_clientmove_t move;
	aas_trace_t trace;
		...
		for (n = 0; n < 1/*2*/; n++)
		{
			...
			if (ret && speed < BFIXED(300,0))
			{
				...
				//if (hordist < BFIXED(1,6) * (facecenter[2] - areastart[2]))
				{
					...
					/*
					//get command movement
					FIXED_VEC3SCALE(dir, speed, velocity);
					velocity[2] = zvel;
					VectorSet(cmdmove, BFIXED_0, BFIXED_0, BFIXED_0);
					*/
					//
					AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue,
												...
												PSE_ENTERWATER|PSE_ENTERSLIME|
												PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE|
												PSE_TOUCHJUMPPAD|PSE_HITGROUND|PSE_HITGROUNDAREA, area2num, visualize);
					//if prediction time wasn't enough to fully predict the movement
					//don't enter slime or lava and don't fall from too high
					if (move.frames < 30 && 
							!(move.stopevent & (PSE_ENTERSLIME|PSE_ENTERLAVA|PSE_HITGROUNDDAMAGE))
								&& (move.stopevent & (PSE_HITGROUNDAREA|PSE_TOUCHJUMPPAD)))
					{
						//create a rocket or bfg jump reachability from area1 to area2
						lreach = AAS_AllocReachability();
						if (!lreach) return qfalse;
						...
						if (n)
						{
							...
							lreach->traveltime = FIXED_TO_INT(aassettings.rs_bfgjump);
						} //end if
						else
						{
							lreach->traveltype = TRAVEL_ROCKETJUMP;
							lreach->traveltime = FIXED_TO_INT(aassettings.rs_rocketjump);
						} //end else
						lreach->next = areareachability[area1num];
						areareachability[area1num] = lreach;
						//
...
void AAS_Reachability_WalkOffLedge(int areanum)
{
	...
	aas_plane_t *plane;
	bvec_t *v1, *v2;
	bvec3_t sharededgevec, mid, dir, testend;
	aas_lreachability_t *lreach;
	aas_trace_t trace;
			...
			for (j = 0; j < area->numfaces; j++)
			{
				...
				for (l = 0; l < face2->numedges; l++)
				{
					...
					if (abs(edge1num) == abs(edge2num))
					{
						...
						VectorAdd(v1, v2, mid);
						FIXED_VEC3SCALE(mid, BFIXED(0,5), mid);
						FIXED_VEC3MA(mid, BFIXED(8,0), dir, mid);
						//
						VectorCopy(mid, testend);
						testend[2] -= BFIXED(1000,0);
						...
						if (FIXED_NOT_ZERO(aassettings.rs_maxfallheight) && FIXED_ABS(mid[2] - trace.endpos[2]) > aassettings.rs_maxfallheight)
							break;
						//
						lreach = AAS_AllocReachability();
						if (!lreach) break;
						...
						VectorCopy(trace.endpos, lreach->end);
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime = FIXED_TO_INT(aassettings.rs_startwalkoffledge + FIXED_ABS(mid[2] - trace.endpos[2]) * BFIXED(50,0) / aassettings.phys_gravity);
...
// Changes Globals:		-
//===========================================================================
int AAS_ContinueInitReachability(gfixed time)
{
	int i, j, todo, start_time;
	static gfixed framereachability, reachability_delay;
	...
	if (aasworld.numreachabilityareas == 1)
	{
		...
		lastpercentage = 0;
		framereachability = GFIXED(2000,0);
		reachability_delay = GFIXED(1000,0);
	} //end if
	//number of areas to calculate reachability for this cycle
	todo = aasworld.numreachabilityareas + FIXED_TO_INT(framereachability);
	...
	for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++)
	{
		...
		if (Sys_MilliSeconds() - start_time > FIXED_TO_INT(reachability_delay)) break;
		//
	...
	{
		botimport.Print(PRT_MESSAGE, "\r%6.1f%%", 100.0);
	...
	else
	{
		...
		botimport.Print(PRT_MESSAGE, "\r%6.1f%%", (double) lastpercentage / 10);
	} //end else
	//not yet finished
	return qtrue;
...
void AAS_InitReachability(void)
{
	...
	{
#ifndef BSPC
		if(FIXED_IS_ZERO(LibVarGetValue("forcereachability")))
		{
...
#ifndef BSPC
	calcgrapplereach = FIXED_TO_INT(LibVarGetValue("grapplereach"));
#endif
