...
int PS_ReadString(script_t *script, token_t *token, int quote)
{
	...
	while(1)
	{
		//minus 2 because trailing lfixed quote and zero have to be appended
		if (len >= MAX_TOKEN - 2)
		{
		...
		else if (*script->script_p == quote)
		{
			...
			//if there's no leading lfixed qoute
			...
			//step over the new leading lfixed quote
			script->script_p++;
		} //end if
		else
		{
...
// Changes Globals:		-
//============================================================================
void NumberValue(char *string, int subtype, unsigned long int *intvalue, lfixed *floatvalue)
{
	...
	*floatvalue = LFIXED_0;
	...
	if (subtype & TT_FLOAT)
	{
		while(*string)
		{
			...
			if (dotfound)
			{
				*floatvalue = *floatvalue + MAKE_LFIXED(*string - '0') / MAKE_LFIXED(dotfound);
				dotfound *= 10;
			} //end if
			else
			{
				*floatvalue = *floatvalue * LFIXED(10,0) + MAKE_LFIXED(*string - '0');
			} //end else
			string++;
		} //end while
		*intvalue = FIXED_TO_INT(*floatvalue);
	} //end if
	else if (subtype & TT_DECIMAL)
	{
		while(*string) *intvalue = *intvalue * 10 + (*string++ - '0');
		*floatvalue = MAKE_LFIXED(*intvalue);
	} //end else if
...
int PS_ReadToken(script_t *script, token_t *token)
{
	...
	//if there is a leading lfixed quote
	if (*script->script_p == '\"')
	{
...
// Changes Globals:		-
//============================================================================
lfixed ReadSignedFloat(script_t *script)
{
	token_t token;
	lfixed sign = LFIXED_1;
	...
	{
		ScriptError(script, "expected gfixed value, found %s\n", token.string);
	} //end else if
