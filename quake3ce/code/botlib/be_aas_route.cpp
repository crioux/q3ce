...
#define DISTANCEFACTOR_CROUCH		BFIXED(1,3)		//crouch speed = 100
#define DISTANCEFACTOR_SWIM			BFIXED_1		//should be BFIXED(0,66), swim speed = 150
#define DISTANCEFACTOR_WALK			BFIXED(0,33)	//walk speed = 300

//cache refresh time
#define CACHE_REFRESHTIME		GFIXED(15,0)	//15 seconds refresh time
...
// Changes Globals:		-
//===========================================================================
__inline gfixed AAS_RoutingTime(void)
{
	return AAS_Time();
...
// Changes Globals:		-
//===========================================================================
unsigned short int AAS_AreaTravelTime(int areanum, bvec3_t start, bvec3_t end)
{
	int intdist;
	bfixed dist;
	bvec3_t dir;
...
void AAS_CalculateAreaTravelTimes(void)
{
	...
	for (i = 0; i < aasworld.numareas; i++)
	{
		...
		settings = &aasworld.areasettings[i];
		//
		off=(settings->numreachableareas * sizeof(unsigned short *));
		off+=(settings->numreachableareas * revreach->numlinks * sizeof(unsigned short));
		off+=((4-(off&3))&3);
		size += off;
	} //end for
...
void AAS_InitRouting(void)
{
	...
	//
	routingcachesize = 0;
	max_routingcachesize = 1024 * FIXED_TO_INT(LibVarValue("max_routingcache", "4096"));
...
//	Com_Memset(aasworld.areaupdate, 0, aasworld.numareas * sizeof(aas_routingupdate_t));
	...
	curupdate->tmptraveltime = FIXED_TO_INT(areacache->starttraveltime);
	//
	areacache->traveltimes[clusterareanum] = FIXED_TO_INT(areacache->starttraveltime);
...
aas_routingcache_t *AAS_GetAreaRoutingCache(int clusternum, int areanum, int travelflags)
{
	...
	if (!cache)
	{
		...
		cache->starttraveltime = GFIXED_1;
...
//	Com_Memset(aasworld.portalupdate, 0, (aasworld.numportals+1) * sizeof(aas_routingupdate_t));
	...
	curupdate->tmptraveltime = FIXED_TO_INT(portalcache->starttraveltime);
	...
	if (clusternum < 0)
	{
		portalcache->traveltimes[-clusternum] = FIXED_TO_INT(portalcache->starttraveltime);
	} //end if
	//put the area to start with in the current read list
	curupdate->next = NULL;
	curupdate->prev = NULL;
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaRouteToGoalArea(int areanum, bvec3_t origin, int goalareanum, int travelflags, int *traveltime, int *reachnum)
{
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaTravelTimeToGoalArea(int areanum, bvec3_t origin, int goalareanum, int travelflags)
{
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaReachabilityToGoalArea(int areanum, bvec3_t origin, int goalareanum, int travelflags)
{
	int traveltime, reachnum;
...
// Changes Globals:		-
//===========================================================================
int AAS_PredictRoute(struct aas_predictroute_s *route, int areanum, bvec3_t origin,
...
// Changes Globals:		-
//===========================================================================
int AAS_RandomGoalArea(int areanum, int travelflags, int *goalareanum, bvec3_t goalorigin)
{
	int i, n, t;
	bvec3_t start, end;
	aas_trace_t trace;
		...
		if (AAS_AreaReachability(n))
		{
			...
			if (t > 0)
			{
				...
				if (!AAS_PointAreaNum(start))
					Log_Write("area %d center %f %f %f in solid?", n, FIXED_TO_DOUBLE(start[0]), FIXED_TO_DOUBLE(start[1]), FIXED_TO_DOUBLE(start[2]));
				VectorCopy(start, end);
				end[2] -= BFIXED(300,0);
				trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1);
				if (!trace.startsolid && trace.fraction < GFIXED_1 && AAS_PointAreaNum(trace.endpos) == n)
				{
					if (AAS_AreaGroundFaceArea(n) > BFIXED(300,0))
					{
						*goalareanum = n;
...
// Changes Globals:		-
//===========================================================================
bfixed DistancePointToLine(bvec3_t v1, bvec3_t v2, bvec3_t point)
{
	bvec3_t vec, p2;
...
// Changes Globals:		-
//===========================================================================
int AAS_NearestHideArea(int srcnum, bvec3_t origin, int areanum, int enemynum, bvec3_t enemyorigin, int enemyareanum, int travelflags)
{
	...
	while (updateliststart)
	{
		...
		for (i = 0; i < numreach; i++, reach++)
		{
			...
			dist2 = FIXED_VEC3LEN(v2);
			//never go through the enemy
			if (dist2 < BFIXED(40,0)) continue;
			//
			VectorSubtract(enemyorigin, curupdate->start, v1);
			dist1 = FIXED_VEC3LEN(v1);
			//
			if (dist2 < dist1)
			{
				t += FIXED_TO_INT((dist1 - dist2) * BFIXED(10,0));
			}
