...
//
#define TRAVELTIME_SCALE		GFIXED(0,01)
//item flags
...
typedef struct levelitem_s
{
	...
	gfixed weight;						//gfixed roam weight
	bvec3_t origin;						//origin of the item
	int goalareanum;					//area the item is in
	bvec3_t goalorigin;					//goal origin within the area
	int entitynum;						//entity number
	gfixed timeout;						//item is removed after this time
...
typedef struct iteminfo_s
{
	...
	gfixed respawntime;					//respawn time
	bvec3_t mins;						//mins of the item
	bvec3_t maxs;						//maxs of the item
...
{"respawntime", ITEMINFO_OFS(respawntime), FT_FIXED},
{"mins", ITEMINFO_OFS(mins), FT_FIXED|FT_ARRAY|FT_IS_BFIXED, 3},
{"maxs", ITEMINFO_OFS(maxs), FT_FIXED|FT_ARRAY|FT_IS_BFIXED, 3},
{0, 0, 0}
};
...
typedef struct bot_goalstate_s
{
	...
	gfixed avoidgoaltimes[MAX_AVOIDGOALS];		//times to avoid the goals
} bot_goalstate_t;
...
//the game type
static int gametype = 0; // bk001206 - init
...
itemconfig_t *LoadItemConfig(char *filename)
{
	...
	max_iteminfo = FIXED_TO_INT(LibVarValue("max_iteminfo", "256"));
	if (max_iteminfo < 0)
	{
...
void InitLevelItemHeap(void)
{
	...
	max_levelitems = FIXED_TO_INT(LibVarValue("max_levelitems", "256"));
		...
		if (!(spawnflags & 1))
		{
			if (!AAS_DropToFloor(origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs))
			{
				botimport.Print(PRT_MESSAGE, "%s in solid at (%1.1f %1.1f %1.1f)\n",
												classname, FIXED_TO_DOUBLE(origin[0]), FIXED_TO_DOUBLE(origin[1]), FIXED_TO_DOUBLE(origin[2]));
			} //end if
		} //end if
...
void BotResetAvoidGoals(int goalstate)
{
	...
	Com_Memset(gs->avoidgoaltimes, 0, MAX_AVOIDGOALS * sizeof(gfixed));
	...
	for (i = 0; i < MAX_AVOIDGOALS; i++)
	{
		if (gs->avoidgoaltimes[i] >= AAS_Time())
		{
			...
			Log_Write("avoid goal %s, number %d for %f seconds", name,
				gs->avoidgoals[i], FIXED_TO_DOUBLE(gs->avoidgoaltimes[i] - AAS_Time()));
		} //end if
	} //end for
...
// Changes Globals:		-
//===========================================================================
void BotAddToAvoidGoals(bot_goalstate_t *gs, int number, gfixed avoidtime)
{
	int i;
	...
	if (avoidtime < GFIXED_0)
	{
		...
		for (li = levelitems; li; li = li->next)
		{
			if (li->number == number)
			{
				...
				if (FIXED_IS_ZERO(avoidtime))
					avoidtime = GFIXED(AVOID_DEFAULT_TIME,0);
				if (avoidtime < GFIXED(AVOID_DEFAULT_TIME,0))
					avoidtime = GFIXED(AVOID_DEFAULT_TIME,0);
...
int BotGetLevelItemGoal(int index, char *name, bot_goal_t *goal)
{
	...
	for (; li; li = li->next)
	{
		...
		if (!Q_stricmp(name, itemconfig->iteminfo[li->iteminfo].name))
		{
			...
			goal->flags = GFL_ITEM;
			if (FIXED_NOT_ZERO(li->timeout)) goal->flags |= GFL_DROPPED;
...
void BotFindEntityForLevelItem(levelitem_t *li)
{
	...
	for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent))
	{
		...
		if (ic->iteminfo[li->iteminfo].modelindex == modelindex)
		{
			...
			if (FIXED_VEC3LEN(dir) < BFIXED(30,0))
			{
...
void BotUpdateEntityItems(void)
{
	...
	for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent))
	{
		...
		li->timeout = AAS_Time() + GFIXED(30,0);
...
// Changes Globals:		-
//===========================================================================
int BotChooseLTGItem(int goalstate, bvec3_t origin, int *inventory, int travelflags)
{
	int areanum, t, weightnum;
	gfixed weight, bestweight;
	gfixed avoidtime;
	iteminfo_t *iteminfo;
	itemconfig_t *ic;
	...
	for (li = levelitems; li; li = li->next)
	{
		...
		if (FIXED_NOT_ZERO(li->timeout))
			weight += MAKE_GFIXED(droppedweight->value);
#endif //DROPPEDWEIGHT
		...
		if (weight > GFIXED_0)
		{
			...
			if (t > 0)
			{
				...
				if (avoidtime - MAKE_GFIXED(t) * GFIXED(0,009) > GFIXED_0)
					continue;
				//
				weight /= MAKE_GFIXED(t) * TRAVELTIME_SCALE;
				//
	...
	goal.flags = GFL_ITEM;
	if (FIXED_NOT_ZERO(bestitem->timeout))
	...
	if (FIXED_NOT_ZERO(bestitem->timeout))
	{
		avoidtime = GFIXED(AVOID_DROPPED_TIME,0);
	} //end if
	else
	{
...
// Changes Globals:		-
//===========================================================================
int BotChooseNBGItem(int goalstate, bvec3_t origin, int *inventory, int travelflags,
														bot_goal_t *ltg, gfixed maxtime)
{
	...
	for (li = levelitems; li; li = li->next)
	{
		...
		if (weight > GFIXED_0)
		{
			...
			if (t > 0 && MAKE_GFIXED(t) < maxtime)
			{
				...
				{
					t = 0;
					if (ltg && FIXED_IS_ZERO(li->timeout))
					{
	...
	else
	{
		...
		if (FIXED_IS_ZERO(avoidtime))
			avoidtime = GFIXED(AVOID_DEFAULT_TIME,0);
		if (avoidtime < GFIXED(AVOID_MINIMUM_TIME,0))
			avoidtime = GFIXED(AVOID_MINIMUM_TIME,0);
	} //end else
	//add the chosen goal to the goals to avoid for a while
	BotAddToAvoidGoals(gs, bestitem->number, avoidtime);
	//push the goal on the stack
...
// Changes Globals:		-
//===========================================================================
int BotTouchingGoal(bvec3_t origin, bot_goal_t *goal)
{
	int i;
	bvec3_t boxmins, boxmaxs;
	bvec3_t absmins, absmaxs;
	bvec3_t safety_maxs = {BFIXED_0, BFIXED_0, BFIXED_0}; //{4, 4, 10};
	bvec3_t safety_mins = {BFIXED_0, BFIXED_0, BFIXED_0}; //{-4, -4, 0};
...
// Changes Globals:		-
//===========================================================================
int BotItemGoalInVisButNotVisible(int viewer, bvec3_t eye, avec3_t viewangles, bot_goal_t *goal)
{
	aas_entityinfo_t entinfo;
	bsp_trace_t trace;
	bvec3_t middle;
	...
	if (trace.fraction >= GFIXED_1)
	{
		...
		if (entinfo.ltime < AAS_Time() - GFIXED(0,5))
			return qtrue;
	} //end if
	return qfalse;
