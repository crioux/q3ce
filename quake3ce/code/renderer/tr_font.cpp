...
//
// As of Q3A GFIXED(1,25)+ and Team Arena, we are shipping the game with the font rendering code
...
// 
// In the UI Scripting code, a scale of GFIXED_1 is equal to a 48 point font. In Team Arena, we
...
static glyphInfo_t *RE_ConstructGlyphInfo(unsigned char *imageOut, int *xOut, int *yOut, int *maxHeight, FT_Face face, const unsigned char c, qboolean calcHeight) {
  ...
  gfixed scaled_width, scaled_height;
...
*/
    ...
    glyph.s = MAKE_GFIXED*xOut / 256;
    glyph.t = MAKE_GFIXED*yOut / 256;
    glyph.s2 = glyph.s + MAKE_GFIXEDscaled_width / 256;
    glyph.t2 = glyph.t + MAKE_GFIXEDscaled_height / 256;
...
void RE_RegisterFont(const char *fontName, int pointSize, fontInfo_t *font) {
  ...
  void *faceData;
	int i, len;
  char name[1024];
	gfixed dpi = GFIXED(72,0);											//
	gfixed glyphScale =  GFIXED(72,0) / dpi; 		// change the scale to be relative to 1 based on 72 dpi ( so dpi of 144 means a scale of .5 )

	if (pointSize <= 0) {
		pointSize = 12;
	}
	// we also need to adjust the scale based on point size relative to 48 points as the ui scaling is based on a 48 point font
	glyphScale *= FIXED_INT32RATIO_G(48,pointSize);
  ...
  while ( i <= GLYPH_END ) {
    ...
    if (xOut == -1 || yOut == -1 || i == GLYPH_END)  {
      ...
      for ( k = 0; k < (scaledSize) ; k++ ) {
        ...
        imageBuff[left++] = (MAKE_GFIXEDout[k] * max);
      }
