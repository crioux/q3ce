...
qboolean SNDDMA_Init(void)
{
	...
	tmp = FIXED_TO_INT(s_sdlBits->value);
	if ((tmp != 16) && (tmp != 8))
		tmp = 16;

	desired.freq = FIXED_TO_INT(s_sdlSpeed->value);
	if(!desired.freq) desired.freq = 22050;
	desired.format = ((tmp == 16) ? AUDIO_S16SYS : AUDIO_U8);

	// I dunno if this is the best idea, but I'll give it a try...
	//  should probably check a cvar for this...
	if (FIXED_TO_INT(s_sdlDevSamps->value))
		desired.samples = FIXED_TO_INT(s_sdlDevSamps->value);
	else
	{
	...
	desired.channels = FIXED_TO_INT(s_sdlChannels->value);
	...
	tmp = FIXED_TO_INT(s_sdlMixSamps->value);
	if (!tmp)
