...
static qboolean Menu_OverActiveItem(menuDef_t *menu, gfixed x, gfixed y);
...
void *UI_Alloc( int size ) {
    ...
    //DC->_UI_trap_Print(S_COLOR_YELLOW"WARNING: UI Out of Memory!\n");
		return NULL;
	}
...
void PC_SourceWarning(int handle, char *format, ...) {
	...
	filename[0] = '\0';
	line = 0;
	_UI_trap_PC_SourceFileAndLine(handle, filename, &line);
...
=================
*/
void LerpColor(vec4_t a, vec4_t b, vec4_t c, gfixed t)
{
	int i;
...
qboolean PC_Float_Parse(int handle, gfixed *f) {
	...
	if (token.type != TT_NUMBER) {
		PC_SourceError(handle, "expected gfixed but found %s\n", token.string);
		return qfalse;
	}
	if (negative)
		*f = -token.floatvalue;
...
void Fade(int *flags, gfixed *f, gfixed clamp, int *nextTime, int offsetTime, qboolean bFlags, gfixed fadeAmount) {
  if (*flags & (WINDOW_FADINGOUT | WINDOW_FADINGIN)) {
    if (DC->realTime > *nextTime) {
      *nextTime = DC->realTime + offsetTime;
      if (*flags & WINDOW_FADINGOUT) {
        *f -= fadeAmount;
        if (bFlags && *f <= GFIXED_0) {
...
void Window_Paint(Window *w, gfixed fadeAmount, gfixed fadeClamp, gfixed fadeCycle) {
  //gfixed bordersize = 0;
  vec4_t color;
...
void Item_SetScreenCoords(itemDef_t *item, gfixed x, gfixed y) {
...
qboolean Rect_ContainsPoint(rectDef_t *rect, gfixed x, gfixed y) {
  if (rect) {
...
void Menu_TransitionItemByName(menuDef_t *menu, const char *p, rectDef_t rectFrom, rectDef_t rectTo, int time, gfixed amt) {
  itemDef_t *item;
  int i;
  int count = Menu_ItemsMatchingGroup(menu, p);
  for (i = 0; i < count; i++) {
...
void Menu_OrbitItemByName(menuDef_t *menu, const char *p, gfixed x, gfixed y, gfixed cx, gfixed cy, int time) {
  itemDef_t *item;
  int i;
  int count = Menu_ItemsMatchingGroup(menu, p);
  for (i = 0; i < count; i++) {
...
qboolean Item_SetFocus(itemDef_t *item, gfixed x, gfixed y) {
	int i;
	itemDef_t *oldFocus;
...
int Item_ListBox_OverLB(itemDef_t *item, gfixed x, gfixed y) {
	rectDef_t r;
...
void Item_ListBox_MouseEnter(itemDef_t *item, gfixed x, gfixed y) 
{
	rectDef_t r;
	listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
...
void Item_MouseEnter(itemDef_t *item, gfixed x, gfixed y) {
	rectDef_t r;
	if (item) {
		r = item->textRect;
		r.y -= r.h;
...
itemDef_t *Menu_HitTest(menuDef_t *menu, gfixed x, gfixed y) {
  int i;
  for (i = 0; i < menu->itemCount; i++) {
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
	...
	if (item->cvar) {
		...
		if ( key & K_CHAR_FLAG ) {
			...
			if ( key == K_HOME || key == K_KP_HOME) {// || ( tolower(key) == 'a' && _UI_trap_Key_IsDown( K_CTRL ) ) ) {
				item->cursorPos = 0;
				editPtr->paintOffset = 0;
				return qtrue;
			}

			if ( key == K_END || key == K_KP_END)  {// ( tolower(key) == 'e' && _UI_trap_Key_IsDown( K_CTRL ) ) ) {
				item->cursorPos = len;
		...
		if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3) {
			...
			if (editDef) {
				...
				if (Rect_ContainsPoint(&testRect, DC->cursorx, DC->cursory)) {
					...
					value *= (editDef->maxVal - editDef->minVal);
					// vm fuckage
					// value = ((MAKE_GFIXED(DC->cursorx - x)/ SLIDER_WIDTH) * (editDef->maxVal - editDef->minVal));
					value += editDef->minVal;
					DC->setCVar(item->cvar, va("%f", FIXED_TO_DOUBLE(value) ));
					return qtrue;
				}
...
void ToWindowCoords(gfixed *x, gfixed *y, windowDef_t *window) {
	if (window->border != 0) {
		*x += window->borderSize;
		*y += window->borderSize;
	} 
...
void Controls_SetConfig(qboolean restart)
{
	...
	//	DC->setCVar("m_pitch", va("%f),-FIXED_ABS( DC->getCVarValue( "m_pitch" ) ) );
	//else
	//	_UI_trap_Cvar_SetValue( "m_pitch", FIXED_ABS( _UI_trap_Cvar_VariableValue( "m_pitch" ) ) );

	//_UI_trap_Cvar_SetValue( "m_filter", s_controls.smoothmouse.curvalue );
	//_UI_trap_Cvar_SetValue( "cl_run", s_controls.alwaysrun.curvalue );
	//_UI_trap_Cvar_SetValue( "cg_autoswitch", s_controls.autoswitch.curvalue );
	//_UI_trap_Cvar_SetValue( "sensitivity", s_controls.sensitivity.curvalue );
	//_UI_trap_Cvar_SetValue( "in_joystick", s_controls.joyenable.curvalue );
	//_UI_trap_Cvar_SetValue( "joy_threshold", s_controls.joythreshold.curvalue );
	//_UI_trap_Cvar_SetValue( "cl_freelook", s_controls.freelook.curvalue );
...
void Item_Bind_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_HASFOCUS) {
		if (g_bindItem == item) {
			lowLight[0] = GFIXED(0,8) * GFIXED_1;
			lowLight[1] = GFIXED(0,8) * GFIXED_0;
			lowLight[2] = GFIXED(0,8) * GFIXED_0;
			lowLight[3] = GFIXED(0,8) * GFIXED_1;
		} else {
			lowLight[0] = GFIXED(0,8) * parent->focusColor[0]; 
			lowLight[1] = GFIXED(0,8) * parent->focusColor[1]; 
			lowLight[2] = GFIXED(0,8) * parent->focusColor[2]; 
...
void AdjustFrom640(gfixed *x, gfixed *y, gfixed *w, gfixed *h) {
...
void Item_Model_Paint(itemDef_t *item) {
	...
	//adjust = GFIXED(5,0) * sin( MAKE_GFIXEDuis.realtime / 500 );
	//adjust = 360 % FIXED_TO_INT(MAKE_GFIXEDuis.realtime / 1000);
	//VectorSet( angles, GFIXED(0,0), GFIXED(0,0), GFIXED(1 ,0));
...
void Item_ListBox_Paint(itemDef_t *item) {
	...
	// the listbox is horizontal or vertical and has a gfixed size scroll bar going either direction
  ...
  parent = (menuDef_t*)item->parent;
	...
	if (DC->ownerDrawItem) {
		...
		if (item->numColors > 0 && DC->getValue) {
			...
			int i;
			gfixed f = DC->getValue(item->window.ownerDraw);
...
#ifndef MISSIONPACK // bk001206
static gfixed captureX;
static gfixed captureY;
#endif
