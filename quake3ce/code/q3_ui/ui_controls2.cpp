...
typedef struct
{
	...
	char				playerModel[64];
	avec3_t				playerViewangles;
	avec3_t				playerMoveangles;
	int					playerLegs;
	int					playerTorso;
	int					playerWeapon;
	qboolean			playerChat;
...
static vec4_t controls_binding_color  = {GFIXED(1,00), GFIXED(0,43), GFIXED(0,00), GFIXED(1,00)}; // bk: Win32 C4305
...
{
	{"cl_run",			LFIXED_0,					LFIXED_0},
	{"m_pitch",			LFIXED_0,					LFIXED_0},
	{"cg_autoswitch",	LFIXED_0,					LFIXED_0},
	{"sensitivity",		LFIXED_0,					LFIXED_0},
	{"in_joystick",		LFIXED_0,					LFIXED_0},
	{"joy_threshold",	LFIXED_0,					LFIXED_0},
	{"m_filter",		LFIXED_0,					LFIXED_0},
	{"cl_freelook",		LFIXED_0,					LFIXED_0},
	{NULL,				LFIXED_0,					LFIXED_0}
};
...
static void Controls_InitCvars( void )
{
	...
	for (i=0; ;i++,cvarptr++)
	{
		...
		// get current value
		cvarptr->value = _UI_trap_Cvar_VariableValue( cvarptr->name );

		// get default value
		_UI_trap_Cvar_Reset( cvarptr->name );
		cvarptr->defaultvalue = _UI_trap_Cvar_VariableValue( cvarptr->name );

		// restore current value
		_UI_trap_Cvar_SetValue( cvarptr->name, cvarptr->value );
	}
}
...
static gfixed Controls_GetCvarDefault( char* name )
{
	...
	return MAKE_GFIXED(cvarptr->defaultvalue);
}
...
static gfixed Controls_GetCvarValue( char* name )
{
	...
	return MAKE_GFIXED(cvarptr->value);
}
...
static void Controls_UpdateModel( int anim ) {
	...
	s_controls.playerViewangles[YAW] = AFIXED(180 - 30,0);
	...
	case ANIM_TURNLEFT:
		s_controls.playerViewangles[YAW] += AFIXED(90,0);
		break;
	...
	case ANIM_STEPLEFT:
		s_controls.playerLegs = LEGS_WALK;
		s_controls.playerMoveangles[YAW] = s_controls.playerViewangles[YAW] + AFIXED(90,0);
		break;
	...
	case ANIM_LOOKUP:
		s_controls.playerViewangles[PITCH] = -AFIXED(45,0);
		break;
...
static void Controls_DrawKeyBinding( void *self )
{
	...
	{
		_UI_trap_Key_KeynumToStringBuf( b1, name, 32 );
		Q_strupr(name);

		b2 = g_bindings[a->generic.id].bind2;
		if (b2 != -1)
		{
			_UI_trap_Key_KeynumToStringBuf( b2, name2, 32 );
			Q_strupr(name2);
	...
	if (c)
	{
		UI_FillRect( MAKE_GFIXED(a->generic.left), MAKE_GFIXED(a->generic.top), MAKE_GFIXED(a->generic.right-a->generic.left+1), MAKE_GFIXED(a->generic.bottom-a->generic.top+1), listbar_color ); 
		...
		if (s_controls.waitingforkey)
		{
			...
			UI_DrawString(SCREEN_WIDTH / 2, FIXED_TO_INT(GFIXED(SCREEN_HEIGHT ,0)* GFIXED(0,80)), "Waiting for new key ... ESCAPE to cancel", UI_SMALLFONT|UI_CENTER|UI_PULSE, colorWhite );
		}
		else
		{
			UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|UI_SMALLFONT, text_color_highlight);
			UI_DrawString(SCREEN_WIDTH /2, FIXED_TO_INT(GFIXED(SCREEN_HEIGHT ,0)* GFIXED(0,78)), "Press ENTER or CLICK to change", UI_SMALLFONT|UI_CENTER, colorWhite );
			UI_DrawString(SCREEN_WIDTH /2, FIXED_TO_INT(GFIXED(SCREEN_HEIGHT ,0)* GFIXED(0,82)), "Press BACKSPACE to clear", UI_SMALLFONT|UI_CENTER, colorWhite );
		}
	}
	else
	{
...
{
	UI_DrawString(SCREEN_WIDTH/2, FIXED_TO_INT(GFIXED(SCREEN_HEIGHT ,0)* GFIXED(0,80)), "Use Arrow Keys or CLICK to change", UI_SMALLFONT|UI_CENTER, colorWhite );
}
...
static void Controls_DrawPlayer( void *self ) {
	...
	b = (menubitmap_s*) self;
	UI_DrawPlayer( MAKE_GFIXED(b->generic.x), MAKE_GFIXED(b->generic.y), MAKE_GFIXED(b->width), MAKE_GFIXED(b->height), &s_controls.playerinfo, uis.realtime/2 );
}
...
static void Controls_GetKeyAssignment (char *command, int *twokeys)
{
	...
	for ( j = 0; j < 256; j++ )
	{
		_UI_trap_Key_GetBindingBuf( j, b, 256 );
		if ( *b == 0 ) {
			continue;
		}
...
static void Controls_GetConfig( void )
{
	...
	s_controls.invertmouse.curvalue  = FIXED_TO_INT(Controls_GetCvarValue( "m_pitch" ) ) < 0;
	s_controls.smoothmouse.curvalue  = FIXED_TO_INT(UI_ClampCvar( GFIXED_0, GFIXED_1, Controls_GetCvarValue( "m_filter" ) ) );
	s_controls.alwaysrun.curvalue    = FIXED_TO_INT(UI_ClampCvar( GFIXED_0, GFIXED_1, Controls_GetCvarValue( "cl_run" ) ) );
	s_controls.autoswitch.curvalue   = FIXED_TO_INT(UI_ClampCvar( GFIXED_0, GFIXED_1, Controls_GetCvarValue( "cg_autoswitch" ) ) );
	s_controls.sensitivity.curvalue  = (UI_ClampCvar( GFIXED(2,0), GFIXED(30,0), Controls_GetCvarValue( "sensitivity" ) ) );
	s_controls.joyenable.curvalue    = FIXED_TO_INT(UI_ClampCvar( GFIXED_0, GFIXED_1, Controls_GetCvarValue( "in_joystick" ) ) );
	s_controls.joythreshold.curvalue = (UI_ClampCvar( GFIXED(0,05), GFIXED(0,75), Controls_GetCvarValue( "joy_threshold" ) ) );
	s_controls.freelook.curvalue     = FIXED_TO_INT(UI_ClampCvar( GFIXED_0, GFIXED_1, Controls_GetCvarValue( "cl_freelook" ) ) );
}
...
static void Controls_SetConfig( void )
{
	...
	for (i=0; ;i++,bindptr++)
	{
		...
		if (bindptr->bind1 != -1)
		{	
			_UI_trap_Key_SetBinding( bindptr->bind1, bindptr->command );

			if (bindptr->bind2 != -1)
				_UI_trap_Key_SetBinding( bindptr->bind2, bindptr->command );
		}
	}

	if ( s_controls.invertmouse.curvalue )
		_UI_trap_Cvar_SetValue( "m_pitch", -FIXED_ABS( _UI_trap_Cvar_VariableValue( "m_pitch" ) ) );
	else
		_UI_trap_Cvar_SetValue( "m_pitch", FIXED_ABS( _UI_trap_Cvar_VariableValue( "m_pitch" ) ) );

	_UI_trap_Cvar_SetValue( "m_filter", MAKE_LFIXED(s_controls.smoothmouse.curvalue) );
	_UI_trap_Cvar_SetValue( "cl_run", MAKE_LFIXED(s_controls.alwaysrun.curvalue) );
	_UI_trap_Cvar_SetValue( "cg_autoswitch", MAKE_LFIXED(s_controls.autoswitch.curvalue) );
	_UI_trap_Cvar_SetValue( "sensitivity", MAKE_LFIXED(s_controls.sensitivity.curvalue) );
	_UI_trap_Cvar_SetValue( "in_joystick", MAKE_LFIXED(s_controls.joyenable.curvalue) );
	_UI_trap_Cvar_SetValue( "joy_threshold", MAKE_LFIXED(s_controls.joythreshold.curvalue) );
	_UI_trap_Cvar_SetValue( "cl_freelook", MAKE_LFIXED(s_controls.freelook.curvalue) );
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "in_restart\n" );
}
...
static void Controls_SetDefaults( void )
{
	...
	s_controls.invertmouse.curvalue  = FIXED_TO_INT(Controls_GetCvarDefault( "m_pitch" )) < 0;
	s_controls.smoothmouse.curvalue  = FIXED_TO_INT(Controls_GetCvarDefault( "m_filter" ));
	s_controls.alwaysrun.curvalue    = FIXED_TO_INT(Controls_GetCvarDefault( "cl_run" ));
	s_controls.autoswitch.curvalue   = FIXED_TO_INT(Controls_GetCvarDefault( "cg_autoswitch" ));
	s_controls.sensitivity.curvalue  = Controls_GetCvarDefault( "sensitivity" );
	s_controls.joyenable.curvalue    = FIXED_TO_INT(Controls_GetCvarDefault( "in_joystick" ));
	s_controls.joythreshold.curvalue = Controls_GetCvarDefault( "joy_threshold" );
	s_controls.freelook.curvalue     = FIXED_TO_INT(Controls_GetCvarDefault( "cl_freelook" ));
}
...
static sfxHandle_t Controls_MenuKey( int key )
{
	...
	for (i=0; ;i++,bindptr++)
	{
		...
		if (bindptr->id == id)
		{
			...
			{
				if( bindptr->bind1 != -1 ) {
					_UI_trap_Key_SetBinding( bindptr->bind1, "" );
					bindptr->bind1 = -1;
				}
				if( bindptr->bind2 != -1 ) {
					_UI_trap_Key_SetBinding( bindptr->bind2, "" );
					bindptr->bind2 = -1;
				}
			}
...
static void Controls_InitWeapons( void ) {
	...
	for ( item = bg_itemlist + 1 ; item->classname ; item++ ) {
		...
		_UI_trap_R_RegisterModel( item->world_model[0] );
	}
}
...
static void Controls_MenuInit( void )
{
	...
	s_controls.sensitivity.minvalue		     = GFIXED(2,0);
	s_controls.sensitivity.maxvalue		     = GFIXED(30,0);
	...
	s_controls.joythreshold.minvalue		  = GFIXED(0,05);
	s_controls.joythreshold.maxvalue		  = GFIXED(0,75);
...
void Controls_Cache( void ) {
	...
	_UI_trap_R_RegisterShaderNoMip( ART_FRAMEL );
	_UI_trap_R_RegisterShaderNoMip( ART_FRAMER );
}
