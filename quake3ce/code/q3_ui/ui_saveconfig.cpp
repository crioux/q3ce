...
static void UI_SaveConfigMenu_SaveEvent( void *ptr, int event ) {
	...
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "writeconfig %s.cfg\n", configname ) );
	UI_PopMenu();
}
...
static void UI_SaveConfigMenu_SavenameDraw( void *self ) {
	...
	UI_FillRect( MAKE_GFIXED(f->generic.x), MAKE_GFIXED(f->generic.y), MAKE_GFIXED(f->field.widthInChars)*GFIXED(SMALLCHAR_WIDTH,0), GFIXED(SMALLCHAR_HEIGHT,0), colorBlack );
...
void UI_SaveConfigMenu_Cache( void ) {
	...
	_UI_trap_R_RegisterShaderNoMip( ART_SAVE0 );
	_UI_trap_R_RegisterShaderNoMip( ART_SAVE1 );
