...
void DriverInfo_Cache( void )
{
	...
	for (i=0; ;i++)
	{
		if (!driverinfo_artlist[i])
			break;
		_UI_trap_R_RegisterShaderNoMip(driverinfo_artlist[i]);
	}
}
...
static void GraphicsOptions_GetInitialVideo( void )
{
	...
	s_ivo.tq          = FIXED_TO_INT( s_graphicsoptions.tq.curvalue );
...
static void GraphicsOptions_CheckConfig( void )
{
	...
	for ( i = 0; i < NUM_IVO_TEMPLATES; i++ )
	{
		...
		if ( s_ivo_templates[i].fullscreen != s_graphicsoptions.fs.curvalue )
			continue;
		if ( MAKE_GFIXED(s_ivo_templates[i].tq) != s_graphicsoptions.tq.curvalue )
			continue;
...
static void GraphicsOptions_UpdateMenuItems( void )
{
	...
	if ( MAKE_GFIXED(s_ivo.tq) != s_graphicsoptions.tq.curvalue )
	{
		s_graphicsoptions.apply.generic.flags &= ~(QMF_HIDDEN|QMF_INACTIVE);
	}
...
static void GraphicsOptions_ApplyChanges( void *unused, int notification )
{
	...
	_UI_trap_Cvar_SetValue( "r_picmip", LFIXED(3 ,0)- MAKE_LFIXED(s_graphicsoptions.tq.curvalue) );
	_UI_trap_Cvar_SetValue( "r_allowExtensions", MAKE_LFIXED(s_graphicsoptions.allow_extensions.curvalue) );
	_UI_trap_Cvar_SetValue( "r_mode", MAKE_LFIXED(s_graphicsoptions.mode.curvalue) );
	_UI_trap_Cvar_SetValue( "r_fullscreen", MAKE_LFIXED(s_graphicsoptions.fs.curvalue) );
	_UI_trap_Cvar_Set( "r_glDriver", ( char * ) s_drivers[s_graphicsoptions.driver.curvalue] );
	...
	_UI_trap_Cvar_SetValue( "r_vertexLight", MAKE_LFIXED(s_graphicsoptions.lighting.curvalue) );
...
static void GraphicsOptions_Event( void* ptr, int event ) {
	...
	case ID_LIST:
		...
		s_graphicsoptions.tq.curvalue          = MAKE_GFIXED(ivo->tq);
...
static void GraphicsOptions_TQEvent( void *ptr, int event ) {
	...
	s_graphicsoptions.tq.curvalue = s_graphicsoptions.tq.curvalue + GFIXED(0,5);
}
...
{
	s_graphicsoptions.mode.curvalue = FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "r_mode" ));
	if ( s_graphicsoptions.mode.curvalue < 0 )
	{
		s_graphicsoptions.mode.curvalue = 3;
	}
	s_graphicsoptions.fs.curvalue = FIXED_TO_INT(_UI_trap_Cvar_VariableValue("r_fullscreen"));
	s_graphicsoptions.allow_extensions.curvalue = FIXED_TO_INT(_UI_trap_Cvar_VariableValue("r_allowExtensions"));
	s_graphicsoptions.tq.curvalue = GFIXED(3,0)-MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "r_picmip"));
	if ( s_graphicsoptions.tq.curvalue < GFIXED_0 )
	{
		s_graphicsoptions.tq.curvalue = GFIXED_0;
	}
	else if ( s_graphicsoptions.tq.curvalue > GFIXED(3 ,0))
	{
		s_graphicsoptions.tq.curvalue = GFIXED(3,0);
	}

	s_graphicsoptions.lighting.curvalue = FIXED_NOT_ZERO(_UI_trap_Cvar_VariableValue( "r_vertexLight" ));
	switch (FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "r_texturebits" )) )
	{
	default:
	case 0:
	...
	if ( _UI_trap_Cvar_VariableValue( "r_lodBias" ) > LFIXED_0 )
	{
		if ( _UI_trap_Cvar_VariableValue( "r_subdivisions" ) >= LFIXED(20 ,0))
		{
	...
	switch ( FIXED_TO_INT( _UI_trap_Cvar_VariableValue( "r_colorbits" ) ) )
	{
	default:
	case 0:
		s_graphicsoptions.colordepth.curvalue = 0;
...
void GraphicsOptions_MenuInit( void )
{
	...
	s_graphicsoptions.tq.minvalue       = GFIXED_0;
	s_graphicsoptions.tq.maxvalue       = GFIXED(3,0);
...
void GraphicsOptions_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_FRAMEL );
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_FRAMER );
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_BACK0 );
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_BACK1 );
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_ACCEPT0 );
	_UI_trap_R_RegisterShaderNoMip( GRAPHICSOPTIONS_ACCEPT1 );
}
