...
static void UI_SoundOptionsMenu_Event( void* ptr, int event ) {
	...
	case ID_EFFECTSVOLUME:
		_UI_trap_Cvar_SetValue( "s_volume", MAKE_LFIXED(soundOptionsInfo.sfxvolume.curvalue) / LFIXED(10 ,0));
		break;

	case ID_MUSICVOLUME:
		_UI_trap_Cvar_SetValue( "s_musicvolume", MAKE_LFIXED(soundOptionsInfo.musicvolume.curvalue) / LFIXED(10 ,0));
		break;

	case ID_QUALITY:
		if( soundOptionsInfo.quality.curvalue ) {
			_UI_trap_Cvar_SetValue( "s_khz", LFIXED(22 ,0));
			_UI_trap_Cvar_SetValue( "s_compression", LFIXED_0 );
		}
		else {
			_UI_trap_Cvar_SetValue( "s_khz", LFIXED(11 ,0));
			_UI_trap_Cvar_SetValue( "s_compression", LFIXED_1 );
		}
		UI_ForceMenuOff();
		_UI_trap_Cmd_ExecuteText( EXEC_APPEND, "snd_restart\n" );
		break;
/*
	case ID_A3D:
		if( soundOptionsInfo.a3d.curvalue ) {
			_UI_trap_Cmd_ExecuteText( EXEC_NOW, "s_enable_a3d\n" );
		}
		else {
			_UI_trap_Cmd_ExecuteText( EXEC_NOW, "s_disable_a3d\n" );
		}
		soundOptionsInfo.a3d.curvalue = (int)_UI_trap_Cvar_VariableValue( "s_usingA3D" );
		break;
*/
	case ID_BACK:
		UI_PopMenu();
...
static void UI_SoundOptionsMenu_Init( void ) {
	...
	soundOptionsInfo.banner.string				= strdup("SYSTEM SETUP");
	...
	soundOptionsInfo.graphics.string			= strdup("GRAPHICS");
	...
	soundOptionsInfo.display.string				= strdup("DISPLAY");
	...
	soundOptionsInfo.sound.string				= strdup("SOUND");
	...
	soundOptionsInfo.network.string				= strdup("NETWORK");
	soundOptionsInfo.network.style				= UI_RIGHT;
	soundOptionsInfo.network.color				= color_red;

	y = 240 - FIXED_TO_INT(GFIXED(1,5) * GFIXED(BIGCHAR_HEIGHT + 2,0));
	...
	soundOptionsInfo.sfxvolume.minvalue			= GFIXED_0;
	soundOptionsInfo.sfxvolume.maxvalue			= GFIXED(10,0);
	...
	soundOptionsInfo.musicvolume.minvalue			= GFIXED_0;
	soundOptionsInfo.musicvolume.maxvalue			= GFIXED(10,0);
...
//	Menu_AddItem( &soundOptionsInfo.menu, ( void * ) &soundOptionsInfo.a3d );
	...
	soundOptionsInfo.sfxvolume.curvalue = MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "s_volume" )) * GFIXED(10,0);
	soundOptionsInfo.musicvolume.curvalue = MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "s_musicvolume" )) * GFIXED(10,0);
	soundOptionsInfo.quality.curvalue = !FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "s_compression" ));
//	soundOptionsInfo.a3d.curvalue = (int)_UI_trap_Cvar_VariableValue( "s_usingA3D" );
}
