...
{
	_UI_trap_Cvar_Set( "model", s_playermodel.modelskin );
	_UI_trap_Cvar_Set( "headmodel", s_playermodel.modelskin );
	_UI_trap_Cvar_Set( "team_model", s_playermodel.modelskin );
	_UI_trap_Cvar_Set( "team_headmodel", s_playermodel.modelskin );
}
...
static void PlayerModel_PicEvent( void* ptr, int event )
{
	...
	if (pdest)
	{
		...
		if( _UI_trap_MemoryRemaining() > LOW_MEMORY ) {
			PlayerModel_UpdateModel();
		}
	}
}
...
static void PlayerModel_DrawPlayer( void *self )
{
	...
	UI_DrawPlayer( MAKE_GFIXED(b->generic.x), MAKE_GFIXED(b->generic.y), MAKE_GFIXED(b->width), MAKE_GFIXED(b->height), &s_playermodel.playerinfo, uis.realtime/2 );
}
...
static void PlayerModel_BuildList( void )
{
	...
	for (i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		for (j=0; j<numfiles && s_playermodel.nummodels < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			if( precache ) {
				_UI_trap_S_RegisterSound( va( "sound/player/announce/%s_wins.wav", skinname), qfalse );
			}
		}
	}	
...
static void PlayerModel_SetMenuItems( void )
{
	...
	// name
	_UI_trap_Cvar_VariableStringBuffer( "name", s_playermodel.playername.string, 16 );
	Q_CleanStr( s_playermodel.playername.string );

	// model
	_UI_trap_Cvar_VariableStringBuffer( "model", s_playermodel.modelskin, 64 );
...
void PlayerModel_Cache( void )
{
	...
	for( i = 0; playermodel_artlist[i]; i++ ) {
		_UI_trap_R_RegisterShaderNoMip( playermodel_artlist[i] );
	}

	PlayerModel_BuildList();
	for( i = 0; i < s_playermodel.nummodels; i++ ) {
		_UI_trap_R_RegisterShaderNoMip( s_playermodel.modelnames[i] );
	}
}
