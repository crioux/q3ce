...
static void UI_SPSkillMenu_SkillEvent( void *ptr, int notification ) {
	...
	SetSkillColor( FIXED_TO_INT(_UI_trap_Cvar_VariableValue( "g_spSkill" )), color_red );

	id = ((menucommon_s*)ptr)->id;
	skill = id - ID_BABY + 1;
	_UI_trap_Cvar_SetValue( "g_spSkill", MAKE_LFIXED(skill) );

	SetSkillColor( skill, color_white );
	skillMenuInfo.art_skillPic.shader = skillMenuInfo.skillpics[skill - 1];

	if( id == ID_NIGHTMARE ) {
		_UI_trap_S_StartLocalSound( skillMenuInfo.nightmareSound, CHAN_ANNOUNCER );
	}
	else {
		_UI_trap_S_StartLocalSound( skillMenuInfo.silenceSound, CHAN_ANNOUNCER );
	}
}
...
void UI_SPSkillMenu_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( ART_FRAME );
	_UI_trap_R_RegisterShaderNoMip( ART_BACK );
	_UI_trap_R_RegisterShaderNoMip( ART_BACK_FOCUS );
	_UI_trap_R_RegisterShaderNoMip( ART_FIGHT );
	_UI_trap_R_RegisterShaderNoMip( ART_FIGHT_FOCUS );
	skillMenuInfo.skillpics[0] = _UI_trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE1 );
	skillMenuInfo.skillpics[1] = _UI_trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE2 );
	skillMenuInfo.skillpics[2] = _UI_trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE3 );
	skillMenuInfo.skillpics[3] = _UI_trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE4 );
	skillMenuInfo.skillpics[4] = _UI_trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE5 );

	skillMenuInfo.nightmareSound = _UI_trap_S_RegisterSound( "sound/misc/nightmare.wav", qfalse );
	skillMenuInfo.silenceSound = _UI_trap_S_RegisterSound( "sound/misc/silence.wav", qfalse );
}
...
static void UI_SPSkillMenu_Init( void ) {
	...
	skillMenuInfo.art_banner.string				= strdup("DIFFICULTY");
	...
	skillMenuInfo.item_baby.string				= strdup("I Can Win");
	...
	skillMenuInfo.item_easy.string				= strdup("Bring It On");
	...
	skillMenuInfo.item_medium.string			= strdup("Hurt Me Plenty");
	...
	skillMenuInfo.item_hard.string				= strdup("Hardcore");
	...
	skillMenuInfo.item_nightmare.string				= strdup("NIGHTMARE!");
	...
	skill = FIXED_TO_INT(Com_Clamp( GFIXED_1, GFIXED(5,0), MAKE_GFIXED(_UI_trap_Cvar_VariableValue( "g_spSkill" )) ));
	SetSkillColor( skill, color_white );
	skillMenuInfo.art_skillPic.shader = skillMenuInfo.skillpics[skill - 1];
	if( skill == 5 ) {
		_UI_trap_S_StartLocalSound( skillMenuInfo.nightmareSound, CHAN_ANNOUNCER );
