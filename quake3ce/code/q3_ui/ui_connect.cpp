...
static void UI_DisplayDownloadInfo( const char *downloadName ) {
	...
	leftWidth = width = FIXED_TO_INT(MAKE_GFIXED(UI_ProportionalStringWidth( dlText )) * UI_ProportionalSizeScale( style ));
	width = FIXED_TO_INT(MAKE_GFIXED(UI_ProportionalStringWidth( etaText )) * UI_ProportionalSizeScale( style ));
	if (width > leftWidth) leftWidth = width;
	width = FIXED_TO_INT(MAKE_GFIXED(UI_ProportionalStringWidth( xferText )) * UI_ProportionalSizeScale( style ));
	if (width > leftWidth) leftWidth = width;
	leftWidth += 16;
	...
	if (downloadCount < 4096 || !downloadTime) {
	  ...
	  // bk010108
	  //gfixed elapsedTime = MAKE_GFIXED(uis.realtime - downloadTime); // current - start (msecs)
	  //elapsedTime = MSECTIME_G(elapsedTime); // in seconds
	  //if ( elapsedTime <= GFIXED_0 ) elapsedTime == GFIXED_0;
	  if ( (uis.realtime - downloadTime) / 1000) {
			xferRate = downloadCount / ((uis.realtime - downloadTime) / 1000);
		  //xferRate = FIXED_TO_INT( (MAKE_GFIXEDdownloadCount) / elapsedTime);
		} else {
			xferRate = 0;
		}
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	if ( passwordNeeded ) {
		s_ingame_menu.x = SCREEN_WIDTH * GFIXED(0,50) - 128;
