...
void Favorites_Add( void )
{
	...
	_UI_trap_Cvar_VariableStringBuffer( "cl_currentServerAddress", serverbuff, sizeof(serverbuff) );
	if (!serverbuff[0])
		return;
	...
	{
		_UI_trap_Cvar_VariableStringBuffer( va("server%d",i+1), adrstr, sizeof(adrstr) );
	...
	if (best)
		_UI_trap_Cvar_Set( va("server%d",best), serverbuff);
}
...
static void ServerInfo_MenuDraw( void )
{
	...
	while ( s ) {
		...
		UI_DrawString(FIXED_TO_INT(GFIXED(SCREEN_WIDTH,0)*GFIXED(0,50)-GFIXED(8,0)),y,key,UI_RIGHT|UI_SMALLFONT,color_red);
		UI_DrawString(FIXED_TO_INT(GFIXED(SCREEN_WIDTH,0)*GFIXED(0,50)+GFIXED(8,0)),y,value,UI_LEFT|UI_SMALLFONT,text_color_normal);
...
void ServerInfo_Cache( void )
{
	...
	for (i=0; ;i++)
	{
		if (!serverinfo_artlist[i])
			break;
		_UI_trap_R_RegisterShaderNoMip(serverinfo_artlist[i]);
	}
}
...
void UI_ServerInfoMenu( void )
{
	...
	_UI_trap_GetConfigString( CS_SERVERINFO, s_serverinfo.info, MAX_INFO_STRING );
