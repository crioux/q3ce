...
static void UI_AddBotsMenu_FightEvent( void* ptr, int event ) {
	...
	_UI_trap_Cmd_ExecuteText( EXEC_APPEND, va("addbot %s %i %s %i\n",
...
static void UI_AddBotsMenu_Draw( void ) {
	...
	UI_DrawNamedPic( GFIXED(320-233,0), GFIXED(240-166,0), GFIXED(466,0), GFIXED(332,0), ART_BACKGROUND );
...
static void UI_AddBotsMenu_Init( void ) {
	...
	_UI_trap_GetConfigString(CS_SERVERINFO, info, MAX_INFO_STRING);   
	...
	addBotsMenuInfo.skill.curvalue			= FIXED_TO_INT(Com_Clamp( GFIXED_0, GFIXED(4,0), MAKE_GFIXED( _UI_trap_Cvar_VariableValue( "g_spSkill" ) ) - GFIXED_1));
...
void UI_AddBots_Cache( void ) {
	...
	_UI_trap_R_RegisterShaderNoMip( ART_BACKGROUND );
	_UI_trap_R_RegisterShaderNoMip( ART_ARROWS );
	_UI_trap_R_RegisterShaderNoMip( ART_ARROWUP );
	_UI_trap_R_RegisterShaderNoMip( ART_ARROWDOWN );
}
