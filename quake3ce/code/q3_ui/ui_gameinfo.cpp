...
static void UI_LoadBots( void ) {
	...
	_UI_trap_Cvar_Register( &botsFile, "g_botsFile", "", CVAR_INIT|CVAR_ROM );
	if( *botsFile.string ) {
	...
	numdirs = _UI_trap_FS_GetFileList("scripts", ".bot", dirlist, 1024 );
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		strcpy(filename, "scripts/");
	...
	_UI_trap_Print( va( "%i bots parsed\n", ui_numBots ) );
}
...
void UI_GetBestScore( int level, int *score, int *skill ) {
	...
	for( n = 1; n <= 5; n++ ) {
		_UI_trap_Cvar_VariableStringBuffer( va( "g_spScores%i", n ), scores, MAX_INFO_VALUE );
...
void UI_LogAwardData( int award, int data ) {
	...
	if( award > AWARD_PERFECT ) {
		_UI_trap_Print( va( S_COLOR_RED "Bad award %i in UI_LogAwardData\n", award ) );
		return;
	}

	_UI_trap_Cvar_VariableStringBuffer( "g_spAwards", awardData, sizeof(awardData) );
	...
	_UI_trap_Cvar_Set( "g_spAwards", awardData );
}
...
qboolean UI_ShowTierVideo( int tier ) {
	...
	_UI_trap_Cvar_VariableStringBuffer( "g_spVideos", videos, sizeof(videos) );
	...
	_UI_trap_Cvar_Set( "g_spVideos", videos );
...
void UI_NewGame( void ) {
	_UI_trap_Cvar_Set( "g_spScores1", "" );
	_UI_trap_Cvar_Set( "g_spScores2", "" );
	_UI_trap_Cvar_Set( "g_spScores3", "" );
	_UI_trap_Cvar_Set( "g_spScores4", "" );
	_UI_trap_Cvar_Set( "g_spScores5", "" );
	_UI_trap_Cvar_Set( "g_spAwards", "" );
	_UI_trap_Cvar_Set( "g_spVideos", "" );
}
...
void UI_SPUnlockMedals_f( void ) {
	...
	_UI_trap_Cvar_VariableStringBuffer( "g_spAwards", awardData, MAX_INFO_VALUE );
	...
	_UI_trap_Print( "All levels unlocked at 100\n" );
}
...
void UI_InitGameinfo( void ) {
	...
	if( FIXED_NOT_ZERO(_UI_trap_Cvar_VariableValue( "fs_restrict" )) || (ui_numSpecialSinglePlayerArenas == 0 && ui_numSinglePlayerArenas == 4) ) {
