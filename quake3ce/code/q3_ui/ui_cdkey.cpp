...
static void UI_CDKeyMenu_Event( void *ptr, int event ) {
	...
	case ID_ACCEPT:
		if( cdkeyMenuInfo.cdkey.field.buffer[0] ) {
			_UI_trap_SetCDKey( cdkeyMenuInfo.cdkey.field.buffer );
		}
		UI_PopMenu();
		break;
...
static void UI_CDKeyMenu_DrawKey( void *self ) {
	...
	UI_FillRect( MAKE_GFIXED(x), MAKE_GFIXED(y), GFIXED(16 * BIGCHAR_WIDTH,0), GFIXED(BIGCHAR_HEIGHT,0), listbar_color );
...
static void UI_CDKeyMenu_Init( void ) {
	_UI_trap_Cvar_Set( "ui_cdkeychecked", "1" );
	...
	cdkeyMenuInfo.banner.string						= strdup("CD KEY");
	...
	cdkeyMenuInfo.cdkey.generic.x					= FIXED_TO_INT(GFIXED(320 - BIGCHAR_WIDTH,0) * GFIXED(2,5));
	...
	_UI_trap_GetCDKey( cdkeyMenuInfo.cdkey.field.buffer, cdkeyMenuInfo.cdkey.field.maxchars + 1 );
	if( _UI_trap_VerifyCDKey( cdkeyMenuInfo.cdkey.field.buffer, NULL ) == qfalse ) {
...
void UI_CDKeyMenu_Cache( void ) {
	_UI_trap_R_RegisterShaderNoMip( ART_ACCEPT0 );
	_UI_trap_R_RegisterShaderNoMip( ART_ACCEPT1 );
